...
- パラメータが2つの場合：`y/x`のアークタンジェントを返し、[ATAN2(y, x)](https://doris.apache.org/docs/dev/sql-manual/sql-functions/scalar-functions/numeric-functions/atan2)と同じ動作をし、結果の範囲は[-π, π]です
...
- `x = +Infinity`かつ有限の`y` > 0の場合、0を返します。有限の`y` < 0の場合、-0を返します
- `x = -Infinity`かつ有限の`y` > 0の場合、πを返します。有限の`y` < 0の場合、-πを返します
