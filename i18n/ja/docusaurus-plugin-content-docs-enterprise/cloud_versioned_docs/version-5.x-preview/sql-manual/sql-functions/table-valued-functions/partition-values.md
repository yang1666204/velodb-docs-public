...
特定のTABLEのパーティション値のリストを表示できるように、パーティション値の一時テーブルを生成するテーブル関数です。
...
クエリ対象のテーブルには複数のパーティションフィールドがあり、それに応じてテーブルには複数の列があります。

## 例

multi_catalog配下のhive3 CATALOGにおける`text_partitioned_columns`のテーブル作成文は以下のとおりです：
...
+----------------------------+-------+---------+----------+
```
hive3 CATALOG の multi_catalog 配下にある `text_partitioned_columns` のパーティション値のリストを表示します：
