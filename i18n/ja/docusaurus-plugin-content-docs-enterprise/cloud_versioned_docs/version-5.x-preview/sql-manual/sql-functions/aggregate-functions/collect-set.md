...
{
  "title": "COLLECT_SET",
  "description": "Aggregation関数は、指定されたカラムのすべての一意の値を集約し、重複する要素を削除して、set型の結果を返します。",
  "language": "ja"
}
---
## Description

集約関数は指定された列のすべての一意な値を集約し、重複要素を削除して、セット型の結果を返します。
...
| -- | -- |
| `<expr>` | 配列に格納する値を決定する式。サポートされる型: Bool、TinyInt、SmallInt、Integer、BigInt、LargeInt、Float、Double、Decimal、Date、Datetime、IPV4、IPV6、String、Array、Map、Struct。 |
| `<max_size>` | 結果の配列サイズをmax_size要素に制限するオプションパラメータ。サポートされる型: Integer。 |

## Return Value

重複除去後のすべてのnon-NULL値を含むARRAY型を返します。グループ内に有効なデータが存在しない場合は、空の配列を返します。
