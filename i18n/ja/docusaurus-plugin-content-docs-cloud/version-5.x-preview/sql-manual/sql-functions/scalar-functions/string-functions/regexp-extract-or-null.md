---
{
  ...
  "description": "テキスト文字列から、対象の正規表現パターンにマッチする最初の部分文字列を抽出します。",
  "language": "ja"
}
---
## 説明
...
| -- | -- |
| `<str>` | 文字列パラメータ。正規表現マッチングが実行されるテキスト文字列を表します。この文字列は任意の文字の組み合わせを含むことができ、関数は<pattern>に一致する部分文字列をその中で検索します。 |
| `<pattern>` | 文字列パラメータ。対象となる正規表現パターンです。このパターンには様々な正規表現メタ文字と文字クラスを含めることができ、マッチさせる部分文字列のルールを正確に定義します。 |
| `<pos>` | 整数パラメータ。抽出する表現グループのインデックスを示します。インデックスは1から始まります。<pos>が0に設定されている場合、最初にマッチした部分文字列全体が返されます。<pos>が負の数であるか、パターン内の表現グループ数を超える場合、関数はNULLを返します。 |
...
入力`<pos>`が無効（負の値または表現グループ数を超過）の場合、NULLを返します。
正規表現のマッチが失敗した場合、NULLを返します。
`<pos>` < 0の場合、NULLを返します。
`pos` > `<str>`の長さの場合、NULLを返します。

## Example

マッチから特定のグループを抽出します。説明：正規表現([[:lower:]]+)C([[:lower:]]+)は、'C'で区切られた1文字以上の小文字の連続を検索します。インデックス1のグループは最初の小文字の連続に対応するため、'b'が返されます。
...
+---------------------------------------------------------------------------+
```
マッチしない正規表現です。文字列 'AbCdE' のどの部分もパターンに完全にマッチしません。
...
+---------------------------------------------------------------------+
```
中国語文字のマッチング。パターン (\p{Han}+)(.+) は最初に1つ以上の中国語文字にマッチし、その後残りの任意の文字にマッチします。インデックス2のグループは、中国語文字の後にある文字列の非中国語部分を表します。
...
+----------------------------------------+
```
`pattern`が正規表現として許可されていない場合は、エラーをスローします。
