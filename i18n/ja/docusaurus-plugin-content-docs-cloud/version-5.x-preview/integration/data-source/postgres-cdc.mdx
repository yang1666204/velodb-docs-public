...
- `max_wal_senders >= 1`
- `REPLICATION, LOGIN`権限とTableに対する`SELECT`権限を持つCDCユーザー
...
```
:::info
**DELETE操作をキャプチャするには:** TableでREPLICA IDENTITY FULLを有効にしてください：
...
```
:::info
データベースはVeloDBに存在している必要があります。Tableは、PostgreSQLスキーマに基づいてFlink Doris Connectorによって自動作成されます。
:::
