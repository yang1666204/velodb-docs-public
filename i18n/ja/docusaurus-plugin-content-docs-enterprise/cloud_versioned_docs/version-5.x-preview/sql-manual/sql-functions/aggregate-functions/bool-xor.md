...
{
  "title": "BOOL_XOR",
  "description": "expression の全てのnon-NULL値に対して論理XOR集約を実行します。",
  "language": "ja"
}
---
## 説明
...
| -- | -- |
| `<expr>` | 論理XORで集約される式。BOOLEAN型と、0/非0ルール（0はFALSE、非0はTRUE）によってbooleanに変換可能な数値型をサポートします。 |
...
+-----------------+-----------------+-----------------+-----------------+
```
BOOL_XOR は数値型も受け入れます。ゼロ以外の値は TRUE として扱われます：
...
```
### Window function
次の例では、条件（id > 2）に基づいて行をパーティション分割し、それらを2つのグループに分けて、ウィンドウ集計結果を表示します：
