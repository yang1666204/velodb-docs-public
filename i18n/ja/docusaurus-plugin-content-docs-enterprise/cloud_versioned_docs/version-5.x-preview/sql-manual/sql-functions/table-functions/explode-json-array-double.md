...
---
## 説明
`explode_json_array_double` テーブル関数は JSON 配列を受け取ります。その実装ロジックは、JSON 配列を配列型に変換してから、処理のために `explode` 関数を呼び出すことです。この動作は次と同等です：`explode(cast(<json_array> as Array<DOUBLE>))`。
