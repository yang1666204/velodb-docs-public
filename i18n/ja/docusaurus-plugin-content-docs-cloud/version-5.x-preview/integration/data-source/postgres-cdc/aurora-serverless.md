---
{
  "title": "Aurora Serverless v2 PostgreSQL ソース設定",
...
Auroraでは、直接`postgresql.conf`を編集する代わりにパラメータグループを使用します。
...
```
もしくは、AWS Consoleでパラメータグループを編集し、`rds.logical_replication`を`1`に設定することで構成できます：
...
```
> **重要:** Auroraを使用する際は、Flink CDCに`--postgres-conf publication.name=velodb_publication`を渡してください。
...
**忘れずに追加してください** AuroraでFlink CDCを実行する際は`--postgres-conf publication.name=velodb_publication`を追加してください。
