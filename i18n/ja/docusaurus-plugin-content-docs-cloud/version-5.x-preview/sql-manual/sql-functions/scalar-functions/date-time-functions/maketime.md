...
{
  "title": "MAKETIME",
  "description": "hour、minute、secondの引数から構成された時刻値を返します。",
  "language": "ja"
}
---
## 説明

`hour`、`minute`、`second`引数から構成された時刻値を返します。

この関数はMySQLの[MAKETIME function](https://dev.mysql.com/doc/refman/8.4/en/date-and-time-functions.html#function_maketime)と一貫した動作をします。
...
`hour:minute:second`の形式で、TIME型の値を返します。入力の`seconde`が整数型の場合、出力値の精度は0となり、小数型の場合、出力値の精度は最大精度の6となります。

- `minute`または`second`が許可された範囲を超える場合、NULLを返します。
...
> 1. `sec`列タイプはFloatのため、すべての出力フォーマットは小数点以下6桁の時間値になります。
> 2. 1 - 2は通常の例です。
...
```
> 注意:
> `sec`の入力型は整数型のため、出力型はマイクロ秒を含まないすべての時間型になります。
