...
CUME_DIST (Cumulative Distribution) は、ソートされた結果セット内の現在の行の値の相対的なランキングを計算するウィンドウ関数です。結果セット内の現在の行の累積分布値を返し、その値は0から1の範囲になります。特定の行について、その累積分布値は次のように計算されます：（現在の行の値以下の行数）/（ウィンドウパーティション内の総行数）。
...
## 例
sales_person（営業担当者名）、sales_amount（売上金額）、sales_date（売上日）を含む売上データを格納したsalesというTableがあるとします。各売上日における各営業担当者の売上金額の累積パーセンテージを計算したいと思います。
...
+--------------+------------+--------------+-----------------------------+
```
この例では、CUME_DIST()関数が各販売日内で売上金額をソートし、その後その日付における各営業担当者の売上金額の累積パーセンテージを計算します。PARTITION BY sales_dateを使用したため、計算は各販売日に対して個別に実行され、営業担当者の売上金額は異なる日付に対して独立して計算されます。
