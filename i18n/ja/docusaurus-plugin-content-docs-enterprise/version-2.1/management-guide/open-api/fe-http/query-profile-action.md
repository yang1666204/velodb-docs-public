...
* `fragment_id`と`instance_id`
    ...
    両方が指定されていない場合、プロファイルのシンプルなツリーが返されます。これは`show query profile '/query_id'`と同等です。  
    両方が指定されている場合、詳細なプロファイルツリーが返されます。これは`show query profile '/query_id/fragment_id/instance_id'`と同等です。
