---
{
  ...
  "description": "struct データカラム内の特定のフィールドを返します。この関数は、フィールドの位置（インデックス）またはフィールド名を通じてstruct内のフィールドへのアクセスをサポートします。",
  "language": "ja"
}
---
## 説明
...
- `<struct>`: 入力する構造体カラム
- `<field_location_or_name>`: フィールドの位置（1から開始）またはフィールド名、定数のみサポート

## 戻り値

戻り値の型: 構造体でサポートされているフィールド値の型
...
- フィールドの位置（インデックス）によるアクセスをサポート、インデックスは1から開始
- フィールド名によるアクセスをサポート、フィールド名は完全に一致する必要がある
- 2番目のパラメータは定数である必要がある（カラムは使用不可）
- この関数はAlwaysNullableとしてマークされており、戻り値がnullになる可能性がある
...
```
Input struct が NULL の場合、エラーが報告されます：
