...
cursor = conn.cursor()
```
接続が完了すると、返されたCursorを使用してSQLを通じてDorisと連携し、テーブルの作成、メタデータの取得、データのインポート、クエリなどの操作を実行できます。
...
3. Q: ARM環境でエラー`get flight info statement failed, arrow flight schema timeout, TimeoutException: Waited 5000 milliseconds for io.grpc.stub.Client`が報告される。
...
12. Q: JDBCまたはJava JDBC ClientでArrow Flight SQLに接続してArray入れ子型を読み取るとエラー`Configuration does not provide a mapping for array column 2`が返される。
...
Python ADBC Client、JAVA ADBC Client、Java JDBC DriverManagerはすべてArray入れ子型の読み取りで問題ありません。JDBCまたはJava JDBC ClientでArrow Flight SQLに接続する場合のみ問題があります。実際、Arrow Flight JDBCの互換性は保証されておらず、Arrowが公式に開発したものではなく、サードパーティのデータベース会社Dremioによるものです。以前にも他の互換性問題が発見されているため、まずJAVA ADBC Clientの使用を推奨します。
