---
{
  ...
  "description": "この文は既存のリソースを変更するために使用されます。rootまたはadminユーザーのみがリソースを変更できます。",
  "language": "ja"
}
---
## 説明
...
`<property>` の形式は `<key>` = `<value>` で、`<key>` が 'type' と等しい場合の `<value>` の変更はサポートされていません。

変更されたプロパティパラメータは [CREATE-RESOURCE](./CREATE-RESOURCE.md) セクションで参照できます。


## Examples

1. spark0 という名前の Spark リソースの作業ディレクトリを変更する:
