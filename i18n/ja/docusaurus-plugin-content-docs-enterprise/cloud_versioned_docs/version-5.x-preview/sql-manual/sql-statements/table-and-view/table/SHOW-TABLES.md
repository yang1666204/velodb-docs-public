...
{
  "title": "SHOW TABLES",
  "description": "この文は、現在のdb配下にあるすべてのテーブルとビューを表示するために使用されます。",
  "language": "ja"
}
---
## 説明
...
**1. `FULL`**
> このパラメータをステートメントに追加すると、返される結果にTable_type（テーブルタイプ）、Storage_format（ストレージ形式）、Inverted_index_storage_format（転置インデックスストレージ形式）の3つの列が追加されます。
...
|:--------------|:-----------|:------------------|
| SELECT_PRIV | Table (Table), View (View) | クエリ権限を持つテーブルとビューのみ表示できます。 |
...
- テーブル名による曖昧検索
     ...
     +----------------+
     ```
- dbの下にあるテーブルとビューを照会するにはFULLを使用してください
