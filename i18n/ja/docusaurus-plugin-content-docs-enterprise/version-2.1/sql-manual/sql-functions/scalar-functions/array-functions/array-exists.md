...
{
  "title": "ARRAY_EXISTS",
  "description": "オプションのlambda式を入力パラメータとして使用し、他の入力ARRAYの内部データに対して対応する式計算を実行します。",
  "language": "ja"
}
---
## 説明
他の入力ARRAY パラメータの内部データに対して対応する式の計算を実行するために、オプションのlambda式を入力パラメータとして使用します。計算が0以外を返すときは1を返し、そうでなければ0を返します。
lambda式には1つまたは複数のパラメータが入力されており、これは後の入力配列列数と一致している必要があります。lambdaでは正当なスカラー関数を実行できますが、集約関数などはサポートされていません。

lambda式がパラメータとして使用されない場合は、array1が計算結果として使用されます。
...
- 配列が`NULL`値を含む場合、または配列自体が`NULL`の場合、結果は`NULL`になります。
