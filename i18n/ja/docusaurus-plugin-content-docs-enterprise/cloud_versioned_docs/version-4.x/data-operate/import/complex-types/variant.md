...
---
VARIANT型は半構造化JSONデータを格納でき、テーブル構造内で特定の列を事前定義することなく、異なるデータ型（整数、文字列、真偽値など）を含む複雑なデータ構造の格納を可能にします。この型は、いつでも変更される可能性のある複雑なネストした構造の処理に特に適しています。書き込みプロセス中に、VARIANT型は列の構造と型を自動的に推測し、書き込まれたスキーマを動的にマージし、JSONキーとそれに対応する値を列と動的サブ列として格納します。詳細なドキュメントについては、VARIANTを参照してください。
...
ロードされたデータを確認するために以下のSQLクエリを使用してください：
...
stream load を例として、以下のコマンドを使用してデータを読み込みます：
...
ロードされたデータを確認するために、以下のSQLクエリを使用してください：
...
スキーマ情報を表示するためにdesc コマンドを実行すると、サブカラムはストレージレイヤーで自動的に展開され、型推論が行われます。
...
```
DESC を使用して、特定のパーティションを指定し、そのパーティションのスキーマを表示することができます。構文は以下の通りです：
