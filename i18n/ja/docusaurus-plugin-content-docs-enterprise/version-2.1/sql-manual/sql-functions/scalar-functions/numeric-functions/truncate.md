...
{
  "title": "TRUNCATE",
  "description": "xをd桁の小数点以下に数値切り捨てを実行する",
  "language": "ja"
}
---
## 説明

xを小数点以下d桁の数値切り捨てを実行します
...
xをd桁の小数点以下で数値的に切り捨てを実行します。切り捨てルール：

dがリテラルの場合：

- d > 0の場合：xのd桁の小数点以下を保持します。
- d = 0の場合：xの小数部分を除去し、整数部分のみを保持します。
- d < 0の場合：xの小数部分を除去し、整数部分をdで指定された桁数に置き換え、数字0を使用します。
