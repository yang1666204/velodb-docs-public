---
{
  ...
  "description": "explodejsonarrayjson テーブル関数は JSON 配列を受け取ります。この配列の各要素は JSON オブジェクト型である必要があります。",
  "language": "ja"
}
---
## 説明

`explode_json_array_json`テーブル関数は、各要素がJSONオブジェクト型のJSON配列を受け取り、配列内の各JSONオブジェクトを複数の行に展開します。各行には1つのJSONオブジェクトが含まれます。この関数はLATERAL VIEWと組み合わせて使用されます。
