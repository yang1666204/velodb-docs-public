...
These properties apply to partitions, meaning that after a partition is created, it will have its own properties. Modifying table properties will only affect partitions created in the future and will not affect existing partitions. For more information about table properties, refer to ALTER TABLE PROPERTY. [Dynamic partitions](data-partitioning/dynamic-partitioning.md) can have these properties set individually.
