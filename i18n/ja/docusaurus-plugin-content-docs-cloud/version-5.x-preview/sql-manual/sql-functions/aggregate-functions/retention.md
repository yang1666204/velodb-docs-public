...
{
  "title": "RETENTION",
  "description": "retention関数は、特定の条件が満たされたかどうかを示すBool型の1から32個の引数による条件のセットを引数として受け取ります",
  "language": "ja"
}
---
## 説明

`retention`関数は、イベントに対して特定の条件が満たされたかどうかを示すBool型の1から32個の引数として条件のセットを受け取ります。任意の条件を引数として指定できます。

最初の条件を除いて、条件はペアで適用されます。2番目の結果は1番目と2番目が真の場合に真になり、3番目は1番目と3番目が真の場合に真になる、というように続きます。

簡単に言うと、戻り値配列の最初の桁は`event_1`が真か偽かを示し、2番目の桁は`event_1`と`event_2`の真偽を表し、3番目の桁は`event_1`が真か偽か、そして`event_3`が真か偽かを表す、といったように続きます。`event_1`が偽の場合、偽で満たされた配列を返します。
...
計算に複数の列が関与する場合、いずれかの列にNULL値が含まれていると、NULL値を含む現在の行は集計計算に参加せず、直接破棄されます。
NULL値を処理するには、計算列でIFNULL関数を使用できます。詳細については、後続の例を参照してください。

## Examples

1. サンプルテーブルを作成し、サンプルデータを挿入する
...
+------+
```
5. 計算にはflag列のみが関与します。uid = 0の場合にflagがtrueになるため、1を返します。
...
+-----------------+
```
6. 列flagとflag2が計算に関与する場合、flag2がNULLであるため、uid = 0の行は集計計算から除外されます。uid = 1の行のみが集計に参加し、結果として戻り値は0になります。
