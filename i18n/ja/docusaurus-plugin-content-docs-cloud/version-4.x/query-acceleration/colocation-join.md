...
Tableのデータは最終的に、バケット列の値のHashとバケット数のモデル化に従ってバケットに配置されます。Table内のバケット数が8であると仮定すると、`[0, 1, 2, 3, 4, 5, 6, 7]`の8つのバケット「Buckets」があります。このようなシーケンスを`Buckets Sequence`と呼びます。各Bucketには1つまたは複数のTabletがあります。Tableが単一パーティションTableの場合、BucketにはTabletが1つだけあります。複数パーティションTableの場合、複数存在します。
