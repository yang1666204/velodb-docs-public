...
{
  "title": "リーディングヒント",
  "description": "Leading Hintは、ユーザーがDorisオプティマイザーにクエリ内のTable結合順序を決定するよう指示することを可能にする強力なクエリ最適化技術です",
  "language": "ja"
}
---
Leading Hintは、ユーザーがDorisオプティマイザーをガイドして、クエリプラン内のTable結合順序を決定できる強力なクエリ最適化手法です。Leading Hintを正しく使用することで、複雑なクエリのパフォーマンスを大幅に向上させることができます。この記事では、DorisでLeading Hintを使用して結合順序を制御する方法について詳しく説明します。

## 通常のLeading Hint

### 構文

Leading Hintを使用すると、オプティマイザーが従うべきTable結合順序を指定できます。Dorisにおいて、Leading Hintの基本構文は以下の通りです：
...
- `tablespec`はTable名またはTableエイリアスであり、少なくとも2つのTableを指定する必要があります。
- 複数のTableはスペースまたは','で区切られます。
