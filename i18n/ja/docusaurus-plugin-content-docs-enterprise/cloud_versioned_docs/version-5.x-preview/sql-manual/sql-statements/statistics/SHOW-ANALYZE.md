...
> テーブル名。指定後、このテーブルに対応するジョブ情報を確認できます。指定されない場合、デフォルトですべてのテーブルのジョブ情報が返されます。

**3. `<job_id>`**

> 統計ジョブID。ANALYZEで非同期収集を実行する際に取得されます。IDが指定されない場合、このコマンドはすべてのジョブに関する情報を返します。

**4. `WHERE STATE = {"PENDING" | "RUNNING" | "FINISHED" | "FAILED"}`**

> ジョブ状態のフィルター条件。指定されない場合、デフォルトですべての状態のジョブに関する情報が表示されます。
