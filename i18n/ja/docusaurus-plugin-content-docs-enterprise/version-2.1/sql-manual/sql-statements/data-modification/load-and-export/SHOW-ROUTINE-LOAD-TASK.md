...
| TxnStatus            | サブタスクのインポートトランザクション状態。Nullはサブタスクがまだスケジュールされていないことを示す |
...
| DataSourceProperties | サブタスクが消費を準備しているKafka PartitionのStarting offset。JSON形式の文字列。KeyはPartition Id、Valueは消費のStarting offset |
...
| 権限 | オブジェクト | 備考 |
| :-------- | :----- | :---- |
| LOAD_PRIV | Table | SHOW ROUTINE LOAD TASKはTableのLOAD権限が必要 |
