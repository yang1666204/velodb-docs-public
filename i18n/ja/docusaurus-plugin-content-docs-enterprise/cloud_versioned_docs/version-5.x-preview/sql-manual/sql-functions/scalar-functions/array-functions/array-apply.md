...
指定されたバイナリ演算子を使用して配列要素をフィルタリングし、条件を満たす要素を含む新しい配列を返します。これは、ラムダ式の代わりに事前定義された演算子を使用する簡素化された配列フィルタリング関数です。
...
- `op`：STRING型、フィルタリング条件の演算子、定数値である必要があります。サポートされる演算子：`=`、`!=`、`>`、`>=`、`<`、`<=`
- `val`：T型、フィルタリング条件の値、定数値である必要があります
...
- 空配列：条件を満たす要素が存在しない場合

使用上の注意点：
- 演算子と条件値は定数である必要があり、カラム名や式は使用できません
- サポートされる型は限定的で、主に数値型、日付型、真偽値型です
...
double_array内の2より大きい要素をフィルタリング:
...
+------------------------------------------+
```
date_array内の指定された日付以上の要素をフィルタリングします：
