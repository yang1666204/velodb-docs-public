...
---
## Description
`explode_json_array_string_outer`テーブル関数はJSON配列を受け取ります。その実装ロジックは、JSON配列を配列型に変換し、その後処理のために`explode_outer`関数を呼び出すことです。この動作は`explode_outer(cast(<json_array> as Array<STRING>))`と同等です。
