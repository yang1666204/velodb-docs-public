...
{
  "title": "AGG_STATE",
  "description": "AGGSTATEはキーカラムとして使用できません。また、テーブル作成時に集約関数のシグネチャを同時に宣言する必要があります。",
  "language": "ja"
}
---
## AGG_STATE
### 説明
AGG_STATEはキー列として使用することはできず、テーブル作成時に集計関数のシグネチャを同時に宣言する必要があります。
...
**注意: agg_stateは集計関数の中間結果を格納するため、読み書きプロセスは集計関数の具体的な実装に強く依存します。Dorisのバージョンアップグレード時に集計関数の実装が変更された場合、非互換の状況が発生する可能性があります。非互換が発生した場合、対応するagg_stateを使用しているマテリアライズドビューを削除して再作成する必要があり、関連する基盤の集計テーブルは直接利用不可能になるため、agg_stateは慎重に使用する必要があります。**
