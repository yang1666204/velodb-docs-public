...
{
  "title": "負荷に基づくバッチ削除",
  "description": "delete操作は、データ更新の特殊な形式です。主キーモデル（Unique Key）Tableでは、",
  "language": "ja"
}
---
## Load に基づくバッチ削除

削除操作は、データ更新の特殊な形式です。主キーモデル（Unique Key）Tableにおいて、Doris はデータのロード時に削除サインを追加することで削除をサポートします。
...
- **Table構造**: 削除サインは主キーTable内の隠しカラム `__DORIS_DELETE_SIGN__` として格納されます。このカラムの値が 1 の場合、削除サインが有効であることを示します。
