...
{
  "title": "TIMESTAMPDIFF",
  "description": "date-diff関数と同じ機能を持ちます。TIMESTAMPDIFF関数は2つのdatetime値の差を計算し、結果を返します。",
  "language": "ja"
}
---
## Description

[date-diff function](./datediff)と同じ機能です。
TIMESTAMPDIFF関数は、2つのdatetime値の差を計算し、指定した時間単位で結果を返します。この関数は複数の時間単位（秒、分、時間、日、週、月、年など）をサポートします。

この関数は、MySQLの[date_diff function](https://dev.mysql.com/doc/refman/8.4/en/date-and-time-functions.html#function_date-diff)と一貫した動作をします。
...
|--------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `<unit>`                 | 時間単位。差分を返す際の単位を指定します。一般的な値には SECOND、MINUTE、HOUR、DAY、MONTH、QUARTER、YEAR などがあります。                                                                                                                                                                                                                                    |
| `<date_or_time_expr1>`   | 最初の日時、開始日時。date/datetime型の入力をサポートします。具体的な datetime と date の形式については、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion) と [date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion) を参照してください |
| `<date_or_time_expr2>`   | 2番目の日時、終了日時。date/datetime型の入力をサポートします                                                                                                                                                                                                                                                                                                       |

## Return Value

2つの日時の差分を返します。型はBIGINTです。
...
- 単位を計算する際、次の単位は無視されません。例えば、実際の差分が1日を満たすかを計算し、不足している場合は0を返します
- 月計算の特殊ケース：例えば、1-31から2-28は1か月の差になります
- date型を入力する場合、時刻部分はデフォルトで00:00:00になります
