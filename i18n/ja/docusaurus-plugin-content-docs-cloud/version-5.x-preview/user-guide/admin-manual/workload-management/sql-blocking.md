...
#### 指定した行数を超えるスキャンのブロック
データのスキャンはBEのIOおよびCPUリソースを大幅に消費します。そのため、不要なデータスキャンはクラスタの安定性に大きな課題をもたらします。日常的な使用では、`SELECT * FROM t`などの盲目的なフルテーブルスキャン操作がしばしば発生します。このようなクエリがクラスタに損害を与えることを防ぐため、単一のクエリが単一のテーブルでスキャンする行数の上限を設定できます。
...
過度に多くのpartitionをスキャンすると、BEのCPU消費量が大幅に増加する可能性があります。さらに、クエリが外部テーブルに対するものである場合、大幅なネットワークオーバーヘッドとメタデータ取得オーバーヘッドが発生する可能性が高くなります。日常の使用では、これはしばしばpartition列のフィルタリング条件を書き忘れたり、間違って書いたりすることが原因です。このようなクエリがクラスターに損害を与えることを防ぐため、単一テーブルでの単一クエリがスキャンするpartition数に上限を設定できます。
...
```
上記のルールが設定されている場合、単一テーブルに対してスキャンされるパーティション数が30を超えると、クエリの実行が禁止されます。
...
あまりにも多くのbucketをスキャンすると、BEのCPU消費量が大幅に増加する可能性があります。このようなクエリがクラスタに害を与えることを防ぐために、単一のクエリが単一テーブルでスキャンできるパーティション数に上限を設定できます。
