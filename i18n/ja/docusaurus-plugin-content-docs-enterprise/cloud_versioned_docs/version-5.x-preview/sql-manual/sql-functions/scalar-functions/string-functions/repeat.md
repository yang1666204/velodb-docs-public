---
{
  "title": "繰り返し",
  "description": "REPEAT関数は、指定された文字列を指定された回数繰り返して新しい文字列を生成するために使用されます。",
  "language": "ja"
}
---
## 説明

REPEAT関数は、指定された文字列を指定された回数繰り返して新しい文字列を生成するために使用されます。この関数は、パディング文字の生成、区切り文字の作成、またはテストデータの生成によく使用されます。

## 構文

```sql
REPEAT(<str>, <count>)
```
## パラメータ

| Parameter | Description |
| -- | -- |
| `<str>` | 繰り返す元の文字列。型: VARCHAR |
| `<count>` | 繰り返し回数、非負の整数である必要があります。型: INT |

## 戻り値

VARCHAR型を返し、指定された回数だけ繰り返された文字列を表します。

繰り返しルール:
- countが0より大きい場合、strをcount回繰り返した結果を返します
- countが0と等しい場合、空文字列を返します
- countが0未満の場合、空文字列を返します
- 結果の文字列が長すぎる場合、文字列長制限の対象となる可能性があります

特殊なケース:
- いずれかのパラメータがNULLの場合、NULLを返します
- strが空文字列の場合、countに関係なく空文字列を返します
- countが0の場合、空文字列を返します
- countが負の値の場合、空文字列を返します

## 例

1. 基本的な文字の繰り返し

```sql
SELECT REPEAT('a', 3);
```
```text
+----------------+
| REPEAT('a', 3) |
+----------------+
| aaa            |
+----------------+
```
2. 複数文字の文字列繰り返し

```sql
SELECT REPEAT('hello', 2);
```
```text
+--------------------+
| REPEAT('hello', 2) |
+--------------------+
| hellohello         |
+--------------------+
```
3. ゼロ回の繰り返し

```sql
SELECT REPEAT('test', 0);
```
```text
+-------------------+
| REPEAT('test', 0) |
+-------------------+
|                   |
+-------------------+
```
4. 負の繰り返し

```sql
SELECT REPEAT('a', -1);
```
```text
+-----------------+
| REPEAT('a', -1) |
+-----------------+
|                 |
+-----------------+
```
5. NULL値の処理

```sql
SELECT REPEAT(NULL, 3), REPEAT('a', NULL);
```
```text
+------------------+-------------------+
| REPEAT(NULL, 3)  | REPEAT('a', NULL) |
+------------------+-------------------+
| NULL             | NULL              |
+------------------+-------------------+
```
6. 空文字列の繰り返し

```sql
SELECT REPEAT('', 5);
```
```text
+----------------+
| REPEAT('', 5)  |
+----------------+
|                |
+----------------+
```
7. 特殊文字の繰り返し

```sql
SELECT REPEAT('-', 10), REPEAT('*', 5);
```
```text
+------------------+-----------------+
| REPEAT('-', 10)  | REPEAT('*', 5)  |
+------------------+-----------------+
| ----------       | *****           |
+------------------+-----------------+
```
8. UTF-8文字の繰り返し

```sql
SELECT REPEAT('ṭṛì', 3), REPEAT('ḍḍu', 2);
```
```text
+-------------------+-------------------+
| REPEAT('ṭṛì', 3)  | REPEAT('ḍḍu', 2)  |
+-------------------+-------------------+
| ṭṛìṭṛìṭṛì          | ḍḍuḍḍu            |
+-------------------+-------------------+
```
9. 混合された数値と記号

```sql
SELECT REPEAT('123', 3), REPEAT('@#', 4);
```
```text
+-------------------+------------------+
| REPEAT('123', 3)  | REPEAT('@#', 4)  |
+-------------------+------------------+
| 123123123         | @#@#@#@#         |
+-------------------+------------------+
```
10. UTF-8文字の繰り返し

```sql
SELECT REPEAT('ṭṛìṭṛì', 3);
```
```text
+--------------------------------------------------+
| REPEAT('ṭṛìṭṛì', 3)                              |
+--------------------------------------------------+
| ṭṛìṭṛìṭṛìṭṛìṭṛìṭṛì                               |
+--------------------------------------------------+
```
## 説明

REPEAT関数は、文字列を指定された回数だけ繰り返すために使用されます。

## 構文

```sql
REPEAT( <str>, <count> )
```
## パラメータ

| パラメータ | 説明                                                                                                               |
|-----------|---------------------------------------------------------------------------------------------------------------------------|
| `<str>`   | 繰り返される文字列。                                                                                                |
| `<count>` | 繰り返す回数。非負の整数である必要があります。1未満の場合、空文字列が返されます。 |

## 戻り値

指定された回数だけ繰り返された文字列を返します。特殊なケース：

- いずれかのパラメータがNULLの場合、NULLが返されます。

## 例

```sql
SELECT repeat("a", 3);
```
```text
+----------------+
| repeat('a', 3) |
+----------------+
| aaa            |
+----------------+
```
```sql
SELECT repeat("a", -1);
```
```text
+-----------------+
| repeat('a', -1) |
+-----------------+
|                 |
+-----------------+
```
