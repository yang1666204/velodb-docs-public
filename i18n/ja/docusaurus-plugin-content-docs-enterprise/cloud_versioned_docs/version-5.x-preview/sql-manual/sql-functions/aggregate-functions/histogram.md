...
histogram関数はデータの分布を記述するために使用されます。この関数は「等高」バケット戦略を使用し、データの値に応じてデータをバケットに分割します。各バケットを、そのバケットに該当する値の数など、いくつかの単純なデータで記述します。NULL以外のデータのみがカウントされます。
...
| `num_buckets` | オプション。ヒストグラムバケット数の上限。デフォルト値は128。サポートされる型：Integer。|
