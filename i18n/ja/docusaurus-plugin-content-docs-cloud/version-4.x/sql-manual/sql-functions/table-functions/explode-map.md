...
{
  "title": "EXPLODE_MAP",
  "description": "explodemap関数はmap（マッピング型）を受け取り、それを複数の行に展開します。各行にはキー値ペアが含まれます。",
  "language": "ja"
}
---
## 説明

`explode_map`関数はmap（マッピング型）を受け取り、それを複数の行に展開します。各行にはキーと値のペアが含まれます。この関数は通常LATERAL VIEWと組み合わせて使用され、複数のlateral viewをサポートできます。新しいオプティマイザーでのみサポートされています。

`explode_map`と`explode_map_outer`の主な違いは、null値の処理にあります。
...
mapが空でないかNULLでない場合、`explode_map`と`explode_map_outer`の戻り値は同じです。

データが空またはNULLの場合：

`explode_map`は空でないmap型のみを処理します。mapが空またはNULLの場合、`explode_map`は行を返しません。
`explode_map_outer`はmapが空またはNULLの場合、空またはNULLのmapを持つレコードを保持し、NULL値を持つ行を返します。
