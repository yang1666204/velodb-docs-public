---
{
    ...
    "description": "Doris-Operator supports mounting PV (Persistent Volume) on pods of various Doris components."
}
---
...
spec:
  feSpec:
    replicas: 3
    image: velodb/doris.fe-ubuntu:2.0.2
    limits:
      cpu: 8
      memory: 16Gi
    requests:
  ...
  beSpec:
    replicas: 3
    image: velodb/doris.be-ubuntu:2.0.2
    limits:
      cpu: 8
      memory: 16Gi
    requests:
...
spec:
  ...
  brokerSpec:
    replicas: 3
    image: velodb/doris.broker-ubuntu:2.0.2
    limits:
      cpu: 2
      memory: 4Gi
    requests:
...
This paragraph is for reference. Containerized deployment solutions that configure other files need to be placed in the conf directory of the Doris node. For example, the common HDFS/Hive configuration file mapping of [Data Lake Multi-catalog](https://doris.apache.org/docs/dev/lakehouse/catalogs/hive-catalog).
...
```
When using multiple disks, the path in the corresponding value of `storage_root_path` in `ConfigMap` should correspond to each mounting path of `persistentVolume` in `doriscluster`. [`storage_root_path`](https://doris.apache.org/docs/admin-manual/config/be-config/#storage_root_path) For the corresponding writing rules, please refer to the document in the link.
