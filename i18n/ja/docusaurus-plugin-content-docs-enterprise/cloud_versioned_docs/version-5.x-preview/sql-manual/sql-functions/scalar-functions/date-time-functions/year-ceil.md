...
{
  "title": "YEAR_CEIL",
  "description": "yearceil関数は、入力されたdatetime値を、指定された年間隔の開始時刻のうち最も近いものに切り上げます。間隔の単位は年です。",
  "language": "ja"
}
---
## 説明
...
year_ceil関数は、入力されたdatetime値を指定された年間隔の開始時刻まで切り上げる関数で、間隔単位は年です。originが指定されている場合はそれを基準として使用し、そうでなければデフォルトの0000-01-01 00:00:00を使用します。
...
|-----------|-------------|
| `<date_or_time_expr>` | 切り上げする日時値で、date/datetimeタイプをサポートします。datetimeおよびdateフォーマットについては、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)および[date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion)を参照してください |
| `<period>` | オプション、各期間が何秒で構成されるかを表し、正の整数タイプ（INT）をサポートします。デフォルトは1秒です。 |
| `<origin_datetime>` | 間隔の開始点で、date/datetimeタイプをサポートします。デフォルトは0000-01-01 00:00:00です。 |

## Return Value

入力タイプと一致する結果（DATETIMEまたはDATE）を返し、切り上げ後の年間隔開始時刻を表します：

- 入力がDATEタイプの場合、DATEタイプを返します（日付部分のみを含む）。入力がDATETIMEまたは適切にフォーマットされた文字列の場合、DATETIMEタイプを返します（時刻部分はoriginと一致し、originがない場合はデフォルトで00:00:00）。
- `<period>`が非正の整数（≤0）の場合、関数はエラーを返します。
- いずれかのパラメータがNULLの場合、NULLを返します。
- `<date_or_time_expr>`が間隔開始点と完全に一致する場合（`<period>`と`<origin>`に基づく）、その開始点を返します。
