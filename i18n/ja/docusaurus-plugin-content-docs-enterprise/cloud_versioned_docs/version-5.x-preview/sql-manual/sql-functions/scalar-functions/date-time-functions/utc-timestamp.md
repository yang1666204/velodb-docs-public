...
---
## 説明
UTC_TIMESTAMP関数は、UTCタイムゾーンでの現在の日付と時刻を返します。この関数はローカルタイムゾーンの影響を受けず、常にUTCタイムゾーンに基づいた現在時刻を返すため、異なるタイムゾーン環境間での時刻の一貫性を保証します。
...
DATETIME型を返します（形式：YYYY-MM-DD HH:mm:ss[.ssssss]）。返された結果を数値演算に使用する場合、[整数形式](../../../../sql-manual/basic-element/sql-data-types/conversion/int-conversion#from-datetime)（形式 YYYYMMDDHHmmss）に変換されます。
