...
COUNT_SUBSTRINGS関数は、文字列内で指定された部分文字列の出現回数をカウントします。注意：現在の実装では、マッチが見つかった場合、部分文字列の長さ分シフトした後に検索を続行します。例えば、str='ccc'でpattern='cc'の場合、返される結果は1です。
...
- strが空文字列の場合、0を返す
- start_posが0以下、または文字列長を超える場合、0を返す
