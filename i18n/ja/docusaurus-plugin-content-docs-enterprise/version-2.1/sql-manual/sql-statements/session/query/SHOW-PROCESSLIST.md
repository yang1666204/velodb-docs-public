...
クラスタ全体の接続情報を表示する必要がある場合は、セッション変数を追加する必要があります：
...
| Command          | このスレッドが現在実行しているコマンド                         |
| Time             | 前のコマンドが現在の状態に送信された時刻（秒単位）                 |
| State            | スレッドの状態                                |
| QueryId          | 現在のクエリ文のID                           |
| Info             | 一般的に、スレッドによって実行された文が記録され、デフォルトでは最初の100文字のみが表示されます         |

一般的なCommandタイプは以下の通りです：

| column | Instructions |
| -- | -- |
| Query | スレッドが文を実行しています |
| Sleep | クライアントが実行文を送信するのを待機しています |
| Quit | スレッドが終了しています |
| Kill | kill文を実行しています |
