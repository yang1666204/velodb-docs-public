---
{
  ...
  "description": "高頻度の小バッチ書き込みシナリオにおいて、従来のローディング方法には以下の問題があります：",
  "language": "ja"
}
---
## 1.ガイド概要
...
本ドキュメントは、**Apache Doris** から **VeloDB Cloud BYOC (Bring Your Own Cloud)** への移行に関する包括的で実用的なガイドを提供します。目標は、ユーザーが **自立して、低リスクで、高効率に** 移行を完了できるよう支援することです。

### 1.2 対象範囲

本ガイドでは以下のトピックを扱います：

1. VeloDB Cloud BYOC データウェアハウスの作成
2. メタデータの移行
3. データの移行
4. ジョブの移行
...
1. Apache Doris クラスターのアクセス認証情報と権限
2. VeloDB Cloud プラットフォームのアクセス認証情報と権限
3. パブリッククラウドプラットフォームのアクセス認証情報と権限
4. パブリッククラウド環境と VeloDB Cloud BYOC 間のネットワーク接続

## 2. VeloDB Cloud BYOC データウェアハウスの作成

### 2.1 前提条件

1. VeloDB Cloud で使用する基盤クラウドリソースの設定に必要なパブリッククラウド管理者アカウントを準備します。
2. VeloDB BYOC ウェアハウスとビジネス環境間のネットワーク接続を確保します。
   1. ウェアハウスとアプリケーションが異なる VPC にデプロイされている場合は、**VPC Peering** を通じて接続を確立することができます。
   2. VPC Peering が使用できない場合は、VeloDB BYOC ウェアハウスをビジネスアプリケーションと **同じ VPC 内** にデプロイすることを推奨します。

### 2.2 作成手順

1. VeloDB Cloud プラットフォーム管理者アカウントを準備してログインします。参考リンク：https://www.velodb.cloud/passport/login
2. VeloDB Cloud プラットフォームで請求設定を完了します。
   1. VeloDB Cloud BYOC アカウントがまだ無料トライアル期間内の場合は、この手順をスキップできます。
   2. 無料トライアルが終了している場合は、請求設定が必要です。参考リンク：https://docs.velodb.io/cloud/management-guide/usage-and-billing
3. VeloDB Cloud BYOC データウェアハウスを作成します。
   1. **Template Mode** がデータウェアハウス作成の推奨アプローチです。Template Mode は AWS、Azure、GCP で利用可能です（以下の参考表を参照）。
   2. **Wizard Mode** も利用可能ですが、現在は AWS でのみサポートされています（以下の参考表を参照）。
...
本ドキュメントでは主に以下のセクションを扱います：

1. テーブル、ビュー、マテリアライズドビューのメタデータ移行（同期・非同期両方）
...
1. Apache Doris クラスターとそのデータウェアハウスの **アクセス認証情報と読み取り権限**。
2. データ移行時の中間ストレージ場所として使用する、**VeloDB Cloud BYOC デプロイメントと同じリージョンの S3 バケット**。必要な認証情報には **S3 Access Key / Secret Key (AK/SK)** または適用可能な **IAM Role** が含まれます。
3. 対象データウェアハウスの読み取り/書き込み権限を持つ **VeloDB Cloud プラットフォームアカウント**。データインポート操作のために専用ユーザーを作成し、読み取り/書き込み権限を割り当てることができます。参考リンク：
...
1. 現在のメタデータ移行の対象範囲には、2つの主要カテゴリが含まれます：
   1. **テーブル、ビュー、マテリアライズドビュー（同期・非同期）**
   2. **ユーザーと権限**
2. 2つの移行アプローチが利用可能です。要件に基づいていずれかのオプションを選択できます。
   1. **手動移行ガイドライン**
   2. **移行スクリプトツール**
...
- **テーブル移行。** ソース Doris クラスターで `SHOW CREATE TABLE` コマンドを使用してテーブル DDL を取得します。必要な調整を行った後、対象 VeloDB Cloud 環境で対応するテーブルを作成します。参考：https://doris.apache.org/docs/dev/sql-manual/sql-statements/table-and-view/table/SHOW-CREATE-TABLE
- **ビュー移行。** ソース Doris クラスターで `SHOW CREATE VIEW` コマンドを使用してビュー DDL を取得します。必要に応じて DDL を調整し、VeloDB Cloud でビューを作成します。参考：https://doris.apache.org/docs/dev/sql-manual/sql-statements/table-and-view/view/SHOW-CREATE-VIEW
- **同期マテリアライズドビュー移行。** ソース Doris クラスターで `SHOW CREATE MATERIALIZED VIEW mv_name ON table_name` コマンドを使用して同期マテリアライズドビューの DDL を取得します。手動調整後、VeloDB Cloud で同期マテリアライズドビューを作成します。参考：https://doris.apache.org/docs/dev/sql-manual/sql-statements/table-and-view/sync-materialized-view/SHOW-CREATE-MATERIALIZED-VIEW
- **非同期マテリアライズドビュー移行。** ソース Doris クラスターで `SHOW CREATE MATERIALIZED VIEW mv_name` コマンドを使用して非同期マテリアライズドビューの DDL を確認します。DDL を適切に調整し、VeloDB Cloud で非同期マテリアライズドビューを再作成します。参考：https://doris.apache.org/docs/dev/query-acceleration/materialized-view/async-materialized-view/functions-and-demands#viewing-materialized-view-creation-statement
...
- 必要なすべてのロールは、`CREATE ROLE`を使用してVeloDBシステムで作成できます。参考: https://doris.apache.org/docs/dev/sql-manual/sql-statements/account-management/CREATE-ROLE/
- 必要なすべてのユーザーは、`CREATE USER`を使用してVeloDBシステムで作成できます。参考: https://doris.apache.org/docs/dev/sql-manual/sql-statements/account-management/CREATE-USER/
...
- 対応する権限は、`GRANT TO`を使用してVeloDBシステム内のユーザーおよびロールに付与できます。参考: https://doris.apache.org/docs/dev/sql-manual/sql-statements/account-management/GRANT-TO
...
```
### 3.4 データとLoad Jobのマイグレーション

#### 3.4.1 Load Jobのマイグレーション

##### 3.4.1.1 Dorisで内部管理されるLoad Jobのマイグレーション

Dorisで内部管理されるload jobには主に4つのタイプがあります：Routine Load、Broker Load、MySQL Load、およびデータレークの`INSERT INTO SELECT`です。VeloDB Cloudは4つのアプローチすべてに対応するサポートを提供し、構文の一貫性を保持しています。詳細については以下のリンクを参照してください：
...
###### 3.4.1.1.1 Routine Load Jobのマイグレーション

- DorisでRoutineloadを収集する

各テーブルに対応するRoutineload名のリスト、PAUSEおよびRESUME文を収集します。スクリプトを実行します
...
以下はデモで、各テーブルに対して生成されたRoutineload CREATE文を使用しています
...
停止したばかりの Doris の Routineload タスクを復元します

```Plain
RESUME ROUTINE LOAD FOR testdb.example_routine_load_csv;
```
##### 3.4.1.2 Doris外で管理される読み込みジョブの移行

Doris外で管理される読み込みジョブは多様で、ネイティブStream Load、ネイティブJDBC `INSERT INTO`、Flink、Spark、Kafka Connect、dbt、SeaTunnel、Kettle、DataX、その他のアプローチが含まれます。VeloDB Cloudはこれらすべての方法に対応したサポートを提供し、構文の一貫性を保ちます。移行には、既存の読み込みジョブとそのスケジューリング設定をコピーまたは再作成し、エンドポイントアドレスを更新してから、VeloDB Cloud上で実行するだけで済みます。詳細については、以下のリンクを参照してください：
...
1. DorisからS3などのオブジェクトストレージにデータをエクスポートする場合は、`SELECT INTO OUTFILE`をParquetファイル形式で使用して、テーブル単位またはパーティション単位で行うことを推奨します。
2. S3などのオブジェクトストレージからVeloDB Cloudにデータをインポートする場合は、TVFをParquetファイル形式で使用して、テーブル単位またはパーティション単位で行うことを推奨します。

###### 3.4.2.1.1 Dorisからのエクスポート

DorisからS3などのオブジェクトストレージにデータをエクスポートする場合は、`SELECT INTO OUTFILE`をParquetファイル形式で使用して、テーブル単位またはパーティション単位で行うことを推奨します。詳細については、以下を参照してください：https://doris.apache.org/docs/dev/data-operate/export/outfile#export-to-object-storage
...
S3などのオブジェクトストレージからVeloDB Cloudにデータをインポートする際は、テーブル単位またはパーティション単位でParquetファイル形式のTVFを使用することを推奨します。詳細については、以下を参照してください：https://docs.velodb.io/cloud/user-guide/data-ingestion/file-format/parquet#tvf-load
...
1. `query_timeout`パラメータを設定することで、エクスポートのタイムアウトを制御でき、大量のデータをエクスポートする際の失敗を防ぐことができます。`query_timeout`の単位は秒です。参考：https://doris.apache.org/docs/dev/data-operate/export/outfile#notice
2. データエクスポート用に専用のWorkload Groupを作成することで、`SELECT INTO OUTFILE`タスクがオンラインのDorisワークロードに与える影響を制御できます。参考：https://doris.apache.org/docs/dev/admin-manual/workload-management/workload-group
3. `insert_timeout`パラメータを設定することで、TVFインポートタスクのタイムアウトを制御できます。ほとんどの場合、設定する必要はありません。デフォルト値は14400秒（4時間）で、通常は十分です。参考：https://doris.apache.org/docs/dev/data-operate/import/import-way/insert-into-manual#data-size
4. VeloDB CloudはAK/SK認証に加えて、IAM RoleによるS3認証をTVFインポートでサポートしています。参考：https://docs.velodb.io/cloud/security/integrations/aws-authentication-and-authorization
5. Dorisの`SELECT INTO OUTFILE`エクスポートは、AK/SK認証に加えて、IAM RoleによるS3認証をサポートしています。参考：https://doris.apache.org/docs/dev/admin-manual/auth/integrations/aws-authentication-and-authorization#assumed-role-authentication

##### 3.4.2.2 ビジュアル移行

VeloDB Cloudは、管理プラットフォーム内で直接操作できるビジュアルなデータ移行機能も提供しています。詳細については、[Migration](https://docs.velodb.io/cloud/5.x-preview/management-guide/migration)を参照してください。

### 3.5 ETLジョブ

1. ETLジョブにおけるSQL互換性は、リプレイツールを使用して検証できます。原理は、DorisクラスターのETLジョブSQLをVeloDB Cloudウェアハウスで実行することです。
2. ETLジョブSQLに互換性がある場合は、ETLジョブとそのスケジュールを単純にコピーして再作成し、エンドポイントアドレスを更新してVeloDB Cloudで実行します。
3. 互換性の問題がある場合は、VeloDB公式技術チームにお問い合わせください。
...
システムパラメータについては、現在信頼できるツールはありません。移行後の検証フェーズで、ケースバイケースで調整・整合することを推奨します。

## 4. 移行後検証

### 4.1 データ整合性検証

1. `SELECT COUNT(*) FROM table`を使用して、データ移行後にDorisソーステーブルとVeloDB Cloudターゲットテーブルのレコード数が一致しているかを検証します。
2. `SELECT SUM(calculable_column) FROM table`を使用して、データ移行後にDorisソーステーブルとVeloDB Cloudターゲットテーブルの詳細データが完全に一致しているかを検証します。
...
ETLジョブやOLAP大規模クエリケースなど、典型的な大規模クエリケースとパフォーマンスメトリクスを収集し、ターゲットシステムVeloDB Cloudでの大規模クエリパフォーマンスがソースDorisシステムと一致しているかを検証します。

VeloDB Cloudでの大規模クエリパフォーマンスが悪化している場合は、VeloDB CloudとDoris間のシステムパラメータの違い、ホット/コールドクエリパターン、CPUとメモリリソース、その他の要因を除外する必要があります。問題がある場合は、VeloDB公式技術チームにお問い合わせください。

#### 4.3.2 クエリ同時実行ストレステスト検証

DorisクラスターからSQL文、QPS、90/95/99パーセンタイルレイテンシなどのクエリ同時実行ケースとパフォーマンスメトリクスを収集し、VeloDB Cloudでのクエリ同時実行パフォーマンスがソースDorisシステムと一致しているかを検証します。

クエリ同時実行ストレステストにはJMeterの使用を推奨します。

VeloDB Cloudでのクエリ同時実行パフォーマンスが悪化している場合は、VeloDB CloudとDoris間のシステムパラメータの違い、ホット/コールドクエリパターン、CPUとメモリリソース、その他の要因を除外する必要があります。問題がある場合は、VeloDB公式技術チームにお問い合わせください。
