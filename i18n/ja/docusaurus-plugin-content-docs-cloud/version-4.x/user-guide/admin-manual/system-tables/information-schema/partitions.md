---
{
  "title": "partitions | Information Schema",
  "description": "データベース内のすべてのTableのPartitionステータスを表示します。3.0.2(inclusive)以前では、Tableは常に空でした。",
  "language": "ja"
}
---
# partitions
...
データベース内の全TableのPartitionステータスを表示します。3.0.2(inclusive)以前では、Tableは常に空でした。
...
| PARTITION_ORDINAL_POSITION    | int           | パーティションの順序位置    |
...
| LOCAL_DATA_SIZE               | text	        | パーティションのローカルデータサイズ            |
| REMOTE_DATA_SIZE              | text          | パーティションのリモートデータサイズ (cloud)   |
| STATE                         | text	        | パーティション状態                      |
...
| LAST_CONSISTENCY_CHECK_TIME   | text          | 最後の整合性チェック時刻          |
| BUCKET_NUM                    | int           | パーティションバケット数 |
| COMMITTED_VERSION             | bigint        | 最後のコミット済みバージョン                |
