...
---
[Fluent Bit](https://fluentbit.io/)は、ストレージシステムにデータを書き込むためのカスタム出力プラグインをサポートする高速ログプロセッサ兼フォワーダーです。Fluent Bit Doris出力プラグインは、Dorisへの出力を行うためのプラグインです。

Fluent Bit Doris出力プラグインは、[Doris Stream Load](../../user-guide/data-operate/import/import-way/stream-load-manual) HTTP インターフェースを呼び出すことで、リアルタイムでDorisにデータを書き込みます。マルチスレッド並行処理、失敗時のリトライ、カスタムStream Loadフォーマットとパラメータ、出力書き込み速度などの機能を提供します。

Fluent Bit Doris出力プラグインを使用するには、主に3つのステップがあります：
1. Doris出力プラグインを含むFluent Bitバイナリプログラムをダウンロードまたはコンパイルする。
2. Fluent Bitの出力先アドレスとその他のパラメータを設定する。
...
https://github.com/joker-star-l/fluent-bit のdevブランチをクローンし、build/ディレクトリで以下のコマンドを実行します
...
`log_request` | トラブルシューティングのために Doris Stream Load リクエストとレスポンスのメタデータをログに出力するかどうか、デフォルトは true
`log_progress_interval` | ログに速度を出力する時間間隔、単位は秒、デフォルトは 10、0 に設定するとこのタイプのログを無効化できます
`retry_limit` | Doris Stream Load リクエスト失敗時のリトライ回数、デフォルト値は 1、false に設定するとリトライ回数を制限しません
`workers` | Doris Stream Load を実行する worker 数、デフォルト値は 2
...
FE ログファイルは通常、Doris インストールディレクトリ下の fe/log/fe.log ファイルに配置されています。これらは典型的な Java プログラムログで、タイムスタンプ、ログレベル、スレッド名、コード位置、ログ内容などのフィールドを含みます。通常のログだけでなく、複数行にわたるスタックトレースを含む例外ログも含まれています。ログの収集と保存では、メインログとスタックトレースを単一のログエントリに結合する必要があります。
...
GitHub events archiveには、GitHubユーザーアクションのアーカイブデータがJSON形式で含まれています。[こちら](https://data.gharchive.org/)からダウンロードできます。例えば、2024年1月1日午後3時のデータです。

```shell
wget https://data.gharchive.org/2024-01-01-15.json.gz
```
以下はデータのサンプルです。通常、各データは1行に記述されますが、表示を見やすくするために、ここではフォーマットされています。
