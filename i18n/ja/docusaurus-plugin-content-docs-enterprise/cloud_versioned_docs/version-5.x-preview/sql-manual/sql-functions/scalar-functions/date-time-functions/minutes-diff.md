...
| --------- | ----------- |
| `<date_or_time_expr1>` | 終了時間。DATEまたはDATETIME型を指定可能。具体的なdatetime/date形式については、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)および[date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion)を参照 |
| `<date_or_time_expr2>` | 開始時間。DATEまたはDATETIME型を指定可能。具体的なdatetime/date形式については、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)および[date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion)を参照 |
...
- 計算では実際の差を考慮し、秒やマイクロ秒を無視しません。
- 入力がDATE型の場合（年、月、日のみを含む）、時刻部分はデフォルトで00:00:00になります。
- 入力datetimeにscale、秒、またはマイクロ秒の部分が含まれ、それらが非ゼロの場合、計算時に無視されません。
