...
{
  "title": "Kafka",
  "description": "Apache KafkaからVeloDBへRoutine Loadを使用してデータをストリーミングするためのガイド",
  "language": "ja"
}
---
...
Routine Loadは、Kafkaトピックからメッセージを継続的に消費し、VeloDBテーブルに読み込む永続的なストリーミングジョブを作成します。
...
3. サンプルデータには、**JSON**フォーマットの**Datagen Source**コネクタを使用
...
- **PLAINTEXT**は内部/信頼できるネットワークで認証なし
...
- **Confluent Cloud**: **SASL/PLAIN**認証をAPI Key/Secretで使用します
- **その他のKafkaプロバイダー**: **SCRAM-SHA-512**を使用してください（SCRAM-SHA-256は使用しないでください。「Broker transport failure」エラーで失敗します）
:::
...
:::caution Important
証明書ファイルは、Routine Loadジョブを作成する**同じデータベース**で作成する必要があります。まずデータベースを作成または選択してから、ファイルを作成してください。
:::
...
この手順をスキップして、[対象テーブルの作成](#3-create-target-table-in-velodb)に進んでください。
...
```
または、ローカル証明書のアップロードについては[CREATE FILE](../../sql-manual/sql-statements/security/CREATE-FILE)を確認してください。
...
Confluent Cloud Consoleの**Datagen Source**コネクタを使用します：

1. クラスター内の**Connectors**に移動
...
4. 残りの手順で**Continue**をクリック
5. コネクタを起動

コネクタは自動的にトピックを作成し、サンプルメッセージを生成します。

:::caution Important
出力フォーマットとして**JSON**を選択してください。AVROおよびJSON_SRフォーマットはVeloDB Routine Loadでサポートされていません。
:::
...
VeloDBデータ型の完全な情報については、[Data Type Overview](../../sql-manual/basic-element/sql-data-types/data-type-overview)を参照してください。
...
```
:::tip
Routine Loadの`jsonpaths`パラメータは、JSONフィールドをテーブルカラムに順序通りにマッピングします。テーブルカラムが`jsonpaths`で指定された順序と一致していることを確認してください。
:::
...
`rpk cluster info`を実行してブローカーアドレスを確認するか、Redpanda Console → Overviewで確認できます。
:::
...
| `property.security.protocol` | クラウドKafkaの場合は`SASL_SSL`、認証なしの場合は`PLAINTEXT` |
| `property.sasl.mechanism` | Confluent Cloudの場合は`PLAIN`、その他の場合は`SCRAM-SHA-512`（SCRAM-SHA-256ではない） |
...
| `property.enable.ssl.certificate.verification` | Confluent Cloudの場合は`false`に設定（証明書不要） |
...
| "Authentication failed" | ユーザー名/パスワードを確認、ユーザーがトピック権限を持つことを確認 |
...
| ジョブの自動一時停止："max_error_number exceeded" | JSONとテーブル間のスキーマ不一致。`jsonpaths`がJSONフィールドと一致することを確認。エラーの一部を許容するために`"max_filter_ratio" = "0.5"`を追加 |
| ジョブステータスが`PAUSED`を表示 | `SHOW ROUTINE LOAD FOR <job_name>;`で理由を確認してから修正し、`RESUME ROUTINE LOAD FOR <job_name>;`を実行 |
...
- [Doris Kafka Connector](./doris-kafka-connector) (Kafka Connectを使用する代替方法)
