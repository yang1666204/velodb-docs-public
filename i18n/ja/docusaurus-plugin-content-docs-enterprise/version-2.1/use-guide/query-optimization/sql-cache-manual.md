...
複数テーブルの結合を含むクエリでは、テーブルの1つが更新された場合、パーティションIDまたはバージョン番号が異なるため、キャッシュミスが発生します。
...
Frontend（FE）がクエリリクエストを受信すると、まずSQL文字列を使用してメモリ内を検索し、同一のクエリが以前に実行されたかを判断し、そのクエリのメタデータ情報の取得を試みます。この情報にはクエリに関わるテーブルとパーティションのバージョンが含まれます。
...
6. Row PolicyまたはData Maskingの変更。特定のテーブルデータをユーザーに対して非表示に設定するなど。
