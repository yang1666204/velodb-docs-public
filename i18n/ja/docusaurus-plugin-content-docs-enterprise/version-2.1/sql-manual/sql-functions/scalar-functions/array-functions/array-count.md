...
lambda式には1つ以上のパラメータが入力され、後で入力する配列列数と一致している必要があります。すべての入力配列の要素数は同じでなければなりません。lambdaでは有効なスカラー関数を実行でき、集約関数などはサポートされていません。
...
ラムダ式を適用した後、ARRAYの非ゼロ要素の数を返します。該当する要素が見つからない場合は0を返します。
