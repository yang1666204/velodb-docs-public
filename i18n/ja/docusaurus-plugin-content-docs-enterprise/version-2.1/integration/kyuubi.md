...
{
  "title": "Kyuubi",
  "description": "Apache Kyuubiは、データウェアハウスとレイクハウス上でサーバーレスSQLを提供する分散型マルチテナントゲートウェイです。",
  "language": "ja"
}
---
# DorisでKyuubiを使用する
...
[Apache Kyuubi](https://kyuubi.apache.org/)は、データウェアハウスとLakehouse上でサーバーレスSQLを提供する分散型マルチテナントゲートウェイです。
Apache Kyuubiは、Spark、Flink、Hive、JDBCなどのエンジンに対して、Thrift、Trino、MySQLなどの様々なプロトコルを提供します。
DorisはApache KyuubiでサポートされているDoris dialectを使用してJDBCデータソースとして接続できます。
Apache Kyuubiは、HA、サービスディスカバリ、統合認証、エンジンライフサイクル管理などの便利な機能も提供します。

## 使用方法

### Apache Kyuubiのダウンロード

<https://kyuubi.apache.org/zh/releases.html>からApache Kyuubiをダウンロードします

Apache Kyuubi 1.6.0以上を取得し、フォルダに展開します。

### KyuubiデータソースとしてDorisを設定する

- `$KYUUBI_HOME/conf/kyuubi-defaults.conf`でKyuubi設定を更新します
...
| kyuubi.engine.jdbc.driver.class        | JDBCドライバクラス名、`com.mysql.cj.jdbc.Driver`を指定 |
...
MySQL JDBCドライバ`mysql-connector-j-8.X.X.jar`を`$KYUUBI_HOME/externals/engines/jdbc`にコピーします。

### Kyuubiサーバーの起動

`$KYUUBI_HOME/bin/kyuubi start`を実行します。
起動後、デフォルトでポート10009がKyuubiサーバーによってThriftプロトコルでリッスンされます。

## 例

以下の例は、beeline CLIでThriftプロトコルを使用してKyuubiでDorisをクエリする基本的な例を示しています。
