...
### 推奨シナリオ
#### 複雑な集約クエリ
- **シナリオ**：複数テーブルの結合、複雑な集約関数（例：SUM、AVG、COUNT）、またはウィンドウ関数を含むクエリ
...
- **シナリオ**：ファクトテーブルと複数のディメンションテーブルの結合、例：売上ファクトテーブルと商品、時間、地域ディメンションテーブルの結合
...
#### 単純なクエリ
- **シナリオ**：単一テーブルスキャンや単純なフィルタリングのみのクエリ
...
#### 小規模ソーステーブル
- **シナリオ**：ベーステーブルが少数のレコード（例：数百行）のみ含む
...
- ベーステーブルのリフレッシュ頻度が低いことが既知（例：数十分毎の変更）
