---
{
  "title": "PostgresからVeloDBにデータを取り込む",
  "description": "Flink CDCを使用してPostgreSQLからVeloDBへのリアルタイムCDCを行うためのガイド",
  "language": "ja"
}
---
...
アクティブなクラスターを持つ VeloDB Cloud アカウントが必要です。アカウントをお持ちでない場合は、[VeloDB Quick Start Guide](../../getting-started/quick-start) に従ってアカウントとクラスターを作成してください。
...
| ソースタイプ | セットアップガイド |
|-------------|-------------|
| セルフホスト PostgreSQL | [Local PostgreSQL Source Setup](./postgres-cdc/generic) |
...
- `max_wal_senders >= 1`
- `REPLICATION, LOGIN` 権限とテーブルに対する `SELECT` 権限を持つ CDC ユーザー
- Aurora の場合：`rds_replication`、`rds_superuser`、`pg_read_all_data` ロールを持つ CDC ユーザー
...
```
:::info
**DELETE操作をキャプチャする場合:** テーブルでREPLICA IDENTITY FULLを有効にしてください:
...
Flink CDCを実行する前に、**SQL Editor**を使用してVeloDBにターゲットデータベースを作成します：

```sql
CREATE DATABASE IF NOT EXISTS <your_velodb_database>;
```
:::info
データベースはVeloDBに存在する必要があります。テーブルはPostgreSQLスキーマに基づいてFlink Doris Connectorによって自動作成されます。
:::
...
- [Flink Doris Connector](https://docs.velodb.io/cloud/4.x/integration/data-source/doris-kafka-connector)
