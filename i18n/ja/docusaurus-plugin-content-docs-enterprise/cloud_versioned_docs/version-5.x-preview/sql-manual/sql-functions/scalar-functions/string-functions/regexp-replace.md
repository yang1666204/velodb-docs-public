---
{
  ...
  "description": "STR文字列の通常マッチングを行い、patternにヒットした部分を新しい文字列で置換します。",
  "language": "ja"
}
---
## Description

STR文字列の正規表現マッチングを行い、パターンにヒットした部分を新しい文字列で置換します。

文字セットマッチングを処理する際には、Utf-8標準文字クラスを使用する必要があることに注意してください。これにより、関数が異なる言語の様々な文字を正しく識別し、処理できることが保証されます。

`pattern`が許可されたregexp正規表現でない場合、エラーを投げます。
...
| -- | -- |
| `<str>` | このパラメータはVarchar型です。正規表現マッチングが実行される文字列を表します。リテラル文字列またはテーブル内の文字列値を含むカラムを指定できます。|
| `<pattern>` | このパラメータはVarchar型です。文字列とのマッチングに使用される正規表現パターンです。パターンには、複雑なマッチングルールを定義するための様々な正規表現メタ文字や構文を含めることができます。|
| `<repl>` | このパラメータはVarchar型です。<pattern>にマッチする<str>の部分を置換する文字列です。パターン内でキャプチャされたグループを参照する場合は、\1、\2などの後方参照を使用できます。\1は最初のキャプチャグループ、\2は2番目のキャプチャグループを表します。|

## Return Value

この関数は置換操作後の結果文字列を返します。戻り値はVarchar型です。<str>のどの部分も<pattern>にマッチしない場合、元の<str>が返されます。
...
+-----------------------------------+
```
キャプチャグループを使用します。ここでは、パターン内のグループ(b)によって文字'b'がキャプチャされ、その後置換文字列内の後方参照\1を使用して<b>に置換されます。
...
+----------------------------------------+
```
中国語文字のマッチング。この例では、文字列内のすべての連続する中国語文字を'123'に置換します。
...
+---------------------------------------------------------------------------------------------+
```
テストケースの挿入とテスト。このテストケースのセットでは、元の文字列、パターン、置換文字列を格納するテーブルを作成します。その後、さまざまなテストデータを挿入し、対応するパターンと置換文字列を使用して元の文字列にREGEXP_REPLACE操作を実行します。最後に、置換された文字列を取得して表示します。
...
+----------------------------------+
```
`pattern`が正規の正規表現として許可されていない場合、エラーをスローする；
