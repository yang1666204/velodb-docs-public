...
```
注意: 違いは、`http`メソッドが`http v2`メソッドよりも多くの`aggregation_type`フィールドを返すことです。`http v2`は`enable_http_server_v2`を設定することで有効になります。詳細なパラメータの説明については、[fe parameter settings](../../config/fe-config.md)を参照してください。
