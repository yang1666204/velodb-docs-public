...
配列が指定された値を含むかどうかをチェックします。見つかった場合はtrueを、そうでない場合はfalseを返します。配列がNULLの場合はNULLを返します。
...
- `value`：T 型、検索する値。配列要素の型と互換性がある型である必要があります。
...
1. 境界条件の動作:
   ...
   - 配列要素の型が検索値の型と一致しない場合は false を返す
   - 配列要素内の null 値について: null 要素は正常に処理され、配列内の null 要素を検索することができる

2. 例外値の動作:
   - 配列要素がサポートされていない型の場合はサポートされていないエラーを返す
...
1. **数値型の互換性**:
   ...
   - 小数型同士で比較可能 (DECIMAL32, DECIMAL64, DECIMAL128I, DECIMALV2, DECIMAL256)
   - 整数と浮動小数点数同士で比較可能
2. **文字列型の互換性**:
...
配列に特定の整数値が含まれているかを確認する: この例では、5がint_arrayに含まれていないため、falseを返します。
...
+------------------------------------------+
```
現在は空の配列です。この例では、空の配列に値が存在しないため、falseを返します。
...
```
配列がnullを含むかどうかを確認する
この例では、value_exprパラメータがnullで、配列にnull要素が存在しないため、falseを返します。
...
+---------------------------------+
```
配列にnullが含まれているかを確認する
この例では、value_exprパラメータがnullで、配列にSQLのnull値が含まれているため、trueを返します。
...
パフォーマンスに関する考慮事項：大きな配列を扱う際に、パフォーマンスが重要な懸念事項である場合、クエリの高速化のためにinverted indexesを使用できますが、注意すべき使用制限がいくつかあります：

1. 配列inverted indexを作成するためのプロパティは、non-tokenized indexのみ可能です
2. 配列の要素型Tは、inverted indexesの作成をサポートするデータ型である必要があります
3. クエリ条件パラメータTがNULLの場合、インデックスを高速化に使用できません
4. インデックス高速化は、関数がpredicate filter conditionとして使用される場合にのみ発生します
