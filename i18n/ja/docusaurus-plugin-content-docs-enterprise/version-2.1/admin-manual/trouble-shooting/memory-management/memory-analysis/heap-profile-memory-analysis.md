...
クラスターでのタスク実行中にHeap Profileのコールスタックでメモリ占有率の大きい`Agg`、`Join`、`Filter`、`Sort`、`Scan`などのフィールドが見られ、タスク完了後にメモリが正常に解放される場合、メモリの大部分は実行中のタスクによって使用されており、リークはないことを意味します。`Label=query, Type=overview` Memory Trackerの値が、Heap Profileで上記フィールドを含むメモリコールスタックよりも全体メモリに対する割合が小さい場合、`Label=query, Type=overview` Memory Trackerの統計が不正確であることを意味し、コミュニティで適時フィードバックを提供できます。
