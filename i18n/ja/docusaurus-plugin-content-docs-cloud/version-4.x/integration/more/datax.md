---
{
  ...
  "description": "DataX Doriswriterプラグインは、MySQL、Oracle、SQL Serverなどの様々なデータソースからDorisへのデータ同期をStreamを使用してサポートします...",
  "language": "ja"
}
---
# DataX Doriswriter

[DataX](https://github.com/alibaba/DataX) Doriswriterプラグインは、MySQL、Oracle、SQL Serverなどの様々なデータソースから、Stream Load方式を使用してDorisにデータを同期することをサポートします。

:::info Note
このプラグインはDataXサービスと併用する必要があります。
DataXは複数のデータソースをサポートしています。詳細はhereを参照してください。
:::
...
DataXはDataXがすでに含まれている公式インストールパッケージを提供しており、直接ダウンロードして使用できます。詳細は[here](https://github.com/alibaba/DataX?tab=readme-ov-file#download-datax%E4%B8%8B%E8%BD%BD%E5%9C%B0%E5%9D%80)を参照してください。

### DorisWriterプラグインを手動でコンパイル

DorisWriterプラグインの[source code](https://github.com/apache/doris/tree/master/extension/DataX)をダウンロードします。
...
3. doriswriterをビルド
    ...
    > DataXプロジェクト全体をコンパイルする必要がある場合は、[here](https://github.com/alibaba/DataX/blob/master/userGuid.md#quick-start)を参照してください
...
以下のソリューションを試すことができます：
        ...
        2. 解凍後、結果として得られる`alibaba/datax/`ディレクトリを、使用するmavenに対応する`.m2/repository/com/alibaba/`にコピーし、再度コンパイルを試してください。

### Datax DorisWriterパラメータの説明：

* **jdbcUrl**

  - 説明：DorisのJDBC接続文字列。ユーザーがpreSqlまたはpostSQLを実行します。
  - 必須：Yes
  - デフォルト：None
* **loadUrl**

  - 説明：Stream Loadの接続先として使用されます。形式は"ip:port"です。IPはFEノードのIP、portはFEノードのhttp_portです。複数を入力でき、英語のカンマ`,`で区切ります。doriswriterはポーリング方式でアクセスします。
  - 必須：Yes
  - デフォルト：None
* **username**
...
* **connection.selectedDatabase**
  - 説明：書き込む必要があるDorisデータベースの名前。
  - 必須：Yes
  - デフォルト：None
* **connection. table**
  - 説明：書き込む必要があるDorisTableの名前。
    - 必須：Yes
    - デフォルト：None
* **flushInterval**
  - 説明：データをバッチで書き込む時間間隔。この時間間隔を小さく設定しすぎると、Dorisの書き込みブロッキング問題（エラーコード-235）が発生し、この時間間隔を小さく設定しすぎて`maxBatchRows`と`batchSize`パラメータを大きく設定した場合、設定したデータサイズに到達できず、データがインポートされない可能性もあります。
  - 必須：No
  - デフォルト：30000 (ms)
* **column**
  - 説明：宛先Tableにデータを書き込む必要があるフィールド。これらのフィールドは生成されるJsonデータのフィールド名として使用されます。フィールドはカンマで区切られます。例："column": ["id","name","age"]。
  - 必須：Yes
  - デフォルト：No
* **preSql**

  - 説明：宛先Tableにデータを書き込む前に、ここの標準文が最初に実行されます。
  - 必須：No
  - デフォルト：None
* **postSql**

  - 説明：宛先Tableにデータを書き込んだ後に、ここの標準文が実行されます。
  - 必須：No
  - デフォルト：None


* **maxBatchRows**
  - 説明：インポートされるデータの各バッチの最大行数。**batchSize**と組み合わせて、バッチごとのインポートレコード行数を制御します。各バッチのデータが2つの閾値のいずれかに到達すると、このバッチのデータのインポートが開始されます。
  - 必須：No
  - デフォルト：500000

* **batchSize**
  - 説明：各バッチでインポートされるデータの最大量。**maxBatchRows**と連携して、バッチごとのインポート数を制御します。各バッチのデータが2つの閾値のいずれかに到達すると、このバッチのデータのインポートが開始されます。
  - 必須：No
  - デフォルト：94371840

* **maxRetries**

  - 説明：各バッチのデータインポートが失敗した後のリトライ回数。
  - 必須：No
  - デフォルト：3
...
* **labelPrefix**
  ...
  - 説明：各バッチのインポートタスクのラベルプレフィックス。最終的なラベルは`labelPrefix + UUID`の形式でグローバルに一意なラベルを形成し、データが重複してインポートされないことを保証します
  - 必須：No
  - デフォルト：`datax_doris_writer_`

* **loadProps**

  - 説明：StreamLoadのリクエストパラメータ。詳細については、StreamLoadの紹介ページを参照してください。[Stream load - Apache Doris](../../user-guide/data-operate/import/import-way/stream-load-manual)

    これにはインポートされるデータ形式：format等が含まれます。インポートされるデータ形式はデフォルトでcsvですが、JSONもサポートしています。詳細については、以下の型変換のセクションを参照するか、上記のStream loadの公式情報を参照してください。
...
#### 1. Streamがデータを読み取りDorisにインポート

doriswriterプラグインの使用方法については、[ここ](https://github.com/apache/doris/blob/master/extension/DataX/doriswriter/doc/doriswriter.md)を参照してください。

#### 2.Mysqlがデータを読み取りDorisにインポート
...
>2. `line_delimiter`はデフォルトで改行文字ですが、データ内の値と競合する可能性があります。インポートエラーを回避するために特殊文字や不可視文字を使用できます
>3. strip_outer_array : インポートデータのバッチ内の複数行データを表します。Dorisは解析時に配列を展開し、その中の各Objectを順番に1行のデータとして解析します。
>4. Stream loadのその他のパラメータについては、[Stream load - Apache Doris](../../user-guide/data-operate/import/import-way/stream-load-manual)を参照してください
>5. CSV形式の場合は、次のように使用できます
>
>```json
>"loadProps": {
>    "format": "csv",
...
>```
>
>**CSV形式では、データ内の特殊文字との競合を避けるため、行と列の区切り文字に特に注意する必要があります。ここでは不可視文字を推奨します。デフォルトの列区切り文字は: \t、行区切り文字: \n**

4.dataxタスクを実行します。詳細については[datax公式サイト](https://github.com/alibaba/DataX/blob/master/userGuid.md)を参照してください
