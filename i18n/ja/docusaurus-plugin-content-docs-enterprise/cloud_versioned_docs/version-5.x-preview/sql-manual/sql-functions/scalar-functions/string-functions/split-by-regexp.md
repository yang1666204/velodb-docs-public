...
SPLIT_BY_REGEXP関数は、指定された正規表現パターンに基づいて文字列を文字列の配列に分割します。SPLIT_BY_STRINGとは異なり、この関数はより柔軟な分割ルールのために複雑な正規表現マッチングをサポートします。オプションで最大分割回数制限をサポートしており、これは構造化テキスト処理、データクレンジング、およびパターンマッチングに有用です。
...
| パラメータ | 説明 |
|-----------|-------------|
| `<str>` | 分割する元の文字列。型: VARCHAR |
| `<pattern>` | 区切り文字として使用する正規表現パターン。型: VARCHAR |
| `<max_limit>` | オプションパラメータ、返される配列の要素数の上限を制限する。型: INT |

## 戻り値

ARRAY<VARCHAR>型を返し、正規表現によって分割された文字列の配列を表します。

分割ルール:
- 正規表現パターンを使用して分割ポイントをマッチする
- 標準的な正規表現構文をサポートする
- 空文字列パターンは文字列を個々の文字に分割する
- パターンが何にもマッチしない場合、元の文字列を含む単一要素配列を返す
- max_limitは結果配列の最大長を制限する

特殊ケース:
- いずれかのパラメータがNULLの場合、NULLを返す
- 文字列が空の場合、空文字列を含む単一要素配列を返す
- 正規表現が空文字列の場合、文字単位で分割する
- max_limitが0または負の値の場合、制限は適用されない
- 連続するマッチは空文字列要素を生成する
