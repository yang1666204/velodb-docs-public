...
{
  "title": "MINUTES_DIFF",
  "description": "MINUTESDIFF関数は、2つのdatetime値の間の分単位の差を計算します。",
  "language": "ja"
}
---
## 説明

MINUTES_DIFF関数は、2つのdatetime値の間の分単位での差分を計算します。結果は、終了時刻から開始時刻を減算して得られる分数です。この関数は、DATEおよびDATETIME（マイクロ秒精度を含む）タイプの処理をサポートしています。
...
| --------- | ----------- |
| `<date_or_time_expr1>` | 終了時間。DATEまたはDATETIME型を指定できます。具体的なdatetime/date形式については、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)および[date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion)を参照してください |
| `<date_or_time_expr2>` | 開始時間。DATEまたはDATETIME型を指定できます。具体的なdatetime/date形式については、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)および[date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion)を参照してください |

## Return Value

`<date_or_time_expr1>`と`<date_or_time_expr2>`の分単位の差分を表すINT型の整数を返します（1時間 = 60分）。

- `<date_or_time_expr1>`が`<date_or_time_expr2>`より後の場合、正の数を返します。
- `<date_or_time_expr1>`が`<date_or_time_expr2>`より前の場合、負の数を返します。
- 計算では実際の差分が考慮され、秒やマイクロ秒は無視されません。
- 入力がDATE型（年、月、日のみを含む）の場合、時刻部分はデフォルトで00:00:00になります。
- 入力されたdatetimeにscale、秒、またはマイクロ秒の部分がゼロ以外で含まれている場合、それらは計算中に無視されません。
