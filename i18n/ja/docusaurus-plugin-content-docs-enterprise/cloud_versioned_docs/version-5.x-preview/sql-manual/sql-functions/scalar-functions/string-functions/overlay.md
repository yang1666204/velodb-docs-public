---
{
  "title": "OVERLAY",
  "description": "OVERLAY関数は、文字列内の指定された位置と長さの部分文字列を置換するために使用されます。指定された位置から開始し、",
  "language": "ja"
}
---
## Description

OVERLAY関数は、文字列内の指定された位置と長さで部分文字列を置換するために使用されます。指定された位置から開始して、指定された長さの文字を新しい文字列で置換します。この関数はマルチバイト対応です。

この関数は、MySQLの[INSERT function](https://dev.mysql.com/doc/refman/8.4/en/string-functions.html#function_insert)と一貫した動作をします。

## Alias

- INSERT

## Syntax

```sql
OVERLAY(<str>, <pos>, <len>, <newstr>)
```
## パラメータ

| Parameter | Description |
| ---------- | ----------------------------------------- |
| `<str>` | 変更対象の元の文字列。型: VARCHAR |
| `<pos>` | 置換の開始位置（1ベース）。型: INT |
| `<len>` | 置換する文字数。型: INT |
| `<newstr>` | 置換に使用する新しい文字列。型: VARCHAR |

## 戻り値

VARCHAR型を返します。置換後の新しい文字列です。

特殊なケース:
- いずれかのパラメータがNULLの場合、NULLを返します
- `<pos>`が1未満または文字列長を超える場合、置換は行われず元の文字列を返します
- `<len>`が0未満または残りの長さを超える場合、`<pos>`から文字列の末尾まで置換します

## 例

1. 基本的な使用法: 中間部分の置換

```sql
SELECT overlay('Quadratic', 3, 4, 'What');
```
```text
+------------------------------------+
| overlay('Quadratic', 3, 4, 'What') |
+------------------------------------+
| QuWhattic                          |
+------------------------------------+
```
2. 負の長さ: 末尾まで置換

```sql
SELECT overlay('Quadratic', 2, -1, 'Hi');
```
```text
+-----------------------------------+
| overlay('Quadratic', 2, -1, 'Hi') |
+-----------------------------------+
| QHi                               |
+-----------------------------------+
```
3. 範囲外の位置: 置換なし

```sql
SELECT overlay('Hello', 10, 2, 'X');
```
```text
+-------------------------------+
| overlay('Hello', 10, 2, 'X')  |
+-------------------------------+
| Hello                         |
+-------------------------------+
```
4. NULL値の処理

```sql
SELECT overlay('Hello', NULL, 2, 'X');
```
```text
+--------------------------------+
| overlay('Hello', NULL, 2, 'X') |
+--------------------------------+
| NULL                           |
+--------------------------------+
```
```sql
SELECT INSERT('🎉🎊🎈', 2, 1, '🎁');
```
```text
+--------------------------------------+
| INSERT('🎉🎊🎈', 2, 1, '🎁')                 |
+--------------------------------------+
| 🎉🎁🎈                                     |
+--------------------------------------+
```
