...
>
> 識別子は文字（またはUnicode名前サポートが有効化されている場合はサポートされている言語の任意の文字）で始まる必要があり、識別子文字列全体がバッククォートで囲まれていない限り（例：My Object）、スペースや特殊文字を含むことはできません。
>
...
| IsUserDrop         | Boolean  | レプリカがユーザー主導の削除にマークされているかどうかを示します。                          |
...
| Privilege  | Object   | 注釈                                                                                                                            |
|:-----------|:---------|:---------------------------------------------------------------------------------------------------------------------------------|
| Admin_priv | Database | Table、パーティション、システムレベルコマンドの管理を含む、データベース上の管理操作を実行するために必要です。                                          |


## 例

- Tableの全レプリカのレプリカステータスを表示する
