...
- バケット化方法が指定されている場合、バケット数のみ変更可能で、バケット化方法やバケット化カラムは変更できません。バケット化方法が指定されているがバケット数が指定されていない場合、テーブル作成時に指定された数値ではなく、デフォルト値の`10`がバケット数として使用されます。バケット数を変更する場合、バケット化方法を同時に指定する必要があります。
- ["key"="value"]セクションでは、パーティションの属性を設定できます。詳細は[CREATE TABLE](./CREATE-TABLE)を参照してください
- テーブル作成時にユーザーが明示的にパーティションを作成しなかった場合、ALTERによるパーティション追加はサポートされていません
