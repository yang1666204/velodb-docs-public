...
CUME_DIST (Cumulative Distribution) は、ソート済みの結果セット内での現在行の値の相対的なランキングを計算するウィンドウ関数です。結果セット内の現在行の累積分布値を返し、その値は0から1の範囲になります。指定された行について、その累積分布値は次のように算出されます：（現在行の値以下の行数）/（ウィンドウパーティション内の総行数）。
...
## 例
sales_person（営業担当者名）、sales_amount（売上金額）、sales_date（売上日）を含む売上データが格納されたsalesというテーブルがあるとします。各売上日における各営業担当者の売上金額の累積パーセンテージを計算したいとします。
...
+------+--------------+------------+--------------+
```
上記のSQLクエリを実行すると、結果には各営業担当者の売上金額と、各販売日における累積パーセンテージランキングが表示されます：
...
+--------------+------------+--------------+-----------------------------+
```
この例では、CUME_DIST()関数が各販売日内で販売金額をソートし、その日付における各営業担当者の販売金額の累積パーセンテージを計算します。PARTITION BY sales_dateを使用したため、計算は各販売日ごとに個別に実行され、営業担当者の販売金額は異なる日付に対して独立して計算されます。
