---
{
  ...
  "description": "QUANTILEPERCENT関数は、指定されたパーセンテージの分位値を計算するために使用されます。",
  "language": "ja"
}
---
## 説明

`QUANTILE_PERCENT`関数は、指定されたパーセンテージに対するquantile値を計算するために使用されます。この関数は2つのパラメータを取ります：quantile_state列とパーセンテージを表す定数浮動小数点数です。この関数は、指定されたパーセンテージ位置でのquantile値を表す浮動小数点数を返します。
