...
| Paramters | デスクリプション |
| -- | -- | 
| `<date>` | 変換される datetime 値は `datetime` または `date` 型で、変換可能な範囲は '1970-01-01 00:00:01.000000 UTC' から '3001-01-19 03:14:07.999999 UTC' までです。|
| `<fmt>` | 'date' パラメータは、タイムスタンプに変換する必要がある特定の部分を指し、string 型のパラメータです。このパラメータが提供された場合、フォーマットに一致する部分のみがタイムスタンプに変換されます。 |

## Return value
入力に基づいて2つの型を返します：

- 入力 `date`（`datetime` 型のみスケールがゼロでない場合）のスケールが 0 でないか、またはフォーマットパラメータが提供されている場合、
最大6桁の小数点精度を持つ Decimal 型のタイムスタンプを返します。
- 入力 datetime のスケールが 0 で、フォーマットパラメータが提供されていない場合、
INT 型のタイムスタンプを返します。

- '1970-01-01 00:00:01.000000 UTC' より前または '3001-01-19 03:14:07.999999 UTC' より後の時刻の場合、関数は 0 を返します。
