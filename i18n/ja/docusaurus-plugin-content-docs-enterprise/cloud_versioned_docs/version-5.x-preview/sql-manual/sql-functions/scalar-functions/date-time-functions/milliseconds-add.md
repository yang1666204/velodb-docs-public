...
`MILLISECONDS_ADD`関数は、入力されたdatetime値に指定されたミリ秒数を加算し、結果として得られる新しいdatetime値を返します。この関数は、ミリ秒精度を持つ`DATETIME`型の処理をサポートします。
...
|--------------|-----------------------------------------------------------------------------------------------|
| `<datetime>` | 入力となる日時値。`DATETIME`型をサポートします。具体的な日時フォーマットについては、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)を参照してください。 |
| `<delta>`    | 追加するミリ秒数。型は`BIGINT`です。1秒 = 1,000ミリ秒 = 1,000,000マイクロ秒です。 |

## Return Value

`DATETIME`型の値を返します。これは、基準時刻に指定されたミリ秒を加算した結果を表します。

- `<delta>`が負の値の場合、この関数は基準時刻から対応するミリ秒を減算するのと同じ動作をします（つまり、`MILLISECONDS_ADD(basetime, -n)`は`MILLISECONDS_SUB(basetime, n)`と等価です）。
- 入力が`DATE`型の場合（年、月、日のみを含む）、デフォルトの時刻部分は`00:00:00.000`に設定されます。
- 計算結果が`DATETIME`型の有効範囲（`0000-01-01 00:00:00`から`9999-12-31 23:59:59.999999`）を超える場合、例外が発生します。
