...
---
Metabaseは、シンプルで使いやすいデータ分析・可視化機能を提供し、豊富なデータソース接続をサポートし、インタラクティブなダッシュボードの迅速な構築を可能にする、オープンソースのビジネスインテリジェンスツールです。その主要機能には、ユーザーフレンドリーなインターフェース、使いやすさ、セルフサービス分析のサポート、可視化ダッシュボードの作成、データドリルダウン探索、SQLクエリとデータエクスポートのための統合SQLクエリエディタが含まれます。
...
1. Metabaseバージョン0.48.0以降をダウンロードしてインストールしてください。詳細は[Metabase Installation Documentation](https://www.metabase.com/docs/latest/installation-and-operation/installing-metabase)を参照してください。
...
2. Metabaseプラグインディレクトリを作成してください（存在しない場合）：
...
MetabaseがDockerを使用して起動されている場合、`doris.metabase-driver.jar`をマウントして起動することが推奨されます。Dockerコンテナ内のプラグインパスは`/plugins/`です。
...
これでデータソースの設定は完了です！次に、Metabaseで可視化を構築できます。
...
コスト分析のため、異なる輸送方法の時間経過による注文金額の成長曲線を分析する必要があるとします。
...
2. 必要に応じてチャートパラメータを設定します（metabaseは自動的に以下のように設定します）：
   - **X軸**: ship_month（出荷月）
   - **Y軸**: revenue（収益）
...
4. チャートを設定した後、右上角の **Save** をクリックします。
...
2. **Add a chart** をクリックして保存した質問をダッシュボードに追加します。
...
これでMetabaseがApache Dorisに正常に接続され、データ分析と可視化ダッシュボードの作成が実現されました！
...
Metabaseは、SQLクエリで変数を使用することをサポートしており、インタラクティブなダッシュボードを簡単に作成できます：
...
1. **パーティション Clippingを使用する**: WHERE句にパーティション列のフィルタ条件を追加します。
   ...
   ```
2. **Materialized Viewの活用:** 複雑な集計クエリについては、DorisでMaterialized Viewを作成することで、クエリ処理を高速化できます。
