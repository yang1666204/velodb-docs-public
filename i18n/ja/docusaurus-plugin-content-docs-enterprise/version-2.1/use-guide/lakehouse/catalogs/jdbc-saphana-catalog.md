...
```
SAP HANA JDBC driverがサポートするJDBC URLの形式とパラメータの詳細については、[SAP HANA](https://help.sap.com/docs/)を参照してください。

## 階層マッピング

SAP HANAをマッピングする際、DorisのDatabaseはSAP HANAの指定されたDataBase下のSchemaに対応します（`jdbc_url`パラメータの"DATABASE"）。DorisのDatabase下のTableは、SAP HANAのSchema下のTablesに対応します。マッピング関係は以下の通りです：
