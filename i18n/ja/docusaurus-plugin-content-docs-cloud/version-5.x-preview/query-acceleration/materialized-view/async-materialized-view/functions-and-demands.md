...
- **`ON MANUAL` 手動トリガー**
  ...
  前回のリフレッシュ以降のベーステーブルパーティションデータの変更をチェックし、変更されたパーティションのみをリフレッシュします：

  ```sql
  REFRESH MATERIALIZED VIEW mvName AUTO;
  ```
:::tip
SQLのマテリアライズドビュー定義で使用されるベーステーブルがJDBCテーブルの場合、Dorisはテーブルデータの変更を感知することができません。マテリアライズドビューを更新する際は、COMPLETEを指定する必要があります。AUTOを指定した場合、ベーステーブルにデータがあるにもかかわらず、更新後にマテリアライズドビューが空になる可能性があります。現在、マテリアライズドビューの更新時において、Dorisは内部テーブルとHiveデータソーステーブルのデータ変更のみを感知することができます。その他のデータソースのサポートは段階的に実装されています。
:::

ベーステーブルの変更をチェックせずに、すべてのマテリアライズドビューパーティションを更新：
...
- **`ON COMMIT` 自動トリガー**
  ...
  ベーステーブルのデータが変更された際にマテリアライズドビューの更新を自動的にトリガーし、更新パーティションの範囲は「スケジュールトリガー」と一致します。

  例：ベーステーブル`lineitem`でパーティション`t1`のデータが変更された場合、対応するマテリアライズドビューのパーティション更新が自動的にトリガーされます：
    ...
    ```
:::caution
頻繁に変更されるベーステーブルには推奨されません。頻繁なマテリアライズド更新タスクが作成され、過剰なリソースを消費するためです。
:::
...
この例では、リフレッシュタイミングは作成時の即座リフレッシュ（`BUILD IMMEDIATE`）に設定され、リフレッシュ方法は完全リフレッシュ（`REFRESH COMPLETE`）に設定され、トリガー方法はトリガーリフレッシュ（`ON COMMIT`）に設定されています。`orders`または`lineitem`テーブルのデータが変更されると、マテリアライズドビューのリフレッシュが自動的にトリガーされます。
...
現在、複数のパーティションカラムをサポートしているのはHive外部テーブルのみです。Hive外部テーブルには、多くの場合、日付による第1レベルのパーティションと地域による第2レベルのパーティションなど、多くの多階層パーティションがあります。マテリアライズドビューは、Hiveのパーティションカラムの1つをマテリアライズドビューのパーティションカラムとして選択できます。
...
一部のベーステーブルには多くのパーティションがありますが、マテリアライズドビューは最近の期間の「ホット」なデータにのみ焦点を当てます。この機能はそれを可能にします。
...
ベーステーブル内のデータが集約される際、各パーティション内のデータ量が大幅に減少する可能性があります。この場合、パーティション集約戦略を採用して、マテリアライズドビューのパーティション数を削減できます。
...
#### Multi-PCT Refresh
「Multi-PCT Refresh」により、非同期マテリアライズドビューは複数のパーティション変更追跡（PCT）テーブルを持つことができます。つまり、複数のベーステーブルでデータ変更が発生した場合、フルリフレッシュではなく、パーティションレベルのリフレッシュのみが実行されます。
...
- 複数のPCTテーブル間でパーティション粒度が一致している必要があります：
    - **許可される例**：

      ベーステーブルt1のパーティション：[2020-01-01, 2020-01-02), [2020-01-02, 2020-01-03)

      ベーステーブルt2のパーティション：[2020-01-02, 2020-01-03), [2020-01-03, 2020-01-04)

      複数のベーステーブルのパーティションは完全に同一ではありませんが、重複していません。

    - **許可されない例**：

      ベーステーブルt1のパーティション：[2020-01-01, 2020-01-03), [2020-01-03, 2020-01-05)

      ベーステーブルt2のパーティション：[2020-01-01, 2020-01-02), [2020-01-03, 2020-01-05)
...
重要な点として、基盤となる内部ビューが変更または再構築された場合、非同期マテリアライズドビューとベーステーブル間でデータの不整合が生じる可能性があることに注意してください。このような場合、マテリアライズドビューのデータは依然として存在しますが、透過的なクエリリライトをサポートできません。

さらに、構造的な変更が非同期マテリアライズドビューが依存するパーティション追跡テーブルやカラムに影響を与えるか、スキーマの変更を引き起こす場合、マテリアライズドビューのリフレッシュは失敗します。変更がこれらの要素に影響しない場合、マテリアライズドビューはリフレッシュ後に正常な動作を再開できます。

## マテリアライズドビューの直接クエリ

マテリアライズドビューはテーブルのように扱うことができ、フィルタ条件と集計を追加して直接クエリできます。
...
JOIN書き換えとは、クエリとマテリアライズドビューが同じテーブルを使用し、条件がマテリアライズドビュー、JOIN入力、またはJOIN外部で記述できる場合を指します。オプティマイザーは、このパターンのクエリに対して透過的な書き換えを試行します。
...
- **State:** SCHEMA_CHANGEに変更された場合、ベーステーブルのスキーマが変更されたことを示します。マテリアライズドビューは透過的な書き換えに使用できません（ただし、直接クエリすることは可能です）。次回の成功したリフレッシュタスク後にNORMALに戻ります。
...
- **SyncWithBaseTables:** ベーステーブルと同期されているかどうか。1は同期済み、0は未同期を意味します。同期されていない場合、`show partitions`を使用してどのパーティションが同期外れかを確認してください。パーティション化されたマテリアライズドビューのSyncWithBaseTablesステータスの確認については、以下のセクションを参照してください。
...
- 0に設定すると、透過的なリライティングにはマテリアライズドビューとベーステーブルのデータ間で正確な整合性が必要です。

- 10に設定すると、マテリアライズドビューとベーステーブルのデータ間で最大10秒の遅延が許可されます。この10秒の時間枠内では、マテリアライズドビューは透過的なリライティングに使用できます。
  :::
...
| SET materialized_view_rewrite_enable_contain_external_table = true; | 外部テーブルを含むマテリアライズドビューが透明な書き換えに参加することを許可します（デフォルトで無効） |
| SET materialized_view_rewrite_success_candidate_num = 3; | CBO 候補で許可される成功した書き換え結果の最大数（デフォルト 3）。透明な書き換えが遅い場合は減らしてください |
| SET enable_materialized_view_union_rewrite = true; | パーティション化されたビューが必要なデータをすべて提供しない場合に、ベーステーブルとマテリアライズドビュー間での UNION ALL を許可します（デフォルトで有効） |
...
| SET enable_dml_materialized_view_rewrite_when_base_table_unawareness = true; | ビューがリアルタイムで追跡できない外部テーブルを含む場合の DML 中の構造ベースのマテリアライズドビュー透明書き換えを有効にします（デフォルトで無効） |
