...
| `<pos>` | 整数パラメータ。抽出する式グループのインデックスを示します。インデックスは1から開始します。<pos>が0に設定されている場合、最初にマッチした部分文字列全体が返されます。<pos>が負の数またはパターン内の式グループ数を超えている場合、関数はNULLを返します。 |
...
文字列型を返し、結果は`<pattern>`にマッチした部分になります。
 ...
 入力`<pos>`が無効（負の数または式グループ数を超えている）の場合、NULLを返します。
