...
{
  "title": "UPDATE",
  "description": "この文は、データを更新するために使用されます。UPDATE文は現在、UNIQUE KEYモデルのみをサポートしています。",
  "language": "ja"
}
---
## Description
...
UPDATE 操作は現在 Value カラムの更新のみをサポートしています。Key カラムの更新については Using FlinkCDC to update Key column を参照してください。
...
+ where condition: 更新対象となる条件。trueまたはfalseを返す式を指定可能

#### オプションパラメータ

+ cte: Common Table Expression。例：'WITH a AS SELECT * FROM tbl'
+ table_alias: テーブルのエイリアス
+ FROM additional_tables: 更新する行を選択するため、または新しい値を設定するために使用する1つ以上のテーブルを指定。ここで対象テーブルを使用したい場合は、明示的にエイリアスを付ける必要があります。
...
`test`テーブルはuniqueモデルテーブルで、k1、k2、v1、v2の4つのカラムを含んでいます。k1、k2はキー、v1、v2は値で、集約方法はReplaceです。

1. 'test'テーブル内でk1 =1、k2 =2の条件を満たすv1カラムを1に更新

```sql
UPDATE test SET v1 = 1 WHERE k1=1 and k2=2;
```
2. 'test'テーブルのk1=1列のv1列を1増加させる
