...
- MoWテーブルの値列に関する条件でmvの作成を禁止
- 動作の変更: MoWテーブルの値列に関する条件でmvを作成することが禁止されました
...
- Partition topn最適化では、すべてのwindow式が同じ順序である必要がある
...
- MaxCompute Catalog: MaxComputeカタログでのproject-schema-table構造の読み取りサポートを追加
- Paimon Catalog: DLF統合でPaimon RESTカタログのサポートを追加
- JDBC Catalog: DM（Dameng）データベースでnvarcharデータ型のサポートを追加

改善

- MTMV: ベーステーブルのスキーマ変更によって、ネストされたMTMVがスキーマ変更状態に入らないようになりました
- Routine Load: トランザクション失敗時にスケジュールを遅延させることでタスクスケジューリングを最適化
- String Performance: 文字列のシリアライゼーションとデシリアライゼーションのパフォーマンスを向上
- I/O Optimization: より良いI/Oチューニングのためにread slice sizeを設定可能にしました
...
- `variant`の`nested`属性はデフォルトで無効です。テーブル作成時に有効にするには、まずセッション変数で次のコマンドを実行する必要があります: `set enable_variant_flatten_nested = true`
...
- unique keyテーブルのポイントクエリにおけるパフォーマンスを改善
...
- 外部ビュークエリ時にビュー内のベーステーブル権限を不正にチェックする問題を修正
...
- Hive
  ...
  - HiveテーブルのプロパティーTMFSI `serialization.null.format`が正しく認識されない問題を修正
  
  - Hiveテーブルパーティション内の中国語による重複パーティションID問題を修正
...
- Hudi
  ...
  - JNIを使用したHudiテーブルパーティションカラム読み取り失敗を修正
  
  - 一部のケースでHudiテーブルTimestamp型パーティションカラムのクエリ失敗問題を修正
...
- Other
  ...
  - 特定のケースでExternal CatalogでDatabaseとTable IDが不正に生成される問題を修正
...
- 同名のテーブルとパーティションを同時に作成・削除する際にFEが終了する可能性のある問題を修正
...
- バックアップ中にパーティションまたはテーブルが削除される際のエラーを修正
...
- Query Optimizer
  ...
  - aggregateモデルテーブルの統計情報収集のサンプリング性能を最適化
...
- Primary Key Model
  - 高頻度同時インポート下でのmowテーブルのインポート性能を最適化
  - mowテーブルのfull compactionで削除データの領域を解放
  - 極端なシナリオでのmowテーブルの潜在的インポート失敗問題を修正
  - mowテーブルのcompaction性能を最適化
  - 同時インポートとスキーマ変更時のmowテーブルの潜在的正確性問題を修正
  - 空のmowテーブルでのスキーマ変更がインポートスタックやスキーマ変更失敗を引き起こす可能性のある問題を修正
...
- Storage
  ...
  - 単一カラムテーブルがリネームできない問題を修正
  - memtableの潜在的メモリリーク問題を修正
  - 空テーブル transaction writerでのサポートされていない操作の一貫性のないエラー報告問題を修正
- Storage-Compute Decoupled
  - File cacheに関する複数の修正
...
動作の変更
- Uniqueテーブルでの時系列Compactionの使用を禁止
- ストレージ・コンピュート分離シナリオで、デフォルトのAuto Bucketサイズを10GBに調整

新機能
- Lakehouse
  - AWS S3 Table BucketのIcebergテーブルフォーマットアクセスをサポート
- Storage
...
- Storage Optimization
  - Compaction ProfileとログをシンプルにSimplified 
...
- Lakehouse
  ...
  - Hiveテーブルプロパティ`skip.header.line.count`をサポート
  - `org.openx.data.jsonserde.JsonSerDe`フォーマット使用のHiveテーブルと互換
