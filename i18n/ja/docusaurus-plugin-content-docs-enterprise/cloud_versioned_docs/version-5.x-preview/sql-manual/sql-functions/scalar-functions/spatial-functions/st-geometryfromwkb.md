...
| パラメータ | 説明 |
| -- |---------|
| `<wkb>` | ジオメトリのバイナリ表現。通常はST_AsBinaryやその他のWKBを生成する関数によって生成される。サポートされるジオメトリタイプには、すべての標準タイプが含まれる：POINT、LINESTRING、LINE、POLYGON |

## 戻り値

入力されたWKBに対応するジオメトリオブジェクトを返す。入力がNULLまたは無効なWKB形式の場合はNULLを返す。
