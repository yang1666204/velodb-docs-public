...
---
## 説明
`explode_json_array_string`テーブル関数はJSON配列を受け取ります。その実装ロジックは、JSON配列を配列型に変換してから処理のために`explode`関数を呼び出すことです。動作は次と同等です：`explode(cast(<json_array> as Array<STRING>))`。
