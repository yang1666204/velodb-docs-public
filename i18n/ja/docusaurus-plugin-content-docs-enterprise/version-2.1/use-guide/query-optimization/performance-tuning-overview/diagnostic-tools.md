...
効率的で効果的なパフォーマンス診断ツールは、データベースシステムのチューニングにおいて極めて重要です。これらのツールによって、問題のあるビジネスSQL queryを迅速に特定し、その後パフォーマンスのボトルネックを素早く突き止めて解決できるかが決まり、データベースシステムがService Level Agreements（SLA）を満たすことが保証されます。

現在、DorisはデフォルトでSQL queryの実行時間が5秒を超えるものをslow SQLと見なします。この閾値は`config.qe_slow_log_ms`によって設定可能です。Dorisは現在、パフォーマンス問題を抱えるslow SQL queryを迅速に特定するために、以下の3つの診断チャネルを提供しています：

## Doris Manager Logs

Doris Managerのlogモジュールは、slow SQLフィルタリング機能を提供します。ユーザーは特定のFEノード上で`fe.audit.log`を選択することによってslow SQLを表示できます。検索ボックスに`slow_query`と入力するだけで、現在のシステムの履歴slow SQL情報がページに表示されます。下図に示すとおりです：

![Doris Manager Monitoring and Logging](/images/doris-manage-trace-log-2.png)

## Audit Log

現在、Doris FEは`slow_query`、`query`、`load`、`stream_load`を含む4種類のAudit Logを提供しています。Managerサービスがインストールおよびデプロイされているクラスタのlogページを通じてlogにアクセスする以外に、Audit LogはFEが配置されているノードの`fe/log/fe.audit.log`ファイルにアクセスすることで直接取得することも可能です。

`fe.audit.log`内で`slow_query`タグを直接検索することで、実行が遅いSQL queryを迅速にフィルタリングできます。以下に示すとおりです：
...
```
`fe.audit.log`を通じて取得されたスロークエリは、実行時間、スキャンした行数、返された行数、およびSQL文そのものなどの詳細情報に簡単にアクセスできるため、パフォーマンス問題のさらなる再現と特定の基盤を築きます。

さらに、Audit Logには`SqlDigest`フィールド（例：上記の例の`SqlDigest=...`）が含まれます。このフィールドは、SQL文の構造から生成されるハッシュ値です（具体的なパラメータ値は除去されます）。`slow_query`内の`SqlDigest`を集約・分析することで、スロークエリの「パターン」を特定できます。これは、具体的なSQL文がパラメータによってわずかに異なっていても、構造が同じである限り、それらの`SqlDigest`は同一になることを意味します。

`SqlDigest`を使用することで、ユーザーはどのSQLパターンが最も頻繁に現れるか、または最も時間を消費するかを判断でき、これらの「高頻度」または「高レイテンシー」パターンの最適化を優先できます。このアプローチは、個々のSQL文を一つずつ分析する非効率性を回避することで、スロークエリ最適化の効率を大幅に向上させます。

`SqlDigest`自体は単なるハッシュ値であり、直接読み取り可能ではないことに注意することが重要です。最適化すべきスロークエリパターンが特定されたら、Audit Logの`Stmt`フィールドを参照して、そのパターンに対応する具体的なSQL文の内容を取得する必要があります。さらに、`QueryId`フィールドを使用してクエリの詳細なProfile情報を取得し（Profileの取得と分析は後続のセクションで詳述されます）、詳細なパフォーマンス分析と最適化を行えます。

### スロークエリ分析例

上記の`fe.audit.log`内の4つのスロークエリログを例に取ると、次のことが観察できます：
...
これは、後者の3つのスロークエリが同じSQLパターン（構造的に同一）に属することを示していますが、それらの具体的な実行時間（`Time(ms)`）と詳細はわずかに異なります。

実際の最適化シナリオでは、特定の`SqlDigest`（`fec5a7136f9375aa968a4de971b994da`など）がスロークエリログに繰り返し現れる場合、または総実行時間の高い割合を占める場合、このパターンの最適化を優先すべきです。
...
1. **ビジネスロジックの特定**：このパターンの任意のログエントリの`Stmt`フィールドを調べることで：
    ...
    ```
特定のSQLロジックを識別することができます。

2.  **詳細分析**: ログから`QueryId`（例：`6e1fae453cb04d9a-b1e5f94d9cea1885`）を使用して、Dorisで対応するQuery Profileを取得します。Profileは、問題が過度なデータスキャンニング、長時間のJoin操作、またはその他の理由によるものかどうかを分析するのに役立ち、対象を絞った最適化戦略（インデックスの追加、SQLの記述の最適化、またはテーブル構造の調整など）を可能にします。

このパターンの問題が解決されると、このパターンに属するすべてのスロークエリが改善されます。

## audit_logシステムテーブル

Dorisバージョン2.1以降、`__internal_schema`データベース下でユーザーがSQLクエリの実行状況を確認できるよう`audit_log`システムテーブルが提供されています。使用する前に、グローバル設定`set global enable_audit_plugin=true`;を有効にする必要があります（このスイッチはデフォルトで無効になっています）。
...
Doris Managerログ、auditログ、および`audit_log`システムテーブルは、スローSQLクエリの自動または手動フィルタリング、およびSQL実行情報の細かい粒度での統計分析などの機能を提供します。これらのツールは、体系的なパフォーマンス診断とチューニングのための強力なサポートを提供します。
