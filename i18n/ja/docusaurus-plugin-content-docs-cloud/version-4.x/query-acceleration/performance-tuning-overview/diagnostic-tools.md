...
{
  "title": "診断ツール",
  "description": "効率的で効果的なパフォーマンス診断ツールは、データベースシステムのチューニングにおいて重要です。",
  "language": "ja"
}
---
## 概要

効率的で効果的なパフォーマンス診断ツールは、データベースシステムのチューニングにおいて重要です。これらのツールは、問題のあるビジネスSQLクエリを迅速に特定できるかどうか、そしてその後、パフォーマンスのボトルネックを迅速に特定し解決できるかどうかを決定し、データベースシステムがService Level Agreements (SLAs)を満たすことを保証します。

現在、Dorisはデフォルトで実行時間が5秒を超えるSQLクエリをslow SQLと見なします。この閾値は`config.qe_slow_log_ms`で設定できます。Dorisは現在、パフォーマンス問題のあるslow SQLクエリを迅速に特定するために、以下の3つの診断チャネルを提供しています：

## Doris Manager Logs

Doris Managerのログモジュールは、slow SQLフィルタリング機能を提供します。ユーザーは特定のFEノードで`fe.audit.log`を選択することでslow SQLを表示できます。検索ボックスに`slow_query`と入力するだけで、現在のシステムの過去のslow SQL情報がページに表示されます。以下の図に示すとおりです：

![Doris Manager Monitoring and Logging](/images/doris-manage-trace-log-2.png)

## Audit Log

現在、Doris FEは`slow_query`、`query`、`load`、`stream_load`を含む4種類のAudit Logsを提供しています。Managerサービスがインストールされデプロイされたクラスターのログページからログにアクセスする以外に、Audit LogsはFEが配置されているノードの`fe/log/fe.audit.log`ファイルに直接アクセスすることでも取得できます。

`fe.audit.log`で`slow_query`タグを直接検索することで、実行の遅いSQLクエリを迅速にフィルタリングできます。以下に示すとおりです：
...
```
`fe.audit.log`で取得されたスローSQLにより、ユーザーは実行時間、スキャンされた行数、返された行数、およびSQL文自体などの詳細情報に簡単にアクセスでき、パフォーマンス問題のさらなる再現と特定のための基盤を築きます。

さらに、Audit Logには`SqlDigest`フィールドが含まれています（例：上記の例の`SqlDigest=...`）。このフィールドは、SQL文の構造から生成されたハッシュ値です（特定のパラメータ値は削除されます）。`slow_query`で`SqlDigest`を集計・分析することで、スロークエリの「パターン」を特定できます。これは、特定のSQL文がパラメータによってわずかに異なっていても、構造が同じである限り、それらの`SqlDigest`は同一になることを意味します。

`SqlDigest`を使用することで、ユーザーはどのSQLパターンが最も頻繁に現れるか、または最も多くの時間を消費するかを判断でき、これらの「高頻度」または「高レイテンシ」パターンの最適化を優先できます。このアプローチにより、個別のSQL文を1つずつ分析する非効率性を回避し、スロークエリ最適化の効率を大幅に向上させます。

`SqlDigest`自体は単なるハッシュ値であり、直接読み取ることはできないことに注意することが重要です。最適化対象のスロークエリパターンが特定されたら、Audit Logの`Stmt`フィールドを参照して、そのパターンに対応する具体的なSQL文の内容を取得する必要があります。さらに、`QueryId`フィールドを使用してクエリの詳細なProfile情報を取得し（Profileの取得と分析については後続のセクションで詳述）、詳細なパフォーマンス分析と最適化を行うことができます。

### スロークエリ分析例

上記の`fe.audit.log`の4つのスロークエリログを例に取ると、以下のことが観察できます：

1. 最初のログ（`Time(ms)=11603`）は`SqlDigest`が`f23c7a7ecff61da33f537b2699e9b053`です。
2. 後続の3つのログ（`Time(ms)=8978`、`8514`、`8660`）はすべて`SqlDigest`が`fec5a7136f9375aa968a4de971b994da`です。

これは、後の3つのスロークエリが同じSQLパターン（構造的に同一）に属していることを示しており、特定の実行時間（`Time(ms)`）と詳細はわずかに異なります。

実際の最適化シナリオでは、特定の`SqlDigest`（`fec5a7136f9375aa968a4de971b994da`など）がスロークエリログに繰り返し現れる、または総実行時間の高い割合を占めることが判明した場合、このパターンの最適化を優先すべきです。
...
2.  **詳細分析**: ログから`QueryId`（例：`6e1fae453cb04d9a-b1e5f94d9cea1885`）を使用して、Doris内の対応するQuery Profileを取得します。Profileは、問題が過度なデータスキャン、長時間のJoin操作、またはその他の理由によるものかを分析するのに役立ち、対象を絞った最適化戦略（インデックスの追加、SQL記述の最適化、テーブル構造の調整など）を可能にします。

このパターンの問題が解決されると、このパターンに属するすべての遅いクエリが改善されます。

## audit_logシステムテーブル

Dorisバージョン2.1から、`__internal_schema`データベースの下で`audit_log`システムテーブルが提供され、ユーザーがSQLクエリの実行状況を確認できます。使用前に、グローバル設定`set global enable_audit_plugin=true`;を有効にする必要があります（このスイッチはデフォルトで無効になっています）。
...
+-------------------+--------------+------+-------+---------+-------+
```
`audit_log`内部テーブルを通じて、ユーザーは詳細なSQL実行情報をクエリし、スロークエリフィルタリングなどの詳細な統計分析を実行できます。

## 概要

Doris Managerログ、監査ログ、および`audit_log`システムテーブルは、スローSQLクエリの自動または手動フィルタリング、およびSQL実行情報の細粒度統計分析などの機能を提供します。これらのツールは、体系的なパフォーマンス診断とチューニングのための強力なサポートを提供します。
