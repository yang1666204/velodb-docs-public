...
---
## Description
JSON型データからjson_pathで指定されたフィールドを抽出します。
...
## Parameters
### Required Parameters:
- `<json_object>`: 抽出対象のJSON型式表現。
- `<path>`: 対象のJSONから目的の要素を抽出するためのJSONパス。
...
- `<path>`に対応する値が文字列の場合、返される文字列はダブルクォート（`"`）で囲まれます。ダブルクォートなしの結果を取得するには、関数[`JSON_UNQUOTE`](./json-unquote.md)を使用してください。
- `<path>`の構文は以下の通りです：
    * `$`はjsonのルートを表します
    ...
    * `[i]`はjson配列内のインデックスiの要素を表します
        * json_arrayの最後の要素を取得するには`$[last]`を使用でき、最後から2番目の要素は`$[last-1]`を使用でき、以下同様です。
    * `*`はワイルドカードを表し、`$.*`はルートオブジェクトのすべてのメンバーを表し、`$[*]`は配列のすべての要素を表します。
    * `**`は'$'と組み合わせて使用され、'$**'はすべてのパス（多層のサブパスを含む）を表します。
...
> 一致するものが1つしかない場合でも、配列形式で返されます
    ...
    +---------------------------------------------------------------------------------+
    ```
すべてのパスにマッチするものがない場合は、NULLを返します
