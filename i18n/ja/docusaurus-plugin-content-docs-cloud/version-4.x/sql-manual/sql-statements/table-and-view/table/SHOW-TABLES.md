...
**1. `FULL`**
> このパラメータをステートメントに追加すると、返される結果にTable_type（テーブルタイプ）、Storage_format（ストレージフォーマット）、Inverted_index_storage_format（転置インデックスストレージフォーマット）の3つの列が追加されます。
...
- テーブル名による曖昧検索
     ...
     +----------------+
     ```
- db配下のテーブルとビューをクエリするにはFULLを使用する
