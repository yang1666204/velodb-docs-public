...
## Hintの例
大量のデータを持つテーブルを想像してください。特定のケースにおいて、テーブルの結合順序がクエリパフォーマンスに影響を与える可能性があることを知っている場合があります。このような状況では、Leading Hintを使用してオプティマイザーに従わせたいテーブル結合順序を指定することができます。
...
+-----------------------------------------------------------------------------------------------------+
```
この例では、Leading Hint `/*+ leading(t2 t1) */` が使用されています。Leading Hintは、指定されたテーブル（t2）を駆動テーブルとして使用し、実行計画において（t1）よりも前に配置するようにオプティマイザに指示します。
