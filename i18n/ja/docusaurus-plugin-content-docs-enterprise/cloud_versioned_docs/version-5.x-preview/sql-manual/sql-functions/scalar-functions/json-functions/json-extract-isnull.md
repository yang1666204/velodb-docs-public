---
{
  ...
  "description": "JSONEXTRACTISNULL は、JSONオブジェクト内の<jsonpath>で指定されたフィールドがnull値であるかどうかを判定します。",
  "language": "ja"
}
---
## 説明
...
- `<json_path>`: String型、対象のJSONから対象要素を抽出するためのJSONパス。

## 戻り値
`Nullable(BOOL)` 値がnullの場合はtrueを返し、それ以外の場合はfalseを返します。
