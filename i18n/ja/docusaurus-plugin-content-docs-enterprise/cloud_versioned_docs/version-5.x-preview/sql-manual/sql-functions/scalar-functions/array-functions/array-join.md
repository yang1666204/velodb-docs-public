...
- `separator`：STRING 型、必須パラメータ、配列要素を区切るために使用される区切り文字
- `null_replacement`：STRING 型、オプションパラメータ、配列内のnull値を置き換えるために使用される文字列。このパラメータが提供されない場合、null値はスキップされます
