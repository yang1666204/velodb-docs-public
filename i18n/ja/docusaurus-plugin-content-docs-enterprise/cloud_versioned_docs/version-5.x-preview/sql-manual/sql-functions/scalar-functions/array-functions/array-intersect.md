...
複数の配列の積集合、つまりすべての配列に存在する要素を返します。この関数は、すべての入力配列に存在する要素を見つけ、重複を除去した後に新しい配列を形成します。
...
- `arr1, arr2, arr3, ...`：ARRAY<T>型、積集合を計算する配列。2つ以上の配列パラメータをサポートします。
...
- 空の配列: すべての入力パラメータ配列の間に共通の要素がない場合

使用上の注意:
- この関数はすべての入力配列に存在する要素を見つけ、結果配列内の要素は重複が除去されます
- 空の配列と任意の非NULL配列は空の配列になります。重複する要素がない場合、この関数は空の配列を返します。
- この関数はNULL配列をサポートしません
- 要素の比較は型互換性ルールに従います。型が互換性がない場合、変換が試行され、失敗するとnullになります
- 配列要素内のnull値について: null要素は操作において通常の要素として扱われ、nullはnullと同じものと見なされます
