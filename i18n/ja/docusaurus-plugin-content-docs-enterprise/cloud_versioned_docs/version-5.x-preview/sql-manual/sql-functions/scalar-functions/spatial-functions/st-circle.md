...
| -- | -- |
| `<center_lng>` | 円の中心の経度。DOUBLE型で、有効範囲は[-180, 180] |
| `<center_lat>` | 円の中心の緯度。DOUBLE型で、有効範囲は[-180, 180] |
| `<radius>` | 円の半径。DOUBLE型で、単位はメートル |


## 戻り値

地球の球面上の円を返します。型はGeoCircleです。そのWKT表現はCIRCLE ((<center_lng> <center_lat>), <radius>)で、中心座標と半径情報が含まれます。
