...
現在、Dorisは2つの独立したDistribute Hint、`[shuffle]`と`[broadcast]`をサポートしており、Joinにおける右側のテーブルのDistribute Typeを指定できます。Distribute Typeは、Joinにおける右側のテーブルの前に配置し、角括弧`[]`で囲む必要があります。また、DorisはLeading HintとDistribute Hintを組み合わせて使用することでshuffleモードを指定することができます（詳細については、[Reordering Join With Leading Hint](reordering-join-with-leading-hint.md)を参照してください）。
