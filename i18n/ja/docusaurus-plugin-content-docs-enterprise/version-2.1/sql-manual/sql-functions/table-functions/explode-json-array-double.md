...
JSON文字列自体がNULLの場合、`OUTER`版は値がNULLの1行を返します。通常版はそのようなレコードを完全に無視します。

JSON配列が空の場合、`OUTER`版は値がNULLの1行を返します。通常版は結果を返しません。
...
JSON配列を展開し、各要素に対して行を作成し、倍精度浮動小数点数の列を返します。
