...
4. Java UDTF：Java UDTFはユーザー定義テーブル関数で、単一の入力行から1つまたは複数の出力行を生成できます。Dorisでは、UDTFは行から列への変換を実現するためにLateral Viewと組み合わせて使用する必要があります。一般的な例としてEXPLODEやEXPLODE_SPLITがあります。**Java UDTFはバージョン3.0.0以降で利用可能です。**
