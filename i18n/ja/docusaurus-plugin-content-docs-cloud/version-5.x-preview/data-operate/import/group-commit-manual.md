---
{
  "title": "高同時実行LOAD最適化（Group Commit）",
  "description": "高頻度の小バッチ書き込みシナリオにおいて、従来のローディング手法には以下の問題があります：",
  "language": "ja"
}
---
高頻度小バッチ書き込みシナリオでは、従来のローディング方法に以下の問題があります：
...
これらの問題を解決するために、DorisはGroup Commitメカニズムを導入しました。Group Commitは新しいローディング方法ではなく、既存のローディング方法の最適化拡張であり、主に以下を対象としています：

- `INSERT INTO tbl VALUES(...)`文
- Stream Load

バックグラウンドで複数の小バッチロードを1つの大きなトランザクションコミットにマージすることで、高並行性小バッチ書き込みパフォーマンスを大幅に改善します。さらに、Group CommitをPreparedStatementと組み合わせて使用することで、より高いパフォーマンス向上を実現できます。
...
* Synchronous Mode（`sync_mode`）
    ...
    DorisはロードとTableの`group_commit_interval`プロパティに基づいて複数のロードを1つのトランザクションでコミットし、トランザクションコミット後に戻ります。これは、ローディング後にデータの即座の可視性を必要とする高並行性書き込みシナリオに適しています。

* Asynchronous Mode（`async_mode`）

    Dorisは最初にデータをWAL（Write Ahead ログ）に書き込み、その後すぐに戻ります。DorisはロードとTableの`group_commit_interval`プロパティに基づいて非同期でデータをコミットし、コミット後にデータを可視化します。WALがディスク容量を過度に占有することを防ぐため、大きな単一ロードに対しては自動的に`sync_mode`に切り替わります。これは書き込みレイテンシーに敏感で高頻度の書き込みシナリオに適しています。

    WAL数はFE httpインターフェース経由で確認するか、BEメトリクスで`wal`キーワードを検索することで確認できます。
...
ユーザーがJDBCの`insert into values`メソッドを使用して書き込みを行う際、SQLの解析と計画のオーバーヘッドを削減するため、FE側でMySQLプロトコルの`PreparedStatement`機能をサポートしています。`PreparedStatement`を使用する場合、SQLとその読み込み計画はセッションレベルのメモリキャッシュにキャッシュされ、後続の読み込みではキャッシュされたオブジェクトを直接使用することで、FEのCPU負荷を軽減します。以下は、JDBCで`PreparedStatement`を使用する例です：
...
```
**2. `group_commit` セッション変数を以下の2つの方法のいずれかで設定します:**

* JDBC url に `sessionVariables=group_commit=async_mode` を追加する方法
...
```
注意: 高頻度のinsert into文は大量の監査ログを出力し、最終的なパフォーマンスに影響するため、prepared statement監査ログの出力はデフォルトで無効になっています。セッション変数設定を通じて、prepared statement監査ログを出力するかどうかを制御できます。
...
Golangはprepared statementのサポートが限定的であるため、手動でクライアント側バッチ処理を行うことでGroup Commitのパフォーマンスを向上させることができます。以下はサンプルプログラムです：
...
データは時間間隔（デフォルト10秒）またはデータ量（デフォルト64MB）のいずれかの条件が満たされた時に自動的にコミットされます。これらのパラメータは一緒に使用し、実際のシナリオに基づいて調整する必要があります。
...
**パラメータ調整の推奨事項**:
- より短い間隔（例：2秒）：
  - 利点：データ可視性の遅延が少なく、高いリアルタイム性能が必要なシナリオに適している
  - 欠点：コミット数が多く、バージョンの増加が早く、バックグラウンドコンパクションの負荷が高い

- より長い間隔（例：30秒）：
  - 利点：コミットバッチが大きく、バージョンの増加が遅く、システムオーバーヘッドが低い
  - 欠点：データ可視性の遅延が高い

データ可視性遅延に対するビジネス許容度に基づいて設定することを推奨します。システム負荷が高い場合は、間隔を増加させることを検討してください。
...
- より小さな閾値（例：32MB）:
  ...
  - 短所: コミットバッチが小さくなる、スループットが制限される可能性がある
...
* **Group Commit制限**
  ...
  * `INSERT INTO VALUES`文は以下の場合にnon-Group Commitモードに退化します：
    ...
    - カラム更新書き込み
    - Tableがlightweight modeの変更をサポートしていない

  * `Stream Load`は以下の場合にnon-Group Commitモードに退化します：
    - 2フェーズコミットの使用
...
* **Unique Model**
  - Group Commitはコミット順序を保証しないため、データ整合性を確保するためにSequenceカラムの使用を推奨します。

* **WAL制限**
  - `async_mode`はデータをWALに書き込み、成功後に削除し、失敗時にはWALを通じて回復します。
  - WALファイルは1つのBE上に単一レプリカで保存されるため、ディスク破損や誤ったファイル削除によりデータ損失が発生する可能性があります。
  - BEノードをオフラインにする際は、データ損失を防ぐために`DECOMMISSION`コマンドを使用してください。
  - `async_mode`は以下の場合に`sync_mode`に切り替わります：
    - データ負荷量が大きすぎる（WAL単一ディレクトリ容量の80%を超過）
    - データ量不明のチャンクストリームロード
    - ディスク容量不足
  - 重量級Schema Change中は、Group Commit書き込みが拒否されるため、クライアントは再試行が必要です。

## パフォーマンス

`Stream Load`と`JDBC`（`async mode`）を使用して、小データ量の高並行シナリオにおけるgroup commitの書き込みパフォーマンスを別々にテストしました。
...
* `non group_commit`と`group_commit=async mode`モード間で、リクエストあたりの異なるデータサイズと並行性レベルでテスト。
...
上記のテストにおいて、BEのCPU使用率は10-40%の間で変動します。

`group_commit`は負荷パフォーマンスを効果的に向上させ、バージョン数を削減することで、compactionへの負荷を軽減します。
...
パフォーマンスを向上させるため、prepared statementの監査ログ出力を無効化。
...
* `txtfilereader`を使用して`mysqlwriter`にデータを書き込み、異なる並行性と`INSERT` SQL当たりの行数を設定。
...
上記のテストにおいて、BEのCPU使用率は10-20%、FEは60-70%の間で変動します。
...
5. インポートする値を設定します：
    ...
    インポートされる値がデータ型と一つずつ一致することを確認してください。
...
* 以下のテストは30、100、500の同時実行数に分かれています。
    ...
    **Syncモード、5 BEs、3レプリカでの30同時実行ユーザーのパフォーマンステスト**
    ...
    **Syncモード、5 BEs、3レプリカでの100同時実行ユーザーのパフォーマンステスト**
    ...
    **Syncモード、5 BEs、3レプリカでの500同時実行ユーザーのパフォーマンステスト**
