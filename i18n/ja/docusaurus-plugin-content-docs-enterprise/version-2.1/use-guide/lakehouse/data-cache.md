...
データキャッシュ機能は、Hive、Iceberg、Hudi、およびPaimonテーブルのクエリにのみ動作します。内部テーブルのクエリや非ファイル外部テーブルのクエリ（JDBCやElasticsearchなど）には効果がありません。
...
ユーザーはシステムテーブル`file_cache_statistics`を通じて、各BackendノードのキャッシュStatisticsを確認できます。
