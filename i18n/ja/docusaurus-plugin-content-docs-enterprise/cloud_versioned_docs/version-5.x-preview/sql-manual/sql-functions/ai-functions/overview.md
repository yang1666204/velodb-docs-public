...
AI Functionは、人工知能（AI）機能に基づいてDorisが提供する組み込み関数です。ユーザーはSQLクエリで直接AIを呼び出して、様々なインテリジェントなテキストタスクを実行できます。AI FunctionはDorisのリソースメカニズムを通じて、複数の主要なAIプロバイダー（OpenAI、Anthropic、DeepSeek、Gemini、Ollama、MoonShotなど）に接続します。

使用するAIは、Dorisによって外部提供され、テキスト解析をサポートしている必要があります。

---

## AI Resourceの設定

AI Functionを使用する前に、AI API のアクセス情報を一元管理するため、AIタイプのResourceを作成する必要があります。
...
`ai.temperature`: オプション。生成コンテンツのランダム性を制御します。0から1の間の浮動小数点値を受け入れます。
デフォルト値は-1で、このパラメータが設定されていないことを意味します。

`ai.max_tokens`: オプション。生成コンテンツの最大トークン数を制限します。
デフォルト値は-1で、このパラメータが設定されていないことを意味します。Anthropicのデフォルト値は2048です。

`ai.max_retries`: オプション。単一リクエストの最大リトライ回数。デフォルト値は3です。

`ai.retry_delay_second`: オプション。リトライ前の遅延時間（秒）。デフォルト値は0です。
...
そうでない場合、モデル呼び出しの失敗を引き起こす可能性があります。
...
- 明示的なリソース指定: 関数呼び出し時に直接リソース名を渡す。
- 暗黙的なリソース指定: 事前にSession変数を設定し、関数が自動的に対応するリソースを使用する。
...
現在、DorisでサポートされているAI Functionsには以下が含まれます：
...
- `AI_AGG`: 複数のテキストに対して行をまたいだ集約分析を実行
...
```
詳細な機能と使用方法については、各関数の個別のドキュメントを参照してください。
