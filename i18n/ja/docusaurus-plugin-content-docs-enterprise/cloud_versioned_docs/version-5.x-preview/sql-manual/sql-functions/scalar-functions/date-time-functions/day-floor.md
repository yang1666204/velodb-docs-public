...
{
  "title": "DAY_FLOOR",
  "description": "DAYFLOOR関数は、指定された日付または時刻の値を、最も近い指定された日期間の開始時刻まで切り下げるために使用されます。",
  "language": "ja"
}
---
## 説明

DAY_FLOOR関数は、指定された日付または時刻の値を、最も近い指定された日期間の開始時点まで切り捨てるために使用されます。この関数は、入力された日付と時刻以下である最大の期間時点を返します。期間ルールは、period（期間内の日数）とorigin（開始基準時刻）によって共同で定義されます。開始基準時刻が指定されない場合、計算基準として0001-01-01 00:00:00がデフォルトになります。
...
| `<period>` | 各期間の日数を指定します。INT型。負の値または0の場合はNULLを返します。指定されない場合、デフォルトの期間は1日です。 |
| `<origin>` | 期間計算の開始基準時刻。date/datetimeタイプをサポートします |

## Return Value

入力値を指定された日期間に切り下げた結果を表すdateまたはtime値を返します。

入力が有効な場合、datetimeタイプと一致する切り下げ結果を返します：

入力がDATEの場合、DATEを返します
入力がDATETIMEの場合、DATETIMEを返します（日付と時刻を含み、時刻部分は00:00:00になります。期間が日ベースのため）。

特殊ケース：

- いずれかのパラメータがNULLの場合、NULLを返します
- periodが負の値または0の場合、エラーを返します
- スケール付きのdatetime入力の場合、出力はすべての小数桁が0のスケールを持ちます
