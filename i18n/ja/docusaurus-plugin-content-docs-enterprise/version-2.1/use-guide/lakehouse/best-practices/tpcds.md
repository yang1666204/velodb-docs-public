---
{
  ...
  "description": "Dorisは、Trino Connector互換フレームワークを通じてTPCDS Connectorを使用し、TPCDSテストセットを迅速に構築することをサポートしています。",
  "language": "ja"
}
---
...
また、コンパイル済みの[trino-tpcds-435.tar.gz](https://github.com/morningman/trino-connectors/releases/download/trino-connectors/trino-tpcds-435.tar.gz)を直接ダウンロードして展開することも可能です。

## TPCDS Connectorのデプロイ

`trino-tpcds-435/`ディレクトリを、すべてのFEおよびBEのデプロイメントパスの`connectors/`ディレクトリ下に配置してください。（存在しない場合は手動で作成できます）
...
...
```
デプロイメント後、Connectorが正しく読み込まれることを確実にするため、FEとBEノードを再起動することを推奨します。
...
```
`tpcds.split-count`は並行処理数であり、最適な並行性を実現するためにBEマシンあたりのコア数の2倍に設定することが推奨されます。データ生成効率を向上させます。

## TPCDS Catalogの使用

TPCDS Catalogには、異なるScale Factorの事前構築されたTPCDSデータセットがあり、`SHOW DATABASES`と`SHOW TABLES`コマンドを使用して表示できます。
...
以下の例では、CTAS文を使用してHive上でTPCDSテストデータセットを素早く構築します：
