...
S3テーブル値関数（TVF）は、S3互換オブジェクトストレージに格納されたファイルを、リレーショナルテーブル形式のデータにアクセスするかのように読み取り、アクセスすることを可能にします。現在、`csv/csv_with_names/csv_with_names_and_types/json/parquet/orc`ファイル形式をサポートしています。
