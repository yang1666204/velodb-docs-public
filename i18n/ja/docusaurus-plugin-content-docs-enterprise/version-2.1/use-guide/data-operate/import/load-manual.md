...
- **リアルタイム書き込み**: HTTPまたはJDBC経由でリアルタイムにDorisTableにデータが書き込まれ、即座の分析とクエリが必要なシナリオに適しています。
...
- **ストリーミング同期**: リアルタイムデータストリーム（例：Flink、Kafka、トランザクションデータベース）をDorisTableにインポートし、リアルタイム分析とクエリに最適です。
...
- **バッチインポート**: 外部ストレージシステム（例：Object Storage、HDFS、ローカルファイル、NAS）からDorisTableにデータをバッチロードし、非リアルタイムデータインポートのニーズに適しています。
...
- **外部データソース統合**: 外部ソース（例：Hive、JDBC、Iceberg）からデータをクエリし、部分的にDorisTableにインポートします。
    ...
    - [カタログ](../../lakehouse/lakehouse-overview.md)を作成して外部ソースからデータを読み取り、[INSERT INTO SELECT](./import-way/insert-into-manual.md)を使用してこのデータをDorisに同期し、JOB経由で非同期実行することができます。
...
| [INSERT INTO SELECT](./import-way/insert-into-manual.md) | カタログ内のTableやObject Storage、HDFSのファイルなどの外部ソースからインポート。      | SQL                     | 同期、Job経由で非同期     |
