...
MONTHS_ADD関数は、入力されたdatetime値に指定された月数を加算し、結果として得られる新しいdatetime値を返します。この関数はDATEとDATETIME型の処理をサポートしています。負の数が入力された場合、対応する月数を減算することと同等です。

この関数は、[date_add function](./date-add)およびMONTHを単位として使用する場合のMySQLの[date_add function](https://dev.mysql.com/doc/refman/8.4/en/date-and-time-functions.html#function_date_add)と一致しています。
...
| --------- | ----------- |
| `<date_or_time_expr>` | 入力となる日時値。date/datetime型をサポートします。具体的なdatetimeおよびdate形式については、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)と[date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion)を参照してください。 |
| `<nums>` | 加算または減算する月数。INT型です。負の数は日時からnums分の月を減算することを示し、正の数はnums分の月を加算することを示します。 |

## Return Value

入力の`<date_or_time_expr>`と同じ型（DATEまたはDATETIME）の値を返します。これは、基準時刻に指定された月数を加算した結果を表します。

- `<nums>`が負の場合、この関数は基準時刻から対応する月数を減算するのと同じ動作をします（つまり、MONTHS_ADD(date, -n)はMONTHS_SUB(date, n)と等価です）。
- 入力がDATE型（年、月、日のみを含む）の場合、結果はDATE型のままです。入力がDATETIME型の場合、結果は元の時刻成分を保持します（例：'2023-01-01 12:34:56'は1ヶ月加算後に'2023-02-01 12:34:56'になります）。
- 入力日付が月の最終日で、対象月がその日付よりも少ない日数しかない場合、対象月の最終日に自動的に調整されます（例：1月31日に1ヶ月を加算すると、うるう年かどうかに応じて2月28日または29日になります）。
