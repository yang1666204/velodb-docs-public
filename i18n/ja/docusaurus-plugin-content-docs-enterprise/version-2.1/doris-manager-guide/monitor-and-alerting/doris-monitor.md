...
| :---------------- | :-------------------------- | :--------------------------------------------------- |
| Cluster Overview | FE Node | クラスター内の FE ノードの総数 |
...
| | BE Compaction Score | 各 BE のコンパクションスコア |
...
| | FE Collect Compaction Score | FE によって収集された各 BE のコンパクションスコア |
...
| | BE Compaction Base | 単位時間内に BE が実行したベースコンパクションタスクの実行率 |
| | BE Compaction Cumulate | 単位時間内に BE が実行した累積コンパクションタスクの実行率 |
...
| | Base Compaction | BE で実行された base_compaction タスクの総数 |
| | Cumulative Compaction | BE で実行された cumulative_compaction タスクの総数 |
...
* **Doris Dashboard Overview**：基本的な Doris とホストモニタリング項目を提供する事前定義済み Doris モニタリングダッシュボード。変更不可。

* **Default Custom Doris Dashboard Overview**：変更可能なユーザー定義モニタリングダッシュボード。

新しいダッシュボードを作成する場合、**Default Custom Doris Dashboard Overview** パネルを変更してカスタムダッシュボードを追加できます。

1. **「Default Custom Doris Dashboard Overview」ダッシュボードを選択**

    モニタリングページの左上で、「Default Custom Doris Dashboard Overview」パネルを選択します：
...
ユーザー設定で「Service Configuration」を選択して、モニタリングとアラートサービスを有効または無効にします。
