...
{
  "title": "REVERSE",
  "description": "REVERSE関数は、入力シーケンスの順序を逆にするために使用されます。文字列引数の場合、文字の順序を逆にした文字列を返します。",
  "language": "ja"
}
---
## 説明

REVERSE関数は、入力シーケンスの順序を逆にするために使用されます。文字列引数の場合、文字の順序を逆にした文字列を返します。配列引数の場合、要素の順序を逆にした配列を返します。この関数は文字単位で逆順にし、UTF-8マルチバイト文字を正しく処理できます。
...
- 文字列入力: VARCHAR 型、文字順序が反転された文字列を返す
- 配列入力: ARRAY 型、要素順序が反転された配列を返す

特殊なケース:
- パラメータが NULL の場合、NULL を返す
- 文字列が空の場合、空文字列を返す
- 配列が空の場合、空配列を返す
- 単一文字の文字列は反転後も同じまま
