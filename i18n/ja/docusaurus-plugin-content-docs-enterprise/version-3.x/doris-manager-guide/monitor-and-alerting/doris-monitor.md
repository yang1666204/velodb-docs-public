...
| :---------------- | :-------------------------- | :--------------------------------------------------- |
| Cluster Overview  | FE Node                     | クラスター内のFEノードの総数                                    |
...
|                   | BE Compaction Score         | 各BEのCompactionスコア                                   |
...
|                   | FE Collect Compaction Score | FEによって収集された各BEのCompactionスコア                      |
...
|                   | BE Compaction Base          | 単位時間内にBEによって実行されるbase compactionタスクの率            |
|                   | BE Compaction Cumulate      | 単位時間内にBEによって実行されるcumulative compactionタスクの率      |
...
|                   | Base Compaction             | BE上で実行されたbase_compactionタスクの総数                    |
|                   | Cumulative Compaction       | BE上で実行されたcumulative_compactionタスクの総数              |
...
* **Doris Dashboard Overview**: 基本的なDorisおよびホスト監視項目を提供する事前定義済みのDoris監視ダッシュボードで、変更はできません。

* **Default Custom Doris Dashboard Overview**: 変更可能なユーザー定義の監視ダッシュボードです。

新しいダッシュボードを作成する際は、**Default Custom Doris Dashboard Overview** パネルを変更してカスタムダッシュボードを追加できます。

1.  **「Default Custom Doris Dashboard Overview」ダッシュボードの選択**

    監視ページの左上隅で、「Default Custom Doris Dashboard Overview」パネルを選択します：
...
ユーザー設定で、「Service Configuration」を選択して監視およびアラートサービスを有効または無効にします。
