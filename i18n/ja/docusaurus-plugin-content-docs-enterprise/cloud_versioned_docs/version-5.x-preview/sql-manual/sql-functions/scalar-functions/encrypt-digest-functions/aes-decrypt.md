...
AES復号化関数。この関数は、MySQLの`AES_DECRYPT`関数と同じ動作をします。デフォルトのアルゴリズムは`AES_128_ECB`で、パディングモードは`PKCS7`です。
...
| `<init_vector>`     | アルゴリズムで使用される初期ベクトルです。特定のアルゴリズムの下でのみ有効です。指定されていない場合、Dorisは組み込みベクトルを使用します                                                                                                                                                          |
