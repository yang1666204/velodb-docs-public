---
{
  ...
  "description": "DataX Doriswriterプラグインは、MySQL、Oracle、SQL Serverなど、さまざまなデータソースからのデータ同期をサポートしています。",
  "language": "ja"
}
---
# DataX Doriswriter
...
:::info Note
このプラグインはDataXサービスと併せて使用する必要があります。
DataXは複数のデータソースをサポートしています。詳細についてはこちらを参照してください。
:::
...
DataXは、DataXが既に含まれている公式インストールパッケージを提供しており、直接ダウンロードして使用できます。詳細については、[こちら](https://github.com/alibaba/DataX?tab=readme-ov-file#download-datax%E4%B8%8B%E8%BD%BD%E5%9C%B0%E5%9D%80)を参照してください。

### DorisWriterプラグインの手動コンパイル

DorisWriterプラグインの[ソースコード](https://github.com/apache/doris/tree/master/extension/DataX)をダウンロードしてください。

1. `init-env.sh`を実行
2. 必要に応じて`DataX/doriswriter`内のdoriswriterのコードを変更
3. doriswriterをビルド
    ...
    > DataXプロジェクト全体をコンパイルする必要がある場合は、[こちら](https://github.com/alibaba/DataX/blob/master/userGuid.md#quick-start)を参照してください
...
以下のソリューションを試すことができます：
        ...
        1. [alibaba-datax-maven-m2-20210928.tar.gz](https://doris-thirdparty-repo.bj.bcebos.com/thirdparty/alibaba-datax-maven-m2-20210928.tar.gz) をダウンロードする
        2. 解凍後、生成された `alibaba/datax/` ディレクトリを、使用するmavenに対応する `.m2/repository/com/alibaba/` にコピーし、再度コンパイルを試行する。

### Datax DorisWriter パラメータ紹介：

* **jdbcUrl**

  - 説明： DorisのJDBC接続文字列。ユーザーはpreSqlまたはpostSQLを実行する。
  - 必須： はい
  - デフォルト： なし
* **loadUrl**

  - 説明： Stream Loadの接続ターゲット。形式は"ip:port"。ここでIPはFEノードのIP、portはFEノードのhttp_port。複数の値をカンマ `,` で区切って入力することができ、doriswriterはポーリング方式でアクセスする。
  - 必須： はい
  - デフォルト： なし
* **username**
...
* **connection. table**
  - 説明： 書き込みが必要なDorisテーブルの名前。
    ...
    - デフォルト： なし
* **flushInterval**
  - 説明： データをバッチで書き込む時間間隔。この時間間隔を小さく設定しすぎると、Dorisの書き込みブロッキング問題（エラーコード-235）が発生し、この時間間隔を小さく設定し、`maxBatchRows` と `batchSize` パラメータを大きく設定しすぎると、設定したデータサイズに到達できない場合でもインポートが行われる可能性がある。
  - 必須： いいえ
  - デフォルト： 30000（ms）
* **column**
...
* **maxBatchRows**
  - 説明： インポートデータの各バッチの最大行数。**batchSize** と合わせて、バッチあたりのインポートレコード行数を制御する。各バッチのデータが2つの閾値のうち1つに到達すると、このバッチのデータのインポートが開始される。
  - 必須： いいえ
  - デフォルト： 500000

* **batchSize**
  - 説明： 各バッチでインポートされるデータの最大量。**maxBatchRows** と連携して、バッチあたりのインポート数を制御する。各バッチのデータが2つの閾値のうち1つに到達すると、このバッチのデータのインポートが開始される。
  - 必須： いいえ
  - デフォルト： 94371840
...
* **labelPrefix**
  ...
  - 説明： 各バッチのインポートタスクのラベルプレフィックス。最終的なラベルは `labelPrefix + UUID` で構成され、グローバルに一意なラベルを形成してデータが重複してインポートされないことを保証する。
  - 必須： いいえ
  - デフォルト： `datax_doris_writer_`

* **loadProps**

  - 説明： StreamLoadのリクエストパラメータ。詳細はStreamLoadの紹介ページを参照。[Stream load - Apache Doris](https://doris.apache.org/docs/data-operate/import/stream-load-manual)

    これには、インポートデータの形式：formatなどが含まれる。インポートデータの形式はデフォルトでcsvであり、JSONをサポートしている。詳細については、以下の型変換セクションを参照するか、上記のStream loadの公式情報を参照。
...
#### 1. Streamがデータを読み取ってDorisにインポートする
...
>2. `line_delimiter`はデフォルトで改行文字ですが、データ内の値と競合する可能性があります。インポートエラーを避けるために特殊文字や非表示文字を使用できます
>3. strip_outer_array : インポートされるデータのバッチ内の複数行のデータを表します。Dorisは解析時に配列を展開し、その中の各Objectを順次データの行として解析します。
>4. Stream loadのその他のパラメータについては、Stream load - Apache Dorisを参照してください
>5. CSV形式の場合、次のように使用できます
>
>```json
>"loadProps": {
>    "format": "csv",
...
>```
>
>**CSV形式では、データ内の特殊文字との競合を避けるため、行と列の区切り文字に特に注意する必要があります。ここでは非表示文字を推奨します。デフォルトの列区切り文字は: \t、行区切り文字: \n**
