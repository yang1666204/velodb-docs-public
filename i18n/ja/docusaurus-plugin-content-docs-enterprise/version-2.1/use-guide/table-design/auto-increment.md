...
Dorisは、auto-incrementカラムで生成する値について**テーブル全体の一意性**を保証します。ただし：
...
auto-incrementカラムを使用するには、テーブル作成（CREATE-TABLE）時に対応するカラムに`AUTO_INCREMENT`属性を追加する必要があります。auto-incrementカラムの開始値を手動で指定するには、テーブル作成時に`AUTO_INCREMENT(start_value)`ステートメントを使用することで可能です。指定されない場合、デフォルトの開始値は1です。

### 例

1. auto-incrementカラムをキーカラムとする重複テーブルの作成。
