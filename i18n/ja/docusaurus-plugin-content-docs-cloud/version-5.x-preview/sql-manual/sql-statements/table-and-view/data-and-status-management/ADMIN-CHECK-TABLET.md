---
{
  "title": "ADMIN CHECK TABLET",
  "description": "この文は、タブレットのセットに対して指定されたチェック操作を実行するために使用されます。",
  "language": "ja"
}
---
## Description

このステートメントは、タブレットセットに対して指定されたチェック操作を実行するために使用されます。

## Syntax

```sql
ADMIN CHECK TABLET ( <tablet_id> [,...] ) PROPERTIES("type" = "<type_value>")
```
## 必須パラメータ

**1. `<tablet_id>`**

指定されたチェック操作を実行するタブレットのID。

## オプションパラメータ

**1. `<type_value>`**

現在、consistencyのみサポートされています。

- consistency:

  タブレットのレプリカデータ一貫性をチェックします。このコマンドは非同期であり、送信後、Dorisは対応するタブレットの一貫性チェックジョブの実行を開始します。

## 戻り値

ステートメントの最終結果は、`SHOW PROC "/cluster_health/tablet_health";`の結果のInconsistentNum列に反映されます。

| Column          | DataType | Note                     |
|-----------------|----------|--------------------------|
| InconsistentNum | Int      | 不整合があるタブレット数 |

## アクセス制御要件

このSQLコマンドを実行するユーザーは、少なくとも以下の権限を持つ必要があります：

| Privilege  | Object   | Notes                                                                                                    |
|:-----------|:---------|:---------------------------------------------------------------------------------------------------------|
| Admin_priv | Database | テーブル、パーティション、システムレベルコマンドの管理を含む、データベースの管理操作を実行するために必要。 |

## 例
- 指定されたタブレットセットでレプリカデータ一貫性チェックを実行

  ```sql
  admin check tablet (10000, 10001) PROPERTIES("type" = "consistency");
  ```
