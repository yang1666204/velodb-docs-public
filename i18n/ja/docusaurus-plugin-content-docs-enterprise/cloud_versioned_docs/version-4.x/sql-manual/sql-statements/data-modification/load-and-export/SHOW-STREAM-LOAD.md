...
3. LABEL LIKEが使用される場合、Stream Loadタスクのlabelがlabel_matcherを含むタスクにマッチします
4. LABEL =が使用される場合、指定されたlabelと正確にマッチします
5. STATUSが指定される場合、STREAM LOADステータスにマッチします
6. ORDER BYを使用して、列の任意の組み合わせでソートできます
7. LIMITが指定される場合、limit件のマッチするレコードが表示されます。そうでなければ全て表示されます
8. OFFSETが指定される場合、クエリ結果はオフセットoffsetから開始して表示されます。デフォルトでオフセットは0です。
...
4. 指定されたdbのStream Loadタスクを表示し、statusを"success"として指定し、StartTimeの降順でソートする
   ...
   ```
5. 指定されたdbのインポートタスクを表示し、StartTimeの降順でソートして、offset 5から始まる10件のクエリ結果を表示する
