...
FIRST_VALUE()は、ウィンドウパーティション内の順序付けられた値のセットから最初の値を返すウィンドウ関数です。NULL値の処理は、IGNORE NULLオプションを使用して制御できます。
...
| ------------------- | ------------------------------------------------------------------------------------------------------------------- |
| expr                | 最初の値を取得する対象の式。サポートされる型: tinyint/smallint/int/bigint/float/double/decimal/string/date/datetime/array/struct/map/bitmap                                                                    |
| ignore_null         | オプションのboolean型。設定された場合、null値は無視され、最初のnull以外の値が返される                                     |
