...
Dorisの転置インデックス実装では、テーブルの各行がドキュメントに対応し、各カラムがドキュメント内のフィールドに対応します。このため、転置インデックスを使用することで、特定のキーワードを含む行を素早く特定でき、WHERE句を高速化できます。
...
- 既存テーブルへの転置インデックス追加をサポート、テーブル内の既存データを書き直すことなく増分インデックス構築

- 既存テーブルからの転置インデックス削除をサポート、テーブル内の既存データを書き直すことなく実行
...
3. Merge-on-Writeが有効なDUPLICATEおよびUNIQUEテーブルモデルは任意のカラムで転置インデックスの構築をサポートします。しかし、Merge-on-Writeが有効でないAGGREGATEおよびUNIQUEモデルはKeyカラムでのみ転置インデックスの構築をサポートし、非Keyカラムには転置インデックスを設定できません。これは、これら2つのモデルがマージのためにすべてのデータを読み取る必要があるため、インデックスを事前フィルタリングに使用できないためです。
...
**1. `idx_column_name(column_name)` は必須です。`column_name` はインデックス用のカラムの名前で、事前に定義されたカラムである必要があります。`idx_column_name` はインデックス名で、テーブルレベルで一意である必要があります。推奨される命名規則: カラム名の前に `idx_` プレフィックスを付ける**
...
**inverted_index_storage_format の値:**
  ...
  <p>- 推奨事項: 大規模テキストデータセットを持つ新しいテーブル、ログ解析ワークロード、またはストレージ最適化が重要な場合はV3を使用してください</p>
...
-- 構文1: テーブルスキーマのINDEXセクションでUSING INVERTEDが指定されている場合は転置インデックスを示す
