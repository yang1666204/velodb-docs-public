...
FEログファイルは通常、Dorisインストールディレクトリの`fe/log/fe.log`ファイルに配置されています。これらは標準的なJavaプログラムログで、タイムスタンプ、ログレベル、スレッド名、コード位置、ログ内容などのフィールドを含みます。通常のログに加えて、スタックトレースを持つ例外ログもあります。スタックトレースは複数行にまたがるため、ログ収集とストレージプロセスでは、メインログとスタックトレースを単一のログエントリに結合する必要があります。
...
ログ収集設定ファイル（例：`opentelemetry_java_log.yml`）は、ETLプロセスに対応する3つの主要部分で構成されます：
...
GitHub Events Archiveには、JSON形式でアーカイブされたユーザーアクティビティイベントが含まれており、[https://www.gharchive.org/](https://www.gharchive.org/)からダウンロード可能です。例えば、2024年1月1日15:00のデータをダウンロードするには：

```bash
wget https://data.gharchive.org/2024-01-01-15.json.gz
```
以下はサンプルエントリです（読みやすさのためにフォーマットされています。実際のデータは1行に1つのJSONオブジェクトです）：
