---
{
  ...
  "description": "ipv6stringtonumornullの別名です。",
  "language": "ja"
}
---
...
- 入力がNULLの場合、`NULL`を返す
- 無効なIPv6文字列の場合、`NULL`を返す
- 入力がIPv4テキストの場合、等価なIPv6アドレス（`::ffff:<ipv4>`）を返す

### 使用上の注意
- `ipv6_string_to_num_or_null`と一貫した動作をする：無効な入力に対して`NULL`を返す
- MySQLの`INET6_ATON`構文との互換性のために一般的に使用される
