...
Vertical compactionはDoris 1.2.2で実装された新しいcompactionアルゴリズムで、大規模で幅の広いTableシナリオにおけるcompactionの実行効率とリソースオーバーヘッドを最適化するために使用されます。compactionのメモリオーバーヘッドを効果的に削減し、compactionの実行速度を向上させることができます。テスト結果により、vertical compactionによるメモリ消費量は元のcompactionアルゴリズムの1/10のみで、compaction速度は15%向上することが示されています。
...
Single replica compactionはTableのPROPERTIESで`enable_single_replica_compaction`パラメータによって指定され、デフォルトではfalse（無効）です。有効にするには、パラメータをtrueに設定します。

このパラメータはTable作成時に指定するか、後で以下を使用して変更できます：
...
compaction strategyは、小さなファイルがいつ、どのファイルがより大きなファイルにマージされるかを決定します。Dorisは現在、Tableプロパティの`compaction_policy`パラメータで指定される2つのcompaction strategyを提供しています。

### Size-based compaction ストラテジー
...
これらのパラメータはTableのPROPERTIESで設定され、Table作成時に指定するか、後で以下を使用して変更できます：
