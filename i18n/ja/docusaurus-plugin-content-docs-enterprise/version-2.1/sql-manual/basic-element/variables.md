...
{
  "title": "変数",
  "description": "Dorisでは、変数はシステム変数とユーザー変数に分けられます。どちらも大文字小文字を区別しません。",
  "language": "ja"
}
---
## 説明

Dorisでは、変数はシステム変数とユーザー変数に分けられます。両方とも大文字と小文字を区別しません。

システム変数はDorisの動作に影響します。システム変数とユーザー変数の両方をユーザークエリで使用できます。
...
- 変数の種類：
    ...
    - 読み取り専用変数：これらの変数はシステムによって設定され、ユーザーが変更することはできません。例えば`version`、`current_timestamp`など。

    - 変更可能変数：ユーザーは実行時にこれらの変数の値を変更できます。例えば`exec_mem_limit`、`time_zone`など。
...
ユーザー定義変数は、セッション内でデータを一時的に格納するメカニズムです。主な特徴は以下の通りです：
...
- スコープ：
    ...
    - 異なるセッションの同じ名前の変数は互いに独立しています。

- 代入方法：

    - `SET @var_name = value`構文を使用して代入します。
...
- SET
    ...
    一部の変数は、グローバルに有効にするか、現在のセッションでのみ有効にするかを設定できます。

    `SET`を使用して現在のセッションでのみ有効になるように設定します。例：
