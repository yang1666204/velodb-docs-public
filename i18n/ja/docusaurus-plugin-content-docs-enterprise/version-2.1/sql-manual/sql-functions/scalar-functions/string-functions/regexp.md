...
| Parameter | デスクリプション |
| -- | -- |
| `<str>` | 文字列型。正規表現とマッチングされる文字列を表します。Tableのカラムまたはリテラル文字列を指定できます。|
| `<pattern>` | 文字列型。文字列 <str> とのマッチングに使用される正規表現パターンです。正規表現は、文字クラス、量詞、アンカーなどを含む複雑な検索パターンを定義するための強力な方法を提供します。|
