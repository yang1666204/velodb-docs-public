---
{
  ...
  "description": "n-gram トークン化は、文章やテキストの一部を複数の隣接する単語グループに分割する手法です。NGram BloomFilter インデックスは、",
  "language": "ja"
}
---
## インデックスの原理
...
BloomFilterインデックスとは異なり、NGram BloomFilterインデックスはテキストのLIKEクエリを高速化するために使用されます。元のテキスト値を保存する代わりに、NGramを使用してテキストをトークン化し、各トークンをBloomFilterに保存します。LIKEクエリにおいて、LIKE '%pattern%'のパターンもNGramを使用してトークン化されます。各トークンはBloomFilterに対してチェックされ、いずれかのトークンが見つからない場合、対応するデータブロックはLIKE条件を満たさないためスキップでき、IOを削減してクエリを高速化します。

## 使用例

NGram BloomFilterインデックスは文字列のLIKEクエリのみを高速化でき、LIKEパターン内の連続文字数はNGramインデックスで定義されたN以上である必要があります。
...
1. **`idx_column_name(column_name)`** は必須です。`column_name` はインデックスを作成する対象の列で、上記の列定義に存在する必要があります。`idx_column_name` はインデックス名で、テーブルレベルで一意である必要があります。プレフィックス `idx_` に続けて列名を付けることを推奨します。
2. **`USING NGRAM_BF`** は必須で、インデックスタイプがNGram BloomFilterインデックスであることを指定します。
3. **`PROPERTIES`** は任意で、NGram BloomFilterインデックスの追加プロパティを指定するために使用されます。サポートされているプロパティは以下の通りです：
   - **gram_size**: NGraphのNで、トークンを形成する連続文字数を指定します。例えば、'This is a simple ngram example'でN = 3の場合、'This is a'、'is a simple'、'a simple ngram'、'simple ngram example'の4つのトークンに分割されます。
...
4. **`COMMENT`** は任意で、インデックスコメントを指定します。

### NGram BloomFilterインデックスの確認

-- 構文1: テーブルスキーマのINDEXセクションでUSING NGRAM_BFが指定されている場合は転置インデックスを示します
...
```
NGram BloomFilterインデックスはLIKEクエリを高速化するために使用されます。例えば：
...
このセクションでは、Amazonの商品レビューデータセット`amazon_reviews`を使用して、NGram BloomFilterインデックスの使用方法と有効性を実証します。
...
```
:::info
データファイルは10 GBを超える場合があり、stream loadのアップロードサイズ制限を超えないようにbe.confのstreaming_road_max_mbを調整する必要があるかもしれません。以下の手順に従って動的に調整できます：
...
**データインポートが成功したことを確認するためにcountクエリを実行してください:**
...
```
**次に、NGram BloomFilter インデックスを追加して、同じクエリを再度実行します。クエリは0.93秒で完了し、8倍のパフォーマンス向上を実現しました:**
