---
{
  "title": "BITMAP_HASH",
  "description": "任意の入力型の32ビットハッシュ値を計算し、そのハッシュ値を含むBitmapを返します。",
  "language": "ja"
}
---
## 説明

任意の入力タイプの32ビットハッシュ値を計算し、そのハッシュ値を含むBitmapを返します。

## 構文

```sql
BITMAP_HASH(<expr>)
```
## Parameters

| Parameter | Description           |
|-----------|-----------------------|
| `<expr>`  | 任意の値またはフィールド式 |

## Return Value

パラメータ `<expr>` の32ビットハッシュ値を含むBitmapを返します。
- パラメータがNULL値の場合、Empty Bitmapを返します

::: note

使用されるハッシュアルゴリズムはMurMur3です。  
MurMur3は高性能で衝突の少ないハッシュアルゴリズムであり、ランダム分布に近い値を生成し、カイ二乗分布テストに合格します。計算されるハッシュ値は、異なるハードウェアプラットフォームやシード値によって異なる場合があることに注意してください。  
このアルゴリズムのパフォーマンスの詳細については、[Smhasher](http://rurban.github.io/smhasher/)ベンチマークを参照してください。

:::

## Examples

値のMurMur3ハッシュを計算するには、次のように使用できます：

```sql
select bitmap_to_array(bitmap_hash('hello'))[1];
```
結果は以下のようになります：

```text
+-------------------------------------------------------------+
| %element_extract%(bitmap_to_array(bitmap_hash('hello')), 1) |
+-------------------------------------------------------------+
|                                                  1321743225 |
+-------------------------------------------------------------+
```
一部のシナリオにおいて`count distinct`よりも効率的である可能性があるbitmapを使用して、列内の重複を除いた値をカウントするには：

```sql
select bitmap_count(bitmap_union(bitmap_hash(`word`))) from `words`;
```
結果は次のようになります：

```text
+-------------------------------------------------+
| bitmap_count(bitmap_union(bitmap_hash(`word`))) |
+-------------------------------------------------+
|                                        33263478 |
+-------------------------------------------------+
```
```sql
select bitmap_to_string(bitmap_hash(NULL));
```
結果は以下のようになります：

```text
+------+
| res  |
+------+
|      |
+------+
```
