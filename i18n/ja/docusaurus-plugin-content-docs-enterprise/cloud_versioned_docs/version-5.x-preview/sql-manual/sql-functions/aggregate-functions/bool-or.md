...
{
  "title": "BOOL_OR",
  "description": "式のすべての非NULL値に対して論理OR集約を実行します。",
  "language": "ja"
}
---
## 説明
...
| -- | -- |
| `<expr>` | 論理ORで集約される式。BOOLEAN型と、0/非0ルールによってbooleanに変換可能な数値型をサポートします（0はFALSE、非0はTRUE）。 |

## Return Value

戻り値はBOOLEANです。すべての非NULL値が存在する場合にTRUEを返し、それ以外の場合はFALSEを返します。
...
```
### Window function:
以下の例では、条件（id > 2）に基づいて行をパーティション分割し、2つのグループに分けて、ウィンドウ集計結果を表示します：
