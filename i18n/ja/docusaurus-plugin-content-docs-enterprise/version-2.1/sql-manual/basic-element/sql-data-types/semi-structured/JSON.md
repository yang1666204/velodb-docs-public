...
{
  "title": "JSON | 半構造化",
  "description": "JSON データ型は、JSON データをバイナリ形式で効率的に格納し、JSON 関数を通じてその内部フィールドへのアクセスを可能にします。",
  "language": "ja"
}
---
# JSON
...
デフォルトでは、最大1048576バイト（1MB）までサポートし、2147483643バイト（2GB）まで増やすことができます。これは`string_type_length_soft_limit_bytes`設定で調整可能です。

通常のSTRING型でJSON文字列を保存する場合と比較して、JSON型には2つの主要な利点があります：
1. データ挿入時のJSON形式検証。
2. より効率的なバイナリストレージ形式により、`get_json_xx`関数と比較して、`json_extract`などの関数を使用したJSON内部フィールドへの高速アクセスが可能。

**注記**: バージョン1.2.xでは、JSON型はJSONBと呼ばれていました。MySQLとの互換性を維持するため、バージョン2.0.0から JSON に名前が変更されました。古いTableでは従来の名前を引き続き使用できます。
...
14	[123, 456]
```
- `'\'`を含むすべての特殊文字、例えば`'\r'`、`'\t'`がJSONに現れる場合、replace関数を使用して`"\"`を`"\\"`に置換する必要があります。例えば、`"\n"`を`"\\n"`に置換する必要があります。
...
```
- JSON型は、整数、文字列、BOOLEAN、ARRAY、およびMAPとの間でキャストできます。例えば：
...
```
- header設定 'max_filter_ratio: 0.3' を設定後、stream loadは成功します
...
```
- stream loadによってロードされたデータを表示するにはSELECTを使用します。JSON型の列はプレーンなJSON文字列として表示されます。
...
```
##### write data using insert into を使用したデータ書き込み

- insert で1行追加後、総行数が18から19に増加
...
##### json_extract関数を使用してjsonからいくつかのフィールドを抽出

1. json全体を抽出、'$'はjsonパス内のルートを表す
...
```
1. 特定のデータ型でフィールドを抽出
- json_extract_string はstring型のフィールドを抽出し、フィールドがstringでない場合はstringに変換します
...
19 rows in set (0.03 sec)
```
- json_extract_bigintはbigint型のフィールドを抽出し、フィールドがbigintでない場合はNULLを返します
...
```
##### json_exists_pathによってJSONにフィールドが存在するかをチェックする
...
```
##### json_typeでjsonのフィールドのデータタイプを取得する

- json pathで指定されたフィールドのデータタイプを返します。存在しない場合はNULLを返します。
...
対照的に、SQL NULL（IS NULLを使用する場合）は、キーがJSONオブジェクトに全く存在しないことを示す場合があります。
...
```
2. `GET_JSON_XXX`関数と`JSON_EXTRACT_XXX`関数の違いは何ですか？どのように選択すればよいですか？
