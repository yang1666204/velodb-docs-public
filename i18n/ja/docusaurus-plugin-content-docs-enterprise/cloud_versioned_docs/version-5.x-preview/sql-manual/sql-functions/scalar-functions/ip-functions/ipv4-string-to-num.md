...
## 説明
A.B.C.D形式（ドット区切りの10進数）のIPv4アドレスを含む文字列を受け取ります。対応するIPv4アドレスのネットワークバイトオーダー（ビッグエンディアン）整数でのアドレスの数値を返します。
...
- 無効なIPv4文字列または`NULL`入力の場合は例外をスローする

### 使用上の注意
- 標準のIPv4テキストのみをサポートし、CIDR（`/24`など）、ポート（`:80`など）、またはその他の拡張フォーマットはサポートしない
- 暗黙的なトリミングや型変換は実行せず、先頭/末尾に空白文字がある文字列は無効とみなす
- 相互変換のため`inet_ntoa`、`to_ipv4`と組み合わせて使用されることが多い

## 例

IPv4テキスト`192.168.0.1`を対応するネットワークバイトオーダー（ビッグエンディアン）整数でのアドレスの数値に変換する。
...
+--------+-----------+
```
無効な入力は例外を引き起こします（セグメント値が範囲外/空白文字を含む/NULL）。
...
```
`inet_ntoa`/`ipv4_num_to_string`と`to_ipv4`による相互変換の例：IPv4テキスト → 整数 → IPv4テキスト → IPv4型。
