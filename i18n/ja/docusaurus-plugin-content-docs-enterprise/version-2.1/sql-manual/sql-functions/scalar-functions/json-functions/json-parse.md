...
{
  "title": "JSON_PARSE",
  "description": "元のJSON文字列をJSONバイナリ形式に解析します。異なる異常データ処理のニーズを満たすため、",
  "language": "ja"
}
---
## 概要
元のJSONストリングをJSONバイナリ形式にパースします。異なる異常データ処理のニーズに対応するため、以下のようなJSON_PARSEシリーズ関数が提供されています：
* JSON_PARSE：JSONストリングをパースし、入力ストリングが有効なJSONストリングでない場合にエラーを報告します。
* JSON_PARSE_ERROR_TO_INVALID：JSONストリングをパースし、入力ストリングが有効なJSONストリングでない場合にNULLを返します。
* JSON_PARSE_ERROR_TO_NULL：JSONストリングをパースし、入力ストリングが有効なJSONストリングでない場合にNULLを返します。
* JSON_PARSE_ERROR_TO_VALUE：JSONストリングをパースし、入力ストリングが有効なJSONストリングでない場合にパラメータdefault_json_strによって指定されたデフォルト値を返します。
* JSON_PARSE_NOTNULL：JSONストリングをパースし、入力ストリングが有効なJSONストリングでない場合にNULLを返します。
...
| パラメータ           | 説明                          |
|--------------|-----------------------------|
| `<json_str>` | 抽出するJSONタイプのパラメータまたはフィールド。         |
