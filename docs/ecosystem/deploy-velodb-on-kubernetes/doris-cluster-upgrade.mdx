---
{
    ...
    "description": "This document describes how to use updates to upgrade an Apache Doris cluster based on a Doris-Operator deployment."
}
---

# Upgrade Apache Doris cluster deployed by Doris-Operator
...
- Before upgrading, you can read the [General Upgrade Manual](https://doris.apache.org/docs/admin-manual/cluster-management/upgrade) to help you understand some principles and precautions during the upgrade. .
...
1. Modify `spec.beSpec.image`
    ...
    Change `velodb/doris.be-ubuntu:2.0.4` to `velodb/doris.be-ubuntu:2.1.0`
```
...
```
    After entering the text editor, you will find `spec.beSpec.image` and change `velodb/doris.be-ubuntu:2.0.4` to `velodb/doris.be-ubuntu:2.1.0`
...
1. Modify `spec.feSpec.image`
    ...
    Change `velodb/doris.fe-ubuntu:2.0.4` to `velodb/doris.fe-ubuntu:2.1.0`
```
$ vim doriscluster-sample.yaml
```
...
$ kubectl edit dcr doriscluster-sample -n doris
```
    After entering the text editor, you will find `spec.feSpec.image` and change `velodb/doris.fe-ubuntu:2.0.4` to `velodb/doris.fe-ubuntu:2.1.0`
...
Access Doris through `mysql-client`.
