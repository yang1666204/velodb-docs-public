---
{
  ...
  "description": "INSERT INTO文は、Dorisクエリの結果を別のTableにインポートすることをサポートします。INSERT INTOは同期インポート方式です。",
  "language": "ja"
}
---
INSERT INTO文は、Dorisクエリの結果を別のTableにインポートすることをサポートしています。INSERT INTOは同期インポート方式であり、インポートの実行後にインポート結果が返されます。インポートが成功したかどうかは、返された結果に基づいて判断できます。INSERT INTOはインポートタスクの原子性を保証し、すべてのデータが正常にインポートされるか、まったくインポートされないかのいずれかになります。

## 適用可能なシナリオ

2. ユーザーがDorisTableの既存データに対してETLを実行し、新しいDorisTableにインポートしたい場合、INSERT INTO SELECT構文が適用できます。
3. Multi-Catalog外部Tableメカニズムと組み合わせることで、MySQLやHiveシステムのTableをMulti-Catalog経由でマップできます。その後、INSERT INTO SELECT構文を使用して外部TableからDorisTableにデータをインポートできます。
4. Table Value Functions（TVF）を活用することで、ユーザーはオブジェクトストレージやHDFS上のファイルに格納されたデータをTableとして直接クエリでき、列タイプの自動推論が行われます。その後、INSERT INTO SELECT構文を使用して外部TableからDorisTableにデータをインポートできます。

## 実装

INSERT INTOを使用する場合、インポートジョブをMySQLプロトコルを使用してFEノードに開始・送信する必要があります。FEは実行プランを生成し、これにはクエリ関連の演算子が含まれ、最後の演算子がOlapTableSinkになります。OlapTableSink演算子は、クエリ結果をターゲットTableに書き込む責任があります。実行プランはその後BEノードに送信されて実行されます。Dorisは1つのBEノードをCoordinatorとして指定し、データを受信して他のBEノードに配布します。
...
VALUES (1, "Emily", 25),
       ...
       (5, "Ava", 17);
```
3. 上記の操作に基づいて、ターゲットTableとして新しいTableを作成する（ソースTableと同じスキーマで）。
...
+--------+-----------------------------------------+----------+--------------------+--------+---------+----------------------------------------------------------------------+----------+---------------------+---------------------+---------------------+---------------------+---------------------+------+-----------------------------------------------------------------------------------------------------------------------+---------------+--------------+------+---------+
| JobId  | Label                                   | State    | Progress           | タイプ   | EtlInfo | TaskInfo                                                             | ErrorMsg | CreateTime          | EtlStartTime        | EtlFinishTime       | LoadStartTime       | LoadFinishTime      | URL  | JobDetails                                                                                                            | TransactionId | ErrorTablets | User | Comment |
+--------+-----------------------------------------+----------+--------------------+--------+---------+----------------------------------------------------------------------+----------+---------------------+---------------------+---------------------+---------------------+---------------------+------+-----------------------------------------------------------------------------------------------------------------------+---------------+--------------+------+---------+
...
Dorisは外部Tableの作成をサポートしています。作成後、外部Tableのデータは`INSERT INTO SELECT`を使用してDorисの内部Tableにインポートするか、SELECT文を使用して直接クエリできます。
...
以下では、Hive外部TableからDoris内部Tableへのデータインポートについて説明します。
...
Dorisは、Table Value Functions（TVFs）を通じて、オブジェクトストレージやHDFSに保存されているファイルを直接Tableとしてクエリおよび分析できます。これは自動列型推論をサポートします。詳細については、レイクハウス/TVFドキュメントを参照してください。
...
`INSERT INTO SELECT`構文と組み合わせて使用することで、ファイルをDorisTableに素早くインポートし、より高速な分析を行うことができます。
