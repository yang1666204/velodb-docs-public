...
{
  "title": "COLLECT_LIST",
  "description": "集約関数。列のすべての値を配列に集約するために使用されます。",
  "language": "ja"
}
---
## 説明
...
| パラメータ | 説明 |
| -- | -- |
| `<expr>` | 配列に格納される値を決定する式。サポートされる型: Bool, TinyInt, SmallInt, Integer, BigInt, LargeInt, Float, Double, Decimal, Date, Datetime, IPV4, IPV6, String, Array, Map, Struct。 |
| `<max_size>` | 結果配列のサイズをmax_size要素に制限するオプションパラメータ。サポートされる型: Integer。 |

## 戻り値

すべての非NULL値を含むARRAY型を返します。グループに有効なデータが存在しない場合は、空の配列を返します。
