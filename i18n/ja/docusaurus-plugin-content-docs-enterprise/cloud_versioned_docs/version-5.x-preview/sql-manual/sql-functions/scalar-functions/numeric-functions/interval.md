...
この関数は、MySQLの[interval function](https://dev.mysql.com/doc/refman/8.4/en/string-functions.html#function_interval)と一貫した動作をします。
...
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `N`               | 検索対象の値、integer型。                                                                                                                       |
| `N1, N2, N3, ...` | 閾値のリスト、integer型、昇順でソートされている必要がある（N1 ≤ N2 ≤ N3 ≤ ... ≤ Nn）。少なくとも1つの閾値パラメータが必要。 |

## Return Value

Nより厳密に大きい最初の閾値のインデックス（0ベース）を返す。戻り値の型はinteger。

NがNULLの場合は-1を返す。

閾値パラメータ（N1, N2, ...）がNULLの場合、0として扱われる。
