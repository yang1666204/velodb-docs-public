...
ストレージポリシーを作成するには、まずストレージリソースを作成し、次にマイグレーションポリシーを作成する際に作成されたストレージリソース名を関連付ける必要があります。詳細については、RESOURCEセクションを参照してください。
...
2. `<storage_resource_name>`: 関連付けられるストレージリソースの名前。作成方法の詳細については、RESOURCEセクションを参照してください

## オプションパラメータ

1. `<cooldown_datetime>`: データ移行ポリシーを作成するためのクールダウン時間を指定します
