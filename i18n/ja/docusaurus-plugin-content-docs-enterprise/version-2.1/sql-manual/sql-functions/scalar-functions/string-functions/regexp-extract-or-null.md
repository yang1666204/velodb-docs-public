---
{
  ...
  "description": "テキスト文字列からターゲットの正規表現パターンにマッチする最初の部分文字列を抽出する。",
  "language": "ja"
}
---
## Description
...
文字セットマッチングを処理する際は、Utf-8標準文字クラスを使用する必要があることに注意してください。これにより、関数は異なる言語の様々な文字を正しく識別し、処理できることが保証されます。
:::info
...
| -- | -- |
| `<str>` | 文字列パラメータ。正規表現マッチングが実行される対象のテキスト文字列を表します。この文字列は任意の文字の組み合わせを含むことができ、関数はその中で<pattern>にマッチする部分文字列を検索します。 |
| `<pattern>` | 文字列パラメータ。対象となる正規表現パターンです。このパターンは様々な正規表現メタ文字と文字クラスを含むことができ、マッチする部分文字列のルールを正確に定義します。 |
| `<pos>` | 整数パラメータ。抽出する式グループのインデックスを示します。インデックスは1から開始します。<pos>が0に設定されている場合、最初にマッチした部分文字列全体が返されます。<pos>が負の数であるか、パターン内の式グループ数を超える場合、関数はNULLを返します。 |

## Return Value

文字列型を返し、結果は`<pattern>`にマッチした部分になります。

 入力された`<pos>`が0の場合、最初にマッチした部分文字列全体を返します。
 入力された`<pos>`が無効（負の値または式グループ数を超える）の場合、NULLを返します。
 正規表現のマッチに失敗した場合、NULLを返します。
 `<pos>` < 0の場合、NULLを返します。
 `pos` > `<str>`の長さの場合、NULLを返します。

## Example

マッチから特定のグループを抽出します。説明：正規表現([[:lower:]]+)C([[:lower:]]+)は、'C'で区切られた1つ以上の小文字の並びを検索します。インデックス1のグループは最初の小文字の並びに対応するため、'b'が返されます。
...
+---------------------------------------------------------------------------+
```
一致しない正規表現です。文字列 'AbCdE' にはパターンと完全に一致する部分がありません。
...
+---------------------------------------------------------------------+
```
中国語文字のマッチング。パターン(\p{Han}+)(.+)は最初に1つ以上の中国語文字にマッチし、次に残りの任意の文字にマッチします。インデックス2のグループは、中国語文字の後にある文字列の非中国語部分を表します。
