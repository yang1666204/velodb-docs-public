...
---
このドキュメントは、`CREATE CATALOG`を通じて**Iceberg tables**または**Hive tables**にアクセスするために**AWS Glue カタログ**を使用する際のパラメータ設定について説明します。
...
| `glue.catalog_id` | Glue カタログ ID（未サポート） | いいえ | Empty |
