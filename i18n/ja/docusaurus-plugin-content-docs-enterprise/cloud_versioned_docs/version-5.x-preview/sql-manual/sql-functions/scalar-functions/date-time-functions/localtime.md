...
---
## 説明
この関数は現在のシステム時刻を取得するために使用され、戻り値は`DATETIME`型です。オプションで精度を指定して、戻り値の秒の小数部分の桁数を調整することができます。
...
|------------------|------------------------------------------------------------------------------------------------------------------------------------|
| `<precision>`    | 戻り値の小数秒部分の精度を指定するオプションパラメータ。範囲は0から6。デフォルトは0で、小数秒が返されないことを意味する。 |

## Return Value
- 現在のシステム時刻を`DATETIME`型で返す。
- 指定された`<precision>`が範囲外（例：負の値または6より大きい値）の場合、関数はエラーを返す。
