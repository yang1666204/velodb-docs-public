...
---
Dorisはパーティション化テーブルに一時パーティションを追加することをサポートしています。一時パーティションは通常のパーティションとは異なり、通常のクエリでは取得されず、特別なクエリステートメントを通じてのみクエリできます。
...
- **アトミック上書き操作**: ユーザーは古いデータの削除と新しいデータのインポートの間でデータ損失なしにパーティションのデータを書き換えることができます。この場合、一時パーティションを作成し、新しいデータを一時パーティションにインポートし、その後replace操作を通じて元のパーティションをアトミックに置き換えることができます。非パーティションテーブルでのアトミック上書き操作については、[replace table documentation](../../data-operate/delete/atomicity-replace)を参照してください。
...
- Drop操作を使用してデータベースやテーブルを直接削除した後、データベースやテーブルはRecover command（制限時間内）を使用して復旧できますが、一時パーティションは復旧されません。
...
- Truncate commandを使用してテーブルを空にすると、テーブルの一時パーティションが削除され、復旧することはできません。
...
- テーブルに一時パーティションがある場合、Alter commandを使用してテーブルに対してSchema Change、Rollup、またはその他の変更操作を実行することはできません。

- テーブルが変更操作を実行中の場合、テーブルに一時パーティションを追加することはできません。
