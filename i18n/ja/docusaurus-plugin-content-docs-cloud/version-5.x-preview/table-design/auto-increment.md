...
auto-increment列を持つテーブルに対して、Dorisは以下のようにデータ書き込みを処理します：
...
Dorisは、auto-increment列でDorisが生成する値について**テーブル全体の一意性**を保証します。ただし：
...
auto-increment列を使用するには、テーブル作成時（[CREATE-TABLE](../sql-manual/sql-statements/table-and-view/table/CREATE-TABLE)）に対応する列に`AUTO_INCREMENT`属性を追加する必要があります。auto-increment列の開始値を手動で指定するには、テーブル作成時に`AUTO_INCREMENT(start_value)`文を使用して行うことができます。指定されていない場合、デフォルトの開始値は1です。

### 例

1. auto-increment列をキー列として持つ重複テーブルの作成。
