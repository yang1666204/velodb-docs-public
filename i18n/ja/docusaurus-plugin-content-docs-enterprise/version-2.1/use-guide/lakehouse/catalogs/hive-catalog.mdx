...
{
  "title": "Hive Catalog",
  "description": "Apache Doris 2.1のHive CatalogはHive Metastoreおよび互換サービスに接続し、HDFSとオブジェクトストレージ全体でHive、Iceberg、Hudiテーブルのクエリと書き込みを行います。設定ガイド、サポートされるサービス、フォーマット、例が含まれています。",
  "language": "ja"
}
---
...
Hive Metastoreまたは Hive Metastore 互換のメタデータサービスに接続することで、Doris は Hive データベースとテーブル情報を自動的に取得してデータクエリを実行できます。

Hive に加えて、多くの他のシステムが Hive Metastore を使用してメタデータを保存しています。そのため、Hive Catalog を通じて、Hive テーブルだけでなく、Iceberg や Hudi など、Hive Metastore をメタデータ保存に使用する他のテーブル形式にもアクセスできます。

## 適用シナリオ

| シナリオ | 説明 |
|----------|------|
| Query Acceleration | Doris の分散コンピューティングエンジンを使用して Hive データに直接アクセスし、クエリを高速化します。 |
| Data Integration | Hive データを読み取って Doris 内部テーブルに書き込むか、Doris コンピューティングエンジンを使用して ZeroETL 操作を実行します。 |
| Data Write-back | Doris がサポートする任意のソースからのデータを処理し、Hive テーブルに書き戻します。 |
...
* `<fs_defaultfs>`
  ...
  このパラメータは、DorisからこのHive Catalogのテーブルにデータを書き込む際に必要です。例：
...
* `{HiveProperties}`
  ...
  HivePropertiesセクションは、Hive Catalogに関連するプロパティを入力するためのものです。

  * `get_schema_from_table`: デフォルト値はfalseです。デフォルトでは、DorisはHive Metastoreからテーブルスキーマ情報を取得します。しかし、一部のケースでは`Storage schema reading not supported`などのエラーといった互換性の問題が発生する可能性があります。この場合、このパラメータをtrueに設定することで、テーブルスキーマがTableオブジェクトから直接取得されます。ただし、この方法では列のデフォルト値情報が無視されることにご注意ください。このプロパティはバージョン2.1.10および3.0.6からサポートされています。

  * `hive.recursive_directories`: パーティションディレクトリをリストする際にサブディレクトリを再帰的に処理するかどうか。このパラメータはバージョン3.0.2からサポートされています。バージョン4.0より前では、このパラメータのデフォルトは`false`でした。それ以降のバージョンでは、デフォルトは`true`です。一部のHiveテーブルのパーティションパスは、テーブルスキーマ内のパーティション情報と一致しない場合があります。サブディレクトリ内のデータファイルを取得するには、このパラメータをtrueに設定する必要があります。
...
* `{CommonProperties}`
  ...
  CommonPropertiesセクションは、共通属性を入力するためのものです。詳細については[Catalog Overview](../catalog-overview.md)の「Common Properties」セクションを参照してください。

### サポートされるHiveバージョン

Hive 1.x、2.x、3.x、4.xをサポートします。

Hiveトランザクショナルテーブルはバージョン3.x以降からサポートされています。詳細については「Hive Transactional Tables」セクションを参照してください。
...
> Dorisを通じてHiveテーブルを作成しデータを書き込むには、Catalog属性に`fs.defaultFS`プロパティを明示的に追加する必要があります。Catalogがクエリのためのみに作成される場合、このパラメータは省略できます。
>
...
| varchar(N)    | varchar(N)    |                                                |
| string        | string        |                                                |
| binary        | string/varbinary  | Catalogの`enable.mapping.varbinary`プロパティによって制御されます（4.0.2からサポート）。デフォルトは`false`で、`string`にマッピングされます。`true`の場合、`varbinary`タイプにマッピングされます。|
| array         | array         |                                                |
| map           | map           |                                                |
| struct        | struct        |                                                |
| other         | unsupported   |                                                |
...
-- パーティション化されていないhiveテーブルの作成
  ...
  -- パーティション化されたhiveテーブルの作成
  -- パーティションカラムはテーブルのカラム定義リストに含まれている必要があります
  ...
  -- textフォーマットテーブルの作成（2.1.7 & 3.0.3以降）
