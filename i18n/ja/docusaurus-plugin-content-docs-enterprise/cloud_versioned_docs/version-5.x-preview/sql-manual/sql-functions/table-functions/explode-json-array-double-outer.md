...
---
## 説明
`explode_json_array_double_outer`テーブル関数は、JSON配列を受け取ります。その実装ロジックは、JSON配列を配列型に変換し、その後`explode_outer`関数を呼び出して処理することです。動作は`explode_outer(cast(<json_array> as Array<DOUBLE>))`と同等です。
