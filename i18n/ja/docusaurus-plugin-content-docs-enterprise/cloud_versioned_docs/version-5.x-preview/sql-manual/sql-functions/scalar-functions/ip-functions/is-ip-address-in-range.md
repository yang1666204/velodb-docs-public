---
{
  ...
  "description": "指定されたIPアドレスが与えられたCIDRネットワーク範囲内にあるかどうかを確認します。IPv4とIPv6の両方のアドレスをサポートしています。",
  "language": "ja"
}
---
...
## 説明
指定されたIPアドレスが与えられたCIDRネットワーク範囲内にあるかどうかを確認します。IPv4とIPv6の両方のアドレスをサポートしています。
...
```
### パラメータ
- `<ip_address>`: チェックするIPアドレス（IPv4、IPv6タイプまたは文字列）
...
- 逆インデックス最適化をサポートし、CIDRパラメータが定数の場合はインデックスアクセラレーションを使用可能
- 無効なCIDR形式の場合は0を返す
- 入力パラメータがNULLの場合はNULLを返す

## 例

IPv4アドレスが指定されたネットワークセグメント内にあるかをチェックする。
...
+----------+
```
指定されたネットワークセグメント内にIPv6アドレスが含まれているかどうかを確認します。
