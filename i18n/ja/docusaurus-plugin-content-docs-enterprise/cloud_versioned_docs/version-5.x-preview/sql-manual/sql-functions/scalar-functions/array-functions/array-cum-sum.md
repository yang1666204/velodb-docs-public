...
配列の累積和を計算します。この関数は配列を左から右に走査し、開始位置から現在位置まで（現在位置を含む）のすべての要素の合計を計算し、元の配列と同じ長さの新しい配列を返します。
...
- `arr`：ARRAY\<T>型、累積和を計算する配列。カラム名または定数値をサポートします。
...
- 入力配列と同じ長さの新しい配列を返します。各位置には、元の配列の開始から現在位置までのすべての要素の合計が含まれます
- NULL: 入力配列がNULLの場合

使用上の注意:
- 配列に他の型（文字列など）が含まれている場合、要素をDOUBLE型に変換を試行します。変換に失敗した要素はnullになります。
- この関数はすべての要素を互換性のある数値型に変換して累積和計算を試行します。累積和の戻り値の型は入力型に基づいて自動的に選択されます:
...
- 累積和の計算順序は左から右で、各位置にはその前のすべての非null要素の合計が含まれます。
- 空の配列は空の配列を返し、NULL配列はNULLを返し、要素が1つだけの配列は元の配列を返します。
- ネストした配列、MAP、STRUCTなどの複雑な型は累積和をサポートしません。呼び出すとエラーになります。
- 配列要素のnull値について: 最初の非null値から開始して、累積値の後のnullは0として累積和計算に参加します。
...
+-----------------------------+
```
double_arrayの累積和: 浮動小数点配列の累積和、結果は浮動小数点です。

2番目の位置の結果が3.3000000000000003となっていることに注意してください。これは浮動小数点のバイナリ表現精度によって引き起こされる小さな誤差によるものです。1.1と2.2はバイナリ浮動小数点（IEEE 754 double）で正確に表現することができず、近似値でしか格納できません。それらを加算すると、誤差が蓄積され、3.3000000000000003という結果になります。後続の累積和（6.6、11、16.5など）は「正常な値」として表示される場合がありますが、実際には近似値であり、四捨五入後に小数点以下が一致しているだけです。これはIEEE 754浮動小数点をベースとするすべてのシステム（MySQL、Snowflake、Python、JavaScriptなどを含む）で発生する現象です。
...
+------------------------------------------+
```
文字列と数値を混在させる場合、数値に変換できる要素は累積和に参加し、変換できない要素はnullになり、null値は累積和の計算において0として扱われます。
...
+----------------------+
```
null値を含む配列において、最初のnull以外の値から開始し、累積値の後のnullは0として扱われ、累積和の計算に参加します。
