...
{
  "title": "EXPLODE_MAP",
  "description": "explodemap関数はmap（マッピング型）を受け取り、それを複数の行に展開します。各行にはキー・バリューペアが含まれます。",
  "language": "ja"
}
---
## 説明

`explode_map`関数はmap（マッピングタイプ）を受け取り、各行にキーと値のペアを含む複数の行に展開します。通常はLATERAL VIEWと組み合わせて使用され、複数のlateral viewをサポートできます。新しいオプティマイザでのみサポートされています。
...
`explode_map` 空でないmapタイプのみを処理します。mapが空またはNULLの場合、`explode_map`は行を返しません。
