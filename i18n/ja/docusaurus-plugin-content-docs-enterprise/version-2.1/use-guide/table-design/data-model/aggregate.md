...
各データインポートはAggregate Key Modelでバージョンを作成し、**コンパクション**段階でバージョンがマージされます。クエリ時には、データはプライマリキーによって集計されます：

* **データインポート段階**

  * データは集計キーTableにバッチでインポートされ、各バッチが新しいバージョンを作成します。
...
Table作成時、**AGGREGATE KEY**キーワードを使用してAggregate Key Modelを指定できます。Aggregate Key ModelはKeyカラムを指定する必要があり、これらはストレージ時にValueカラムを集計するために使用されます。
...
```
上記の例では、ユーザー情報とアクセス行動のファクトTableが定義されており、`user_id`、`load_dt`、`city`が集計用のKeyカラムとして使用されています。データインポート時に、Keyカラムは1行に集計され、Valueカラムは指定された集計タイプに従って集計されます。
...
Aggregate KeyTableでは、データはプライマリキーに基づいて集計されます。データ挿入後、集計操作が完了します。

![aggrate-key-model-insert](/images/table-desigin/aggrate-key-model-insert.png)

上記の例では、Tableに元々4行のデータがありました。2行を挿入した後、Keyカラムに基づいてディメンションカラムの集計操作が実行されます：
...
```
Table内の計算方法は以下の図に示されています：

![state-func-group-concat-state-result-1](/images/table-desigin/state-func-group-concat-state-result-1.png)

Tableをクエリする際、merge操作を使用して複数の`state`値をマージし、最終的な集約結果を返すことができます。`group_concat`は順序付けが必要であるため、結果が不安定になる可能性があります。
