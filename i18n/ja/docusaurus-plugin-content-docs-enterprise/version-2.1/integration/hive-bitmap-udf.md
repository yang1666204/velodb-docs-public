---
{
  ...
  "description": "Hive Bitmap UDFは、hiveテーブルでビットマップの生成とビットマップ操作を行うためのUDFを提供します。HiveのビットマップはDorisビットマップと全く同じです。",
  "language": "ja"
}
---
# Hive UDF

Hive Bitmap UDFは、hiveテーブルでbitmapの生成とbitmap操作を行うためのUDFを提供します。HiveのbitmapはDorisのbitmapと完全に同じです。Hiveのbitmapは(spark bitmap load)を通じてdorisにインポートできます。
...
HiveテーブルをTEXT形式で作成する際、Binary型の場合、Hiveはbash64エンコード文字列として保存されます。そのため、DorisのHive Catalogを使用することで、bitmap_from_base64関数を通じてバイナリデータを直接Bitmapとして保存できます。
