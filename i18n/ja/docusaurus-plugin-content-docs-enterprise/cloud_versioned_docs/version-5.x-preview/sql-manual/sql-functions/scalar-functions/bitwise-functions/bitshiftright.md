---
{
  ...
  "description": "右シフトビット演算を実行し、二進数のすべてのビットを指定された位置数だけ右に移動します。",
  "language": "ja"
}
---
## 説明
右シフトビット演算を実行します。これは、二進数のすべてのビットを指定された位置数だけ右に移動します。この演算は、バイナリデータの処理や効率的な数学的計算（除算など）によく使用されます。

-1を1つの位置で論理右シフトすると、BIGINT_MAX（9223372036854775807）になります。
