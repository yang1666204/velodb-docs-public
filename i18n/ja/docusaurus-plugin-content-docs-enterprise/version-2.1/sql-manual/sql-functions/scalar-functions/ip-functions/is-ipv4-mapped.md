---
{
  ...
  "description": "この関数は、INET6ATON()によって返される、バイナリ文字列として数値形式で表現されたIPv6アドレスを受け取ります。",
  "language": "ja"
}
---
## 説明
この関数は、INET6_ATON()によって返される、バイナリ文字列として数値形式で表現されたIPv6アドレスを受け取ります。INET6_ATONはIPV6_STRING_TO_NUM_OR_NULLとも呼ばれます。
- IPv4マップアドレスは`::ffff:ipv4_address`の形式を持ちます。
...
## Return Value
引数が有効なIPv4マップIPv6アドレスの場合は1を返し、そうでない場合は0を返します。
