...
条件に基づいて配列要素をフィルタリングし、条件を満たす要素で構成される新しい配列を返します。この関数は2つの呼び出し方法をサポートしています：lambda式を使用した高階関数形式と、boolean配列を直接使用するフィルタリング形式です。
...
戻り値の意味：
- フィルタリング条件を満たすすべての要素で構成される新しい配列を返す
- NULL：入力配列がNULLの場合
- 空配列：条件を満たす要素がない場合

使用上の注意：
- Lambda形式：lambda式のパラメータ数は配列パラメータ数と一致する必要がある
- Boolean配列形式：`array1`と`filter_array`の長さは完全に一致することが理想的。boolean配列の方が長い場合、余分なboolean値は無視される。boolean配列の方が短い場合、boolean配列の対応する位置の要素のみが処理される
- 複数の配列と複合型配列のフィルタリングをサポート
- 空配列は空配列を返し、NULL配列はNULLを返す
- Lambdaは任意のスカラー式を使用可能、集計関数は使用不可
...
+------------------------------------------+
```
string_array内で長さが2より大きい要素をフィルタリングするためのlambda式の使用:
...
+---------------------------------------------------+
```
boolean配列の長さが元の配列より短い場合、boolean配列内の対応する位置にある要素のみが処理されます：
...
+-------------------------------------+
```
null値を含む配列では、lambdaはnullを評価できます：
...
+------------------------------------------------------------+
```
複数配列のフィルタリング、int_array > double_arrayの条件で要素をフィルタリング：
...
ネストされた配列のフィルタリング、各サブ配列の長さが2より大きい要素をフィルタリング：
...
+-------------------------------------------------------------------+
```
Map型フィルタリング、キー'a'の値が10より大きい要素をフィルタリングする場合：
...
現在の例では、内側のarray_countがスカラー値（INT64）を返すため、array_filterが処理できるのでネストが可能です。
