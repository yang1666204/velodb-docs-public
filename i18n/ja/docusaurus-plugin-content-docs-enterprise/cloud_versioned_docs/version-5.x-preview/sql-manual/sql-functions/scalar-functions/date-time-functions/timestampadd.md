...
TIMESTAMPADD関数は、指定された日時値に対して指定された単位の時間間隔を加算（または減算）し、計算された日時値を返します。この関数は複数の時間単位（秒、分、時間、日、週、月、年など）をサポートし、日時のオフセット計算を柔軟に処理できます。負の間隔は対応する時間の減算を示します。
...
| `<interval>`          | 追加する時間間隔。通常は整数で、正の値または負の値を指定して時間の長さの追加または減算を示すことができます                                                                                                                                                                                                            |
| `<date_or_time_expr>` | 有効な対象日付。date/datetime型の入力をサポートします。具体的なdatetimeとdateの形式については、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)および[date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion)を参照してください |

## Return Value

戻り値は、基準となるdatetimeに指定された間隔を追加した結果を表します。
...
- `<datetime_expr>`が無効な日付（0000-00-00、2023-13-01など）の場合、または`<unit>`がサポートされていない単位の場合、例外をスローします
- いずれかのパラメータがNULLの場合、NULLを返します
- 月/年を処理する際、月末日を自動的に調整します（例：2023-01-31に1ヶ月を追加すると、うるう年かどうかによって2023-02-28または2023-02-29になります）
