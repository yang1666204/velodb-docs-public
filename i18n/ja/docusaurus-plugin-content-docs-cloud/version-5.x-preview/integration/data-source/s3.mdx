...
| **Existing Table** | 既存のVeloDBテーブルに読み込み |
...
| **UNIQUE** | 更新のあるデータ - キーごとに最新の行のみを保持 | ユーザープロファイル、商品カタログ、ディメンションテーブル |
...
| **Partition** | 時間ベースのパーティション分割を有効化（時系列データに推奨） |
| **Bucket Key** | ノード間でのハッシュ分散用カラム |
| **Bucket Number** | データバケット数（ほとんどの場合 `AUTO` を推奨） |
...
- **Bucket Key**: 均等なデータ分散のための高カーディナリティカラム
