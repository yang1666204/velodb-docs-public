...
配列の要素にラムダ式を適用し、各要素が条件を満たすかどうかを示すブール配列を返します。この関数は配列の各要素にラムダ式を適用し、対応するブール値を返します。
...
戻り値の意味:
- 入力配列と同じ長さのboolean配列を返し、各位置にはlambda式を対応する要素に適用した結果が含まれる
- NULL: 配列パラメータのみがNULLでlambda式が存在しない場合

使用上の注意:
- lambda式のパラメータ数は配列パラメータの数と一致する必要がある
- すべての入力配列は同じ長さでなければならない
- 多次元配列と複合型配列の評価をサポートしている
- 空配列は空配列を返す。配列パラメータのみがNULLでlambda式が存在しない場合、NULLを返す。lambda式が存在し配列がNULLの場合、エラーとなる
- lambdaは任意のスカラ式を使用できるが、集約関数は使用できない
- lambda式は他の高階関数を呼び出すことができるが、戻り値の型に互換性が必要である
...
+-------------------------------------+
```
NULL配列とラムダ式の組み合わせ。NULLを含むラムダ式がある場合、エラーが発生します。ラムダ式がない場合、NULLを返します：
...
nullとの比較演算（>、<、=、>=、<=など）は、特別なIS NULLやIS NOT NULL以外では、nullを返します。これは、未知の値（NULL）が2より大きいかどうかを判定することが不可能であるためです。この動作は、MYSQL、POSTGRESSQLなどと一致しています。
...
+-------------------------------------------------+
```
多配列評価、最初の配列が2番目の配列より大きいかどうかをチェック:
...
ネストされた配列の評価、各サブ配列の長さが2より大きいかどうかをチェック:
...
+----------------------------------------------------------------+
```
Map型の評価で、キー'a'の値が10より大きいかどうかをチェックする：
