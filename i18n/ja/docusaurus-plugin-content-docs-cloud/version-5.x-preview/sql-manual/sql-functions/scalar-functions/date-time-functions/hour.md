...
{
  "title": "HOUR",
  "description": "HOUR関数は、datetime型またはtime型の式から時間部分を抽出します。この関数は複数の時間型入力をサポートしており、",
  "language": "ja"
}
---
## 説明

HOUR関数は、datetimeまたはtime式から時間部分を抽出します。この関数は、DATE/DATETIMEおよびTIMEを含む複数の時間型入力をサポートし、対応する時間値を返します。

DATETIME（'2023-10-01 14:30:00'など）の場合、戻り値は0-23の範囲（24時間形式）です。
TIME型（'456:26:32'など）の場合、戻り値は24を超えることがあり、[0,838]の範囲です。

この関数は、MySQLの[hour関数](https://dev.mysql.com/doc/refman/8.4/en/date-and-time-functions.html#function_hour)と一貫した動作をします。
