...
場合によっては、ユーザーがUnique KeyモデルのTableに対して同じSQLを使用してクエリを実行すると、複数回のクエリの結果が一致しない場合があります。そして、クエリ結果は常に2-3種類の間で変化します。
...
バージョン1.1.xで、ベクトル化が有効になっており、クエリデータTableのbitmap型フィールドがNULL結果を返す場合：
...
バージョン1.1.xで、ベクトル化が有効になっており、クエリデータTableのbitmp型フィールドがNULL結果を返す場合：
