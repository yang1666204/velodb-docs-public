...
| `<delimiter>` | 区切り文字列。大文字小文字を区別し、マルチバイト文字をサポート。型：VARCHAR |
| `<field>` | 区切り文字の出現回数。正の値は左から、負の値は右から数え、0は空文字列を返す。型：INT |
...
抽出ルール：
- field > 0の場合：左からfield番目の区切り文字より前の内容を返す
- field < 0の場合：右から|field|番目の区切り文字より後の内容を返す
- field = 0の場合：空文字列を返す（contentがNULLの場合はNULLを返す）
- 大文字小文字を区別した区切り文字の完全一致

特別なケース：
- いずれかのパラメータがNULLの場合、NULLを返す
- 区切り文字が文字列に存在しない場合、元の文字列を返す
- 指定された回数が実際の区切り文字の出現回数を超える場合、抽出可能な最大部分を返す
- 区切り文字が空文字列の場合、空文字列を返す
- ソース文字列が空の場合、空文字列を返す
