...
VeloDB のハイブリッド検索を使用して RAG (Retrieval-Augmented Generation) アプリケーションを構築します - 転置インデックスと BM25 スコアリングによる全文検索と、ベクトルインデックスによる類似度検索を単一の SQL クエリで組み合わせます。

:::warning
ベクトルインデックスによるハイブリッド検索は VeloDB 5.x-preview で利用可能です。
:::
