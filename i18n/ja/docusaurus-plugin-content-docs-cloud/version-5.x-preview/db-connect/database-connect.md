...
---
Apache Dorisは、MySQLネットワーク接続プロトコルを採用しています。MySQLエコシステム内のコマンドラインツール、JDBC/ODBCドライバー、および各種ビジュアライゼーションツールと互換性があります。さらに、Apache Dorisには組み込みの使いやすいWeb UIが付属しています。このガイドでは、MySQL クライアント、MySQL JDBC Connector、DBeaver、および組み込みのDoris Web UIを使用してDorisに接続する方法について説明します。

## MySQL クライアント

LinuxのMySQL Clientを[MySQL公式サイト](https://dev.mysql.com/downloads/mysql/)からダウンロードします。現在、DorisはMySQL 5.7以降のクライアントと主に互換性があります。

ダウンロードしたMySQL clientを展開します。`bin/`ディレクトリで、`mysql`コマンドラインツールを見つけます。Dorisに接続するには、以下のコマンドを実行します：
...
Doris FEにはビルトインのWeb UIが搭載されています。これにより、ユーザーはMySQLクライアントをインストールする必要なく、SQLクエリを実行し、その他の関連情報を表示できます。

Web UIにアクセスするには、WebブラウザでURLを入力するだけです：http://fe_ip:fe_port、例えば`http://172.20.63.118:8030`。これによりDorisのビルトインWebコンソールが開きます。
...
:::tip 
Playgroundで特定のデータベース/Tableに関連しないステートメントを正常に実行するには、左側のデータベースパネルからデータベースをランダムに選択する必要があります。この制限は後で削除される予定です。
