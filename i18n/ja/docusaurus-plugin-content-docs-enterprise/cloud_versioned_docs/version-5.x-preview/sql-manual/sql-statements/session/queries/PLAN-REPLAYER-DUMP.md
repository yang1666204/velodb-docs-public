---
{
  ...
  "description": "PLAN REPLAYER DUMPは、Dorisユーザーが実行プラン診断ファイルを生成するためのツールです。クエリオプティマイザーの状態と入力データを取得し、",
  "language": "ja"
}
---
## 説明

PLAN REPLAYER DUMPは、Dorisユーザーが実行プラン診断ファイルを生成するためのツールです。クエリオプティマイザーの状態と入力データをキャプチャし、クエリ最適化の問題のデバッグと分析を促進します。出力は対応する診断ファイルのhttpアドレスです。
