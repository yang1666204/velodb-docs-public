...
{
  "title": "リーディングヒント",
  "description": "Leading Hintは、ユーザーがDorisオプティマイザーをガイドしてクエリ内のテーブル結合順序を決定できる強力なクエリ最適化技術です。",
  "language": "ja"
}
---
Leading Hintは、ユーザーがDorisオプティマイザーにクエリプランにおけるテーブル結合順序を決定するよう導くことができる強力なクエリ最適化技術です。Leading Hintの正しい使用により、複雑なクエリのパフォーマンスを大幅に向上させることができます。この記事では、DorisでLeading Hintを使用して結合順序を制御する方法について詳しく説明します。

## 通常のLeading Hint

### 構文

Leading Hintは、オプティマイザーが従うべきテーブル結合順序を指定することができます。Dorisにおいて、Leading Hintの基本構文は以下の通りです：
...
- `tablespec`はテーブル名またはテーブルエイリアスで、少なくとも2つのテーブルを指定する必要があります。
...
| `SyntaxError` | 対応するテーブルが見つからないなど、Leading Hintの構文エラーを示します。 |
