...
{
  "title": "UPDATE",
  "description": "このステートメントはデータの更新に使用されます。UPDATE ステートメントは現在 UNIQUE KEY モデルのみをサポートしています。",
  "language": "ja"
}
---
## デスクリプション
...
+ target_table: 更新対象データのターゲットTable。'db_name.table_name'の形式で指定可能
+ assignment_list: 更新対象カラム。'col_name = value, col_name = value'の形式で指定
...
+ FROM additional_tables: 更新する行を選択するため、または新しい値を設定するために使用する1つ以上のTableを指定します。ここでターゲットTableを使用したい場合は、明示的にエイリアスを与える必要があります。
...
```
2. 'test'Tableのk1=1列のv1列を1増加させる

```sql
UPDATE test SET v1 = v1+1 WHERE k1=1;
```
3. `t2`と`t3`のjoinの結果を使用して`t1`を更新する
