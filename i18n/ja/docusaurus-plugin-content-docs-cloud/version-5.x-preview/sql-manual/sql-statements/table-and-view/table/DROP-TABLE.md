...
**1. `TEMPORARY` **
> 指定した場合、ステートメントはTEMPORARYテーブルのみを削除します。

**2. `IF EXISTS`**
> 指定した場合、存在しないテーブルに対してエラーが発生しません。
...
**4.`FORCE`**
> 指定した場合、システムはテーブルに未完了のトランザクションがあるかどうかをチェックしません。テーブルは直接削除され、復旧することはできません。この操作は一般的に推奨されません。
...
- `DROP TABLE FORCE`を実行した場合、システムはテーブルに未完了のトランザクションがあるかどうかをチェックしません。テーブルは直接削除され、復元することはできません。一般的に、この操作は推奨されません。
