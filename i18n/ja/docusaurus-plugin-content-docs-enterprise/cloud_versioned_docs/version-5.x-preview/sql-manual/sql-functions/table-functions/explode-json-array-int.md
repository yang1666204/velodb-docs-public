...
---
## 説明
`explode_json_array_int`テーブル関数はJSON配列を受け取ります。その実装ロジックは、JSON配列を配列型に変換し、その後処理のために`explode`関数を呼び出すことです。この動作は`explode(cast(<json_array> as Array<BIGINT>))`と同等です。
