...
システム ActionはDorisに組み込まれたProcシステムに関する情報を取得するために使用されます。
...
```
データ部分の`column_names`はヘッダー情報であり、`href_columns`はTable内のどの列がハイパーリンク列であるかを示します。`rows`配列の各要素は1行を表します。その中で、`__hrefPaths`はTableデータではなく、ハイパーリンク列のリンクURLであり、`href_columns`の列と一対一で対応しています。
