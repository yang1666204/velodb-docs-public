...
>
> **スキャン制限カテゴリ**
> ユーザーがクエリを開始すると、クエリオプティマイザは各テーブルでスキャンする必要があるパーティション数、タブレット数、データ行数を計算します。以下のプロパティを使用して、これら3つの数値をすべて同時に、または一部のみを制限することができます。
> - partition_num: テーブルがスキャンするパーティションの最大数。
> - tablet_num: テーブルがスキャンするタブレットの最大数。
...
ルールで定義されたSQLを実行すると、以下に示すように例外エラーが返されます：
   ...
   ```
2. 同じテーブルの30個を超えるパーティションのスキャンを防止し、クエリのデータ量を100億行以下に制限するルールを作成する
