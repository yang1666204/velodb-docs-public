---
{
    ...
    "description": "Doris Operator extends Kubernetes with Custom Resource Definition (CRD)."
}
---
...
```bash
wget https://raw.githubusercontent.com/velodb/doris-operator/master/config/crd/bases/doris.velodb.com_dorisclusters.yaml
kubectl apply -f ./doris.velodb.com_dorisclusters.yaml
```

The following is the expected output:

```bash
customresourcedefinition.apiextensions.k8s.io/dorisclusters.doris.velodb.com created
```
...
```bash
dorisclusters.doris.velodb.com                      2024-02-22T16:23:13Z
```
...
```bash
velodb/doris.k8s-operator:latest
```
...
docker pull velodb/doris.k8s-operator:latest
## save the doris operator image as a tar package
docker save -o doris.k8s-operator-latest.tar velodb/doris.k8s-operator:latest
```
...
containers:
  - command:
    ...
    args:
      - --leader-elect
    image: velodb/doris.k8s-operator:v1.0.0
    name: dorisoperator
    securityContext:
...
```bash
helm repo add doris-repo https://charts.velodb.com
```
...
```bash
wget https://charts.velodb.com/doris-operator-1.4.0.tgz
```
