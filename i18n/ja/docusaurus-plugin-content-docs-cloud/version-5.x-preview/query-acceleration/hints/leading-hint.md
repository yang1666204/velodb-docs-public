...
{
  "title": "リーディングヒント",
  "description": "Leading Hintは、ユーザーがDorisオプティマイザーにクエリ内のテーブル結合順序を決定するよう指示することを可能にする強力なクエリ最適化技術です",
  "language": "ja"
}
---
Leading Hintは、ユーザーがDorisオプティマイザーをガイドして、クエリプラン内のテーブル結合順序を決定できる強力なクエリ最適化手法です。Leading Hintを正しく使用することで、複雑なクエリのパフォーマンスを大幅に向上させることができます。この記事では、DorisでLeading Hintを使用して結合順序を制御する方法について詳しく説明します。

## 通常のLeading Hint

### 構文

Leading Hintを使用すると、オプティマイザーが従うべきテーブル結合順序を指定できます。Dorisにおいて、Leading Hintの基本構文は以下の通りです：
...
- `tablespec`はテーブル名またはテーブルエイリアスであり、少なくとも2つのテーブルを指定する必要があります。
- 複数のテーブルはスペースまたは','で区切られます。
- 波括弧`{}`を使用してJoin Treeの形状を明示的に指定できます。
...
+------------------------------------------------------------------------------+
```
Leading Hintが効果的でない場合、通常のプロセスがプランの生成に使用されます。EXPLAINは、使用されたHintが効果的かどうかを表示し、主に3つのタイプに分かれます：

| Status        | Description |
|--------------|--------------------------------------------------------------------------------------------------------------------------------|
| `Used`       | Leading Hintが正常に効果的です。 |
| `Unused`     | ここでのサポートされていないケースには、Leading Hintで指定された結合順序が元のSQLと等価でない場合や、この版でその機能がサポートされていない場合が含まれます（詳細は制限事項を参照してください）。 |
| `SyntaxError` | Leading Hintの構文エラーを示します。対応するテーブルが見つからない場合などです。 |

1. Leading Hintのデフォルト構文は、left-deepツリーを構築します：
    ...
    +--------------------------------------------------------------------------------+
    ```
2. 同時に、波括弧を使用してJoinツリーの形状を指定できます：
    ...
    +----------------------------------------------------------------------------------+
    ```
3. ViewがJoinReorderに参加するためのエイリアスとして使用される場合、対応するViewをLeading Hintのパラメータとして指定することができます。例：
...
2. 元の計画：
    ...
    +-------------------------------------------+
    ```
3. t1とt2の結合順序を交換する必要がある場合、前に`leading(t2 t1)`を追加するだけです。`explain`を実行すると、このヒントが使用されているかどうかが表示されます。以下はLeadingプランです：`Used`はHintが正常に有効であることを示します。
    ...
    +------------------------------------------------------------------------------+
    ```
4. Leading Hintに構文エラーがある場合、`explain`を実行すると、対応する情報が`SyntaxError`に表示されますが、プランは依然として正常に生成されます。ただし、Leadingは使用されません。例：
...
1. Left-Deep Tree
    ...
    上記で述べたように、Dorisのクエリ文で括弧が使用されていない場合、Leadingはデフォルトでleft-deep treeを生成します。
...
プランの形状をright-deep tree、Bushy tree、またはzig-zag treeにする必要がある場合、中括弧を追加してプランの形状を制限するだけで済み、Oracleのようにswapを使用してleft-deep treeから段階的に調整する必要はありません。
...
5. Non-inner Join
    ...
    非内部結合（Outer JoinやSemi/Anti Joinなど）に遭遇した場合、Leading Hintは元のSQLセマンティクスに従って各結合の結合方法を自動的に導出します。Leading Hintが元のSQLセマンティクスと異なる場合、または生成できない場合は、UnUsedに配置されますが、これはプランの正常な生成には影響しません。
    以下は交換できない例です：
    ...
    +--------------------------------------------------------------------------------+
    ```
読者が自分で検証できるように、Leading Hintを使ったOuter、Semi、およびAnti Joinの例をいくつか示します。
...
6. View
    ...
    Alias を含む場合、エイリアスは完全で独立したサブツリーとして指定でき、結合順序はテキスト順序に従って生成されます
...
Ordered hintは、leading hintの特殊なケースと見なすことができ、テキストの順序に従ってjoinの順序を制御するために使用されます。
...
Ordered HintとLeading Hintが同時に使用される場合、Ordered HintがLeading Hintよりも優先されます。これは、Leading Hintが指定されていても、Ordered Hintも存在する場合、クエリプランはOrdered Hintのルールに従って実行され、Leading Hintは無視されることを意味します。以下は両方が同時に使用される場合の状況を示す例です：
...
Leading Hintは結合順序を手動で制御するための強力な機能であり、本番環境でのビジネスチューニングで広く使用されています。leading hintを適切に使用することで、現場での結合順序に対するチューニング要件を満たし、システム制御の柔軟性を向上させることができます。
