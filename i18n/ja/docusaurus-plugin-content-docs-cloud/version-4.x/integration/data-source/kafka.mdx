...
この手順をスキップして、[対象Tableの作成](#3-create-target-table-in-velodb)に進んでください。
...
VeloDBデータ型の完全な情報については、[Data タイプ 概要](../../sql-manual/basic-element/sql-data-types/data-type-overview)を参照してください。
...
**SQL Editor**でデータベースとTableを作成します:
...
```
:::tip
Routine Loadの`jsonpaths`パラメータは、JSONフィールドをTableカラムに順序通りにマッピングします。Tableカラムが`jsonpaths`で指定された順序と一致していることを確認してください。
:::
...
| ジョブの自動一時停止："max_error_number exceeded" | JSONとTable間のスキーマ不一致。`jsonpaths`がJSONフィールドと一致することを確認。エラーの一部を許容するために`"max_filter_ratio" = "0.5"`を追加 |
