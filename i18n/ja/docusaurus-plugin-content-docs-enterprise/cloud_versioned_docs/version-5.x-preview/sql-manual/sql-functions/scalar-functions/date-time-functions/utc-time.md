...
{
  "title": "UTC_TIME",
  "description": "UTCTIME関数は、UTCタイムゾーンの現在時刻を返します。",
  "language": "ja"
}
---
## 説明
UTC_TIME関数は、UTCタイムゾーンでの現在時刻を返します。この関数はローカルタイムゾーンの影響を受けず、常にUTCタイムゾーンに基づく現在時刻を返すため、異なるタイムゾーンシナリオ間での時刻の一貫性を保証します。
...
Time型を返します（形式：HH:mm:ss）。返された結果を数値演算に使用する場合、[整数形式](../../../../sql-manual/basic-element/sql-data-types/conversion/int-conversion#from--time)に変換されます（00:00:00からの経過時間値、単位はマイクロ秒）。
