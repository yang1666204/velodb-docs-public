...
{
  "title": "ALTER USER",
  "description": "ALTER USER文は、パスワードやパスワードポリシーなどを含む、ユーザーのアカウント属性を変更するために使用されます。",
  "language": "ja"
}
---
## 説明
...
> `user_identity`は2つの部分、user_nameとhostで構成されており、usernameはユーザー名です。Hostはクライアントが接続するホストアドレスを識別します。host部分はあいまい一致に%を使用できます。hostが指定されていない場合、デフォルトは'%'になり、これはユーザーが任意のホストからDorisに接続できることを意味します。
...
>
> アカウントがロックされた時のロック時間を設定します。例えば、`PASSWORD_LOCK_TIME 1 DAY`はアカウントが1日間ロックされることを意味します。
>
> `ACCOUNT_UNLOCK`
>    
...
1. このコマンドはバージョン2.0からユーザーロールの変更をサポートしなくなりました。関連操作には[GRANT](./GRANT-TO.md)と[REVOKE](./REVOKE-FROM.md)を使用してください

2. ALTER USERコマンドでは、以下のアカウント属性のうち同時に変更できるのは1つのみです：
- パスワードの変更
