...
このステートメントはColocation Groupのプロパティを変更するために使用されます。
...
注意:
1. colocate groupがグローバル、つまり名前が__global__で始まる場合、それはどのDatabaseにも属しません
...
```
2. 非グローバルグループのレプリカ数を変更し、Table作成時に "colocate_with" = "bar" を設定します。このTableはデータベース example_db に属します。
