...
- Boolean 型: BOOLEAN (true は 1 に変換、false は 0 に変換)
...
戻り値の意味:
- 配列内の全ての要素の積を返す
- NULL: 配列が空の場合、配列が NULL の場合、または全ての要素が NULL の場合

使用上の注意:
- この関数は配列内の NULL 値をスキップし、NULL でない要素のみの積を計算する
- 配列内の全ての要素が NULL の場合、NULL を返す
- 空の配列は NULL を返す
- 複合型 (MAP, STRUCT, ARRAY) は積の計算をサポートしておらず、エラーの原因となる
- 配列要素内の null 値について: null 要素は積の計算に参加しない
