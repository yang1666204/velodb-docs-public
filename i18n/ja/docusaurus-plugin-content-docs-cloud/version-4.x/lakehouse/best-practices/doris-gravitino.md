...
---
データレイク技術の急速な発展に伴い、統合された安全で効率的なlakehouseアーキテクチャの構築は、企業のデジタルトランスフォーメーションにおける中核的な課題となっています。Apache Gravitinoは、次世代の統合メタデータ管理プラットフォームとして、マルチクラウド・マルチエンジン環境におけるデータガバナンスの完全なソリューションを提供します。様々なデータソースとコンピュートエンジンの統合管理をサポートするだけでなく、credential vendingメカニズムを通じてデータアクセスのセキュリティと制御可能性を確保します。

この記事では、Apache DorisとApache Gravitinoを深く統合し、Iceberg REST Catalogベースのモダンなlakehouseアーキテクチャを構築する方法について詳しく紹介します。Gravitinoの統合メタデータ管理と動的認証情報配布機能により、S3上のIcebergデータへの効率的で安全なアクセスを実現できます。

**このドキュメントを通じて、以下を迅速に理解できます：**

* **AWS環境の準備**: AWSでS3バケットとIAMロールを作成し、Gravitino用の安全な認証情報管理システムを設定し、一時的な認証情報の動的配布を実装する方法。
...
開始する前に、S3バケットと慎重に設計されたIAMロールシステムを含む完全なインフラストラクチャをAWS上に準備する必要があります。これは安全で信頼性の高いlakehouseアーキテクチャを構築するための基盤となります。

### 1.1 S3バケットの作成

まず、Icebergデータを格納する専用のS3バケットを作成します：
...
```
### 1.2 IAM ロールアーキテクチャの設計

安全な認証情報管理を実装するために、Gravitino が STS AssumeRole メカニズムを通じて使用する IAM ロールを作成する必要があります。この設計は、最小権限の原則と職務の分離というセキュリティのベストプラクティスに従います。
...
2. IAMロールの作成
   ...
   デモンストレーションを簡単にするため、AWSマネージドポリシーを直接使用します。本番環境では、より細かい粒度の権限制御を使用する必要があります。
...
```
## 3. Doris と Gravitino の接続

### 3.1 Vended Credentials の使用
