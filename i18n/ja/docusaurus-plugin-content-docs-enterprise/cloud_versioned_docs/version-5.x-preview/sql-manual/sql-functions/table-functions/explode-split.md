...
---
## Description
`explode_split`テーブル関数は、指定されたデリミタに従って文字列を複数の部分文字列に分割し、各部分文字列を個別の行に展開するために使用されます。
ネストされたデータ構造を標準的なフラットテーブル形式に平坦化するために、[`LATERAL VIEW`](../../../query-data/lateral-view.md)と一緒に使用する必要があります。
