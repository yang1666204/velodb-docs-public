---
{
  ...
  "description": "STR文字列の正規表現マッチングを行い、patternにヒットした部分を新しい文字列で置換します。",
  "language": "ja"
}
---
## Description

STR文字列の正規表現マッチングを行い、patternにマッチした部分を新しい文字列に置換します。

文字セットマッチングを扱う際は、Utf-8標準文字クラスを使用する必要があることに注意してください。これにより、関数は異なる言語の様々な文字を正しく識別し、処理することができます。
...
| -- | -- |
| `<str>` | このパラメータはVarchar型です。正規表現マッチングが実行される文字列を表します。リテラル文字列または文字列値を含むテーブルの列を指定できます。|
| `<pattern>` | このパラメータはVarchar型です。文字列をマッチするために使用される正規表現パターンです。パターンには、複雑なマッチングルールを定義するための様々な正規表現メタ文字や構造を含めることができます。|
| `<repl>` | このパラメータはVarchar型です。<pattern>にマッチする<str>の部分を置換する文字列です。パターン内のキャプチャグループを参照したい場合は、\1、\2などの後方参照を使用できます。\1は最初のキャプチャグループ、\2は2番目のキャプチャグループを参照します。|

## 戻り値

この関数は置換操作後の結果文字列を返します。戻り値はVarchar型です。<str>の部分が<pattern>にマッチしない場合、元の<str>が返されます。

## 例

基本的な置換の例。この例では、文字列'a b c'内のすべてのスペースがハイフンに置換されます。
...
+-----------------------------------+
```
キャプチャグループの使用。ここでは、パターン内のグループ(b)によって文字'b'がキャプチャされ、置換文字列内の後方参照\1を使用して<b>に置き換えられます。
...
+---------------------------------------------------------------------------------------------+
```
Insert とテストケース。このテストケースのセットでは、元の文字列、パターン、置換文字列を格納するテーブルを作成します。次に、様々なテストデータを挿入し、対応するパターンと置換文字列を使用して元の文字列に対してREGEXP_REPLACE操作を実行します。最後に、置換された文字列を取得して表示します。
...
+----------------------------------+
```
`pattern`が正規の正規表現として許可されていない場合、エラーをスローします。
