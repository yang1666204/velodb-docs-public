...
場合によっては、ユーザーが同じSQLを使用してUnique Keyモデルのテーブルをクエリする際に、複数回のクエリの結果が一致しない場合があります。そして、クエリ結果は常に2-3種類の間で変化します。
...
バージョン1.1.xで、ベクター化が有効になっており、クエリデータテーブルのbitmap型フィールドがNULL結果を返す場合、
...
バージョン1.1.xで、ベクター化が有効になっており、クエリデータテーブルのbitmp型フィールドがNULL結果を返す場合、
