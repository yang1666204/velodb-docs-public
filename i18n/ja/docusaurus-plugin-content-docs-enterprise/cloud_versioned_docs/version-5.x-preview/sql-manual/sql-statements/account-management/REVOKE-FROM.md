...
- catalog_name.*.*: 指定されたカタログ内のすべてのデータベースとテーブル
- catalog_name.db.*: 指定されたデータベース内のすべてのテーブル
- catalog_name.db.tbl: 指定されたデータベース内の特定のテーブル
...
ユーザーアイデンティティを指定します。ユーザーはCREATE USER文で作成されたユーザーである必要があります。ユーザーアイデンティティのホスト部分はドメイン名でも良く、ドメイン名の場合は権限取り消し時に1分の遅延が生じる可能性があります。
...
取り消すロールのカンマ区切りリスト。指定されたすべてのロールは存在している必要があります。
...
- ロールからコンピュートグループの使用権限を取り消す:
   ...
   ```
- ユーザーからすべてのstorage vaultに対する使用権限を取り消す：
