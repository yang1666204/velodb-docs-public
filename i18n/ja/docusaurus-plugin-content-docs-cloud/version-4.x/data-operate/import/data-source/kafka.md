...
Dorisでは、CREATE ROUTINE LOADコマンドを通じて永続的なRoutine Loadロードタスクを作成できます。これは単一テーブルロードと複数テーブルロードに分けることができます。詳細な構文については、CREATE ROUTINE LOADを参照してください。
...
```
**ステップ3: 単一テーブルにデータを読み込むためのRoutine Loadジョブを作成する**
...
複数のテーブルを同時にロードする必要があるシナリオでは、Kafka内のデータにテーブル名情報を含める必要があり、フォーマットは次のようになります：`table_name|data`。例えば、CSVデータをロードする場合、フォーマットは次のようになります：`table_name|val1,val2,val3`。テーブル名はDoris内のテーブル名と完全に一致する必要があることに注意してください。そうでない場合、ロードは失敗し、後で紹介するcolumn_mapping設定はサポートされません。
...
Dorisにロードするテーブルを作成します。構文は以下の通りです：
...
DISTRIBUTED BY HASH(user_id) BUCKETS 10;
```
**ステップ 3: 複数のテーブルにデータをロードするためのRoutine Loadジョブを作成する**
