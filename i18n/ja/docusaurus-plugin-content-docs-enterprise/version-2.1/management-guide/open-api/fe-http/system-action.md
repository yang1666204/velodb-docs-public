...
System ActionはDorisに組み込まれたProcシステムに関する情報を取得するために使用されます。
...
`/dbs/10003/10054/partitions/10053/10055`を例として：
...
```
データ部分の`column_names`はヘッダー情報であり、`href_columns`はテーブル内のどの列がハイパーリンク列であるかを示します。`rows`配列の各要素は1行を表します。その中で、`__hrefPaths`はテーブルデータではなく、ハイパーリンク列のリンクURLであり、`href_columns`の列と一対一で対応しています。
