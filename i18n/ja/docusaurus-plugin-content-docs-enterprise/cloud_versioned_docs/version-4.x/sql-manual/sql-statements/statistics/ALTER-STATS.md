...
> 同期マテリアライズドビュー（「同期マテリアライズドビュー」の章を参照）の識別子（つまり、名前）。テーブルは0個から複数のマテリアライズドビューを持つことができます。マテリアライズドビュー内のカラムの統計を設定する必要がある場合、<index_name>を使用してマテリアライズドビューの名前を指定する必要があります。指定されていない場合、ベーステーブル内のカラムのプロパティが設定されます。
...
ユーザーがテーブルに統計を手動で注入した後、そのテーブルはユーザーが手動で注入した統計を上書きしないよう、統計の自動収集（「統計の自動収集」の章を参照）に参加しなくなります。注入された統計が不要になった場合、drop stats文を使用してすでに注入された情報を削除することができ、これによりテーブルは再び自動収集を有効にできます。
...
- partテーブルのp_partkeyカラムに統計を注入します（index_nameが指定されていないため、ベーステーブルのカラム）。
  ...
  ```
- part テーブルの index1 マテリアライズドビューの col1 カラムに統計情報を注入する（index_name が指定されているため、マテリアライズドビューのカラム）。
