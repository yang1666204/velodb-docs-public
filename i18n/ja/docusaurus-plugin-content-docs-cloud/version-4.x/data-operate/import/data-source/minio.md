...
DorisはMinIOからファイルを読み込むための2つの方法を提供しています：
- S3 Loadを使用してMinIOファイルをDorisに読み込む方法（非同期読み込み方式）
- TVFを使用してMinIOファイルをDorisに読み込む方法（同期読み込み方式）
...
:::caution 注意
MinIOからS3 Loadを使ってデータをインポートする際は、以下の点に注意してください：

- MinIOがTLSなしのローカルネットワークにデプロイされている場合は、エンドポイントに明示的に`http://`を追加してください。例：`"s3.endpoint" = "http://localhost:9000"`
- S3 SDKはデフォルトでvirtual-hosted styleを使用しますが、MinIOではデフォルトで有効になっていません。path-styleアクセスを強制するため`"use_path_style" = "true"`を追加してください。
:::
...
- MinIOがTLSなしのローカルネットワークに展開されている場合、エンドポイントに明示的に`http://`を追加してください。例：`"s3.endpoint" = "http://localhost:9000"`
- S3 SDKはデフォルトでvirtual-hosted styleを使用しますが、MinIOはデフォルトでそれを有効にしていません。path-styleアクセスを強制するために`"use_path_style" = "true"`を追加してください。
:::
