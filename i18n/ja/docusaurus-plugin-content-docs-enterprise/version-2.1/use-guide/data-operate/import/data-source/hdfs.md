...
{
  "title": "HDFS | データソース",
  "description": "Dorisは、HDFSからファイルをロードする2つの方法を提供します：",
  "language": "ja"
}
---
# HDFS

DorisはHDFSからファイルを読み込む2つの方法を提供しています：
- HDFS Loadを使用してHDFSファイルをDorisに読み込む方法（非同期読み込み方式）
- TVFを使用してHDFSファイルをDorisに読み込む方法（同期読み込み方式）
...
CSVファイルhdfsload_example.csvを作成します。このファイルはHDFS上に保存され、内容は以下の通りです：
...
CSV ファイル hdfsload_example.csv を作成します。このファイルは HDFS に保存され、その内容は以下の通りです：
