...
---
## 説明
他の入力ARRAY パラメータの内部データに対して対応する式計算を実行するために、オプションのlambda式を入力パラメータとして使用します。計算結果が0以外の値を返す場合は1を返し、それ以外の場合は0を返します。
lambda式には1つ以上のパラメータが入力され、これは後の入力配列列数と一致している必要があります。lambdaでは正当なスカラー関数を実行できますが、集約関数などはサポートされていません。
...
|---|---|
| `<lambda>` | 有効なスカラー関数を実行できるが、集約関数はサポートしないlambda式。 |
| `<arr1>` | 計算が必要な配列arr1 |
| `<arr2>` | 計算が必要な配列arr2 |
