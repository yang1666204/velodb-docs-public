...
{
  "title": "一時パーティション",
  "description": "Dorisはパーティションテーブルへの一時パーティションの追加をサポートしています。",
  "language": "ja"
}
---
Dorisは、パーティション化されたテーブルに一時的なパーティションを追加することをサポートしています。一時的なパーティションは、通常のクエリでは取得されず、特別なクエリ文を通してのみクエリできる点で、通常のパーティションとは異なります。
...
- **アトミックな上書き操作**: ユーザーは、古いデータを削除してから新しいデータをインポートする間のデータ損失なしに、パーティションのデータを書き換えることができます。この場合、一時的なパーティションを作成し、新しいデータを一時的なパーティションにインポートし、replace操作を通して元のパーティションをアトミックに置き換えることができます。パーティション化されていないテーブルでのアトミックな上書き操作については、[replace table documentation](../../data-operate/delete/atomicity-replace)を参照してください。
...
- Drop操作を使用してデータベースやテーブルを直接削除した後、そのデータベースやテーブルはRecoverコマンドを使用して復旧できますが（制限時間内）、一時パーティションは復旧されません。
...
- Truncateコマンドを使用してテーブルを空にすると、そのテーブルの一時パーティションが削除され、復旧することはできません。
...
- テーブルに一時パーティションがある場合、Alterコマンドを使用してそのテーブルに対してSchema Change、Rollup、またはその他の変更操作を実行することはできません。

- テーブルが変更操作中の場合、そのテーブルに一時パーティションを追加することはできません。
