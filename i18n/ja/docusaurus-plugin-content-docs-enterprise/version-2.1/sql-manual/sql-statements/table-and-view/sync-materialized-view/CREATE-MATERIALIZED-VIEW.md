...
- 同期マテリアライズドビューは単一TableのSELECT文のみをサポートし、WHERE、GROUP BY、ORDER BY句をサポートしますが、JOIN、HAVING、LIMIT句、またはLATERAL VIEWはサポートしません。
...
- 単一Table上のマテリアライズドビューが多すぎると、データインポートの効率に影響を与える可能性があります：データをインポートする際、マテリアライズドビューとBaseTableのデータが同期的に更新されます。Table上のマテリアライズドビューが多すぎる場合、単一のインポート操作で複数のTableに同時にデータをインポートするのと同様に、インポート速度が遅くなる可能性があります。
