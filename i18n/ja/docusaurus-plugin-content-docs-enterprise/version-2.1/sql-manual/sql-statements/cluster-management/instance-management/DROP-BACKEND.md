...
1. このコマンドを使用してBEノードをオフラインにすることは推奨されません。このコマンドはBEノードをクラスタから直接削除します。現在のノード上のデータは他のBEノードに負荷分散されません。クラスタに単一レプリカTableがある場合、データ損失が発生する可能性があります。より良いアプローチは、[DECOMMISSION BACKEND](./DECOMMISSION-BACKEND.md)コマンドを使用してBEノードを適切にオフラインにすることです。
2. この操作は高リスクな操作であるため、このコマンドを直接実行する場合：
   ...
   ```
上記のプロンプトメッセージが表示されます。何をしているかを理解している場合は、`DROP`キーワードを`DROPP`に置き換えて続行できます：
