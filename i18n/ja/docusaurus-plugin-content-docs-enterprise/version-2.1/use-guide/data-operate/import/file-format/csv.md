...
---
この文書では、DorisにCSV形式のデータファイルを読み込む方法について説明します。Dorisは柔軟なCSV形式の設定をサポートしており、カスタムデリミタ、フィールド囲み文字を含み、さまざまなシナリオでのデータ読み込み要件を満たすための多様な読み込み方法を提供します。
...
以下の表は、各読み込み方法でサポートされているCSV形式パラメータを示しています：
...
| 列区切り文字 | `\t` | column_separator | COLUMNS TERMINATED BY | COLUMNS TERMINATED BY | COLUMNS TERMINATED BY | column_separator |
...
| スキップ行数 | 0 | skip_lines | PROPERTIES.skip_lines | サポートなし | IGNORE LINES | skip_lines |
| ダブルクォート除去 | false | trim_double_quotes | サポートなし | PROPERTIES.trim_double_quotes | サポートなし | trim_double_quotes |
| 圧縮形式 | plain | compress_type | PROPERTIES.compress_type | サポートなし | サポートなし | compress_type |

:::tip 注意
1. Stream Load: パラメータはHTTPヘッダーを通じて直接指定します（例：`-H "line_delimiter:\n"`）
2. Broker Load: パラメータはSQL文で指定します：
   - 区切り文字は`COLUMNS TERMINATED BY`、`LINES TERMINATED BY`で指定
   - その他のパラメータは`PROPERTIES`で指定（例：`PROPERTIES("compress_type"="gz")`）
3. Routine Load: パラメータはSQL文で指定します：
   - 区切り文字は`COLUMNS TERMINATED BY`で指定
   - その他のパラメータは`PROPERTIES`で指定（例：`PROPERTIES("enclose"="\"")`）
4. MySQL Load: パラメータはSQL文で指定します：
   - 区切り文字は`LINES TERMINATED BY`、`COLUMNS TERMINATED BY`で指定
...
- MySQL Protocolでの特別な処理:
  - 不可視文字には追加のバックスラッシュが必要
  - 例えば、Hiveの `\x01` は Broker Load では `\\x01` と記述する必要がある
- 使用例とケース:
  - 一般的な可視文字:
...
- フィールドに列区切り文字と行区切り文字の両方が含まれている場合:
    ...
    ```
#### Escape Character
- 目的: エンクロージャ文字と同じ文字をフィールド内でエスケープします
- 制限事項: 単一バイト文字のみサポート、デフォルトは `\` です
- 使用例:
...
- csv_with_names_and_types フォーマットを使用する：
    ...
    ```
#### Trim Double Quotes
- 目的: CSVファイル内の各フィールドから最外側のダブルクォートを削除します
- タイプ: Boolean
- デフォルト値: false
- 使用例とサンプル:
  - Trim Double Quotes:
...
- 注意:
  - tarはファイルパッケージ形式であり、圧縮形式ではないため、サポートされていません
  - tarでパッケージされたファイルを使用する場合は、まず展開してからロードしてください
