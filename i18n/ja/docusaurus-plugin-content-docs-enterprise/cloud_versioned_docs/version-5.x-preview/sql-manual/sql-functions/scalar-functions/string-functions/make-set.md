...
{
  "title": "MAKE_SET",
  "description": "MAKESET関数は、ビットマスク（ビット）に基づいて複数の文字列パラメータから文字列を選択し、結合します。",
  "language": "ja"
}
---
## 説明
...
動作はMySQLの[MAKE_SET](https://dev.mysql.com/doc/refman/8.4/en/string-functions.html#function-make-set)と一致します。
...
| -------- | ----------------------------------------- |
| `<bit>` | ビットマスク値。バイナリビットで選択する文字列を指定します。型: BIGINT |
| `<str1>`, `<str2>`, ... | 結合対象の文字列パラメータ（可変引数）。型: VARCHAR |
...
- 対応するビットがパラメータの範囲を超える場合、そのビットを無視します
- バイナリビットは右から左へカウントし、ビット0は最初の文字列パラメータに対応します
- `<bit>`が0の場合、空文字列を返します
...
+-------------------------------+
```
5. ビットがパラメータ範囲を超えています: bit = 15 (2進数 1111、4ビットを選択しますが、パラメータは2個のみ)
