...
{
  "title": "負荷に基づくバッチ削除",
  "description": "delete操作はデータ更新の特殊な形式です。プライマリキーモデル（Unique Key）テーブルにおいて、",
  "language": "ja"
}
---
## 負荷に基づくバッチ削除

削除操作はデータ更新の特殊な形式です。主キーモデル（Unique Key）テーブルでは、Dorisはデータロード時に削除サインを追加することで削除をサポートします。
...
- **テーブル構造**: 削除サインは主キーテーブル内の隠しカラム`__DORIS_DELETE_SIGN__`として格納されます。このカラムの値が1の場合、削除サインが有効であることを示します。
