...
この関数は、指定されたフォーマットに基づいて、入力されたdatetime文字列をDATETIME値に変換します。
...
|------------------|------------------------------------------------------------------------------------------------------------------|
| `<datetime_str>` | 必須。変換対象の入力datetime文字列。                                                             |
| `<format>`       | 必須。指定されたdatetimeフォーマット文字列。`%Y-%m-%d %H:%i:%s`など。具体的なフォーマットパラメータについては、DATE_FORMATドキュメントを参照してください。 |
...
- 入力の`<datetime_str>`または`<format>`が無効な場合、関数はNULLを返します。
