...
セカンドテーブルまたはサブクエリの値に基づいて、テーブルの値を挿入、更新、削除します。セカンドテーブルがターゲットテーブルに挿入する新しい行、更新する変更された行、または削除するマークされた行を含む変更ログである場合、マージは有用です。
...
> 新しいカラム値の対応する式を使用して、ターゲットテーブルの指定されたカラムを更新します（ターゲットとソースの両方のリレーションを参照できます）。  
> 単一のSETサブクローズでは、複数のカラムを更新対象として指定できます。

**DELETE**

> ソースとマッチした場合に、ターゲットテーブルの行を削除します。
...
> オプションで、ソースから値を挿入するターゲットテーブルの1つ以上のカラムを指定します。
> デフォルト: 値なし（ターゲットテーブルの全カラムが挿入されます）
...
結合後に、同一のターゲットテーブル行に対して複数の操作（更新、削除、または挿入など）が同時に適用される場合、動作はINSERT文と類似します：Sequenceカラムが存在する場合、最終的に書き込まれるデータはSequenceカラムの値によって決定されます。存在しない場合は、いずれかの行が任意に書き込まれます。
