---
{
    ...
    "description": "After planning the cluster topology, you can deploy the Doris cluster in Kubernetes."
}
---
...
```text
velodb/doris.fe-ubuntu:2.0.2
velodb/doris.be-ubuntu:2.0.2
```
...
docker pull velodb/doris.fe-ubuntu:2.0.2
docker pull velodb/doris.be-ubuntu:2.0.2

## save docker image as a tar package
docker save -o doris.fe-ubuntu-v2.0.2.tar velodb/doris.fe-ubuntu:2.0.2
docker save -o doris.be-ubuntu-v2.0.2.tar docker pull velodb/doris.be-ubuntu:2.0.2
```
...
Before installation, you need to add a deployment warehouse. If it has been added, you can directly install Doris Cluster. Otherwise, please refer to the operation of **Add Deployment Warehouse** when adding [Deploy Doris Operator](<./install-operator#Option 3: Helm deploys Doris Operator>)
...
```bash
wget https://charts.velodb.com/doris-2.0.6.tgz
```
