...
banker's roundingアルゴリズムの規則に従い、指定された小数位に丸める際：

- 丸める桁が5で、その後に他の非ゼロ桁がない場合、直前の桁がチェックされます：
   - 直前の桁が偶数の場合、5は単純に削除されます。
   - 直前の桁が奇数の場合、数値は切り上げられます。

- 丸める桁が5より大きいか、その後に非ゼロ桁がある場合、従来の丸め規則が適用されます：桁が5以上の場合は切り上げ、それ以外は切り下げます。

例：

- 値2.5の場合、5の前の桁2が偶数であるため、結果は2に丸められます。

- 値3.5の場合、5の前の桁3が奇数であるため、結果は4に丸められます。
...
- デフォルトでは、パラメータd = 0の場合、banker's rounding algorithmを使用してxを計算して得られる整数を返します。

- dが負の場合、小数点の左側の最初の桁が0である整数を返します。

- xとdの両方がNULLの場合、NULLを返します。

- dがカラムを表し、xがDecimal型の場合、同じ精度の浮動小数点数を返します。
