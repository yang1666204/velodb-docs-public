...
CONCAT関数は複数の文字列を順番に連結して1つの文字列にします。この関数は可変個の引数をサポートしており、文字列処理において最も基本的でよく使用される関数の一つです。データの連結、レポート生成、動的SQLの構築などのシナリオで広く使用されています。いずれかの引数がNULLの場合、結果全体がNULLになることに注意してください。
