...
- `arr`：ARRAY<T> 型、位置インデックスを生成する配列。列名または定数値をサポートします。
...
戻り値の意味:
- 入力配列と同じ長さの新しい配列を返します。各位置には配列内の対応する要素の位置インデックス（1から開始）が含まれます
- NULL: 入力配列が NULL の場合

使用上の注意:
- この関数は配列内の各要素に対して位置インデックスを生成し、1から開始して増加します
- 空の配列は空の配列を返し、NULL 配列は NULL を返します
- 配列要素内の null 値について: null 要素も対応する位置インデックスを生成します
