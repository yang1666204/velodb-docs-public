...
{
  "title": "転置インデックス",
  "description": "転置インデックスは、情報検索の分野で一般的に使用されるインデックス技術です。",
  "language": "ja"
}
---
## インデックスの原理

[転置インデックス](https://en.wikipedia.org/wiki/Inverted_index)は、情報検索の分野で一般的に使用されるインデックス技術です。テキストを個々の単語に分割し、単語 -> 文書IDのインデックスを構築することで、特定の単語を含む文書を素早く検索できます。

バージョン2.0.0以降、Dorisは転置インデックスをサポートしており、テキストタイプでの全文検索、通常の数値型と日付型での等価・範囲クエリに使用でき、大量のデータから条件を満たす行を素早くフィルタリングできます。

Dorisの転置インデックス実装では、テーブルの各行が文書に対応し、各列が文書内のフィールドに対応します。従って、転置インデックスを使用することで、特定のキーワードを含む行を素早く特定し、WHERE句を高速化できます。

Dorisの他のインデックスとは異なり、転置インデックスはストレージ層で独立したファイルを使用し、データファイルと一対一で対応しながらも物理的には独立して保存されます。このアプローチにより、データファイルを書き換えることなくインデックスの作成と削除が可能になり、処理オーバーヘッドを大幅に削減できます。

## 使用シナリオ

転置インデックスは幅広い用途があり、等価検索、範囲検索、全文検索（キーワードマッチング、フレーズマッチングなど）を高速化できます。テーブルには複数の転置インデックスを持つことができ、クエリ時に複数の転置インデックスの条件を任意に組み合わせることができます。

転置インデックスの機能を簡単に紹介します：

**1. 文字列タイプの全文検索を高速化**
  - キーワード検索をサポート。複数キーワードの同時マッチング`MATCH_ALL`と任意のキーワードマッチング`MATCH_ANY`を含む
...
**2. 通常の等価・範囲クエリを高速化し、BITMAPインデックスの機能を包含・置換**
...
- AND、OR、NOTを使用した複数条件の任意の論理結合をサポート

**4. 柔軟で効率的なインデックス管理**

- テーブル作成時の転置インデックス定義をサポート

- 既存テーブルへの転置インデックス追加をサポート。テーブル内の既存データを書き換えずに増分インデックス構築

- 既存テーブルからの転置インデックス削除をサポート。テーブル内の既存データを書き換えずに実行

:::tip

転置インデックスの使用にはいくつかの制限があります：

1. 浮動小数点型のFLOATとDOUBLEは精度の問題があるため、精度が不正確で転置インデックスをサポートしません。解決策として、正確な精度を持つDECIMAL型を使用することで、転置インデックスをサポートできます。

2. 一部の複合データ型はまだ転置インデックスをサポートしていません。MAP、STRUCT、JSON、HLL、BITMAP、QUANTILE_STATE、AGG_STATEが含まれます。

3. DUPLICATEとMerge-on-Writeが有効なUNIQUEテーブルモデルは任意の列での転置インデックス構築をサポートします。ただし、AGGREGATEとMerge-on-Writeが無効なUNIQUEモデルはKey列でのみ転置インデックス構築をサポートします。非Key列には転置インデックスを設定できません。これは、これら2つのモデルではマージのためにすべてのデータを読み取る必要があり、事前フィルタリングにインデックスを使用できないためです。
...
**1. `idx_column_name(column_name)` は必須です。`column_name` はインデックス用のカラム名で、事前に定義されたカラムである必要があります。`idx_column_name` はインデックス名で、テーブルレベルでユニークである必要があります。推奨される命名規則: カラム名の前に `idx_` プレフィックスを付ける**
...
<details>
  ...
  <p>- デフォルトでは未指定で、トークン化が行われないことを意味します</p>
  <p>- `english`: 英語のトークン化。英語テキストを含むカラムに適しており、スペースと句読点でトークン化し、高性能です</p>
  <p>- `chinese`: 中国語のトークン化。主に中国語テキストを含むカラムに適していますが、英語のトークン化よりも性能は劣ります</p>
  <p>- `unicode`: Unicodeトークン化。中国語と英語の混在、および多言語テキストの混在に適しています。メールのプレフィックスとサフィックス、IPアドレス、文字と数字の混在文字列をトークン化でき、中国語を文字単位でトークン化できます。</p>
...
<details>
  ...
  **トークン化モードを指定します。`parser = chinese` で現在サポートされているモードは以下の通りです:**
  ...
  <p>- デフォルトは coarse_grained です</p>
</details>

<details>
  <summary>support_phrase</summary>

  **インデックスが MATCH_PHRASE フレーズクエリ加速をサポートするかどうかを指定します**
  <p>- true: サポートされますが、インデックスはより多くのストレージ容量を必要とします</p>
  <p>- false: サポートされず、ストレージ効率が良く、MATCH_ALL を使用して複数のキーワードをクエリできます</p>
  <p>- バージョン 2.0.14, 2.1.5, 3.0.1 以降では、parser が設定されている場合のデフォルトは true です。それ以外の場合はデフォルトで false になります。</p>

  例えば、以下の例では中国語のトークン化、粗粒度モード、フレーズクエリ加速のサポートを指定しています。
...
<details>
  ...
  **トークン化前にテキストを前処理することを指定し、通常はトークン化の動作に影響を与えます**
  ...
  例えば、以下の例ではドットとアンダースコアをスペースで置換し、それらを単語区切り文字として扱うことで、トークン化の動作に影響を与えます。
...
<details>
  ...
  **非トークン化文字列インデックスの長さ制限を指定します（parserが指定されていない場合）**
...
<details>
  ...
  **大文字小文字を区別しないマッチングのためにトークンを小文字に変換するかどうか**
  <p>- true: 小文字に変換する</p>
  <p>- false: 小文字に変換しない</p>
  <p>- バージョン2.1.2以降、デフォルトはtrueで、自動的に小文字に変換されます。それ以前のバージョンのデフォルトはfalseです。</p>
</details>
...
<details>
  ...
  **使用するストップワードリストを指定し、トークナイザーの動作に影響します**
  <p>- デフォルトの組み込みストップワードリストには、'is'、'the'、'a'などの意味のない単語が含まれています。書き込みまたはクエリ時に、トークナイザーはストップワードリストに含まれる単語を無視します。</p>
  <p>- none: 空のストップワードリストを使用する</p>
</details>

**4. `COMMENT`はインデックスコメントを指定するためのオプションです**

### 既存のテーブルへの転置インデックスの追加
...
`CREATE / ADD INDEX`操作はインデックス定義の追加のみを行います。この操作後に書き込まれる新しいデータは転置インデックスを生成しますが、既存のデータについては`BUILD INDEX`を使用してインデックス作成をトリガーする必要があります：
...
2.1.4より前のバージョンでは、`BUILD INDEX`は成功するまで再試行を続けていました。このバージョン以降、失敗とタイムアウトメカニズムにより無限の再試行を防ぎます。
...
`DROP INDEX`はインデックス定義を削除するため、新しいデータはそのインデックスに書き込まれなくなります。これにより、各BEで複数のスレッドによって実行されるインデックス削除を実行する非同期タスクが作成されます。スレッド数はBEパラメータ`alter_index_worker_count`を使用して設定でき、デフォルト値は3です。

:::

### 転置インデックスの表示

-- 構文1: テーブルスキーマのINDEXセクションでUSING INVERTEDが指定されている場合は転置インデックスを示します
SHOW CREATE TABLE table_name;

-- 構文2: IndexTypeがINVERTEDの場合は転置インデックスを示します
...
転置クエリ高速化は、セッション変数 `enable_inverted_index_query` を使用して切り替えることができ、デフォルトで true に設定されています。インデックスの高速化効果を確認するには、false に設定してオフにすることができます。
...
トークン化の実際の効果を確認したり、テキストの一部をトークン化するには、検証のために `TOKENIZE` 関数を使用できます。
...
--   USING INVERTED でインデックスタイプを転置インデックスとして指定
--   PROPERTIES("parser" = "english") で "english" トークナイザーの使用を指定。その他のオプションには中国語トークン化用の "chinese" や混合言語トークン化用の "unicode" があります。"parser" パラメータが指定されない場合、トークン化は適用されません。
...
- Adding an inverted index for the `timestamp` column
  ...
  ```sql
-- 日付時刻型の場合、USING INVERTEDはパーサーの指定を必要としません
  -- CREATE INDEXはインデックスを作成するための一つの構文です。別の方法は後で示されます
...
SELECT count() FROM hackernews_1m WHERE parent = 11189;
  ...
  -- 数値型の場合、USING INVERTEDはパーサーの指定が不要
  -- ALTER TABLE t ADD INDEXはインデックス作成の第二の構文
  ALTER TABLE hackernews_1m ADD INDEX idx_parent(parent) USING INVERTED;


  -- BUILD INDEXを実行して既存データに対する転置インデックスを作成
...
SELECT count() FROM hackernews_1m WHERE author = 'faster';
  ...
  -- ここでは、`author`列をトークン化せずにUSING INVERTEDを使用し、単一の用語として扱います
  ALTER TABLE hackernews_1m ADD INDEX idx_author(author) USING INVERTED;

  
  -- 既存データに対して転置インデックスを追加するためにBUILD INDEXを実行します
  BUILD INDEX idx_author ON hackernews_1m;

  
100万件のauthor記録に対する増分インデックスの作成は、わずか1.5秒で完了しました。
...
+-------+---------------+---------------+----------------------------------------------------+-------------------------+-------------------------+---------------+----------+------+----------+
```
インデックス作成後、文字列等価マッチングも大幅な高速化を示しました。
