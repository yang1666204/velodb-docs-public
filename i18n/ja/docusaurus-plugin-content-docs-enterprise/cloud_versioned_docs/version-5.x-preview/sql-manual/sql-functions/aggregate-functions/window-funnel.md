...
WINDOW_FUNNEL関数は、指定された時間窓内でイベントチェーンを検索し、イベントチェーンで完了したステップの最大数を計算することで、ユーザー行動シーケンスを分析します。この関数は、Webサイト訪問から最終購入までのユーザーコンバージョンの分析など、コンバージョンファネル分析に特に有用です。
...
| `<timestamp>` | timestamp はDATETIME型の列を指定し、スライディング時間ウィンドウはその列に対して動作します |
...
**Mode**
    ...
    - `deduplication`: イベントの系列において同一のイベントが継続する場合、そのような繰り返しイベントは以降の処理を中断します。例えば、配列パラメータが[event1='A', event2='B', event3='C', event4='D']で、元のイベントチェーンが"A-B-C-B-D"の場合、イベントBが繰り返されるため、フィルタリング後のイベントチェーンは"A-B-C"のみとなり、最大イベントレベルは3となります。

    - `fixed`: 他のイベントの介入を許可しません。例えば、配列パラメータが[event1='A', event2='B', event3='C', event4='D']で、元のイベントチェーンがA->B->D->Cの場合、DでA->B->Cの検索が停止し、最大イベントレベルは2となります。

    - `increase`: 厳密に増加するタイムスタンプを持つイベントにのみ条件を適用します。

## 戻り値
指定された時間ウィンドウ内で完了した連続するステップの最大数を表す整数を返します。

## 例

### example1: default モード

`default`モードを使用して、異なる`user_id`に対応する連続するイベントの最大数を、時間ウィンドウ`5`分で見つけます:
...
+---------+-------+
```
`uesr_id=100123`の場合、`payment`イベントが発生した時刻が時間窓を超えているため、マッチした イベントチェーンは`login-visit-order`です。

### example2: deduplicationモード

`deduplication`モードを使用して、異なるuser_idに対応する連続イベントの最大数を求めます。時間窓は1時間です：
...
+---------+-------+
```
`uesr_id=100123`の場合、`order`イベントがマッチした後、イベントチェーンは`login2`イベントによって中断されるため、マッチしたイベントチェーンは`login-visit-order`となります。

### example4: increaseモード

`increase`モードを使用して、異なる`user_id`に対応する連続イベントの最大数を、`1`時間のタイムウィンドウで調べます：
...
+---------+-------+
```
`uesr_id=100123`の場合、`payment`イベントのタイムスタンプと`order`イベントのタイムスタンプが同一秒内で発生し、インクリメントされないため、マッチしたイベントチェーンは`login-visit-order`となります。
