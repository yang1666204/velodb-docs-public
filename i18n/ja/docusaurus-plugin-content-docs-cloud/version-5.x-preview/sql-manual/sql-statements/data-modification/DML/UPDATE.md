...
{
  "title": "UPDATE",
  "description": "この文はデータを更新するために使用されます。UPDATE文は現在UNIQUE KEYモデルのみをサポートしています。",
  "language": "ja"
}
---
## Description
...
UPDATE 操作は現在 Value 列の更新のみをサポートしています。Key 列の更新については Using FlinkCDC to update Key column を参照してください。
...
+ where condition: 更新されることが期待される条件。trueまたはfalseを返す式を指定可能

#### オプションパラメータ

+ cte: Common Table Expression、例：'WITH a AS SELECT * FROM tbl'
+ table_alias: テーブルのエイリアス
+ FROM additional_tables: 更新する行を選択するため、または新しい値を設定するために使用する1つ以上のテーブルを指定します。ここで対象テーブルを使用したい場合は、明示的にエイリアスを指定する必要があります。

#### 注意

現在のUPDATE文はUNIQUE KEYモデルでの行更新のみをサポートしています。

## 例

`test`テーブルはuniqueモデルテーブルで、4つのカラム：k1、k2、v1、v2を含みます。k1、k2はキー、v1、v2は値で、集約メソッドはReplaceです。
