...
{
  "title": "TOPN_WEIGHTED",
  "description": "TOPNWEIGHTED関数は、指定された列で重み付きカウントを用いてN個の最も頻繁な値を返します。通常のTOPN関数とは異なり、",
  "language": "ja"
}
---
## 概要

TOPN_WEIGHTED関数は、重み付きカウントを使用して、指定された列で最も頻度の高いN個の値を返します。通常のTOPN関数とは異なり、TOPN_WEIGHTEDでは重みを通じて値の重要度を調整することができます。
