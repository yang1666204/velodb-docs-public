---
{
  "title": "Confluent Cloud セットアップ",
  "description": "Confluent Cloud クラスターとVeloDB統合用のサンプルデータを設定する",
  "language": "ja"
}
---
# Confluent Cloud セットアップ

このガイドでは、VeloDBへの接続用にサンプルデータを含むConfluent Cloud Kafkaクラスターをセットアップする方法について説明します。
...
3. クラスタータイプを選択します：
   - **Basic** - 無料ティア、開発およびテストに適している
   - **Standard** - より高い制限を持つプロダクションワークロード

4. **Provider and region**を選択します：
   - 最適なパフォーマンスのため、VeloDBウェアハウスと同じクラウドプロバイダーとリージョンを選択してください
   - 例: AWS us-east-1
...
3. **Sample Data**または**Datagen Source**を検索します

4. **Sample Data**をクリックしてクイック起動ダイアログを開きます

![Launch Sample Data](/images/cloud/integration/confluent-cloud/confluent3.png)

5. コネクターを設定します：
   - **Topic**: `sample_data_orders`（またはお好みの名前）
   - **Select a template**: **Orders**を選択（このガイドでは推奨）
...
JSON形式を使用するには、起動前に**Additional configuration**をクリックして以下を設定します：
- **Output record value format**: `JSON`

AVROでコネクターを既に作成した場合は、JSON出力用に設定された新しいDatagenコネクターで新しいトピックを作成してください。
:::
...
1. クラスター下の左サイドバーで**API Keys**に移動します
...
4. **Next**をクリックし、次に**Create**をクリックします

5. **両方の値を保存**してください - VeloDBで必要になります：
   - **API Key**（ユーザー名）
   - **API Secret**（パスワード）
