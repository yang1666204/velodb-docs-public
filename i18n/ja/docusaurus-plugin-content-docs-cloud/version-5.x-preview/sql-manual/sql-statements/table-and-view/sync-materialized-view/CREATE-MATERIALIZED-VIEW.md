...
> マテリアライズドビューの識別子（すなわち、名前）を指定します。同期マテリアライズドビューはテーブルに基づいて作成されるため、名前は同じテーブル内で一意である必要があります。
>
...
- 単一テーブル上のマテリアライズドビューが多すぎると、データインポートの効率に影響を与える可能性があります：データをインポートする際、マテリアライズドビューとBaseテーブルのデータが同期的に更新されます。テーブル上にマテリアライズドビューが多すぎる場合、単一のインポート操作で複数のテーブルに同時にデータをインポートするのと同様に、インポート速度が遅くなる可能性があります。
