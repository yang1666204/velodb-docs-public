...
{
  "title": "HLL",
  "description": "HLLは近似重複除去に使用され、大量のデータを扱う際にCount Distinctよりも優れた性能を発揮します。",
  "language": "ja"
}
---
HLLは近似重複除去に使用され、大量のデータを扱う際にはCount Distinctよりも優れたパフォーマンスを発揮します。HLLのロードはhll_hashのような関数と組み合わせる必要があります。詳細なドキュメントについては、HLLを参照してください。

## 使用例

### ステップ1：データの準備

次のCSVファイルを作成します：test_hll.csv
