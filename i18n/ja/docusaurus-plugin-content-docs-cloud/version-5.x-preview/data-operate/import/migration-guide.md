...
1. Table、ビュー、マテリアライズドビューのメタデータ移行（同期・非同期両方）
...
1. 現在のメタデータ移行の対象範囲には、2つの主要カテゴリが含まれます：
   1. **Table、ビュー、マテリアライズドビュー（同期・非同期）**
   2. **ユーザーと権限**
...
- **Table移行。** ソース Doris クラスターで `SHOW CREATE TABLE` コマンドを使用してTable DDL を取得します。必要な調整を行った後、対象 VeloDB Cloud 環境で対応するTableを作成します。参考：https://doris.apache.org/docs/dev/sql-manual/sql-statements/table-and-view/table/SHOW-CREATE-TABLE
...
各Tableに対応するRoutineload名のリスト、PAUSEおよびRESUME文を収集します。スクリプトを実行します
...
1. DorisからS3などのオブジェクトストレージにデータをエクスポートする場合は、`SELECT INTO OUTFILE`をParquetファイル形式で使用して、Table単位またはパーティション単位で行うことを推奨します。
2. S3などのオブジェクトストレージからVeloDB Cloudにデータをインポートする場合は、TVFをParquetファイル形式で使用して、Table単位またはパーティション単位で行うことを推奨します。

###### 3.4.2.1.1 Dorisからのエクスポート

DorisからS3などのオブジェクトストレージにデータをエクスポートする場合は、`SELECT INTO OUTFILE`をParquetファイル形式で使用して、Table単位またはパーティション単位で行うことを推奨します。詳細については、以下を参照してください：https://doris.apache.org/docs/dev/data-operate/export/outfile#export-to-object-storage
...
1. `SELECT COUNT(*) FROM table`を使用して、データ移行後にDorisソースTableとVeloDB CloudターゲットTableのレコード数が一致しているかを検証します。
2. `SELECT SUM(calculable_column) FROM table`を使用して、データ移行後にDorisソースTableとVeloDB CloudターゲットTableの詳細データが完全に一致しているかを検証します。
