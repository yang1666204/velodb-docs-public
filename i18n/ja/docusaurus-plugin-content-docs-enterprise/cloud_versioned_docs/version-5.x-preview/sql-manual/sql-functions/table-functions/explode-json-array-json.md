...
---
## 説明
`explode_json_array_json`テーブル関数はJSON配列を受け取ります。その実装ロジックは、JSON配列をarray型に変換してから`explode`関数を呼び出して処理することです。この動作は`explode(cast(<json_array> as Array<JSON>))`と同等です。
