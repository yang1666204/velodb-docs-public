...
{
  "title": "TOPN_WEIGHTED",
  "description": "TOPNWEIGHTED関数は、指定された列において重み付きカウントによる最も頻度の高いN個の値を返します。通常のTOPN関数とは異なり、",
  "language": "ja"
}
---
## 説明

TOPN_WEIGHTED関数は、重み付きカウントを使用して、指定された列の最も頻度の高いN個の値を返します。通常のTOPN関数とは異なり、TOPN_WEIGHTEDでは重みを通じて値の重要度を調整することができます。
...
| `<top_num>` | 返却する最頻値の数。正の整数である必要があります。 |
