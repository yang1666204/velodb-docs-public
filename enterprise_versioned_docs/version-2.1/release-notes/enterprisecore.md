...
- Hudi
  - Enabled the Hadoop Hudi JNI Scanner to improve the scanning performance and compatibility of Hudi tables.
- JDBC Catalog
  - Added JDBC Catalog support for **Dameng** and **Kingbase**.
  - Supported the Dameng NVARCHAR data type.
- Planner & SQL Governance
  - Validated SQL regex blocking rules in advance before the execution plan phase to enhance security and controllability.
- Privileges
  - Optimized the query and write performance of PrivTable to improve the efficiency of privilege management.

Bug Fixes

- Load
  - Fixed the issue where Stream Load incorrectly redirected to HTTPS in certain scenarios.
  - Fixed the source slot mapping error when the file column was not explicitly specified.
  - Reset MemTable immediately after insertion failure to prevent BE Crash.
- Query Engine / Nereids
  - Fixed issues related to Runtime Filter.
  - Fixed the exception when CTEConsumer was used as the Runtime Filter Target.
  - Fixed the issue where SQL Pattern information was lost in the SQL Block Rule.
- Functions & Data Types
  - Fixed the row count statistics error of the `json_extract_string` function.
  - Fixed the multiplication overflow issue of the `decimal256` type.
- Storage (MOW)
  - Fixed the issue where the Delete Bitmap was not properly cleaned up when removing unused Rowsets.
- Exception & Stability
  - Fixed the use-after-free issue caused by the non-thread-safe implementation of `Exception::to_string()`.
- View & Auth
  - Fixed the issue where column definitions were not displayed in the `SHOW CREATE VIEW` command.
