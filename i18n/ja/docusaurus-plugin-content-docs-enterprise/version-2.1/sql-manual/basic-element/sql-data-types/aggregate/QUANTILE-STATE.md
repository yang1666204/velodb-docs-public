...
QUANTILE_STATEは分位数の近似値を計算するための型です。同じキーを持つ異なる値は、読み込みプロセス中に事前集約されます。集約される値の数が2048を超えない場合、すべてのデータが詳細に記録されます。集約される値の数が2048を超える場合、[TDigest](https://github.com/tdunning/t-digest/blob/main/docs/t-digest-paper/histo.pdf)アルゴリズムを使用してデータを集約（クラスタ化）し、クラスタ化後の重心点を保存します。
...
関連関数:
    ...
    TO_QUANTILE_STATE(DOUBLE raw_data [,FLOAT compression]):
       ...
       値が大きいほど分位数近似計算の精度が高くなり、メモリ消費量が増加し、計算時間が長くなります。
...
```
この方法では、FEプロセスの再起動後にconfigがリセットされます。永続的な設定のためには、fe.conf内にconfig `enable_quantile_state_type=true`を追加できます。
