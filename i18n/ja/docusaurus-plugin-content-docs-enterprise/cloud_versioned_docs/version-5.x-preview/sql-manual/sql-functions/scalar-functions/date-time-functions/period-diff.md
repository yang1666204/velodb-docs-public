...
`<period>`は整数で、下2桁が月（01-12）を表し、前の桁が年を表します。
この関数はperiod_1 - period_2の絶対値を返します。
...
| パラメータ      | 説明                                                                                     |
|----------------|-------------------------------------------------------------------------------------------------|
| `<period_1>`   | 年と月で構成される期間を表します。<ul><li>**フォーマット**: 月は最後の2桁を占め、`[1, 12]`の範囲内でなければなりません。前の桁は年を表し、年の桁数に制限はありません。4桁を超えることもできます。</li><li>**年の推定**: 年の値は最後の2桁を除くすべての桁から直接取得されます。年が2桁の数値の場合（範囲: [00, 99]）、年が70未満の場合は20YYとして解釈され、70以上の場合は19YYとして解釈されます。</li><li>**値の範囲**: `[0, 2^63-1]`の範囲内の整数パラメータを受け入れます。</li></ul> |
...
パラメータが負の値であるか、月の部分が無効な場合、関数はエラーをスローします。
