...
{
  "title": "AI_AGG",
  "description": "大規模言語モデルを使用して、ユーザーが提供した指示に従って特定の列を集約します。",
  "language": "ja"
}
---
## 説明

ユーザー提供の指示に従って、大規模言語モデルを使用して特定の列を集約します。
...
| `<expr>`          | 集約するテキスト列。単一のテキストの文字数は128K未満である必要があります。|
| `<instruction>`   | 実行する指示、リテラルのみ受け付けます。                 |
...
結果は言語モデルによって生成されるため、出力は固定されていません。

## 例

以下の表は顧客サポートチケットをシミュレートしています：
...
```
`AI_AGG`を使用して、問題の件名別に顧客の課題を要約できます：
...
以下の表は、ECプラットフォームのユーザーレビューテーブルをシミュレートしたものです。
