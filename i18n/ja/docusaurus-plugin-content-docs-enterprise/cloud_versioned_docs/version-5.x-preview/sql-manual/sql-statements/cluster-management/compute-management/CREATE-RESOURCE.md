...
このステートメントはリソースを作成するために使用されます。rootユーザーまたはadminユーザーのみがリソースを作成できます。現在、Spark、ODBC、S3外部リソースをサポートしています。
...
`<type>` に応じて、PROPERTIES のパラメータは異なります。詳細については例を参照してください。
...
Spark関連のパラメータは以下の通りです：
   ...
   - working_dir: ETLで使用されるディレクトリ。sparkがETLリソースとして使用される場合に必須。例：hdfs://host:port/tmp/doris。
   - broker: broker名。sparkがETLリソースとして使用される場合に必須。`ALTER SYSTEM ADD BROKER`コマンドを使用して事前に設定を行う必要があります。
...
ODBCの関連パラメータは以下の通りです：
   ...
   - driver: ODBC外観のドライバ名。be/conf/odbcinst.ini内のDriver名と同じである必要があります。
...
S3関連のパラメータは以下の通りです：
   ...
   - オプションパラメータ
    ...
    DorisはS3 Resourceを作成するための`AWS Assume Role`もサポートしています。AWS intergrationを参照してください。
...
HDFS関連パラメータは以下の通りです：
   ...
   - dfs.nameservices: hadoopでHAが有効になっている場合は、fsのnameserviceを設定してください。hdfs-site.xmlを参照してください
   - dfs.ha.namenodes.[nameservice ID]：nameservice内の各NameNodeの一意な識別子。hdfs-site.xmlを参照してください
   - dfs.namenode.rpc-address.[nameservice ID].[name node ID]：各NameNodeがリッスンする完全修飾RPCアドレス。hdfs-site.xmlを参照してください
   - dfs.client.failover.proxy.provider.[nameservice ID]：HDFSクライアントがアクティブなNameNodeに接続するために使用するJavaクラス。通常はorg.apache.hadoop.hdfs.server.namenode.ha.ConfiguredFailoverProxyProviderです
...
ES関連のパラメータは以下の通りです：
   ...
   - enable_keyword_sniff: ESの文字列セグメンテーションタイプtext.fieldsを調査し、keywordによってクエリするかどうか（デフォルトはtrue、falseはセグメンテーション後のコンテンツとマッチします）
   - nodes_discovery: ESノード検出を有効にするかどうか、デフォルトはtrueです。ネットワーク分離において、このパラメータをfalseに設定します。指定されたノードのみが接続されます
