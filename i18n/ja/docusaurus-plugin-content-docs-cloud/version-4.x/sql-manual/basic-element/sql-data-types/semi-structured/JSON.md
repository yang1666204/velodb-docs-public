...
JSONデータタイプは[JSON](https://www.rfc-editor.org/rfc/rfc8785)データをバイナリ形式で効率的に格納し、JSON関数を通じてその内部フィールドへのアクセスを可能にします。

デフォルトでは、最大1048576バイト（1MB）をサポートし、最大2147483643バイト（2GB）まで増加できます。これは`string_type_length_soft_limit_bytes`設定で調整できます。

通常のSTRING型にJSON文字列を格納することと比較して、JSON型には2つの主な利点があります：
1. データ挿入時のJSON形式検証。
2. より効率的なバイナリストレージ形式により、`get_json_xx`関数と比較して`json_extract`などの関数を使用したJSON内部フィールドへの高速アクセスが可能。

**注意**: バージョン1.2.xでは、JSON型はJSONBという名前でした。MySQLとの互換性を維持するため、バージョン2.0.0からJSONに名前が変更されました。古いTableでは以前の名前を引き続き使用できます。
...
```
**挿入:**
- `INSERT INTO VALUES`を使用し、フォーマットをクォートで囲まれた文字列として指定します。例：

```sql
INSERT INTO table_name(id, json_column_name) VALUES (1, '{"k1": "100"}')
```
- STREAM LOADの場合、対応する列の形式は追加の引用符を含まない文字列です。例：
...
14	[123, 456]
```
- JSON内で`'\'`を含む特殊文字（`'\r'`、`'\t'`など）が出現する場合、replace関数を使用して`"\"`を`"\\"`に置換する必要があります。例えば、`"\n"`を`"\\n"`に置換する必要があります。
...
```
JSON型は整数、文字列、BOOLEAN、ARRAY、MAPとの間でキャストできます。例えば：
...
```
:::tip
JSON型は現在、`GROUP BY`、`ORDER BY`、または比較演算には使用できません。
:::

### example
Table作成、データロード、クエリを含むJSONデータ型のチュートリアル。
...
25	[123, abc]
```
- 28%の行が無効なため、デフォルト設定でのstream loadは「too many filtered rows」というエラーメッセージで失敗します
...
```
- header設定'max_filter_ratio: 0.3'を設定した後、stream loadは成功します
...
```
- stream loadによって読み込まれたデータを表示するにはSELECTを使用します。JSON型の列はプレーンなJSON文字列として表示されます。
...
```
- json_extract_intはint型のフィールドを抽出し、フィールドがintでない場合はNULLを返します
...
19 rows in set (0.03 sec)
```
- json_extract_bigint はbigint型のフィールドを抽出し、フィールドがbigintでない場合はNULLを返します
...
19 rows in set (0.03 sec)
```
- json_extract_boolはboolean型のフィールドを抽出し、そのフィールドがbooleanでない場合はNULLを返します
...
- json nullはSQL NULLとは異なります。SQL NULLはフィールドに値が存在しないことを表しますが、json nullは特別な値nullを持つフィールドを表します。
...
```
##### json_exists_pathでJSONにフィールドが存在するかチェックする
...
```
##### json_type による JSON フィールドのデータ型取得

- json path で指定されたフィールドのデータ型を返します。存在しない場合は NULL を返します。
...
一方、SQL NULL（IS NULLを使用する場合）は、そのキーがJSONオブジェクト内に全く存在しないことを示す場合があります。
...
`GET_JSON_XXX`関数は文字列型での使用を想定して設計されており、生のJSON文字列から直接値を抽出します。一方、`JSON_EXTRACT_XXX`関数はJSONデータ型専用に実装されており、そのために最適化されています。
