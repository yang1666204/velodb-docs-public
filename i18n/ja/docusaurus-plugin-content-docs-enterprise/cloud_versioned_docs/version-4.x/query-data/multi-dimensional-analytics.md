...
データベースにおいて、ROLLUP、CUBE、およびGROUPING SETSは、多次元データ集約に使用される高度なSQL文です。これらの機能はGROUP BY句の機能を大幅に強化し、ユーザーが単一のクエリで複数レベルの集計結果を取得することを可能にします。これは、UNION ALLを使用して複数の集約文を接続することと意味的に同等です。

- **ROLLUP**: ROLLUPは階層的な集計を生成するために使用される操作です。指定された列の順序に従ってデータを集約し、最も細かい粒度から最高レベルまで段階的に集計します。例えば、売上データにおいて、ROLLUPを使用して地域と時間で集計することで、各地域の月ごとの売上、各地域の合計売上、および全体の合計売上を提供できます。ROLLUPは段階的な集計が必要なシナリオに適しています。

- **CUBE**: CUBEはより強力な集約操作であり、可能なすべての集計の組み合わせを生成します。ROLLUPとは異なり、CUBEはすべての次元のサブセットを計算します。例えば、製品と地域で集約される売上データの場合、CUBEは各地域の各製品の売上、各製品の合計売上、各地域の合計売上、および全体の合計売上を計算します。CUBEは、ビジネス分析や市場調査など、包括的な多次元分析が必要なシナリオに適用可能です。

- **GROUPING SETS**: GROUPING SETSは特定のグループ化セットを集約する際の柔軟性を提供します。ROLLUPやCUBEのようにすべての可能な組み合わせを生成するのではなく、ユーザーが独立した集約のための列の組み合わせのセットを指定することを可能にします。例えば、各次元のすべての組み合わせを必要とせずに、地域と時間の特定の組み合わせの集計を定義できます。GROUPING SETSはカスタマイズされた集計が必要なシナリオに適しており、柔軟な集約制御を提供します。

ROLLUP、CUBE、およびGROUPING SETSは強力な多次元データ集計機能を提供し、様々なデータ分析とレポート作成のニーズに対応し、複雑な集約計算をより簡単かつ効率的にします。以下のセクションでは、これらの機能の使用シナリオ、構文、および例について詳しく説明します。

## ROLLUP

### 使用例

ROLLUPは、時間、地理、カテゴリなどの階層的な次元に沿ってデータを集計するのに特に有用です。例えば、クエリで`ROLLUP(year, month, day)`や`(country, Province, city)`を指定できます。
...
```
このクエリは時間別にデータを集計し、年別売上、各年内の月別売上、および売上の総合計のサブトータルを計算します。クエリの結果は以下の通りです：
...
CUBEは、単一ディメンションの異なるレベルを表すカラムではなく、複数の独立したディメンションからのカラムを含むクエリに最も適しています。たとえば、一般的な使用シナリオは、月、地域、製品のすべての組み合わせを要約することです。これらは3つの独立したディメンションであり、すべての可能な小計の組み合わせを分析することが一般的です。対照的に、年、月、日のすべての可能な組み合わせをクロス集計すると、時間ディメンションの自然な階層により、いくつかの不要な値が含まれることになります。ほとんどの分析では、月と日で計算された利益などの小計は不要です。「年間を通じて各月の16日の総売上は何か？」と問う必要があるユーザーは比較的少数です。
...
```
クエリ結果は以下の通りで、次の項目を計算します：

- 総売上合計

- 年別、商品カテゴリ別、州別の売上小計

- 各年内での商品カテゴリ別売上小計、各商品の州別売上小計、各年内での州別売上小計、各州および年内での商品カテゴリ別売上小計
...
1. 結果セットのどの行が小計を表しているかをプログラム的に識別し、特定の小計に対応する集約レベルを正確に判定する方法。小計は総計に対する割合などの計算によく必要となるため、これらの小計行を識別する便利な方法が必要です。

2. クエリ結果に実際に格納されているNULL値とROLLUPまたはCUBE操作によって生成された"NULL"値の両方が含まれている場合、別の問題が発生します：これら2種類のNULL値をどのように区別するかということです。

GROUPING、GROUPING_ID、およびGROUPING SETSは、上記の課題を効果的に解決できます。
...
order by  
        year(d_date), month(d_date);
```
- (YEAR(d_date), MONTH(d_date)) グループに対するGROUPING関数の結果は、年月による集計の場合(0,0)です。

- (YEAR(d_date)) グループに対するGROUPING関数の結果は、年による集計の場合(0,1)です。

- () グループに対するGROUPING関数の結果は、総計集計の場合(1,1)です。
...
order by year(d_date), i_category, ca_state;   
```
HAVING句でGROUPING関数を使用すると、総売上、年別に集計された売上、地域別に集計された売上のみが保持されます。クエリ結果:
...
データベースにおいて、GROUPING_IDとGROUPING関数はともに、ROLLUPやCUBEなどの多次元データ集約クエリを処理するための補助関数として機能し、ユーザーが異なる集約レベルの結果を区別するのに役立ちます。特定の行の集約レベルを判定したい場合は、GROUPING関数を使用してすべてのGROUP BY列を計算する必要があります。単一列の計算結果だけでは不十分だからです。

GROUPING_ID関数は、複数の列を同時に検出できるため、GROUPINGよりも強力です。GROUPING_ID関数は複数の列をパラメータとして受け取り、これらの列の集約状態をバイナリビットで表現する整数を返します。テーブルやマテリアライズドビューを使用して計算結果を保存する場合、GROUPINGを使用して異なる集約レベルを表現すると、かなりのストレージ容量を消費する可能性があります。このようなシナリオでは、GROUPING_IDの方が適しています。
...
CUBE クエリは通常、大量のリソースを消費するため、少数の次元のみが関心事である場合、`GROUPING SETS`を使用することでクエリ実行効率を向上させることができます。
...
```
上記のアプローチは、CUBEを使用することと同等ですが、具体的な`grouping_id`を指定することで、不要な計算を削減します：
...
`CUBE`を使用すると、可能なすべての集約レベル（この場合は8つ）が計算されますが、実際には、そのうちの数個にのみ関心がある場合があります。
...
- GROUPING SETS vs. GROUP BY UNION ALL
  ...
  ```
クエリ結果において、`UNION ALL`で接続された複数の`GROUP BY`クエリと等価です：
