...
配列内でlambda式の条件を満たす最初の要素を返します。この関数は配列の要素にlambda式を適用し、条件を満たす最初の要素を見つけてそれを返します。
...
- `lambda`：配列要素を評価するために使用されるlambda式。true/falseまたはboolean値に変換可能な式を返します
...
+--------------------------------------------------------------+
```
マルチ配列検索において、最初の配列が2番目の配列より大きい最初の要素を見つける:
