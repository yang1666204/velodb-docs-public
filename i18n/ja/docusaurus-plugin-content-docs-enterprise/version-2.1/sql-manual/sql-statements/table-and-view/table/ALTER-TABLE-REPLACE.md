---
{
  ...
  "description": "2つのTableのアトミック置換。この操作はOLAPTableにのみ適用されます。",
  "language": "ja"
}
---
## デスクリプション
...
Table A をTable B で置き換え、`swap` が `true` の場合は、以下を実行します：
...
3. 置き換え操作は2つの OLAP Table間でのみ発生し、2つのTableのTable構造が一致しているかどうかはチェックしません。
4. 元の権限設定は変更されません。権限チェックはTable名に基づいて行われるためです。

## 例

1. Tableを削除することなく `tbl1` と `tbl2` をアトミックに交換する（注意：削除する場合は、実際には tbl1 を削除して tbl2 を tbl1 に名前変更します。）
...
```
2. `tbl1`と`tbl2`をアトミックスワップし、`tbl2`Tableを削除する（`tbl1`と元の`tbl2`のデータを保持する）
...
```
## Best Practice
1. Atomic overlay write 運用

  場合によっては、ユーザーは特定のTableのデータを書き換えたいが、データを最初に削除してからインポートすると、その間の期間中にデータを表示できなくなります。この場合、ユーザーはまず`CREATE TABLE LIKE`文を使用して同じ構造の新しいTableを作成し、新しいデータを新しいTableにインポートし、置換操作を使用して古いTableを原子的に置換することで目標を達成できます。パーティションレベルでのAtomic overwrite write 運用については、temp partitionドキュメントを参照してください。
