---
{
  "title": "JSON_TYPE",
  "description": "jsonpath で指定されたJSONBデータ内のフィールドの型を判定するために使用されます。フィールドが存在しない場合は、NULLを返します。フィールドが存在する場合は、",
  "language": "ja"
}
---
## Description

JSONB データ内で `json_path` によって指定されたフィールドの型を判定するために使用されます。フィールドが存在しない場合は NULL を返します。フィールドが存在する場合は、以下の型のいずれかを返します：

- object
- array
- null
- bool
- int
- bigint
- largeint
- double
- string

## Syntax

```sql
JSON_TYPE( <json>, <json_path> )
```
## パラメータ

- `<json>` 型をチェックするJSON文字列。
- `<json_path>` String型。JSON内のフィールドの場所を指定します。パスは通常$.で始まり、階層構造を表すために.を使用します。

## 戻り値

`Nullable<String>`: 対応するフィールドの型を返します。

## 使用上の注意
- `<json_object>`または`<json_path>`がNULLの場合、NULLを返します。
- `<json_path>`が有効なパスでない場合、関数はエラーを報告します。
- `<json_path>`で指定されたフィールドが存在しない場合、NULLを返します。

## 例
1. JSONがstring型の場合:

    ```sql
    SELECT JSON_TYPE('{"name": "John", "age": 30}', '$.name');
    ```
    ```text
    +-------------------------------------------------------------------+
    | jsonb_type(cast('{"name": "John", "age": 30}' as JSON), '$.name') |
    +-------------------------------------------------------------------+
    | string                                                            |
    +-------------------------------------------------------------------+
    ```
2. JSONが数値型の場合：

    ```sql
    SELECT JSON_TYPE('{"name": "John", "age": 30}', '$.age');
    ```
    ```text
    +------------------------------------------------------------------+
    | jsonb_type(cast('{"name": "John", "age": 30}' as JSON), '$.age') |
    +------------------------------------------------------------------+
    | int                                                              |
    +------------------------------------------------------------------+
    ```
3. NULLパラメータ

    ```sql
    select json_type(NULL, '$.key1');
    ```
    ```text
    +---------------------------+
    | json_type(NULL, '$.key1') |
    +---------------------------+
    | NULL                      |
    +---------------------------+
    ```
4. NULLパラメータ 2

    ```sql
    select json_type('{"key1": true}', NULL);
    ```
    ```text
    +-----------------------------------+
    | json_type('{"key1": true}', NULL) |
    +-----------------------------------+
    | NULL                              |
    +-----------------------------------+
    ```
5. `json_path`パラメータで指定されたフィールドが存在しません

    ```sql
    select json_type('{"key1": true}', '$.key2');
    ```
    ```text
    +---------------------------------------+
    | json_type('{"key1": true}', '$.key2') |
    +---------------------------------------+
    | NULL                                  |
    +---------------------------------------+
    ```
6. 無効な `json_path` パラメータ

    ```sql
    select json_type('{"key1": true}', '$.');
    ```
    ```text
    ERROR 1105 (HY000): errCode = 2, detailMessage = [INVALID_ARGUMENT]Json path error: Invalid Json Path for value: $.
    ```
