...
{
  "title": "一時パーティション",
  "description": "Dorisはパーティションテーブルへの一時パーティションの追加をサポートします。",
  "language": "ja"
}
---
Dorisはパーティション化されたテーブルに一時パーティションを追加することをサポートしています。一時パーティションは通常のパーティションとは異なり、通常のクエリでは取得されず、特別なクエリ文を通じてのみクエリできます。
...
- **アトミックな上書き操作**：ユーザーは古いデータの削除と新しいデータのインポートの間でデータを失うことなく、パーティションのデータを書き換えることができます。この場合、一時パーティションを作成し、新しいデータを一時パーティションにインポートし、replace操作を通じて元のパーティションをアトミックに置換することができます。非パーティション化テーブルでのアトミックな上書き操作については、[replace table documentation](../../data-operate/delete/atomicity-replace)を参照してください。
...
- Drop操作を使用してデータベースやテーブルを直接削除した後、データベースやテーブルはRecover コマンドを使用して（制限時間内に）復旧できますが、一時パーティションは復旧されません。
...
- Truncateコマンドを使用してテーブルを空にすると、テーブルの一時パーティションが削除され、復旧することはできません。
...
- テーブルが変更操作中の場合、テーブルに一時パーティションを追加することはできません。
