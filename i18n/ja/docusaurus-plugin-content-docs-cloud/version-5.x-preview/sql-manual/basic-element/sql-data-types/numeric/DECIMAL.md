...
### 説明
    DECIMAL(P[,S])
    高精度固定小数点数。Pは有効桁数の総数（精度）を表し、Sは小数点の右側の小数部分の桁数を表します。
    ...
    enable_decimal256のデフォルト値はfalseです。trueに設定するとより正確な結果を得ることができますが、パフォーマンスが低下します。

    decimal型を出力する際、小数点の後には常にS桁の数字が続きます（末尾の数字が0であっても）。例えば、decimal(18, 6)型の数値123.456は123.456000として出力されます。

### 精度推論

DECIMALには非常に複雑な型推論ルールのセットがあります。異なる式に対して、精度推論には異なるルールが適用されます。

#### 算術演算

e1(p1, s1)とe2(p2, s2)を2つのDECIMAL数とすると、演算結果の精度推論ルールは以下の通りです：

|演算|結果の精度|結果のスケール|オーバーフロー時の結果の精度|オーバーフロー時の結果のスケール|中間e1型|中間e2型|
|-------|------------|---------|-----------------|--------------|--------|------|
|e1 + e2|max(p1 - s1,p2 - s2) + max(s1, s2) + 1|max(s1, s2)|MAX_P|min(MAX_P, p) - max(p1 - s1,p2 - s2)|結果に従ってキャスト|結果に従ってキャスト|
|e1 - e2|max(p1 - s1,p2 - s2) + max(s1, s2) + 1|max(s1, s2)|MAX_P|min(MAX_P, p) - max(p1 - s1,p2 - s2)|結果に従ってキャスト|結果に従ってキャスト|
|e1 * e2|p1 + p2|s1 + s2|MAX_P|<ol><li>precision - scale < MAX_P - `decimal_overflow_scale`: min(scale, 38 - (precision - scale))</li><li>precision - scale > MAX_P - `decimal_overflow_scale`、かつscale < `decimal_overflow_scale`: s1 + s2</li><li>precision - scale > MAX_P - `decimal_overflow_scale`、かつscale >= `decimal_overflow_scale`：`decimal_overflow_scale`</li></ol>|変更なし|変更なし|
|e1 / e2|p1 + s2 + `div_precision_increment`|s1 + `div_precision_increment`|MAX_P|<ol><li>precision - s1がmax_precision - `decimal_overflow_scale`未満の場合: (max_precision - (precision - s1)) + `div_precision_increment`</li><li>precision - s1がmax_precision - `decimal_overflow_scale`より大きく、s1が`decimal_overflow_scale`未満の場合: s1 + `div_precision_increment`</li><li>precision - s1がmax_precision - `decimal_overflow_scale`より大きく、s1が`decimal_overflow_scale`以上の場合: `decimal_overflow_scale` + `div_precision_increment`</li></ol>|pは結果に従ってキャスト、sは結果+e2.scaleに従ってキャスト||
|e1 % e2|max(p1 - s1,p2 - s2) + max(s1, s2)|max(s1, s2)|MAX_P|min(MAX_P, p) - max(p1 - s1,p2 - s2)|結果に従ってキャスト|結果に従ってキャスト|

Table内の`Result scale if overflow`を計算するルールにおいて、`precision`は`Result precision`列の`precision`を指し、`scale`は`Result scale`列の`scale`を指します。

`div_precision_increment`はFEの設定パラメータです。div_precision_incrementを参照してください。

`decimal_overflow_scale`はFEのセッション変数で、decimal値の計算結果の精度がオーバーフローした際に計算結果で保持可能な小数点以下の最大桁数を示します。デフォルト値は6です。

注目すべきは、除算計算のプロセスが以下の通りであることです：
DECIMAL(p1, s1) / DECIMAL(p2, s2)はまずDECIMAL(p1 + s2 + div_precision_increment, s1 + s2) / DECIMAL(p2, s2)に変換されてから計算が実行されます。そのため、DECIMAL(p1 + s2 + div_precision_increment, s1 + div_precision_increment)はDECIMALの範囲を満たしていても、
DECIMAL(p1 + s2 + div_precision_increment, s1 + s2)への変換により
範囲を超える可能性があり、Dorisはデフォルトで`Arithmetic overflow`エラーを報告します。
...
```
乗算結果精度の計算ルールに従って、結果の型はdecimal(38, 18)となり、オーバーフローしません：
...
```
乗算結果の精度の計算ルールによると、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）の下では、通常の計算結果タイプは`decimal(41, 11)`です。精度がオーバーフローするため、オーバーフロールールに従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - `scale` = 41 - 11 = 30 < 32、ルール1が適用され、最終結果のscale = min(11, 38 - 30) = 8、最終結果タイプはdecimal(38, 8)です：
...
+----------------------+-----------------------+--------------------------------------+
```
`decimal_overflow_scale`の値が増加した場合、例えば`set decimal_overflow_scale=9;`では、オーバーフロールールに従って計算が実行されます：`MAX_P` - `decimal_overflow_scale` = 38 - 9 = 29、`precision` - `scale` = 41 - 11 = 30 > 29、かつscale > `decimal_overflow_scale`であるため、オーバーフロールール3が適用され、最終的な計算結果の型は`decimal(38,9)`となります：
...
+----------------------+-----------------------+---------------------------------------+
```
`decimal_overflow_scale`の値を引き続き増加させる場合、例えば`set decimal_overflow_scale=12;`とすると、オーバーフロールールに従って計算されます：`MAX_P` - `decimal_overflow_scale` = 38 - 12 = 26、`precision` - `scale` = 41 - 11 = 30 > 26、かつscale < `decimal_overflow_scale`。この場合、オーバーフロールール2が適用され、最終的に計算される結果の型は`decimal(38,11)`となります：
...
```
デフォルトでは（`enable_decimal256`=false）、乗算の実際の結果はオーバーフローします。この場合、decimal256を有効にできます：`set enable_decimal256=true` を使用して正確な結果を計算すると、結果の型はdecimal(76, 38)になります：
...
```
除算結果の精度の計算ルールに従って、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）の下では、通常の計算結果の型は`decimal(19 + 9 + 4, 9 + 4)`、つまり`decimal(32, 13)`であり、精度はオーバーフローしません。結果の最終的な型は`decimal(32, 13)`です：
...
+----------------------+---------------------+-----------------+
```
結果により多くの小数点以下の桁数を保持したい場合は、`div_precision_increment`を増やすことができます。例えば、`admin set frontend config('div_precision_increment'='8');`のようにします。その後、上記の計算ルールに従って、計算結果の型は`decimal(36, 17)`になります：
...
```
除算結果の精度の計算ルールに従い、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）では、通常の計算結果型は`decimal(27 + 8 + 4, 8 + 4)`、すなわち`decimal(39, 12)`となります。精度がオーバーフローしたため、オーバーフロールールに従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - s1 = 39 - 8 = 31 < 32であるため、`Result scale if overflow`ルール1が適用され、結果のスケールは（`MAX_P` - （`precision` - `s1`）） + `div_precision_increment` = (38 - (39 - 8)) + 4 = 11となり、結果型は`decimal(38, 11)`となります：
...
+-----------------------------+----------------------------+---------------+
```
`decimal_overflow_scale`の値が増加された場合、例えば`set decimal_overflow_scale=8;`の場合、計算はオーバーフロールールに従って実行されます：`MAX_P` - `decimal_overflow_scale` = 38 - 8 = 30、`precision` - s1 = 39 - 8 = 31 > 30、そしてs1 == `decimal_overflow_scale`であるため、オーバーフロールール3が適用され、最終的な計算結果の型は`decimalv3(38,12)`になります：
...
```
除算結果の精度の計算ルールに従って、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）では、通常の計算結果タイプは`decimal(38 + 4 + 4, 4 + 4)`、つまり`decimal(46, 8)`です。精度がオーバーフローしたため、オーバーフロールールに従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - s1 = 46 - 4 = 42 > 32、s1 = 4 < `decimal_overflow_scale`であるため、`Result scale if overflow`ルール2が適用され、結果のスケールは`s1` + `div_precision_increment` = 4 + 4 = 8となり、結果タイプは`decimal(38, 8)`です：
...
+-------------------------+------------------------+-------------+
```
結果により多くの小数点以下桁数を保持したい場合は、`div_precision_increment`を増加させることができます。例えば、`admin set frontend config('div_precision_increment'='8');`のようにします。すると、上記の計算ルールに従って、計算結果の型は`decimal(38, 12)`になります：
...
+-------------------------+------------------------+-----------------+
```
decimal256が有効な場合（`set enable_decimal256 = true;`）、結果の精度はオーバーフローなしで通常通り計算され、結果の型は`decimal(46, 8)`になります：
...
```
除算結果の精度の計算ルールに従い、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）では、通常の計算結果タイプは`decimal(38 + 7 + 4, 7 + 4)`、すなわち`decimal(49, 11)`になります。精度がオーバーフローするため、オーバーフロールールに従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - s1 = 49 - 7 = 42 > 32、s1 = 7 > `decimal_overflow_scale`であるため、`Result scale if overflow`ルール3が適用され、結果のスケールは`decimal_overflow_scale` + `div_precision_increment` = 6 + 4 = 10となり、結果タイプは`decimal(38, 10)`になります：
...
上記で言及した式以外では、他の式は精度推論にデフォルトルールを使用します。つまり、式 `expr(DECIMAL(a, b))` の場合、結果の型も DECIMAL(a, b) になります。

#### 結果精度の調整

異なるユーザーはDECIMALに対して異なる精度要件を持ちます。上記のルールはDorisのデフォルト動作です。**ユーザーが異なる精度要件を持つ場合、以下の方法で精度を調整できます**：

* 期待する結果精度がデフォルト精度より大きい場合、パラメータの精度を調整することで結果精度を調整できます。例えば、ユーザーが `AVG(col)` を計算してDECIMAL(x, y)を結果として得たい場合、`col` の型がDECIMAL (a, b)であれば、式を `AVG(CAST(col as DECIMAL (x, y))` に書き換えることができます。
* 期待する結果精度がデフォルト精度より小さい場合、出力結果を近似することで望ましい精度を得ることができます。例えば、ユーザーが `AVG(col)` を計算してDECIMAL(x, y)を結果として得たい場合、`col` の型がDECIMAL(a, b)であれば、式を `ROUND(AVG(col), y)` に書き換えることができます。

### DECIMALが必要な理由

DorisのDECIMALは真の高精度固定小数点数です。Decimalには以下の主要な利点があります：
1. より広い範囲を表現できます。DECIMALでは精度とスケール両方の値範囲が大幅に拡張されています。
2. より高いパフォーマンス。旧バージョンのDECIMALはメモリで16バイト、ストレージで12バイトを必要としましたが、DECIMALは以下に示すような適応的調整を行っています。

|     precision        | 占有容量 (memory/disk) |
|----------------------|-------------------|
...
3. より完全な精度推論。異なる式に対して、異なる精度推論ルールが適用され、結果の精度が推論されます。
