...
---
Redshiftの移行プロセスでは、通常、中間媒体としてオブジェクトストレージを使用する必要があります。コアプロセスは以下の通りです：まず、RedshiftのUNLOADステートメントを使用してデータをオブジェクトストレージにエクスポートします。次に、DorisのS3 Load機能を使用してオブジェクトストレージからデータを読み取り、Dorisにインポートします。詳細については、S3 Loadを参照してください。

## 注意事項

1. 移行前に、Redshiftのテーブル構造に応じて、DorisのData Modelや、PartitioningおよびBucketingの戦略を選択する必要があります。その他のテーブル作成戦略については、Load Best Practicesを参照してください。
...
```
このテーブル構造に従って、Dorisプライマリキーパーティションテーブルを作成することができます。パーティションフィールドはビジネスシナリオに応じて選択する必要があります。ここでは、パーティションフィールドは"order_date"であり、日次ベースでパーティション化されています。
...
S3 Loadは非同期のデータロード方式です。実行後、Dorisがデータソースからアクティブにデータを取得します。データソースはS3プロトコルと互換性のあるオブジェクトストレージをサポートしており、([AWS S3](./amazon-s3.md)、[GCS](./google-cloud-storage.md)、[AZURE](./azure-storage.md)など)が含まれます。

この方式は、大量のデータを含み、バックグラウンドでの非同期処理が必要なシナリオに適しています。同期的に処理する必要があるデータインポートについては、TVF Loadを参照してください。
...
S3 Load importは非同期で実行されるため、SHOW LOADを使用して特定のラベルの状況を確認できます：
...
LoadFinishTime: 2025-04-15 17:33:02
 ...
 TransactionId: 769213
  ErrorTablets: {}
       ...
       Comment: 
```
上記の例に示すように、この問題は**データ品質エラー**(ETL_QUALITY_UNSATISFIED)です。詳細なエラーを確認するには、結果で提供されているURLにアクセスする必要があります。例えば、データがテーブルスキーマのcountryカラムで定義された長さを超えた場合：
...
```
データ品質エラーについて、エラーのあるレコードをスキップすることを許可したい場合は、S3 Loadタスクのプロパティセクションでフォルトトレラント率を設定できます。詳細については、Import Configuration Parametersを参照してください。

**3.4 複数パーティションのデータロード**

大量の履歴データを移行する場合は、バッチロード戦略の使用を推奨します。各バッチはDorisの1つまたは少数のパーティションに対応します。システム負荷を軽減し、ロード失敗時の再試行コストを下げるため、バッチあたりのデータサイズは100GB未満に保つことを推奨します。
