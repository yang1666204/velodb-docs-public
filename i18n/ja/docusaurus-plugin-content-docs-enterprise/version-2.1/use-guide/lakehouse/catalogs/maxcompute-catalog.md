...
* パーティションテーブル、クラスターテーブル、マテリアライズドビューの読み取りをサポートしています。
...
3. [Alibaba Cloudエンドポイントドキュメント](https://help.aliyun.com/zh/maxcompute/user-guide/endpoints)に基づいて`mc.endpoint`を設定
   ...
   VPC経由でアクセスするユーザーは、**"リージョナルエンドポイントテーブル（Alibaba Cloud VPCネットワーク接続方法）"** の **"VPCネットワークエンドポイント"** 列を参照して`mc.endpoint`を設定してください。

   パブリックネットワーク経由でアクセスするユーザーは、**"リージョナルエンドポイントテーブル（Alibaba Cloudクラシックネットワーク接続方法）"** の **"クラシックネットワークエンドポイント"** 列、または **"リージョナルエンドポイントテーブル（外部ネットワーク接続方法）"** の **"外部ネットワークエンドポイント"** 列から選択して`mc.endpoint`を設定できます。
...
- `mc.odps_endpoint`：MaxCompute Endpoint、MaxComputeメタデータ（例：データベースとテーブル情報）の取得に使用。
