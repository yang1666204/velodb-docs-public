...
{
  "title": "PERIOD_ADD",
  "description": "<period>に<month>か月を加算した結果を計算します。",
  "language": "ja"
}
---
## 説明
...
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<period>` | 年と月で構成される期間を表します。<ul><li>**Format**: 月は末尾2桁を占め、`[1, 12]`の範囲内である必要があります。先頭の桁は年を表し、年の桁数は制限がありません。4桁を超えることも可能です。</li><li>**Year Inference**: 年の値は末尾2桁を除くすべての桁から直接取得されます。年が2桁の数値の場合（範囲: [00, 99]）、年が70未満の場合は20YYとして解釈され、70以上の場合は19YYとして解釈されます。</li><li>**Value Range**: `[0, 2^63-1]`の範囲内の整数パラメータを受け入れます。</li></ul> |
...
いずれかのパラメータがNULLの場合、または`period`パラメータがBIGINTに変換できない場合、この関数はNULLを返します。

`period`パラメータが負の値であるか、その月の部分が無効な場合、この関数はエラーをスローします。
