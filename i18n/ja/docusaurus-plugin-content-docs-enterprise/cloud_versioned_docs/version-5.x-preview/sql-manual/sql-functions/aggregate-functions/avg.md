...
| `[DISTINCT]` | expr から重複する値を除去した後に平均値を計算することを示すオプションのキーワードです。 |

## Return Value

選択された列または式の平均値を返します。グループ内のすべてのレコードが NULL の場合、この関数は NULL を返します。
decimal 型の入力の場合、戻り値も decimal になります。その他の数値型は double 型を返します。
...
```
SmallInt型の平均計算において、[10,20,30]の平均は20です。
...
```
すべての入力データがNULL値である場合、NULL値を返します。
