...
**1. `<db_name>`**: データベース名、オプション。指定しない場合、デフォルトで現在のデータベースが使用されます。
...
| Column_name    | string | インデックス付きカラムの名前。                                                                                   |
...
| Index_type     | string | インデックスの種類: <br> - `BTREE`: B+ツリーインデックス（MySQLのデフォルト型） <br> - `HASH`: ハッシュインデックス <br> - `RTREE`: R-ツリーインデックス <br> - `INVERTED`: 転置インデックス（例：全文インデックス） |
