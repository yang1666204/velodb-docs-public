...
AUTO_PARTITION_NAME関数は自動パーティションのパーティション名を生成するために使用されます。この関数は2つのモードをサポートしています：RANGEモードは時間単位に基づいてパーティション名を生成し、LISTモードは文字列値に基づいてパーティション名を生成します。
...
| パラメータ | 説明 |
| ----------- | ----------------------------------------- |
| `'RANGE'` | RANGEパーティションモード、時間に基づいてパーティション名を生成します |
| `'LIST'` | LISTパーティションモード、文字列値に基づいてパーティション名を生成します |
