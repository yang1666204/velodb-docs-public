...
2. ソートフィールドc1、c2がテーブルキーのプレフィックスと完全に一致する場合、さらなる最適化が適用されます。データ読み込み時に、データファイルのヘッダーまたはテールのみが読み込まれ、読み込みデータ量が必要なn行だけに削減されます。
...
3. `topn_filter_ratio`、LIMIT nとテーブル内の総データの比率で、デフォルト値は0.5です。これは、LIMITの数がテーブル内のデータの半分を超える場合、フィルタが生成されないことを意味します。
