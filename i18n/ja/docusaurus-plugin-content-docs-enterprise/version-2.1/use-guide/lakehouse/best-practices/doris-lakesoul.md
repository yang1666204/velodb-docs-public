...
- バージョン0.15以降、Apache DorisはHiveとIceberg外部テーブルを導入し、データレイク用のApache Icebergとの組み合わせ機能を探求しました。
...
Flink CDC JobをmysqlテーブルをsyncするためにLaunchします。mysqlテーブルはdocker composeをlaunchする際にロードされます。
...
```
Flink CDC Jobが起動されます。`doris client`でlakesoulカタログを再作成することで、起動プロセスを確認できます。Flink CDC Jobが起動された後、`doris client`で同期中のLakeSoul CDCテーブルを確認できます。
