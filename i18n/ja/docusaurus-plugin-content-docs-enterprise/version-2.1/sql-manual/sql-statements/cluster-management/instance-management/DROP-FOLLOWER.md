...
{
  "title": "DROP FOLLOWER",
  "description": "この文は FRONTEND の FOLLOWER ロールを持つノードを削除します。（管理者のみ！）",
  "language": "ja"
}
---
## 説明

この文は、FRONTENDのFOLLOWERロールを持つノードを削除します。（管理者のみ！）
...
2. FOLLOWERノードを削除する前に、オフライン後のクラスタ内のFOLLOWERノード数が奇数になることを確認してください。
...
1. FOLLOWERノードを削除する
   ...
   ```
このコマンドはクラスター内のFOLLOWERノード（IPアドレス host_ip、ポート 9010）を削除します
