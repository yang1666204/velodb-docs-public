...
{
  "title": "Kyuubi",
  "description": "Apache Kyuubiは、データウェアハウスとLakehouse上でサーバーレスSQLを提供する分散型マルチテナントゲートウェイです。",
  "language": "ja"
}
---
# Kyuubi を Doris と使用する

## はじめに

[Apache Kyuubi](https://kyuubi.apache.org/) は、データウェアハウスやレイクハウスでサーバーレス SQL を提供する分散・マルチテナントゲートウェイです。
Apache Kyuubi は、Spark、Flink、Hive、JDBC などのエンジンに対して、Thrift、Trino、MySQL などの様々なプロトコルを提供しています。
Doris は、Apache Kyuubi でサポートされている Doris 方言を使用して、JDBC データソースとして接続することができます。
Apache Kyuubi は、HA、サービスディスカバリ、統合認証、エンジンライフサイクル管理など、一連の有用な機能も提供しています。

## 使用方法

### Apache Kyuubi のダウンロード

<https://kyuubi.apache.org/zh/releases.html> から Apache Kyuubi をダウンロードしてください。

Apache Kyuubi 1.6.0 以上を取得し、フォルダに展開してください。

### Kyuubi データソースとして Doris を設定

- `$KYUUBI_HOME/conf/kyuubi-defaults.conf` の Kyuubi 設定を更新してください
...
以下の例では、ThriftプロトコルでbeeLine CLIを使用してKyuubi経由でDorisにクエリを実行する基本的な例を示します。
