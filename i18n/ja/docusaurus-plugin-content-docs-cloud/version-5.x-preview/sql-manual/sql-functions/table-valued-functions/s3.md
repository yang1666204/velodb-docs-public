...
S3テーブル値関数（TVF）により、ユーザーはS3互換オブジェクトストレージに保存されたファイルを、リレーショナルテーブル形式のデータにアクセスするかのように読み取りおよびアクセスできます。現在、`csv/csv_with_names/csv_with_names_and_types/json/parquet/orc`ファイル形式をサポートしています。
