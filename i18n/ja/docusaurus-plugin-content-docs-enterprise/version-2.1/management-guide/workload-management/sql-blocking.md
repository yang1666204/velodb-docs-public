...
#### 指定した行数を超えるスキャンをブロック
データのスキャンはBEのIOおよびCPUリソースを大幅に消費します。そのため、不要なデータスキャンはクラスタの安定性にとって大きな課題となります。日常的な使用において、`SELECT * FROM t`のような盲目的なフルTableスキャン操作がしばしば発生します。このようなクエリがクラスタに損害を与えることを防ぐため、単一のクエリが単一のTableでスキャンする行数の上限を設定できます。
...
多くのパーティションをスキャンすると、BEのCPU消費量が大幅に増加する可能性があります。さらに、クエリが外部Tableに対するものである場合、大きなネットワークオーバーヘッドとメタデータ取得オーバーヘッドが発生する可能性が高くなります。日常の使用では、これはパーティション列のフィルタリング条件の記述を忘れたり、間違って記述したりすることが原因であることが多いです。このようなクエリがクラスタに損害を与えることを防ぐため、単一のTableに対する単一のクエリでスキャンされるパーティション数の上限を設定できます。
...
```
上記のルールが設定されている場合、単一Tableでスキャンされるパーティション数が30を超えると、クエリの実行が禁止されます。
...
あまりにも多くのバケットをスキャンすることは、BEのCPU消費量を大幅に増加させる可能性があります。このようなクエリがクラスターに害を与えることを防ぐために、単一のクエリが単一Tableでスキャンできるパーティション数に上限を設定することができます。
...
```
上記のルールが設定されている場合、単一のTableでスキャンされるバケット数が200を超えると、クエリの実行が禁止されます。
