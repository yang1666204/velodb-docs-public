...
| -- | -- |
| `<date_or_time_part>` | datetime型をサポートする有効な日付表現。具体的なdatetimeおよびdate形式については、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)を参照してください |

## Return Value

入力が有効な場合、時刻部分を含まないDATE型の純粋な日付値（YYYY-MM-DD形式）を返します。
特殊なケース：
