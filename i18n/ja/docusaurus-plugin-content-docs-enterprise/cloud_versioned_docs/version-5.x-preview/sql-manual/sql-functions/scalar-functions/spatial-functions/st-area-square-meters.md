---
{
  ...
  "description": "地球表面の閉じた領域の面積を平方メートルで計算します。",
  "language": "ja"
}
---
## 説明
...
閉じていない幾何オブジェクト（点、線分など）の場合、その面積は0になります。無効な幾何オブジェクト（自己交差するポリゴンなど）の場合、NULLを返します。
...
| パラメータ | 説明     |
| -- |--------|
| `<geo>` | 地球表面上の幾何オブジェクトで、GeoPolygon、GeoCircle、GeoMultiPolygonなどの閉じた領域タイプをサポートします。 |

## 戻り値

領域の面積を平方メートル単位で返し、型はDOUBLEです。

ST_AREA_SQUARE_METERSには以下のエッジケースがあります：

- 入力パラメータがNULLの場合、NULLを返します。
- 入力が閉じていない幾何オブジェクト（点GeoPoint、線分GeoLineStringなど）の場合、0を返します。
- 入力幾何オブジェクトが無効な場合（自己交差するポリゴン、閉じていないポリゴンなど）、NULLを返します。
