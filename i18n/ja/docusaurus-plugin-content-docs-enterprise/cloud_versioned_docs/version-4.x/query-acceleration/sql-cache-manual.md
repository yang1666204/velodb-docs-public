...
これらのメタデータが変更されていない場合、対応するテーブル内のデータが変更されていないことを示し、以前のSQL Cacheの再利用が可能です。この場合、FEはSQLの解析と最適化プロセスをスキップし、一貫したハッシュアルゴリズムに基づいて直接対応するBEを特定し、そこからクエリ結果の取得を試みることができます。
...
1. テーブル/ビュー構造の変更。`drop table`、`replace table`、`alter table`、または`alter view`の実行など。
...
6. Row Policy または Data Masking の変更。特定のテーブルデータをユーザーに非表示に設定するなど。
