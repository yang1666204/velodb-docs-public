...
KettleはオープンソースのETL（Extract、Transform、Load）ツールで、最初にPentahoによって開発されました。KettleはPentaho製品スイートのコアコンポーネントの1つで、主にデータ統合とデータ処理に使用され、様々なソースからデータを抽出し、データのクリーニングと変換を行い、ターゲットシステムに読み込むタスクを簡単に完了できます。
...
|--------------|----------------| -------- |--------------------------------|
| ステップ name | -- | Y | ステップ名 |
| fenodes | -- | Y | Doris FE httpアドレス、複数アドレスをサポート、カンマ区切り |
| Database | -- | Y | Doris書き込みデータベース |
| Target table | -- | Y | Dorisの書き込みTable |
...
| Delete Mode | N | N | 削除モードを有効にするかどうか。デフォルトでは、Stream Loadはinsert操作を実行します。削除モードを有効にすると、すべてのStream Load書き込みが削除操作になります。 |
