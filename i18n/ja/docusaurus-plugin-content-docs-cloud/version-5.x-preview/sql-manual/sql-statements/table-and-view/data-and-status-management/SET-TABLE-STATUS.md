...
- Tableステータスによって引き起こされるタスクのブロッキングを解決するために使用されます。
...
| 権限            | オブジェクト | 注意事項                                      |
| :-------------- | :---------- | :-------------------------------------------- |
| ADMIN           | システム      | ユーザーはこのコマンドを実行するためにADMIN権限を持つ必要があります。 |

## 使用上の注意

- このコマンドは緊急時の障害復旧を目的としており、慎重に使用してください。
- OLAPTableのみをサポートし、他のタイプのTableはサポートしていません。
- Tableが既に目標状態にある場合、このコマンドは無視されます。
- 不適切な状態設定はシステム異常を引き起こす可能性があります。技術サポートの指導の下でこのコマンドを使用することをお勧めします。
- ステータス変更後は、システムの動作状況を速やかに監視することをお勧めします。

## 例

- TableのステータスをNORMALに設定：

    ```sql
    ADMIN SET TABLE tbl1 STATUS PROPERTIES("state" = "NORMAL");
    ```
- TableステータスをSCHEMA_CHANGEに設定する：
