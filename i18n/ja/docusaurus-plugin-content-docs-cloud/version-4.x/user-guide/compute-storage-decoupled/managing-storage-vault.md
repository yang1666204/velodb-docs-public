...
{
  "title": "ストレージVaultの管理",
  "description": "Storage Vaultは、Dorisが分離ストレージ・コンピュートモデルで使用するリモート共有ストレージです。",
  "language": "ja"
}
---
# Storage Vault の管理
...
HDFSベースの分離ストレージ・コンピュートDorisクラスターを作成するには、すべてのノード（FE/BEノード、Meta Serviceを含む）が指定されたHDFSにアクセスする権限を持っていることを確認してください。これには、事前にKerberos認証設定と接続性チェックを完了することが含まれます（対応する各ノードでHadoop Clientを使用してテストできます）。
...
```
より詳細なパラメータの説明と例については、[CREATE-STORAGE-VAULT](../../sql-manual/sql-statements/cluster-management/storage-management/CREATE-STORAGE-VAULT)を参照してください。

**注意**
提供されるオブジェクトストレージのパスには、head/get/list/put/multipartUpload/deleteのアクセス権限が必要です。
...
```
`PROPERTY_VALUE=3`を対応するストレージボールトID値に置き換えてください。
