...
```
標準インストールモードでは、プラグインが依存するrubyモジュールが自動的にインストールされます。ネットワークが利用できない場合、処理が停止して完了できません。このような場合、依存関係を含むzipインストールパッケージをダウンロードして完全にオフラインでインストールできます。ローカルファイルシステムを指定するために`file://`を使用する必要があることに注意してください。
...
`log_speed_interval` | ログ内に速度を出力する時間間隔、単位は秒、デフォルトは10、0に設定するとこのタイプのロギングを無効にできます
...
この例ではDoris FEログを例として、TEXTログ収集を説明します。
...
Logstashには主に2つのタイプの設定ファイルがあります。1つはLogstashシステム全体用で、もう1つは特定のログ収集用です。

Logstashシステム全体の設定ファイルは通常config/logstash.ymlに配置されています。Dorisへの書き込み時のパフォーマンスを向上させるには、バッチサイズとバッチ遅延を変更する必要があります。1行あたり平均数百バイトのサイズのログの場合、1,000,000行のバッチサイズと10秒のバッチ遅延が推奨されます。

``` 
pipeline.batch.size: 1000000
pipeline.batch.delay: 10000
```
logstash_doris_log.confなどの特定のログ収集用設定ファイルは、主にETLの各段階に対応する3つの部分で構成されています：
...
GitHub イベントアーカイブには、GitHubユーザーのアクションがJSONフォーマットでアーカイブされたデータが含まれています。これは[こちら](https://data.gharchive.org/)からダウンロードできます。例えば、2024年1月1日午後3時のデータなどがあります。

```shell
wget https://data.gharchive.org/2024-01-01-15.json.gz
```
以下はデータのサンプルです。通常、各データは1行に記述されますが、表示を見やすくするため、ここではフォーマットを整えています。
...
1. ファイル入力のcodecパラメータはjsonです。Logstashは各行のテキストをJSON形式として解析し、解析されたフィールドを後続の処理に使用します。
