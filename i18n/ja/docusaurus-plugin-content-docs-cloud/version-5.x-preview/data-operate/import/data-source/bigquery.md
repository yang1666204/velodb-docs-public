...
1. 移行前に、BigQueryのTable構造に応じて、Dorisの[Data Model](../../../table-design/data-model/overview.md)、および[Partitioning](../../../table-design/data-partitioning/dynamic-partitioning.md)と[Bucketing](../../../table-design/data-partitioning/data-bucketing.md)の戦略を選択する必要があります。Table作成戦略の詳細については、[Load Best Practices](../load-best-practices.md)を参照してください。
...
BigQueryのTableをDorisに移行する際は、まずDorisのTableを作成する必要があります。

BigQueryに以下のTableとデータが既に存在するとします。
...
```
このTable構造に従って、Doris プライマリキーパーティションTableを作成できます。パーティションフィールドは BigQuery のものと同じにする必要があり、Tableは日次ベースでパーティション分割する必要があります。
   ...
   *************************** 1. row ***************************
   ...
   TransactionId: 769213
     ErrorTablets: {}
         ...
         Comment: 
   ```
上記の例で示されているように、問題は**data quality error**(ETL_QUALITY_UNSATISFIED)です。詳細なエラーを確認するには、結果に提供されているURLにアクセスする必要があります。例えば、データがTableスキーマのcountry列の定義された長さを超えていました：
   ...
   ```
データ品質エラーについて、エラーレコードのスキップを許可したい場合は、S3 Loadタスクのプロパティセクションでフォルトトレラント率を設定できます。詳細については、[Load 構成 パラメータ](../../import/import-way/broker-load-manual.md#related-configurations)を参照してください。
