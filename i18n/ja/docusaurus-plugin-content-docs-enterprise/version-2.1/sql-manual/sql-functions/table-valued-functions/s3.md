...
S3Table値関数（TVF）は、S3互換オブジェクトストレージに格納されたファイルを、リレーショナルTable形式のデータにアクセスするかのように読み取り、アクセスすることを可能にします。現在、`csv/csv_with_names/csv_with_names_and_types/json/parquet/orc`ファイル形式をサポートしています。
...
| Parameter                       | Default | デスクリプション                                                                                                                             | 注釈                                                                              |
|---------------------------------|---------|-----------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------|
| `s3.session_token`              | -       | S3セッショントークン                                                                                                                        |                                                                                    |
| `use_path_style`                | `false` | S3 SDKはデフォルトでVirtual-hosted Styleを使用します。一部のオブジェクトストレージシステムはVirtual-hosted Styleをサポートしない場合があります。このパラメータを設定してPath Styleアクセスを強制します。例えば、MinIOはデフォルトでPath Styleのみを許可するため、MinIOにアクセスする際は`use_path_style=true`を設定する必要があります |                                                                                    |
| `force_parsing_by_standard_uri` | `false` | 非標準URIを標準URIとして強制的にパースします                                                                                                    |                                                                                    |
| `column_separator`              | `\t`    | カラム区切り文字                                                                                                                        |                                                                                    |
| `line_delimiter`                | `\n`    | 行区切り文字                                                                                                                          |                                                                                    |
| `compress_type`                 | `UNKNOWN` | 圧縮タイプ。現在`UNKNOWN`/`PLAIN`/`GZ`/`LZO`/`BZ2`/`LZ4FRAME`/`DEFLATE`/`SNAPPYBLOCK`をサポートし、`uri`の拡張子に基づいて自動検出します |                                                                                    |
| `read_json_by_line`             | `"true"` | JSON形式インポート用                                                                                                                  | JSON Load                        |
| `strip_outer_array`             | `"false"` | JSON形式インポート用                                                                                                                  | JSON Load                        |
| `json_root`                     | Empty   | JSON形式インポート用                                                                                                                  | JSON Load                        |
| `jsonpaths`                     | Empty   | JSON形式インポート用                                                                                                                  | JSON Load                        |
| `num_as_string`                 | `false` | JSON形式インポート用                                                                                                                  | JSON Load                        |
| `fuzzy_parse`                   | `false` | JSON形式インポート用                                                                                                                  |                                                                                    |
| `trim_double_quotes`            | `false` | CSV形式インポート用、各フィールドから最外側のダブルクォートを削除します                                                              |                                                                                    |
| `skip_lines`                    | `0`     | CSVファイルの最初のN行をスキップします。`csv_with_names`または`csv_with_names_and_types`形式を使用する場合、このパラメータは効果がありません   |                                                                                    |
...
| `enable_mapping_varbinary`      | `false` | PARQUET/ORCを読み取る際にBYTE_ARRAY型をSTRINGにマッピングします。有効にすると、代わりにVARBINARY型にマッピングされます。                                 | バージョン4.0.3からサポート                                                       |
...
- パススタイル
      ...
      ```
これらの3つの標準的なURIスタイルに加えて、他のURIスタイルもサポートされています（ただし、あまり一般的ではありません）：
...
2. TVFを直接クエリするか、このTVFに基づいてViewを作成するには、ResourceのUSAGE権限が必要です。TVFに基づいて作成されたViewをクエリするには、ViewのSELECT権限のみが必要です。
...
- **CSV Format**
  ...
  S3 table-valued-function は事前にTable Schema を知らないため、Table Schema を解析するためにファイルを一度読み取ります。

  `csv` format: S3 table-valued-function はS3上のファイルを読み取り、CSVファイルとして処理し、最初の行を使用してTable Schema を解析します。最初の行の列数 `n` がTable Schema の列数として使用され、列名は `c1, c2, ..., cn` として自動的に命名され、すべての列タイプは `String` に設定されます。例：
...
- **`csv_with_names` フォーマット**
  ...
  `csv_with_names` フォーマット: ファイルの最初の行をTable Schemaの列数と列名として解析し、すべての列タイプは `String` に設定されます。例:
...
- **`csv_with_names_and_types` フォーマット**
  ...
  `csv_with_names_and_types` フォーマット：現在、CSVファイルからの列型の解析はサポートされていません。このフォーマットを使用する場合、S3 TVFは最初の行をTable Schemaの列数と列名として解析し、すべての列型をStringに設定して、ファイルの2行目を無視します。
...
- **JSON Format**
  ...
  `json` format: JSON formatは多くのオプションパラメータを含みます。各パラメータの意味については、Json Loadを参照してください。S3 TVFでJSON formatファイルをクエリする際、`json_root`と`jsonpaths`パラメータに基づいてJSONオブジェクトを特定し、オブジェクト内の`key`をTable Schemaの列名として使用し、すべての列タイプをStringに設定します。例：
...
- **ワイルドカードを使用したURI**
  ...
  URIはワイルドカードを使用して複数のファイルを読み取ることができます。注意：ワイルドカードを使用する場合、すべてのファイルが同じフォーマットである（特に`csv`、`csv_with_names`、`csv_with_names_and_types`は異なるフォーマットです）ことを確認してください。S3 TVFは最初のファイルを使用してTable Schemaを解析します。
