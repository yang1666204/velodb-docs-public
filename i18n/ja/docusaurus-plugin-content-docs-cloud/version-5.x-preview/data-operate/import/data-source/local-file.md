...
{
  "title": "ローカルファイル",
  "description": "Dorisは、ローカルソースからデータをロードする複数の方法を提供しています：",
  "language": "ja"
}
---
Dorisはローカルソースからデータを読み込むための複数の方法を提供します：

### 1. Stream Load

HTTPプロトコルを介してローカルファイルやデータストリームをDorisに読み込みます。CSV、JSON、Parquet、ORCフォーマットをサポートします。詳細については、[Stream Loadドキュメント](../import-way/stream-load-manual.md)を参照してください。

### 2. Streamloaderツール

StreamloaderツールはStream Loadをベースとした、Dorisデータベースにデータを読み込むための専用クライアントツールです。マルチファイルと並行読み込み機能を提供でき、大容量データの読み込みに必要な時間を短縮できます。詳細なドキュメントについては、Streamloaderを参照してください。
...
+----------+
```
## Streamloader Toolを使用したデータの読み込み
...
Streamloader toolを使用してデータを読み込みます：
