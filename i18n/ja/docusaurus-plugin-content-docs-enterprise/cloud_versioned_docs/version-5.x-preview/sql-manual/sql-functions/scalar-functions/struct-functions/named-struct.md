...
{
  "title": "NAMED_STRUCT",
  "description": "指定されたフィールド名と値に基づいて構造体を構築して返します。この関数は偶数個のパラメータを受け取ります。",
  "language": "ja"
}
---
## 説明
...
- `<field_name>`: 構造体を構築するための奇数位置の入力内容、フィールドの名前、定数文字列である必要があります
- `<field_value>`: 構造体を構築するための偶数位置の入力内容、フィールドの値、複数の列または定数が可能です
...
- この関数はすべてのフィールド名と値のペアを構造体に結合します。奇数位置はフィールド名（定数文字列である必要があり、重複できません。大文字小文字は区別されません）、偶数位置はフィールド値（複数の列または定数が可能）です
...
```
重複するフィールド名、フィールド名は大文字と小文字を区別しません：
