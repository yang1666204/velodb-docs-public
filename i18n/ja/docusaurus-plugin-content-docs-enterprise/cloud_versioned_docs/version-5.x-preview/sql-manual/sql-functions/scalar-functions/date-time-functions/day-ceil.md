...
{
  "title": "DAY_CEIL",
  "description": "dayceil関数は、指定された日付または時刻値を、指定された最も近い日の期間の開始時刻まで切り上げます。",
  "language": "ja"
}
---
## 説明

day_ceil関数は、指定された日付または時刻値を、指定された日単位期間の最も近い開始時刻に切り上げます。期間ルールは、period（日数）とorigin（基準時刻）によって定義されます。originが指定されていない場合、デフォルトで0001-01-01 00:00:00になります。
...
| `<period>` | 各期間の日数を指定します。型はINTです。指定されない場合、デフォルトの期間は1日です。 |
| `<origin>` | 期間計算の開始基準時刻。date/datetimeタイプをサポートします |

## Return Value

指定された日期間に入力値を切り上げた結果を表す日付または時刻値を返します。
...
- 丸め結果が日付タイプのサポート範囲（'9999-12-31'以降など）を超える場合、エラーが報告されます。
- スケール付きのdatetime入力の場合、出力はすべての小数部を0に切り詰め、戻り値にはスケールが含まれます
