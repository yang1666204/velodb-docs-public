...
分析関数で使用される「パーティション」という用語は、テーブルパーティショニング機能とは無関係です。この章では、「パーティション」という用語は分析関数に関連する意味のみを指します。
...
LAG関数とLEAD関数は、値間の比較に適しています。両方の関数は、自己結合を必要とせずにテーブル内の複数行に同時にアクセスできるため、クエリ処理の速度が向上します。具体的には、LAG関数は現在の行より指定されたオフセット分前の行へのアクセスを提供し、LEAD関数は現在の行より指定されたオフセット分後の行へのアクセスを提供します。
...
```text
 ...
 | JDR          | 13.98         | 2014-10-08 00:00:00 |
```
2. このクエリは分析関数を使用してmoving_average列を生成し、3日間の平均株価（前日、当日、翌日）を計算します。初日には前日の値がなく、最終日には翌日の値がないため、これらの行は2日間の平均のみを計算します。Partition By句は、すべてのデータがJDRのものであるため、ここでは効果がありませんが、他の株式情報がある場合、Partition Byは分析関数が自身のパーティション内でのみ動作することを保証します。
