---
{
    ...
    "language": "en",
    "description": "This document explains basic concepts of table creation and data partitioning in Apache Doris 3.x, including rows and columns, partition and bucket design, example CREATE TABLE syntax, and advanced features like auto partitioning and auto bucketing."
}
---
...
For detailed suggestions on this feature, see Auto Partition Conjunct with Dynamic Partition.
...
The `partitions` table function and the `information_schema.partitions` system table( supported since 3.0.2) record partition information for the cluster. The partition information can be extracted from the corresponding table for use when automatically managing partitions:
