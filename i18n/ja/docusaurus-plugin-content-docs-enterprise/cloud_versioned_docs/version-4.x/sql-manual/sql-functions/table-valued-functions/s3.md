...
S3 table-valued function (TVF) を使用すると、ユーザーはS3互換オブジェクトストレージに保存されているファイルを、リレーショナルテーブル形式のデータにアクセスするように読み取りおよびアクセスできます。現在、`csv/csv_with_names/csv_with_names_and_types/json/parquet/orc` ファイル形式をサポートしています。
