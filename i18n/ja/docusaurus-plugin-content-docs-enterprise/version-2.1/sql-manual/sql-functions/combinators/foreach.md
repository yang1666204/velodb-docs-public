...
{
  "title": "FOREACH",
  "description": "Table用の集約関数を、対応する配列要素を集約し、配列を返す配列用の集約関数に変換します",
  "language": "ja"
}
---
## description
Table用の集約関数を、対応する配列項目を集約し結果の配列を返す配列用の集約関数に変換します。例えば、配列 [1, 2]、[3, 4, 5]、[6, 7] に対する sum_foreach は、対応する配列項目を合計した後、結果 [10, 13, 5] を返します。
