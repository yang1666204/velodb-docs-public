...
---
`KILL`コマンドを使用して、現在実行中の操作をキャンセルしたり、現在の接続セッションを切断したりできます。このドキュメントでは、関連する操作と考慮事項を紹介します。
...
- `"query_id"`
	...
	この操作は、すべてのFEノード上でQuery IDを検索し、対応するクエリをキャンセルしようとします。
...
- `"trace_id"`
	...
	この操作は、すべてのFEノード上でTrace IDを検索し、対応するクエリをキャンセルしようとします。
