...
---
VARIANT型は半構造化JSONデータを格納でき、Table構造内で特定の列を事前定義することなく、異なるデータ型（整数、文字列、真偽値など）を含む複雑なデータ構造の格納を可能にします。この型は、いつでも変更される可能性のある複雑なネストした構造の処理に特に適しています。書き込みプロセス中に、VARIANT型は列の構造と型を自動的に推測し、書き込まれたスキーマを動的にマージし、JSONキーとそれに対応する値を列と動的サブ列として格納します。詳細なドキュメントについては、VARIANTを参照してください。
