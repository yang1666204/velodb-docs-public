...
---
この文書では、Tencent Cloud COSにアクセスするために必要なパラメータについて説明します。これらは以下のシナリオに適用されます：
...
* 設定パラメータには `cos.` プレフィックスを使用することを推奨します。これにより、Tencent Cloud COSとの一貫性と明確性が保たれます。
* バージョン3.1より前では、従来の `s3.` プレフィックスを使用してください。
* `cos.region` を設定することでアクセスの精度とパフォーマンスが向上するため、設定することを推奨します。
* コネクションプールパラメータは、接続のブロッキングを避けるために並行処理要件に応じて調整できます。
