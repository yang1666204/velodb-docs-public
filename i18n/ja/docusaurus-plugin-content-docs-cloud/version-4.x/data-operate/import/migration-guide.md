...
1. テーブル、ビュー、マテリアライズドビュー（同期および非同期）のメタデータ移行
...
1. メタデータ移行の現在の対象範囲には、2つの主要カテゴリが含まれます：
   1. **テーブル、ビュー、マテリアライズドビュー（同期および非同期）**
   2. **ユーザーと権限**
...
- **テーブル移行。** ソースDorisクラスターで`SHOW CREATE TABLE`コマンドを使用してテーブルDDLを取得します。必要な調整を行った後、ターゲットVeloDB Cloud環境で対応するテーブルを作成します。参考：https://doris.apache.org/docs/dev/sql-manual/sql-statements/table-and-view/table/SHOW-CREATE-TABLE
...
各テーブルに対応するRoutineload名のリスト、PAUSEおよびRESUME文を収集します。スクリプトを実行してください
...
以下はデモで、各テーブルに対して生成されたRoutineloadのCREATE文を使用しています
...
1. DorisからS3などのオブジェクトストレージにデータをエクスポートする際は、テーブル単位またはパーティション単位で、Parquetファイル形式を使用した`SELECT INTO OUTFILE`の使用を推奨します。
2. S3などのオブジェクトストレージからVeloDB Cloudにデータをインポートする際は、テーブル単位またはパーティション単位で、Parquetファイル形式を使用したTVFの使用を推奨します。

###### 3.4.2.1.1 Dorisからのエクスポート

DorisからS3などのオブジェクトストレージにデータをエクスポートする際は、テーブル単位またはパーティション単位で、Parquetファイル形式を使用した`SELECT INTO OUTFILE`の使用を推奨します。詳細については、https://doris.apache.org/docs/dev/data-operate/export/outfile#export-to-object-storage を参照してください
...
VeloDB Cloudは、管理プラットフォーム内で直接操作できるビジュアルデータ移行機能も提供しています。詳細については、[Migration](https://docs.velodb.io/cloud/5.x-preview/management-guide/migration)を参照してください。
...
1. `SELECT COUNT(*) FROM table`を使用して、データ移行後のDorisソーステーブルとVeloDB Cloudターゲットテーブルのレコード数が一致しているかを検証します。
2. `SELECT SUM(calculable_column) FROM table`を使用して、データ移行後のDorisソーステーブルとVeloDB Cloudターゲットテーブルの詳細データが完全に一致しているかを検証します。
