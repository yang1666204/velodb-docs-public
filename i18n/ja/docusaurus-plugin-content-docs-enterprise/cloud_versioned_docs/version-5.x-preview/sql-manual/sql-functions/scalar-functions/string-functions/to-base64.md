...
{
  "title": "TO_BASE64",
  "description": "TOBASE64関数は、入力文字列をBase64エンコード形式に変換します。Base64は64個の印刷可能文字に基づくエンコード方式で、",
  "language": "ja"
}
---
## 説明

TO_BASE64関数は、入力文字列をBase64エンコード形式に変換します。Base64は64個の印刷可能文字に基づくエンコード方式で、電子メール、URLパラメータ、JSONなど、バイナリデータをサポートしないプロトコルでデータを送信する際によく使用されます。この関数はRFC 4648標準に従い、標準のBase64文字セット（A-Z、a-z、0-9、+、/）とパディング文字（=）を使用します。
