...
{
  "title": "ADD_TIME",
  "description": "指定された時間間隔を日付/時刻または時刻式に追加します。2番目のパラメータが負の値の場合、",
  "language": "ja"
}
---
## 説明

指定された時間間隔を日付/時刻または時刻式に加算します。2番目のパラメータが負の値の場合、1番目のパラメータから間隔を減算することと同等です。
...
| `<time>`             | 有効な時刻式で、`<date_or_time_expr>`に加算される時刻値を表します。負の値の場合は減算を意味します。time型の入力をサポートします。 |

## 戻り値

`<date_or_time_expr>`に`<time>`を加算した結果を返します。戻り値の型は第一パラメータの型に依存します：
- 第一パラメータがdatetime型の場合、datetime型を返します。
- 第一パラメータがtime型の場合、time型を返します。

特殊なケース：
- 入力パラメータのいずれかがnullの場合、nullを返します。
- 第一パラメータがtime型で結果がtime型の範囲を超える場合、最大値（または最小値）の時刻値を返します。
- 第一パラメータがdatetime型で結果がdatetime型の範囲を超える場合、エラーがスローされます。
