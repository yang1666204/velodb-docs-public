...
{
  "title": "JSON_PARSE",
  "description": "生のJSON文字列をJSONバイナリ形式に解析します。異なる例外データ処理要件を満たすために、",
  "language": "ja"
}
---
## 説明
生のJSON文字列をJSONバイナリ形式に解析します。異なる例外データ処理要件を満たすために、以下のようなJSON_PARSEシリーズ関数が提供されています：
...
## パラメータ
### 必須パラメータ
- `<json_str>` String型、有効なJSON文字列である必要があります。
### オプションパラメータ
- `<default_json_value>` JSON型、NULLにすることができます。`<json_str>`の解析が失敗した場合、`<default_json_value>`がデフォルト値として返されます。
...
2. `JSONB_PARSE`/`JSONB_PARSE_ERROR_TO_NULL`/`JSONB_PARSE_ERROR_TO_VALUE`は基本的に同じ動作をしますが、解析が失敗した場合に取得される結果が異なります。
