---
{
  "title": "SHOW TABLE STATSコマンドは、Tableの統計情報を表示します。",
...
| columns | 解析されたカラムリスト           |
| trigger |   最後の解析トリガー方法           |
| new_partition |  新しいパーティション初回読み込みフラグ           |
| user_inject | ユーザー統計情報注入フラグ         |
| enable_auto_analyze | 自動解析有効化フラグ         |
...
| Privilege | Object | 注釈                                    |
|:--------------| :------------- |:------------------------------------------------|
| SELECT_PRIV   | Table    | SHOWを実行する際、照会対象TableのSELECT_PRIV権限が必要です。 |

## 例

1. Tabletest1の概要統計を表示します。
