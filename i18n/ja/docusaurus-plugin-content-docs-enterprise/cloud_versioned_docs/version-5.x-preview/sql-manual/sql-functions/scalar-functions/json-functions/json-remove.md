...
{
  "title": "JSON_REMOVE",
  "description": "JSONREMOVE関数は、JSONドキュメントからデータを削除して結果を返すために使用されます。",
  "language": "ja"
}
---
## Description
...
- `<path>` String型の式、パスパラメータは左から右の順序で評価されます。パスを評価することで生成されたJSONドキュメントが、次のパス評価の新しい値になります。
...
+------------------------------------------------+
```
2. JSON オブジェクトから `<path>` で指定された値を削除する
