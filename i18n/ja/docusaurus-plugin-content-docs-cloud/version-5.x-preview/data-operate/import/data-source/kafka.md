...
Dorisでは、CREATE ROUTINE LOADコマンドを通じて永続的なRoutine Load読み込みタスクを作成します。これは単一テーブル読み込みと複数テーブル読み込みに分けることができます。詳細な構文については、[CREATE ROUTINE LOAD](../../../sql-manual/sql-statements/data-modification/load-and-export/CREATE-ROUTINE-LOAD)を参照してください。
...
```
**ステップ3: 単一テーブルにデータをロードするRoutine Loadジョブの作成**
...
複数のテーブルを同時にロードする必要があるシナリオでは、Kafka内のデータにテーブル名情報を含める必要があり、フォーマットは`table_name|data`となります。例えば、CSVデータをロードする際のフォーマットは`table_name|val1,val2,val3`とする必要があります。テーブル名はDoris内のテーブル名と完全に一致する必要があり、そうでなければロードは失敗し、後述するcolumn_mapping設定はサポートされないことにご注意ください。
...
DISTRIBUTED BY HASH(user_id) BUCKETS 10;
```
**ステップ3: 複数のテーブルにデータを読み込むためのRoutine Loadジョブを作成する**
