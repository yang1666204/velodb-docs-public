...
{
  "title": "REGEXP",
  "description": "文字列 str に対して正規表現マッチを実行し、マッチが成功した場合は true を、そうでなければ false を返します。",
  "language": "ja"
}
---
## 説明
...
文字セットマッチングを処理する際は、Utf-8標準文字クラスを使用する必要があることに注意してください。これにより、関数は異なる言語の様々な文字を正しく識別し処理できるようになります。

`pattern`が許可された正規表現でない場合、エラーをスローします。
...
| -- | -- |
| `<str>` | String型。正規表現とマッチングされる文字列を表します。テーブル内のカラムまたはリテラル文字列を指定できます。|
| `<pattern>` | String型。文字列<str>とのマッチングに使用される正規表現パターンです。正規表現は、文字クラス、量詞、アンカーを含む複雑な検索パターンを定義するための強力な方法を提供します。|

## 戻り値

REGEXP関数はBOOLEAN値を返します。文字列<str>が正規表現パターン<pattern>とマッチする場合、関数はtrue（SQLでは1として表現）を返し、マッチしない場合はfalse（SQLでは0として表現）を返します。
...
+--------------------+
```
`pattern`が正規表現として無効な場合、エラーをthrowする；
