...
---
MySQL/SqlServer/Oracle などの他のTPシステムからDorisへデータを移行する方法は様々あります。

## Multi-カタログ

Catalogを使用して外部Tableとしてマッピングし、INSERT INTOまたはCREATE-TABLE-AS-SELECT文を使用してデータロードを完了します。
...
SELECT * FROM iceberg_catalog.iceberg_db.table1;
```
詳細については、[カタログ Data Load](../../../lakehouse/catalog-overview.md#data-import)を参照してください。

## Flink Doris Connector

Flinkを活用してTPシステムのオフライン同期とリアルタイム同期を実現できます。
...
- FlinkCDCを使用してフルデータと増分データの両方を読み取ることで、リアルタイム同期を実現できます。例えば、FlinkSQLを使用する場合：
  ...
  ```
TP データベース内のデータベース全体または複数のTableを同期する場合、Flink Doris Connector が提供するフルデータベース同期機能を使用して、ワンクリックで TP データベースの書き込みを完了できます。以下に示します：
  ...
  ```    
より詳細については、Full Database Synchronizationを参照してください。
