---
{
  ...
  "description": "frontendsdisks テーブル関数は、現在の Doris クラスター内の FE ノードのディスク情報を表示するための一時テーブルを生成します。",
  "language": "ja"
}
---
## 説明

`frontends_disks`テーブル関数は、現在のDorisクラスター内のFEノードのディスク情報を表示するための一時テーブルを生成します。その結果は本質的に`show frontends disks`文によって表示される情報と同じですが、`frontends_disks()`のフィールドタイプはより明示的であり、TVFによって生成されたテーブルをフィルタリング、結合、およびその他の操作に使用できます。
