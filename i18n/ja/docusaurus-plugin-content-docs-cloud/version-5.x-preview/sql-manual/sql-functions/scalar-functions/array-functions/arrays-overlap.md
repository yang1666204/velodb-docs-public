...
- `arr1`: 最初の配列、型は `ARRAY<T>`。

- `arr2`: 2番目の配列、型は `ARRAY<T>`。
...
3. **テーブル作成文で転置インデックスを指定して実行を高速化できます**（例を参照）。
...
1. 簡単な例
    ...
    +----------------------------------------------------------------+
    ```
2. 無効なパラメータ型: サポートされていない型が渡された場合、`INVALID_ARGUMENT`を返します
...
3. 入力 `ARRAY` が `NULL` の場合、戻り値は `NULL` です
    ...
    +----------------------------+
    ```
4. 入力の`ARRAY`が`NULL`を含む場合、`NULL`と`NULL`は等しいとみなされます
