---
{
  ...
  "description": "ARRAYSOVERLAPは、2つの配列に少なくとも1つの共通要素があるかどうかを◊するために使用されます。共通要素がある場合はtrueを返し、ない場合はfalseを返します。",
  "language": "ja"
}
---
## Function

`ARRAYS_OVERLAP`は2つの配列が少なくとも1つの共通要素を持つかどうかを◊するために使用されます。共通要素がある場合は`true`を返し、そうでなければ`false`を返します。
...
- `arr1`: 最初の配列、型は `ARRAY<T>` です。

- `arr2`: 2番目の配列、型は `ARRAY<T>` です。

    - 両方の配列の要素型 `T` は同じであるか、互いに暗黙的に変換可能である必要があります。
    - 要素型 `T` は数値、文字列、日付/時刻、またはIP型にすることができます。

## 戻り値

- `BOOLEAN` 型を返します：

    - 2つの配列が交差している場合、`true` を返します；
    - 交差していない場合、`false` を返します。

## 使用上の注意

1. **比較は要素の等価性（`=` 演算子）を使用して行われます**。
2. **この関数では `NULL` と `NULL` は等しいとみなされます**（例を参照）。
3. **テーブル作成文で転置インデックスを指定することで実行を高速化できます**（例を参照）。
   - 関数が述語条件として使用される場合、転置インデックスが実行を高速化します。
   - 関数がクエリ結果で使用される場合、転置インデックスは実行を高速化しません。
4. データクリーニング、タグマッチング、ユーザー行動の交差シナリオでよく使用されます。
...
1. 簡単な例
    ...
    +----------------------------------------------------------------+
    ```
2. 無効なパラメータタイプ：サポートされていないタイプが渡された場合、`INVALID_ARGUMENT`を返す
