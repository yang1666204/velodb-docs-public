...
---
BITMAP型は、Duplicate table、Unique table、Aggregate tableで使用できますが、Key型としてのみ使用可能で、Value列としては使用できません。Aggregate tableでBITMAP型を使用する場合、テーブルは集約タイプBITMAP_UNIONで作成する必要があります。ユーザーは長さやデフォルト値を指定する必要はありません。長さは、データ集約の度合いに基づいてシステムによって制御されます。詳細なドキュメントについては、Bitmapを参照してください。
