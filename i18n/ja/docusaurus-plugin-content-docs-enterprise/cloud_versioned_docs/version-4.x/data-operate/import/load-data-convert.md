...
- **Column Mapping**: ソースデータのカラムをターゲットテーブルの異なるカラムにマッピングします。
...
Column mappingは、ソースデータ列とターゲットテーブル列の対応関係を定義するために使用されます。以下のシナリオに対応できます：
- ソースデータ列とターゲットテーブル列の順序が一致しない
- ソースデータ列とターゲットテーブル列の数が一致しない
...
- **ステップ2: Column Mappingと代入** - 列名によって中間変数をターゲットテーブルフィールドにマッピング
...
4,\N,chongqing,1.4
```
対象テーブルには4つの列があります：k1、k2、k3、k4です。次のように列をマッピングしたいと思います：
...
4,\N,chongqing,1.4
```
ターゲットテーブルは3つの列を持ちます：k1、k2、k3。ソースファイルからは1番目、2番目、4番目の列のみが必要で、以下のマッピング関係になります：
...
column4 -> k3
```
ソースファイルの特定の列をスキップするには、列マッピング時にターゲットテーブルに存在しない任意の列名を使用できます。これらの列名はカスタマイズ可能で、制限はありません。これらの列のデータは読み込み時に自動的に無視されます。
...
FROM KAFKA (...);
```
> 注意: 例の`tmp_skip`は、対象テーブルの列定義に含まれていない限り、任意の名前に置き換えることができます。
...
4,\N,chongqing,1.4
```
対象テーブルには5つの列があります：k1、k2、k3、k4、k5。ソースファイルからは1番目、2番目、3番目、4番目の列のみが必要で、以下のマッピング関係になります：
...
4,\N,chongqing,1.4
```
ターゲットテーブルには4つのカラムがあります：k1、k2、k3、k4です。カラムの値を以下のように変換したいと思います：
...
4,\N,chongqing,1.4
```
ターゲットテーブルは4つのカラムを持ちます：k1、k2、k3、k4。次のようにカラムの値を変換したいと思います：
...
4,\N,chongqing,1.4
```
対象のテーブルには4つの列があります：k1、k2、k3、およびk4。これらの列の値を次のように変換したいと思います：
...
例えば、ソースデータに複数のテーブルのデータが含まれている場合（または複数のテーブルのデータが同じKafkaメッセージキューに書き込まれている場合）。各データ行には、そのデータがどのテーブルに属するかを示すカラムがあります。ユーザーはpre-filtering条件を使用して、ロードする対応するテーブルデータをフィルタリングできます。
...
4,\N,chongqing,1.4
```
ターゲットテーブルには4つのカラムがあります：k1、k2、k3、k4。4番目のカラムが1.2より大きいデータのみをロードしたいと思います。
...
4,\N,chongqing,1.4
```
対象テーブルには4つのカラムがあります：k1、k2、k3、k4。これらのカラム値を以下のように変換したいと思います：
...
4,\N,chongqing,1.4
```
対象テーブルには4つの列があります：k1、k2、k3、k4。k1がNULLで、k4が1.2未満のデータを除外したいと考えています。
