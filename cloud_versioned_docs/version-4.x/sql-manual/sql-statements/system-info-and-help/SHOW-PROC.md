...
7. cluster_balance : To check the balance of the cluster, please refer to Data Copy Management
...
9. colocation_group : This command can view the existing Group information in the cluster. For details, please refer to the Colocation Join chapter
...
16. jobs: show statistics of all kind of jobs. If a specific `dbId` is given, will return statistics data of the database. If `dbId` is -1, will return total statistics data of all databases
...
18. monitor : shows the resource usage of FE JVM
19. resources : View system resources, ordinary accounts can only see resources that they have USAGE_PRIV permission to use. Only the root and admin accounts can see all resources. Equivalent to [SHOW RESOURCES](../cluster-management/compute-management/SHOW-RESOURCES)
20. routine_loads: Display all routine load job information, including job name, status, etc.
21. statistics: It is mainly used to summarize and view the number of databases, tables, partitions, shards, and replicas in the Doris cluster. and the number of unhealthy copies. This information helps us to control the size of the cluster meta-information in general. It helps us view the cluster sharding situation from an overall perspective, and can quickly check the health of the cluster sharding. This further locates problematic data shards.
22. stream_loads: Returns the stream load task being executed.
23. tasks : Displays the total number of tasks of various jobs and the number of failures.
...
|------------|-----------|-------------|
| Name | String | Database or table name. |
| Type | String | Type of data object, value can be "db" (database) or "table" (table) |
| Id | Number | Database ID or Table ID |
| Dropped | Boolean | Whether the database or table has been deleted. A value of "true" indicates that the object has been removed from Doris, but its Binlog records are still retained; a value of "false" indicates that the object still exists in the system. Even if a database or table is deleted, the system will still retain Binlog records for a period of time until the TTL expires or is manually cleaned up |
| BinlogLength | Number | Total number of binary log entries for this database or table |
| BinlogSize | Number | Total size of binary logs (bytes) |
| FirstBinlogCommittedTime | Number | Timestamp of the first binary log commit (Unix timestamp, milliseconds) |
| ReadableFirstBinlogCommittedTime | String | Commit time of the first binary log (readable format) |
| LastBinlogCommittedTime | Number | Timestamp of the last binary log commit (Unix timestamp, milliseconds) |
| ReadableLastBinlogCommittedTime | String | Commit time of the last binary log (readable format) |
| BinlogTtlSeconds | Number | Time-to-live for binary logs (seconds), logs exceeding this time may be cleaned up |
| BinlogMaxBytes | Number | Maximum size of binary logs (bytes), exceeding this size may trigger cleanup |
| BinlogMaxHistoryNums | Number | Maximum number of binary log history records to retain, exceeding this count may trigger cleanup |

Notes:
- Only databases and tables with binary logging enabled will appear in the output of this command
- If database-level binary logging is enabled, the binary log information for the database is displayed; otherwise, information for individual tables with binary logging enabled in that database is displayed
- 
## Example
