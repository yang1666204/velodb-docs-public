...
複数テーブルの結合を含むクエリでは、テーブルの1つが更新されると、パーティションIDまたはバージョン番号が異なるため、キャッシュミスが発生します。
...
Frontend（FE）がクエリリクエストを受信すると、最初にSQL文字列を使用してメモリ内を検索し、同じクエリが以前に実行されたかどうかを判断し、そのクエリのメタデータ情報を取得しようとします。この情報には、クエリに関与するテーブルとパーティションのバージョンが含まれます。

これらのメタデータが変更されていない場合、対応するテーブル内のデータが変更されていないことを示し、以前のSQL Cacheを再利用できます。この場合、FEはSQL解析と最適化プロセスをスキップし、一貫したハッシュアルゴリズムに基づいて直接対応するBEを特定し、そこからクエリ結果を取得しようとします。
...
1. `drop table`、`replace table`、`alter table`、`alter view`の実行など、テーブル/ビュー構造の変更。

2. `insert`、`delete`、`update`、`truncate`の実行など、テーブルデータの変更。
...
6. 特定のテーブルデータをユーザーに非表示に設定するなど、Row PolicyまたはData Maskingの変更。
