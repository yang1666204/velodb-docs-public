...
GROUP BY クエリにおける行のグルーピングレベルを計算します。GROUPING_ID 関数は、特定の出力行に対して GROUP BY リスト内のどの列が集約されていないかを示す整数ビットマップを返します。この関数は、GROUP BY が指定されている場合に SELECT リスト、HAVING、または ORDER BY 句で使用できます。
