...
配列から連続する重複要素を削除し、異なる各値の最初の出現のみを保持します。この関数は配列を左から右に走査し、前の要素と同じ要素をスキップして、各値の最初の出現のみを保持します。
...
- `arr`：ARRAY<T>型、重複排除する配列。カラム名または定数値をサポート。
...
戻り値の意味:
- 重複排除された配列、連続する重複要素の最初の出現のみを保持
- NULL: 入力配列がNULLの場合

戻り値の動作説明:

1. 通常の重複排除動作:
   - 配列を左から右に走査し、各要素の最初の出現を保持し、前の要素と同じ連続する要素をスキップ
   - 連続する重複要素のみを削除し、非連続の重複要素は保持される
   - null値を保持（nullとnullは同じとみなされる）
...
- 関数は配列要素の元の順序を維持する
- 連続する重複要素のみを削除し、グローバルな重複排除は実行しない
- map、structは重複排除ロジックをサポートしない
- 配列要素のnull値について: null要素は通常通り処理され、連続する複数のnull要素は1つにマージされる
...
string_array での連続する重複の削除: 隣接する 'a' または 'b' のみが削除され、'c' は保持されます。
