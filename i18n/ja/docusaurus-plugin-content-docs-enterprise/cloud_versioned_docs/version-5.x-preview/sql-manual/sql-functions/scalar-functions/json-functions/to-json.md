...
{
  "title": "TO_JSON",
  "description": "DorisのインテグナルなデータタイプをJSONBタイプに変換します。この関数は、互換性のあるDorisのデータタイプを次の形式のJSON表現に変換することを可能にします",
  "language": "ja"
}
---
## 説明

Dorisの内部データ型をJSONB型に変換します。この関数は、互換性のあるDorisデータ型を精度を失うことなくJSON表現に変換することを可能にします。
...
さらに、この関数は以下の型の変換もサポートしています：
- 日付型：DATETIME、DATE、TIME
- IP型：IPV4、IPV6
- 複合型：MAP

対応するJSONB型を持たないDATETIME、DATE、TIME、IPV4、IPV6型については、STRING型に変換されます。
MAP型については、JSONB Object型に変換されます。JSON標準ではObjectキーが文字列である必要があるため、Mapキーは文字列型である必要があります。

## 戻り値

JSONB型の値を返します。

入力の`value`がSQL NULLの場合、この関数はSQL NULLを返します（JSON null値ではありません）。配列や構造体内にNULL値が現れる場合、それらはJSON null値に変換されます。
...
1. 一部の型には直接的なJSONマッピングが存在しません（DATEなど）。これらの型については、まずSTRINGに変換してから`TO_JSON`を使用する必要があります。

2. `TO_JSON`を使用してDoris内部型をJSONBに変換する場合、テキスト表現を通じて変換する場合とは異なり、精度の損失はありません。

3. DorisのJSONBオブジェクトは、デフォルトで1,048,576バイト（1 MB）のサイズ制限があり、BE設定`string_type_length_soft_limit_bytes`を通じて最大2,147,483,643バイト（約2 GB）まで調整できます。
