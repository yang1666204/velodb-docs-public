...
配列内のすべての要素が指定された条件に一致する場合はtrueを返し、そうでなければfalseを返します。配列にNULL要素が含まれ、NULL以外のすべての要素が条件に一致する場合は、NULLを返します。
...
1. この関数はNULL値を以下の方法で処理します：
   - NULL要素が存在し、すべての非NULL要素が条件に一致する場合、NULLを返します
   - いずれかの非NULL要素が条件に一致しない場合、NULL要素の有無に関係なくfalseを返します
