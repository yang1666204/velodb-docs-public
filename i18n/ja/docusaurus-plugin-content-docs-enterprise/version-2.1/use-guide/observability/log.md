...
---
この文書では、中核的なオブザーバビリティコンポーネントの一つであるLogsのストレージと分析プラクティスを紹介します。完全なオブザーバビリティソリューションの概要については、Overviewを参照してください。
...
| -          | `max_tablet_version_num = 20000`                             | Table作成用のtime_series compactionストラテジーと連携し、より多くのバージョンが一時的に未マージのまま残ることを許可 |
| コンパクション | `max_cumu_compaction_threads = 8`                            | CPUコア数 / 4に設定。CPUリソースの1/4を書き込み用、1/4をバックグラウンドcompaction用、2/4をクエリやその他の操作用に使用することを示す。 |
...
ログデータの書き込みとクエリの両方の特徴的な性質により、パフォーマンスを向上させるため、対象を絞った設定でTableを構成することを推奨します。
...
1. Logstash Doris 出力プラグイン をダウンロードしてインストールします。以下の2つの方法のうちいずれかを選択できます：
...
3. 以下のコマンドに従ってFilebeatを実行し、ログを収集してApache Dorisに出力します。
    ...
    ```
Beatに関する詳細情報については、Beats Doris 出力プラグインを参照してください。
...
- 詳細なログ情報の表示、JSONまたはTable形式への展開可能
