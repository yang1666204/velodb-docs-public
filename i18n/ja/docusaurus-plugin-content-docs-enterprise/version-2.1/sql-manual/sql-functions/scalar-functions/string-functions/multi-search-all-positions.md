---
{
  ...
  "description": "i番目の要素がneedles（すなわちneedle）のi番目の要素の文字列haystack内での最初の出現位置を示すARRAYを返します。",
  "language": "ja"
}
---
...
`ARRAY`を返します。`i`番目の要素は、`needles`の`i`番目の要素（つまり`needle`）が文字列`haystack`内で**最初に**出現する位置です。位置は1から数えられ、0は要素が見つからなかったことを意味します。**大文字小文字を区別します**。
