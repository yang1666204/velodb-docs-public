...
STRUCT型は複数のフィールドを単一の構造に結合するために使用されます。各フィールドは独自の名前と型を持つことができ、ネストされた複雑なビジネスデータ構造を表現するのに適しています。

- `STRUCT<field_name:field_type [COMMENT 'comment_string'], ... >`

  - `field_name`は名前を表します。**空にすることはできず、重複することもできず、大文字小文字を区別しません**。

  - `field_type`は型を表します。nullableであり、NOT NULLとして指定することはできません。サポートされる型には以下があります：`BOOLEAN, TINYINT, SMALLINT, INT, BIGINT, LARGEINT, FLOAT, DOUBLE, DECIMAL, DATE, DATETIME, CHAR, VARCHAR, STRING, IPV4, IPV6, ARRAY, MAP, STRUCT`。

  - `[COMMENT 'comment-string']`はオプションのコメントを表します。

## 型の制約

- `STRUCT`型がサポートする最大ネスト深度は9です。

- `STRUCT`型間の変換は、内部の型が変換可能かどうかに依存します（名前は変換に影響しません）。`STRUCT`型は他の型に変換することはできません。

  - 文字列型は`STRUCT`型に変換できます（解析を通じて、解析に失敗した場合はNULLを返します）。

- `AGGREGATE`Tableモデルでは、`STRUCT`型は`REPLACE`と`REPLACE_IF_NOT_NULL`のみをサポートします。**いずれのTableモデルでも、KEYカラムとして使用することはできず、パーティションカラムやバケットカラムとしても使用できません。**

- `STRUCT`型のカラムは比較演算や算術演算をサポートせず、**`ORDER BY`と`GROUP BY`演算をサポートせず、`JOIN KEY`として使用することもできず、`DELETE`文で使用することもできません。**

- `STRUCT`型のカラムはいかなるインデックスの作成もサポートしません。

## 型の構築

- `STRUCT()`を使用してSTRUCT型の値を構築します。内部の名前はcol1から始まります。
  ...
  +--------------------------------------+
  ```
- 特定の`STRUCT`型の値を構築するには`NAMED_STRUCT()`を使用します。
...
- 変更は`STRUCT`のサブカラム型が`VARCHAR`の場合のみ許可されます。
   ...
   - `VARCHAR`のパラメータを小さい値から大きい値への変更のみ許可され、その逆は許可されません。
...
- `STRUCT`内の特定のサブカラムにアクセスするには、`STRUCT_ELEMENT(struct, k/field_name)`を使用します。
