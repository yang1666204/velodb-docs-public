...
DorisはJavaを使用したUDF、UDAF、UDTFの開発をサポートしています。特に指定がない限り、以下のテキストでの「UDF」は、すべてのタイプのユーザー定義関数を指します。

1. Java UDF：Java UDFは一般的に使用されるスカラー関数で、各入力行に対応する出力行を生成します。一般的な例としてABSやLENGTHがあります。注目すべきことに、Hive UDFはDorisに直接移行できるため、ユーザーにとって便利です。

2. Java UDAF：Java UDAFは複数の入力行を単一の出力行に集約するユーザー定義集約関数です。一般的な例としてMIN、MAX、COUNTがあります。

3. Java UDWF：User-Defined Window Functionの略で、ウィンドウ（1行または複数行）に基づいて各行の計算値を返します。一般的な例としてROW_NUMBER、RANK、DENSE_RANKがあります。

4. Java UDTF：Java UDTFはユーザー定義Table関数で、単一の入力行から1つまたは複数の出力行を生成できます。Dorisでは、UDTFは行から列への変換を実現するためにLateral Viewと組み合わせて使用する必要があります。一般的な例としてEXPLODEやEXPLODE_SPLITがあります。**Java UDTFはバージョン3.0.0以降で利用可能です。**
...
:::caution Warning
関数を作成する際は、`string`の代わりに`varchar`を使用することは避けてください。関数が失敗する原因となる可能性があります。
:::

## 使用上の注意

1. 複雑なデータ型（HLL、Bitmap）はサポートされていません。

2. ユーザーは現在JVMヒープサイズの最大値を指定することができます。設定項目は`be.conf`の`JAVA_OPTS`の`-Xmx`部分です。デフォルトは1024mです。データを集約する必要がある場合は、パフォーマンスを向上させ、メモリオーバーフローのリスクを軽減するためにこの値を増やすことを推奨します。

3. JVMが同じ名前のクラスをロードする問題により、同じ名前の複数のクラスをUDF実装として同時に使用しないでください。同じ名前のクラスでUDFを更新したい場合は、classpathを再ロードするためにBEを再起動する必要があります。

## はじめに
このセクションでは主にJava UDFの開発方法を紹介します。参考例は`samples/doris-demo/java-udf-demo/`に提供されています。詳細については[こちら](https://github.com/apache/doris/tree/master/samples/doris-demo/java-udf-demo)をクリックしてご確認ください。

UDFの使用法は標準関数と同じですが、主な違いは、組み込み関数がグローバルスコープを持つのに対し、UDFはDB内にスコープが限定されることです。

セッションがデータベース内でリンクされている場合、UDF名を直接使用すると現在のDB内で対応するUDFが検索されます。そうでない場合は、ユーザーはUDFのデータベース名を明示的に指定する必要があります。例：`dbName.funcName`。

以下のセクションでは、例として`test_table`Tableを使用します。対応するTable作成スクリプトは以下の通りです：
...
```
### Java-UDF例の紹介
JavaでUDFを記述する際、メインエントリーポイントは`evaluate`関数である必要があります。これはHiveなどの他のエンジンと一致しています。この例では、整数入力に対してインクリメント操作を実行する`AddOne` UDFを記述します。
...
1. 対応するJavaコードを記述し、JARファイルにパッケージ化します。
    ...
    ```
2. DorisでJava-UDF関数を登録および作成します。構文の詳細については、CREATE FUNCTIONを参照してください。
    ...
    ```
3. UDFを利用するには、ユーザーは対応するデータベースに対する`SELECT`権限を持っている必要があります。そして、UDFの登録が成功したことを確認するには、SHOW FUNCTIONSコマンドを使用できます。
    ...
    +------+----------------------+
    ```
4. UDFが不要になった場合は、DROP FUNCTIONで詳しく説明されている以下のコマンドを使用してドロップできます。

さらに、UDFで大きなリソースファイルの読み込みやグローバル静的変数の定義が必要な場合は、このドキュメントで後述する静的変数の読み込み方法を参照してください。

### Java-UDAF例の紹介

Javaを使用して`UDAF`を記述する際、実装する必要がある関数（必須としてマークされている）と内部クラスStateがあります。以下の例では、これらの実装方法を説明します。

1. 対応するJava UDAFコードを記述し、JARファイルにパッケージ化します。

<details>
<summary> 例1：SimpleDemoは、入力パラメータがINTで出力パラメータがINTのsum関数に似たシンプルな関数を実装します。</summary>
...
<details>
<summary> 例2：MedianUDAFは中央値を計算する関数です。入力型は(DOUBLE, INT)で、出力型はDOUBLEです。 </summary>
...
3. Java-UDAFを使用する場合、グループ化による集約またはすべての結果の集約のいずれかを実行できます：
    ...
    +-----------------+
    ```
### Java-UDWF Example の紹介

1. 実装は Java UDAF と同様ですが、状態をクリアするための追加の reset() メソッドが必要です。

    ```JAVA
    void reset(State state)
    ```
2. DorisでUDAFと同様にJava-UDWF関数を登録・作成します。構文の詳細については、CREATE FUNCTIONを参照してください。
    ...
    ```
3. Java UDWFは、特定のウィンドウフレーム内で計算結果をクエリすることを可能にします。詳細な構文については、[Window ファンクション](../window-function.md)を参照してください。
...
現在、Dorisでは、UDF関数を実行する際（例：`select udf(col) from table`）、各並行インスタンスに対してudf.jarパッケージをロードし、インスタンスが終了するとudf.jarパッケージをアンロードします。

udf.jarファイルが数百MBのファイルをロードする必要がある場合、並行性によりメモリ使用量が急激に増加し、OOM（Out of Memory）を引き起こす可能性があります。

また、コネクションプールを使用したい場合、このアプローチでは静的エリアで一度だけ初期化することができません。

ここでは2つの解決策を示しますが、2番目の解決策にはDorisバージョンbranch-3.0以降が必要です。

*解決策1:*

解決策は、リソースロードコードを分離し、別のjarパッケージを生成して、他のパッケージがこのリソースjarパッケージを直接参照するようにすることです。
...
2. 次に、前のステップのリソースパッケージを直接参照してFunctionUdfファイルをコンパイルし、FunctionUdf.jarパッケージを生成します：
    ...
    ```
3. 上記の2つのステップの後、2つのjarパッケージが得られます。リソースjarパッケージがすべての並行インスタンスから参照できるようにするため、デプロイメントパス`be/custom_lib`に配置してください。再起動後、JVMの起動時にロードされます。その結果、リソースはサービス開始時にロードされ、サービス停止時に解放されます。
...
BE（Backend）はJARファイルをグローバルにキャッシュし、有効期限と削除時間をカスタマイズします。関数を作成する際に、2つの追加プロパティが追加されます：

static_load: 静的キャッシュローディング方式を使用するかどうかを定義します。
expiration_time: JARファイルの有効期限を分単位で定義します。

静的キャッシュローディング方式を使用する場合、UDFインスタンスは最初の呼び出しと初期化後にキャッシュされます。その後のUDF呼び出しでは、システムはまずキャッシュ内を検索します。見つからない場合、初期化プロセスがトリガーされます。
...
```
ご覧の通り、結果は増加し続けており、これは読み込まれたJARファイルがアンロードされて再読み込みされていないことを証明しています。代わりに、変数が0に再初期化されています。
