...
TOPN_ARRAYは、指定された列でN個の最も頻出する値の配列を返します。これは近似計算関数で、カウント数の降順で並べられた結果を返します。
...
| `<space_expand_rate>` | オプションパラメータで、Space-Savingアルゴリズムで使用されるカウンタ数を設定するために使用されます。`counter_numbers = top_num * space_expand_rate`で、space_expand_rateの値が大きいほど結果はより正確になり、デフォルト値は50です。 |
