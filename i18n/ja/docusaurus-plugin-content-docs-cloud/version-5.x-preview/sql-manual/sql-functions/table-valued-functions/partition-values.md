---
{
  ...
  "description": "特定のTABLEのパーティション値のリストを表示することを可能にする、パーティション値の一時テーブルを生成するテーブル関数。",
  "language": "ja"
}
---
## 説明

特定のTABLEのパーティション値のリストを表示できるよう、パーティション値の一時テーブルを生成するテーブル関数です。
...
クエリ対象のテーブルには複数のパーティションフィールドがあり、それに応じてテーブルには複数の列が存在します。

## 例

multi_catalogの下にあるhive3 CATALOGの`text_partitioned_columns`のテーブル作成文は以下の通りです：
