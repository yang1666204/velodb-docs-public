...
{
  "title": "SET PROPERTY",
  "description": "ユーザーに割り当てられたリソース、インポートするクラスターなど、ユーザー属性を設定します。",
  "language": "ja"
}
---
## Description
...
スーパーユーザー権限：
 ...
 - `max_user_connections`: 最大接続数です。
 - `max_query_instances`: ユーザーが同時にクエリを実行するために使用できるインスタンス数です。
 - `sql_block_rules`: SQLブロックルールを設定します。一度設定されると、このユーザーが送信するクエリがルールに一致する場合は拒否されます。
 - `cpu_resource_limit`: クエリのCPUリソースを制限します。セッション変数`cpu_resource_limit`の詳細な説明を参照してください。-1は未設定を意味します。
 - `exec_mem_limit`: クエリのメモリ使用量を制限します。セッション変数`exec_mem_limit`の詳細な説明を参照してください。-1は未設定を意味します。
...
プロパティを設定するユーザーのユーザー名です。省略した場合、現在のユーザーにプロパティが設定されます。
