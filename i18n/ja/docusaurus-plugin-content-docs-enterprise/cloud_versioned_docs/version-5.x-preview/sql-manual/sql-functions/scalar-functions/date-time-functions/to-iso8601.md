---
{
  ...
  "description": "DATETIME型とDATE型の入力タイプをサポートし、datetime値をISO8601形式の文字列に変換します。",
  "language": "ja"
}
---
# TO_ISO8601
...
DATETIME および DATE の入力タイプをサポートして、datetime値をISO8601形式の文字列に変換します。
返されるISO8601形式のdatetimeは、YYYY-MM-DDTHH:MM:SSとして表現され、Tは日付と時刻の間のセパレータです。
...
| パラメータ | 説明 |
|-----------|-------------|
| `<date_or_date_expr>` | 入力datetime値、date/datetimeタイプをサポートします。datetimeおよびdateフォーマットについては、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)と[date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion)を参照してください |
...
- 入力がDATETIME（例：'2023-10-05 15:30:25'）の場合、YYYY-MM-DDTHH:MM:SS.xxxxxx形式（日付と時間がTで区切られ、xxxxxxはすべてゼロ、入力datetimeの小数秒は秒に丸められます）を返します；
