...
| `<pattern>` | 文字列パラメータ。対象となる正規表現パターンです。このパターンには様々な正規表現メタ文字と文字クラスを含めることができ、マッチする部分文字列のルールを正確に定義します。 |
| `<pos>` | 整数パラメータ。抽出する式グループのインデックスを示します。インデックスは1から開始します。<pos>が0に設定された場合、最初にマッチした部分文字列全体が返されます。<pos>が負の数であるか、パターン内の式グループ数を超える場合、関数はNULLを返します。 |
...
文字列型を返し、結果は`<pattern>`にマッチする部分です。
 ...
 入力`<pos>`が無効（負の値または式グループ数を超える）の場合、NULLを返します。
...
+---------------------------------------------------------------------------+
```
無効なグループインデックス。パターンには2つのグループのみがあるため、インデックス5は範囲外となり、NULLが返されます。
...
+----------------------------------------+
```
`pattern`が許可されたregexp正規表現でない場合、エラーをthrowする;
