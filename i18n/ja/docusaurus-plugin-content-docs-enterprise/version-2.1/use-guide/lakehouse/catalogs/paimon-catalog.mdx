...
{
  "title": "Paimon カタログ",
  "description": "Apache Doris 2.1のPaimon カタログは、複数のPaimonメタデータサービスに接続し、HDFSとクラウドオブジェクトストレージ全体でPaimonTableをクエリします。詳細な設定、プロパティ、クエリ操作を提供し、書き込みサポートが計画されています。",
  "language": "ja"
}
---
...
DorisはPaimonTableのメタデータに様々なメタデータサービス経由でアクセスし、Paimonデータをクエリすることを現在サポートしています。

現時点では、PaimonTableへの読み取り操作のみがサポートされています。PaimonTableへの書き込み操作は将来的にサポートされる予定です。
...
| データ統合 | Paimonデータを読み取りDoris内部Tableに書き込む、またはDoris計算エンジンを使用してZeroETL操作を実行します。 |
...
* `<paimon_warehouse>`
  ...
  `warehouse`パスは`Database`パスの1階層上を指定する必要があります。例えば、Tableパスが`s3://bucket/path/to/db1/table1`の場合、`warehouse`は`s3://bucket/path/to/`にする必要があります。
...
* `{StorageProperties}`
  ...
  StoragePropertiesセクションは、ストレージシステムに関連する接続および認証情報を入力するために使用されます。詳細は[Supported Storage システム]のセクションを参照してください。
...
* `{CommonProperties}`
  ...
  CommonPropertiesセクションは共通プロパティを入力するために使用されます。[Common Properties]の詳細については、[カタログ 概要](../catalog-overview.md)セクションを参照してください。
...
| varchar                            | string        |                                                                         |
| char                               | string        |                                                                         |
| binary, varbinary                             | string/varbinary | カタログの`enable.mapping.varbinary`プロパティによって制御されます（4.0.2以降でサポート）。デフォルトは`false`で`string`にマップされます。`true`の場合は`varbinary`タイプにマップされます。|
| date                               | date          |                                                                         |
...
指定されたPaimonTableのスナップショットは`$snapshots`Table関数を通じてクエリできます:
...
`$branches`と`$tags`システムTableを使用して、PaimonTableのブランチとタグを表示できます：
...
DorisはPaimonシステムTableのクエリをサポートし、Table関連のメタデータを取得できます。システムTableを使用して、スナップショット履歴、マニフェストファイル、データファイル、パーティション、その他の情報を表示できます。
...
Tableの現在および履歴のスキーマ情報を表示します。`ALTER TABLE`、`CREATE TABLE AS`、または `CREATE DATABASE AS` ステートメントを使用してTableスキーマを変更する際、各変更は schemas Table内にレコードを生成します：
...
Tableのすべてのタグ情報を表示します。タグ名と関連するスナップショットが含まれます：
...
#### Tableの最新のスナップショット情報を表示して現在の状態を把握する
