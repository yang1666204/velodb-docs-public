---
{
  ...
  "description": "2つ以上のビットマップの積集合を計算する 使用法: intersectcount(bitmapcolumntocount, filtercolumn, filtervalues ...",
  "language": "ja"
}
---
## Description
...
例: intersect_count(user_id, event, 'A', 'B', 'C')、これはA/B/C 3つのビットマップすべてにおけるuser_idの積集合カウントを求めることを意味します
filter_values内でcolumn_to_filterと一致するbitmap_column内の要素の積集合カウント、すなわちビットマップ積集合カウントを計算します。
...
| `<column_to_filter>` | フィルタリングに使用するディメンションカラム。サポートされる型: TinyInt, SmallInt, Integer, BigInt, LargeInt。 |
| `<filter_values>`  | ディメンションカラムをフィルタリングするために使用される異なる値。サポートされる型: TinyInt, SmallInt, Integer, BigInt, LargeInt。 |


## 戻り値

指定されたビットマップの積集合の要素数を返します。
