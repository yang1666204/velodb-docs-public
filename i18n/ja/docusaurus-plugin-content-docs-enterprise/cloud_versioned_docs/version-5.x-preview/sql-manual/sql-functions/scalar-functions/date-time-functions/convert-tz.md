...
指定されたタイムゾーンfrom_tzから指定されたタイムゾーンto_tzへdatetime値を変換し、結果を返します。タイムゾーン設定については、Time Zone Managementドキュメントを参照してください。

この関数は、MySQLの[convert_tz function](https://dev.mysql.com/doc/refman/8.4/en/date-and-time-functions.html#function_convert_tz)と一致しています。
...
| `<to_tz>` | 変換先のターゲットタイムゾーンで、このパラメータは`varchar`型です |

## 戻り値

- datetime型の変換された値
- 戻り値のスケールは入力のスケールと同じです
  - スケールのないdatetime入力の場合、戻り値にもスケールはありません
  - スケールのある入力の場合、戻り値は同じスケールを持ちます

特別なケース:
- いずれかのパラメータがNULLの場合、NULLを返します。
- 入力タイムゾーンが無効な場合、エラーを返します。タイムゾーン設定については、Time Zone Managementを参照してください。
