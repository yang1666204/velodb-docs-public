...
- バージョン0.15以降、Apache DorisはHiveとIcebergの外部テーブルを導入し、データレイク向けのApache Icebergとの結合機能を探求しました。
...
Flink SQLを使用してPaimonテーブルのデータを更新することができます：
...
```
Flink SQLによって更新されたテーブルには4つのシャードが含まれており、すべてのシャードがNative Reader (paimonNativeReadSplits=4/4) を通じてアクセス可能であることが確認できます。さらに、最初のシャードのhasDeletionVectorプロパティがtrueになっており、これはそのシャードに対応するDeletion Vectorが存在することを示し、読み取り時にDeletion Vectorに従ってデータがフィルタリングされることを意味します。
