...
{
  "title": "WEEKS_SUB",
  "description": "WEEKSSUB関数は、指定された日付または時刻の値から指定された週数を減算（または加算）するために使用されます。",
  "language": "ja"
}
---
## 説明
WEEKS_SUB関数は、指定された日付や時刻の値から指定した週数を減算（または加算）し、調整された日付や時刻を返すために使用されます（基本的にweeks_value × 7日を減算します）。DATEおよびDATETIME型の処理をサポートしています。
...
|-----------|-------------|
| `<date_or_time_expr>` | 入力datetime値、date/datetimeタイプをサポートします。datetimeおよびdate形式については、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)と[date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion)を参照してください |
| `week_period` | INTタイプの整数で、減算する週数を表します（正の値で減算、負の値で加算）。 |

## Return Value
指定された週数を減算した日付または時刻を返します：

- 入力がDATEタイプの場合、戻り値はDATEタイプのままです（年、月、日のみ調整）。
- 入力がDATETIMEタイプの場合、戻り値はDATETIMEタイプのままです（年、月、日が調整され、時、分、秒は変更されません）。
