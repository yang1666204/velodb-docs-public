...
{
  "title": "WIDTH_BUCKET",
  "description": "等幅ヒストグラムを構築します。ここでは、ヒストグラムの範囲が等しいサイズの区間に分割され、計算後に、",
  "language": "ja"
}
---
## 説明

等幅ヒストグラムを構築します。ヒストグラムの範囲は等しいサイズの区間に分割され、計算後、式の値が属するバケット番号を返します。特殊なケース：

- この関数は整数値またはNULLを返します（いずれかの入力がNULLの場合、NULLを返します）
...
| Parameter | デスクリプション |
| -- | -- |
| `<expr>` | ヒストグラムを作成するための式。この式は数値または数値に暗黙的に変換可能な値として評価される必要があります。値の範囲は `-(2^53 - 1)` から `2^53 - 1`（両端を含む）である必要があります |
| `<min_value>` | 式の許容範囲の最小値ポイント。このパラメータは数値である必要があり、`<max_value>` と等しくてはいけません。範囲は `-(2^53 - 1)` から `2^53 - 1`（両端を含む）である必要があります。さらに、最大値ポイントと最小値ポイントの差は `2^53` 未満である必要があります（例：`abs(max_value - min_value) < 2^53)`） |
| `<max_value>` | 式の許容範囲の最大値ポイント。このパラメータは数値である必要があり、`<min_value>` と等しくてはいけません。範囲は `-(2^53 - 1)` から `2^53 - 1`（両端を含む）である必要があります。さらに、最大値ポイントと最小値ポイントの差は `2^53` 未満である必要があります（例：`abs(max_value - min_value) < 2^53)`） |
| `<num_buckets>` | バケットの数。正の整数である必要があります。この関数は式からの値を各バケットに割り当て、対応するバケット番号を返します |

## Return Value

式の値が該当するバケット番号を返します。式が範囲外の場合、関数は以下のルールに従って値を返します：

- 式の値が `min_value` 未満の場合、0を返します。
- 式の値が max_value 以上の場合、`num_buckets + 1` を返します。
- いずれかのパラメータが `null` の場case、`null` を返します
