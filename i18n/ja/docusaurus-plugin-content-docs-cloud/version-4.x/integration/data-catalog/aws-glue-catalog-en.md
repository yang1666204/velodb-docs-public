---
{
  ...
  "description": "AWS Glueは、AWSが提供する完全マネージド型のサーバーレスメタデータストレージサービスです。AWS Glue Catalogを作成することで、VeloDB CloudでAWS Glueによって管理されたメタデータを使用して、Amazon S3に保存されたデータレークテーブルを直接クエリできます。",
  "language": "ja"
}
---
AWS Glueは、AWSが提供する完全管理型のサーバーレスメタデータストレージサービスです。AWS Glue Catalogを作成することで、VeloDB CloudでAWS Glueによって管理されたメタデータを使用して、Amazon S3に保存されているデータレークテーブルを直接クエリできます。

VeloDB CloudはAWS Glue経由で以下の2つのテーブル形式へのアクセスをサポートしています：

*   **Iceberg**: ACIDトランザクション、Schema Evolution、Time Travelをサポートする現代的なオープンテーブル形式。
*   **Hive**: Hadoopエコシステムと幅広く互換性のある従来のHiveテーブル形式。
...
*   AWSアカウントを所有していること。
*   Amazon S3バケットを準備すること（IcebergまたはHiveテーブルデータの保存用）。
...
VeloDBを使用してAWS Glue Catalog経由でS3に保存されているIcebergまたはHiveテーブルにアクセスするには、以下のIAM権限ポリシーを設定する必要があります：
...
>
> *   `<bucket-name>`をIcebergまたはHiveテーブルデータが保存されている実際のS3バケット名に置き換えてください。
>
> *   より細かい権限制御のため、`*`を具体的なデータベース名とテーブル名に置き換えてください。

#### 2. Lake Formation権限設定

あなたのAWSアカウントでLake Formation権限制御が有効になっている場合、Glue Data Catalogにアクセスするには、IAMポリシーの設定だけでは不十分で、Lake Formationでも対応する権限を付与する必要があります。
...
4.  以下のオプションを設定します：
    ...
    *   **Catalog**: あなたの**Catalog**を選択します。
    *   **Databases**: 対象データベースまたは**All databases**を選択します。
    *   **Tables**: 対象テーブルまたは**All tables**を選択します。
...
| フィールド | 必須 | 説明 |
| :--- | :--- | :--- |
| **Catalog Name** | ✓ | Catalogの一意名で、SQLクエリでこのデータソースを識別するために使用されます。 |
| **Comment** | | オプションの説明情報。 |
...
まず、アクセスしたいテーブル形式を選択します。テーブル形式が異なれば、必要なパラメータも異なります。
...
> **注意**: Hive形式は現在クエリのみをサポートし、データベースやテーブルの作成はサポートしていません。
...
**Authentication**セクションで、AWS Glueメタデータサービスにアクセスするための認証情報を設定します。
...
| **Authentication** | ✓ | Access KeyまたはCross-account IAMを選択；設定方法はMetastore認証と同じです。 |
