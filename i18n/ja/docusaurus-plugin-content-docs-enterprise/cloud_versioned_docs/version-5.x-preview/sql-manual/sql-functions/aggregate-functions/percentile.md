...
小さなデータセットに適した正確なパーセンタイルを計算します。まず指定されたカラムを降順でソートし、その後正確なp番目のパーセンタイルを取得します。`p`の値は`0`と`1`の間です。`p`が正確な位置を指していない場合、位置`p`における隣接する値の[linear interpolation](https://en.wikipedia.org/wiki/Linear_interpolation)を返します。これは2つの数値の平均ではないことに注意してください。特別なケース：
