...
1. enabled: trueまたはfalseを指定でき、デフォルト値はtrueです。現在のポリシーが有効であることを示します。falseは現在のポリシーが無効であることを示します。
2. priority: 0から100までの正の整数で、デフォルト値は0です。これはポリシーの優先度を表します。値が高いほど、優先度が高くなります。このプロパティの主な役割は、複数のポリシーがマッチした場合に最も優先度の高いポリシーを選択することです。
