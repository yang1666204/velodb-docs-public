...
{
  "title": "YEARWEEK",
  "description": "YEARWEEK関数は、指定された日付の「年 + 週番号」の組み合わせを返すために使用されます（形式はYYYYWW、例：",
  "language": "ja"
}
---
## 説明

YEARWEEK関数は、指定された日付の「年 + 週番号」の組み合わせを返すために使用されます（形式はYYYYWW、例：202301は2023年の第1週を表します）。この関数は、オプションパラメータmodeを通じて、週の開始曜日と「第1週」を決定する基準を柔軟に定義し、デフォルトではmode=0となります。

週番号は、mode設定に応じて1-53の範囲となります。

パラメータmodeの効果を下記の表に示します：

|Mode |週の最初の曜日     |週番号の範囲        |第1週の定義                                             |
|:----|:-----------------|:-----------------|:-----------------------------------------------------|
|0    |日曜日             |1-53              |その年の最初の日曜日を含む週                             |
|1    |月曜日             |1-53              |その年に4日以上含む最初の週                              |
|2    |日曜日             |1-53              |その年の最初の日曜日を含む週                             |
|3    |月曜日             |1-53              |その年に4日以上含む最初の週                              |
|4    |日曜日             |1-53              |その年に4日以上含む最初の週                              |
|5    |月曜日             |1-53              |その年の最初の月曜日を含む週                             |
|6    |日曜日             |1-53              |その年に4日以上含む最初の週                              |
|7    |月曜日             |1-53              |その年の最初の月曜日を含む週                             |
...
YYYYWW形式のINT型整数を返します（最初の4桁が年、最後の2桁が週番号）。例えば、202305は2023年の第5週を表し、202052は2020年の第52週を表します。

- 日付を含む週が前年に属する場合、前年の年と週番号を返します（例：2021年1月1日は202052を返す場合があります）。
- 日付を含む週が翌年に属する場合、翌年の年と週番号1を返します（例：2024年12月30日は202501を返す場合があります）。
