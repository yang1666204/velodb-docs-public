...
>
> パーティション化されていないテーブルの場合、パーティション名はテーブル名と同じです。
...
- データ分散は、パーティションとバケットの2つの次元に沿って表示されます。
- Graph列は、文字`>`を使用してデータ分散比を視覚的に表現します。
- パーセンテージは小数点以下2桁まで正確です。
- パーティション化されていないテーブルの場合、クエリ結果のパーティション名はテーブル名と同じです。

## 例

- パーティション化されたテーブルを作成し、そのデータ分散を表示する：
