...
---
Dorisは、Kafkaからデータをロードする以下の方法を提供します：

- **Routine Loadを使用してKafkaデータを消費する**

DorisはRoutine Loadを通じてKafka Topicsからデータを継続的に消費します。Routine Loadジョブを投稿すると、Dorisはリアルタイムでロードタスクを生成し、Kafkaクラスター内の指定されたTopicからメッセージを消費します。Routine LoadはCSVとJSON形式をサポートし、Exactly-Onceセマンティクスにより、データの損失や重複を防ぎます。詳細なドキュメントについては、[Routine Load](../import-way/routine-load-manual.md)を参照してください。

- **Doris Kafka ConnectorでKafkaデータを消費する**

Doris Kafka Connectorは、KafkaデータストリームをDorisデータベースにロードするためのツールです。ユーザーはKafka Connectプラグインを通じて、様々なシリアル化形式（JSON、Avro、Protobufなど）を簡単にロードでき、Debeziumコンポーネントからのデータ形式の解析もサポートします。詳細なドキュメントについては、Doris Kafka Connectorを参照してください。

ほとんどの場合、外部コンポーネントを統合してKafkaデータを消費する必要はなく、直接Routine Loadを選択してデータをロードできます。AvroやProtobuf形式のデータ、またはDebezium経由で上流データベースから収集されたデータをロードする必要がある場合は、Doris Kafka Connectorを使用できます。

## Routine Loadを使用してKafkaデータを消費する

### 使用制限

1. サポートされるメッセージ形式はCSVとJSONです。各CSVメッセージは1行で、行の末尾に改行文字は含まれません。
2. デフォルトでは、Kafkaバージョン0.10.0.0以上をサポートします。古いバージョン（0.9.0、0.8.2、0.8.1、0.8.0など）を使用する必要がある場合は、BE設定を変更して`kafka_broker_version_fallback`を互換性のある古いバージョンに設定するか、Routine Load作成時に`property.broker.version.fallback`を設定する必要があります。古いバージョンを使用すると、時間に基づくKafkaパーティションオフセットの設定など、一部の新機能が使用できない場合があります。

### 操作例

Dorisでは、CREATE ROUTINE LOADコマンドを通じて永続的なRoutine Loadロードタスクを作成します。これは単一テーブルロードと複数テーブルロードに分けられます。詳細な構文については、CREATE ROUTINE LOADを参照してください。
...
複数のテーブルを同時にロードする必要があるシナリオでは、Kafka内のデータにテーブル名情報を含める必要があり、フォーマットは`table_name|data`となります。例えば、CSVデータをロードする場合、フォーマットは`table_name|val1,val2,val3`となります。テーブル名はDoris内のテーブル名と完全に一致する必要があることに注意してください。そうでなければロードに失敗し、後で紹介するcolumn_mapping設定はサポートされません。
...
DISTRIBUTED BY HASH(user_id) BUCKETS 10;
```
**ステップ3: 複数のテーブルにデータをロードするRoutine Loadジョブを作成する**
...
Doris Kafka Connectorは、KafkaデータストリームをDorisデータベースに読み込むためのツールです。ユーザーはKafka Connectプラグインを通じて様々なシリアライゼーション形式（JSON、Avro、Protobufなど）を簡単に読み込むことができ、Debeziumコンポーネントからのデータ形式の解析もサポートしています。

### 分散モードでの開始

[分散](https://docs.confluent.io/platform/current/connect/index.html#distributed-workers)モードは、Kafka Connectにスケーラビリティと自動障害耐性を提供します。このモードでは、同じ`group.id`を使用して複数のワーカープロセスを開始でき、利用可能なすべてのワーカープロセス間でコネクターとタスクの実行を調整します。
...
以下の構文を使用して、Dorisにロードするテーブルを作成します：
...
DebeziumによってMySQLテーブルから収集されたデータが`mysql_debezium.test.test_user` Topicにあると仮定します：
