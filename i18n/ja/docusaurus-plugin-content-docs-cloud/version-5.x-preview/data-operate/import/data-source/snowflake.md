...
移行前に、Snowflakeのテーブル構造に基づいて、Dorisの[データモデル](../../../table-design/data-model/overview.md)、[パーティショニング](../../../table-design/data-partitioning/dynamic-partitioning.md)、および[バケッティング](../../../table-design/data-partitioning/data-bucketing.md)戦略を選択してください。テーブル作成戦略の詳細については、[Load Best Practices](../load-best-practices.md)を参照してください。
...
```
この構造に基づいて、Snowflakeのclustering keyに対応したDoris Primary Keyパーティションテーブルを作成し、日単位でパーティション分割してください：
   ...
   LoadFinishTime: 2025-04-15 17:33:02
    ...
    TransactionId: 769213
     ErrorTablets: {}
          ...
          Comment: 
   ```
上記の例で示されているように、この問題は**データ品質エラー**(ETL_QUALITY_UNSATISFIED)です。詳細なエラーを確認するには、結果に提供されているURLにアクセスする必要があります。例えば、データがテーブルスキーマのcountryカラムで定義された長さを超過した場合などです：
   ...
   ```
データ品質エラーについて、エラーのあるレコードをスキップすることを許可したい場合は、S3 Loadタスクのプロパティセクションでフォルトトレラント率を設定できます。詳細については、[Import Configuration Parameters](../../import/import-way/broker-load-manual.md#related-configurations)を参照してください。
