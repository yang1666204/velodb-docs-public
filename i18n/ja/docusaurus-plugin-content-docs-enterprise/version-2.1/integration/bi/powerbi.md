...
最終的に、ナビゲータービューでデータベースとテーブルが表示されるはずです。希望するテーブルを選択し、「Load」をクリックして、テーブル構造をロードし、Apache Doris からデータをプレビューしてください。
...
1. まず、テーブルモデルの関係を作成してください。Model ビューをクリックしてください。

![](/images/ecomsystem/powerbi/V7PsbP3oKoJpLjxK5HdcPsnLnKf.png)

2. 必要に応じてこれらの4つのテーブルを同じ画面にドラッグアンドドロップで配置し、関連するフィールドをドラッグアンドドロップしてください。
