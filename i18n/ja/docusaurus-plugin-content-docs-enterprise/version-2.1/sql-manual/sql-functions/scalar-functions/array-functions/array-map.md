...
{
  "title": "ARRAY_MAP",
  "description": "他の入力ARRAY パラメータの内部データに対応する式を計算するために、ラムダ式を入力パラメータとして使用します。",
  "language": "ja"
}
---
## description
...
| パラメータ | 説明 |
| --- |---|
| `lambda` | 1つ以上の入力パラメータを持つlambda式。入力配列の数と一致する必要があります。lambdaは有効なスカラー関数を実行できますが、集約関数はサポートしていません。 |
| `<arr>` | ARRAY配列 |

## 戻り値

lambda式を通して処理されたARRAY。
