...
戻り値の意味：
- 入力配列と同じ長さの新しい配列を返します。各位置には、対応する要素にlambda式を適用した結果が含まれます

使用上の注意：
- lambda式のパラメータ数は配列パラメータの数と一致する必要があります
- 入力配列パラメータにNULLはサポートされていません
- 複数の配列パラメータがある場合、すべての配列は同じ長さである必要があります
- Lambdaは任意のスカラ式を使用できますが、集約関数は使用できません
