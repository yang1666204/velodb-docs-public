...
DAY関数は日付や時刻の式から「日」の部分を抽出し、1から31（月と年に応じて）の範囲の整数値を返すために使用されます。

この関数はMySQLの[day function](https://dev.mysql.com/doc/refman/8.4/en/date-and-time-functions.html#function_day)と一貫した動作をします。
