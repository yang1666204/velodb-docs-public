...
{
  "title": "Snowflake",
  "description": "Snowflake から Doris への移行中、オブジェクトストレージは通常、中間媒体として使用されます。コアプロセスは以下の通りです：まず、",
  "language": "ja"
}
---
SnowflakeからDorisへの移行では、通常オブジェクトストレージが中間メディアとして使用されます。コアプロセスは以下の通りです：まず、Snowflakeの[COPY INTO](https://docs.snowflake.com/en/user-guide/data-unload-overview)文を使用してオブジェクトストレージにデータをエクスポートします。次に、DorisのS3 Load機能を使用してオブジェクトストレージからデータを読み取り、Dorisにロードします。詳細については、[S3 Load](./amazon-s3.md)を参照してください。


## 考慮事項

移行前に、SnowflakeのTable構造に基づいて、Dorisの[データモデル](../../../table-design/data-model/overview.md)、[パーティショニング](../../../table-design/data-partitioning/dynamic-partitioning.md)、および[バケッティング](../../../table-design/data-partitioning/data-bucketing.md)戦略を選択してください。Table作成戦略の詳細については、[Load Best Practices](../load-best-practices.md)を参照してください。
...
SnowflakeTableをDorisに移行するには、まずDorisTableを作成します。

Snowflakeに以下のTableとデータがあると仮定します：
...
```
この構造に基づいて、Snowflakeのclustering keyに対応したDoris Primary KeyパーティションTableを作成し、日単位でパーティション分割してください：
...
2.1. **COPY INTOによるS3 Parquetファイルへのエクスポート**
   ...
   Snowflakeは[AWS S3](https://docs.snowflake.com/en/user-guide/data-unload-s3)、[GCS](https://docs.snowflake.com/en/user-guide/data-unload-gcs)、[AZURE](https://docs.snowflake.com/en/user-guide/data-unload-azure)へのエクスポートをサポートしています。**Dorisのパーティションフィールドでパーティション分割されたデータをエクスポートします**。AWS S3の例：
...
S3 Loadは非同期データロード方式です。実行後、Dorisはデータソースから能動的にデータを取得します。データソースはS3プロトコルと互換性のあるオブジェクトストレージをサポートしており、([AWS S3](./amazon-s3.md)、[GCS](./google-cloud-storage.md)、[AZURE](./azure-storage.md)など)が含まれます。

この方式は、バックグラウンドでの非同期処理が必要な大量データのシナリオに適しています。同期処理が必要なデータインポートについては、[TVF Load](./amazon-s3.md#load-with-tvf)を参照してください。
   ...
   LoadFinishTime: 2025-04-15 17:33:02
    ...
    TransactionId: 769213
     ErrorTablets: {}
          ...
          Comment: 
   ```
上記の例で示されているように、この問題は**データ品質エラー**(ETL_QUALITY_UNSATISFIED)です。詳細なエラーを確認するには、結果に提供されているURLにアクセスする必要があります。例えば、データがTableスキーマのcountryカラムで定義された長さを超過した場合などです：
   ...
   ```
データ品質エラーについて、エラーのあるレコードをスキップすることを許可したい場合は、S3 Loadタスクのプロパティセクションでフォルトトレラント率を設定できます。詳細については、[Import 構成 パラメータ](../../import/import-way/broker-load-manual.md#related-configurations)を参照してください。

3.4. **複数パーティションのデータロード**

   大量の履歴データを移行する際は、バッチロード戦略を使用することを推奨します。各バッチはDoris内の1つまたは少数のパーティションに対応します。システム負荷を軽減し、ロード失敗時の再試行コストを下げるため、バッチあたりのデータサイズを100GB未満に保つことを推奨します。

   スクリプト[s3_load_demo.sh](https://github.com/apache/doris/blob/master/samples/load/shell/s3_load_demo.sh)を参照してください。このスクリプトはS3上のパーティションディレクトリをポーリングし、DorisにS3 Loadタスクを送信してバッチロードを実現できます。
