...
以下では、テーブルt1とt2、および関連するSQLを使用して、subqueryの基本的な特徴と使用法を紹介します。テーブル作成文は以下の通りです：
...
メインクエリ（外部クエリとも呼ばれる）から1つ以上の列を参照するサブクエリです（参照される外部列は、多くの場合サブクエリのWHERE条件にあります）。相関サブクエリは、外部に関連付けられたテーブルに対するフィルタリング操作として見ることができます。外部テーブルの各データ行に対して、サブクエリが計算され、対応する結果を返します。
...
`right semi join`は似ていますが、左右のテーブルが異なります。ここでは、`left semi join`を例として使用します。
...
```
`Mark Join`と通常の`left semi join`の違いは、通常の`left semi join`が条件を満たす左テーブルの行を直接出力するのに対し、`Mark Join`は元の左テーブルに追加のフラグ列（例では`mark_join_flag`）を付けて出力することです。このフラグ列は`true`、`false`、または`null`の値を取ることができます。フラグの値は`join`条件式`t1.c2 = t2.c2`によって決定され、各行がフラグ値に対応します。フラグ値の計算は以下のテーブルに示されています：
