...
| Range                     | パーティションの範囲                                                         |
| DistributionKey           | パーティション分散のためのディストリビューションキー                                             |
...
| LastConsistencyCheckTime  | パーティションの最後の整合性チェック時刻                          |
| DataSize                  | パーティション内のデータサイズ                                             |
| IsInMemory                | パーティションがメモリに格納されているかどうか                                    |
| ReplicaAllocation         | パーティションのレプリケーション戦略                                        |
| IsMutable                 | パーティションが変更可能かどうか                                             |
| SyncWithBaseTables        | パーティションがベーステーブルと同期されているかどうか（非同期マテリアライズドビュー用） |
| UnsyncTables              | 非同期のベーステーブル（非同期マテリアライズドビューパーティション用）   |



## 例
internalカタログのtestの下にあるexample_tableのパーティションリストを表示
...
+-------------+---------------+----------------+---------------------+--------+--------------+--------------------------------------------------------------------------------+-----------------+---------+----------------+---------------+---------------------+---------------------+--------------------------+----------+------------+-------------------------+-----------+--------------------+--------------+
```
example_table配下のパーティション名p1のパーティション情報を表示します
...
+-------------+---------------+----------------+---------------------+--------+--------------+--------------------------------------------------------------------------------+-----------------+---------+----------------+---------------+---------------------+---------------------+--------------------------+----------+------------+-------------------------+-----------+--------------------+--------------+
```
example_table配下のパーティション名p1のパーティションIDを表示する
