...
---
BigQuery CatalogはBigQuery ConnectorによってBigQueryテーブルにアクセスするため、[Trino Connector](https://doris.apache.org/zh-CN/community/how-to-contribute/trino-connector-developer-guide/)互換性フレームワークを使用します。
...
| シナリオ       | 説明                          |
| -------------- | ------------------------------------ |
| Data Integration | BigQueryデータを読み取り、Doris内部テーブルに書き込みます。 |
| Data Writeback  | サポートされていません。                     |
...
また、事前にコンパイルされた [trino-bigquery-435-20240724.tar.gz](https://github.com/apache/doris-thirdparty/releases/download/trino-435-20240724/trino-bigquery-435-20240724.tar.gz) を直接ダウンロードして展開することもできます。

### BigQuery Connectorのデプロイ

すべてのFEおよびBEノードのデプロイメントパスの `connectors/` ディレクトリに `trino-bigquery-435/` ディレクトリを配置してください。（ディレクトリが存在しない場合は、手動で作成できます。）
...
* `{TrinoProperties}`
  ...
  TrinoPropertiesセクションは、Trino Connectorに渡されるプロパティを指定するために使用されます。これらのプロパティは`trino.`プレフィックスを使用します。理論的には、Trinoでサポートされているすべてのプロパティがここでもサポートされます。BigQueryの詳細については、[Trino documentation](https://trino.io/docs/435/connector/bigquery.html)を参照してください。
