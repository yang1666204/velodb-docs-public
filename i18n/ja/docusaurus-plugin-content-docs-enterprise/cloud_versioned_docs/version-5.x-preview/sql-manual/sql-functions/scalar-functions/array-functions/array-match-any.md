---
{
  ...
  "description": "配列内の任意の要素が指定された条件に一致する場合はtrueを返し、そうでなければfalseを返します。",
  "language": "ja"
}
---
## Description

配列内のいずれかの要素が指定された条件に一致する場合はtrueを返し、そうでなければfalseを返します。配列にNULL要素が含まれ、すべての非NULL要素がfalseの場合、NULLを返します。
...
- `NULL` 配列にNULL要素が含まれ、すべての非NULL要素が条件に一致しない場合
...
2. この関数は以下の用途に有用です：
   - 配列内のいずれかの要素が特定の条件を満たすかどうかを確認する
