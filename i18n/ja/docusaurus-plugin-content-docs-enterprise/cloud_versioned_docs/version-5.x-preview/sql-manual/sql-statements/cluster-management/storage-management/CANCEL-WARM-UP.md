...
{
  "title": "WARM UPをキャンセル",
  "description": "Dorisで指定されたウォームアップジョブを終了するためのものです。",
  "language": "ja"
}
---
## Description

Dorisにおいて指定されたウォームアップジョブを終了するためのものです。
...
```
以下の内容が返される場合、指定されたidに対応するwarm-upジョブが存在しないことを示します：

```C++
ERROR 1105 (HY000): errCode = 2, detailMessage = job id: 110 does not exist.
```
正常に返された後、`SHOW WARM UP JOB`を再度使用して、ジョブステータスがRUNNINGからCANCELLEDに変更されたことを確認できます：
