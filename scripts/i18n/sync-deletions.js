...
const SOURCE_MAP = {
  cloud_versioned_docs: {
    pluginDir: 'docusaurus-plugin-content-docs-cloud',
    sidebarDir: 'cloud_versioned_sidebars',
  },
  enterprise_versioned_docs: {
    pluginDir: 'docusaurus-plugin-content-docs-enterprise',
    sidebarDir: 'enterprise_versioned_sidebars',
  },
};
...
function parseDeletedSourcePath(filePath) {
  ...
  for (const [sourceRoot, conf] of Object.entries(SOURCE_MAP)) {
    ...
    const sidebarPath = path.join(conf.sidebarDir, `${version}-sidebars.json`);

    return { sourceRoot, version, docId, jaPath, sidebarPath };
  }
