...
---
:::warning 注意！
この機能は3.1.0から非推奨です。使用に関するお問い合わせは、開発者までご連絡ください。
:::

DorisはPostgreSQLに保存されたメタデータを使用してLakeSoulテーブルデータへのアクセスと読み取りをサポートしています。
...
| シナリオ   | 説明                                               |
| ---------- | --------------------------------------------------------- |
| データ統合 | LakeSoulデータを読み取ってDoris内部テーブルに書き込む、またはDorisコンピューティングエンジンを使用してZeroETL操作を実行する。 |
| データ書き戻し | サポートされていません。                                       |
...
* `[CommonProperties]`
  ...
  CommonPropertiesセクションは、一般的なプロパティを記入するためのものです。「Common Properties」セクションの[Data Catalog Overview](../catalog-overview.md)を参照してください。

LakeSoulデータがHDFS上に保存されている場合は、FEとBEの両方の`conf/`ディレクトリに`core-site.xml`、`hdfs-site.xml`、`hive-site.xml`を配置してください。`conf/`ディレクトリ内のHadoop設定ファイルが最初に読み込まれ、次に`HADOOP_CONF_DIR`環境変数で指定された設定ファイルが読み込まれます。
...
- LakeSoul主キーテーブルおよび非主キーテーブルをサポートします。
