...
> テーブルのキー列。Dorisでは、キー列はテーブルの最初のK列である必要があります。キーの制限とキー列の選択方法の詳細については、「データモデル」の章の該当セクションを参照してください。
...
>
> パーティションフィールドが指定されている場合、フィールドの由来となるベーステーブルが自動的に推論され、ベーステーブルと同期されます（現在、内部テーブルとHiveテーブルをサポート）。内部テーブルの場合、1つのパーティションフィールドのみが許可されます。
>
...
内部テーブルで使用されるプロパティで、その大部分はマテリアライズドビューでも使用でき、マテリアライズドビュー固有のプロパティもあります。以下にリストします：

| プロパティ名                      | 説明                                                         |
| -------------------------------- | ------------------------------------------------------------ |
| grace_period                     | クエリリライト時にマテリアライズドビューデータに許可される最大遅延秒数。マテリアライズドビューのパーティションAとベーステーブルデータが一致せず、パーティションAの最後のリフレッシュ時刻が10:15:00で、現在のシステム時刻が10:15:08の場合、パーティションは透過的にリライトされません。ただし、`grace_period`が10に設定されている場合、パーティションは透過的リライトに使用されます。 |
...
| partition_sync_limit             | ベーステーブルのパーティションフィールドが時刻型の場合、このプロパティを`partition_sync_time_unit`と組み合わせて使用し、ベーステーブルと同期するパーティションの範囲を設定できます。例えば、2に設定し、`partition_sync_time_unit`を`MONTH`に設定すると、ベーステーブルの過去2ヶ月分のパーティションとデータのみが同期されます。最小値は`1`です。時間の経過とともに、マテリアライズドビューは各リフレッシュ時に自動的にパーティションを追加・削除します。例えば、マテリアライズドビューが現在2月と3月のデータを持っている場合、来月は自動的に2月のデータを削除し、4月のデータを追加します。 |
| partition_sync_time_unit         | パーティションリフレッシュの時間単位で、DAY/MONTH/YEAR（デフォルトはDAY）をサポート。 |
| partition_date_format            | ベーステーブルのパーティションフィールドが文字列型の場合、`partition_sync_limit`機能を使用したい場合は、日付フォーマットを設定して`partition_date_format`の設定に従ってパーティション時刻を解析できます。 |
...
- パーティション化されたマテリアライズドビューの作成条件：
  ...
  > 2. マテリアライズドビューで使用されるパーティション化されたベーステーブルは、ListまたはRangeパーティション戦略を使用する必要があります。
  > 3. マテリアライズドビュー定義SQLは、Partition By句で1つのパーティションフィールドのみを持つことができます。
  ...
  > 7. データ変更はパーティションテーブルで発生する必要があります。非パーティションテーブルで発生する場合、マテリアライズドビューはフルビルドが必要です。
...
以下に示すように、パーティションカラムが指定された場合、システムは自動的にそのカラムがどのベーステーブルから来ているかを識別し、ベーステーブルのパーティションを同期します。ベーステーブルは日単位でパーティション化されています（パーティションカラム: o_orderdate、パーティションタイプ: RANGE）。マテリアライズドビューは月単位でパーティション化され、DATE_TRUNC関数を使用してベーステーブルのパーティションを月単位でロールアップします。
