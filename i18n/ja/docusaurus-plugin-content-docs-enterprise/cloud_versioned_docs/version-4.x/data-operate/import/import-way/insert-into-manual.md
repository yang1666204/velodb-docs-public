---
{
  ...
  "description": "INSERT INTO文は、Dorisクエリの結果を別のテーブルにインポートすることをサポートしています。INSERT INTOは同期インポート方式であり、",
  "language": "ja"
}
---
INSERT INTO文は、Dorisクエリの結果を別のテーブルにインポートすることをサポートしています。INSERT INTOは同期的なインポート方法で、インポートが実行された後にインポート結果が返されます。返された結果に基づいて、インポートが成功したかどうかを判断できます。INSERT INTOはインポートタスクの原子性を保証します。つまり、すべてのデータが正常にインポートされるか、どのデータもインポートされないかのいずれかです。

## 適用シナリオ

2. ユーザーがDorisテーブル内の既存データに対してETLを実行し、その後新しいDorisテーブルにインポートしたい場合、INSERT INTO SELECT構文が適用可能です。
3. Multi-Catalog外部テーブルメカニズムと組み合わせて、MySQLやHiveシステムからのテーブルをMulti-Catalog経由でマッピングできます。その後、INSERT INTO SELECT構文を使用して外部テーブルからDorisテーブルにデータをインポートできます。
4. Table Value Functions (TVF)を利用して、ユーザーはオブジェクトストレージやHDFS上のファイルに格納されたデータを、自動的な列型推論機能付きのテーブルとして直接クエリできます。その後、INSERT INTO SELECT構文を使用して外部テーブルからDorisテーブルにデータをインポートできます。

## 実装

INSERT INTOを使用する場合、インポートジョブはMySQLプロトコルを使用してFEノードに開始され、送信される必要があります。FEは実行プランを生成し、これにはクエリ関連のオペレーターが含まれ、最後のオペレーターがOlapTableSinkです。OlapTableSinkオペレーターは、クエリ結果をターゲットテーブルに書き込む役割を担います。実行プランはその後、実行のためにBEノードに送信されます。DorisはCoordinatorとして1つのBEノードを指定し、このノードがデータを受信して他のBEノードに配布します。
...
VALUES (1, "Emily", 25),
       ...
       (5, "Ava", 17);
```
3. 上記の操作に基づいて、ターゲットテーブルとして新しいテーブルを作成します（ソーステーブルと同じスキーマを使用）。
...
```
I'm ready to translate the English technical documentation text into Japanese following your requirements. However, I don't see the actual text to translate after "Example:" - it appears to be cut off. Could you please provide the complete text you'd like me to translate?
...
以下では、Hive外部テーブルからDoris内部テーブルへのデータインポートの例を示します。
...
Dorisは、Table Value Functions (TVFs)を通じて、オブジェクトストレージやHDFSに保存されたファイルを直接テーブルとしてクエリおよび解析できます。これは自動カラムタイプ推論をサポートします。詳細については、Lakehouse/TVFドキュメントを参照してください。
...
`INSERT INTO SELECT`構文と組み合わせて使用することで、ファイルをDorisテーブルに迅速にインポートし、より高速な分析を実現できます。
