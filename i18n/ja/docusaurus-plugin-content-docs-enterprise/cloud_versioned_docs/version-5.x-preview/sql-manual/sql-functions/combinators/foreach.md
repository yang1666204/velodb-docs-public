...
{
  "title": "FOREACH",
  "description": "テーブル用の集約関数を、対応する配列アイテムを集約し、配列を返す配列用の集約関数に変換します",
  "language": "ja"
}
---
## Description

テーブル用の集計関数を、対応する配列項目を集計して結果の配列を返す配列用の集計関数に変換します。例えば、配列[1, 2]、[3, 4, 5]、[6, 7]に対するsum_foreachは、対応する配列項目を加算した後、結果[10, 13, 5]を返します。
