...
{
  "title": "ファイル作成",
  "description": "この文は Doris クラスタにファイルを作成してアップロードするために使用されます。",
  "language": "ja"
}
---
## 説明
...
この機能は通常、証明書、公開鍵、秘密鍵など、他のコマンドで使用する必要があるファイルを管理するために使用されます。
...
> - **catalog**: 必須。ファイル分類用のカテゴリ名（ユーザー定義）。特定のコマンドでファイルを特定する際に使用されます（例：スケジュールインポートでKafkaがデータソースの場合、'kafka'カタログ下のファイルを検索）。
