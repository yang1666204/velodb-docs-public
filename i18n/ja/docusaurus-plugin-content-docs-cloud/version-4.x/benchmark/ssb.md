...
以下のスクリプトを実行して、[ssb-tools](https://github.com/apache/doris/tree/master/tools/ssb-tools) ツールをダウンロードしコンパイルします。

```shell
sh bin/build-ssb-dbgen.sh
```
インストールが正常に完了すると、`dbgen`バイナリが`ssb-dbgen/`ディレクトリ下に生成されます。
...
```
> Note 1: `sh gen-ssb-data.sh -h`でスクリプトのヘルプを確認してください。
>
> Note 2: データは`ssb-data/`ディレクトリの下に`.tbl`サフィックス付きで生成されます。総ファイルサイズは約600GBで、生成には数分から1時間程度かかる場合があります。
>
