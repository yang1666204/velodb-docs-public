...
戻り値の意味:
- 対応するIPv4アドレスのネットワークバイトオーダー（ビッグエンディアン）表現でのアドレスの数値を表す整数を返す
- 入力が NULLの場合、`NULL`を返す
- 無効なIPv4文字列の場合、`NULL`を返す

### 使用上の注意
- `ipv4_string_to_num_or_null`と一貫した動作をする：無効な入力に対して`NULL`を返す
- MySQL の `INET_ATON`構文との互換性のために一般的に使用される

## 例

IPv4テキスト`192.168.0.1`を対応するネットワークバイトオーダー（ビッグエンディアン）でのアドレスの数値の整数に変換する。
