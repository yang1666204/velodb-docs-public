---
{
  ...
  "description": "この文は、カスタム関数を作成するために使用されます。",
  "language": "ja"
}
---
## 説明
...
> `function_name`が`db1.my_func`のようなデータベース名を含む場合、カスタム関数は対応するデータベースに作成されます。そうでなければ、関数は現在のセッションのデータベースに作成されます。新しい関数の名前とパラメータは、現在の名前空間の既存の関数と同一であってはいけません。そうでなければ、作成は失敗します。

**2. `<arg_type>`**

> 関数の入力パラメータタイプです。可変長パラメータの場合は、`, ...`を使用してそれらを示します。可変長タイプの場合、可変長パラメータのタイプは最後の非可変長パラメータのタイプと一致している必要があります。

**3. `<ret_type>`**

> 関数の戻りパラメータタイプです。これは新しい関数を作成するために必須のパラメータです。既存の関数のエイリアスを作成する場合、このパラメータは必要ありません。

## オプションパラメータ

**1. `GLOBAL`**

> 指定した場合、作成される関数はグローバルに有効になります。

**2. `AGGREGATE`**

> 指定した場合、作成される関数は集約関数になります。
...
> 指定した場合、作成される関数はエイリアス関数になります。

> 関数タイプを表す上記のパラメータのいずれも選択されていない場合、作成される関数がスカラー関数であることを示します。

**5. `<inter_type>`**

> 集約関数の中間段階でのデータタイプを示すために使用されます。
...
> - `always_nullable`: UDF結果がNULL値を含む可能性があるかどうかを示します。これはオプションパラメータで、デフォルト値はtrueです。
