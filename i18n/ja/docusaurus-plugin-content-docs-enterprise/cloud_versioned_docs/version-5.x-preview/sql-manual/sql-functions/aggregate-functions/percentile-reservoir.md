---
{
  ...
  "description": "この関数は、最大8192のリザーバーサイズとサンプリング用の乱数生成器を使用してリザーバーサンプリングを適用します。",
  "language": "ja"
}
---
## 説明

この関数は、最大8192のリザーバサイズとサンプリング用の乱数ジェネレータを使用して[reservoir sampling](https://en.wikipedia.org/wiki/Reservoir_sampling)を適用します。これは位置`p`での近似パーセンタイルを計算するために使用されます。
`p`の値は`0`と`1`の間です。
...
| -- | -- |
| `<col>` | 近似パーセンタイルとして計算する列。サポートされる型：Double |
| `<p>` | 計算する近似パーセンタイル、定数値。サポートされる型：値の範囲が `[0.0, 1.0]` のDouble |

## Return Value

指定された列の近似パーセンタイルを返します。戻り値の型は `DOUBLE` です。
- 入力列が `NULL` の場合は `NULL` を返します
