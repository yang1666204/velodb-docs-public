---
{
  ...
  "description": "データベース下のすべてのカスタムキーを表示します。ユーザーがデータベースを指定した場合、対応するデータベースを確認し、",
  "language": "ja"
}
---
## 説明

データベース配下のすべてのカスタムキーを表示します。ユーザーがデータベースを指定した場合は、対応するデータベースを確認します。そうでなければ、現在のセッションが配置されているデータベースを直接クエリします。
...
| 権限         | オブジェクト    | 備考                                                                          |
|:-------------|:------------|:------------------------------------------------------------------------------|
| `ADMIN_PRIV` | User / Role | 暗号化キーを表示するには、対象のユーザー/ロールに対して `ADMIN_PRIV` 権限を保持する必要があります |

## 使用上の注意

データベースが明示的に指定された場合、システムは指定されたデータベース内の暗号化キーを照会します。そうでない場合は、現在のセッションのデータベース内のキーを自動的に照会します。
