...
* **Polarisのデプロイと設定**：サーバー上でPolarisサービスをダウンロードして起動する方法、およびPolarisでIceberg Catalog、Namespace、対応するPrincipal/Role/権限を作成してDorisに安全なメタデータアクセスエンドポイントを提供する方法。
...
* `--storage-type s3`: 基盤となるストレージをS3として指定します。
    * `--default-base-location`: Icebergテーブルデータのデフォルトルートパスです。
...
Icebergテーブルの管理にDorisを使用する詳細については、以下をご覧ください：
