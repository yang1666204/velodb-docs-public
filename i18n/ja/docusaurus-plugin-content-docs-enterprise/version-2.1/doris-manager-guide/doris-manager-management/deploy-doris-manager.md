...
2.  **サービスコンポーネント配備情報の設定**
    ...
    | Proxy 構成     | 本番環境が外部ネットワークから分離されている場合、パブリックオフィス通信ソフトウェアに通知を送信するためのプロキシを設定できます。                                                                                                                             |
    | Installation Package 構成 | Doris Core と Manager のインストールパッケージのローカルストレージパスを設定します。新しいクラスターの作成と既存クラスターのアップグレードに使用されます。 |
