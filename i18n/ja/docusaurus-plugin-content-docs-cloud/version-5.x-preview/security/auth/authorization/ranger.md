...
#### Table権限
> ここで、「table」という用語は一般的にテーブル、ビュー、および非同期マテリアライズドビューを指します。
...
2. Row Level Filterポリシーが設定されているが、ユーザーがクエリ実行時にpermission deniedエラーが発生する。
   ...
   Row Level Filterポリシーは、テーブルのデータ内の特定のレコードへのユーザーアクセスを制限するためにのみ使用されます。ユーザーに対する認可は、依然としてACCESS POLICYを通じて付与される必要があります。
