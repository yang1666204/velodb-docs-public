---
{
  "title": "転置インデックス | Index",
  "description": "Inverted Indexは、情報検索の分野で一般的に使用されるインデックス技術です。",
  "language": "ja"
}
---
# Inverted Index
...
[Inverted Index](https://en.wikipedia.org/wiki/Inverted_index)は情報検索の分野で一般的に使用されるインデックス技術です。テキストを個別の単語に分割し、単語 -> ドキュメントIDのインデックスを構築することで、特定の単語を含むドキュメントを素早く検索できるようにします。
...
- キーワード検索をサポート、複数キーワード同時マッチング`MATCH_ALL`および任意のキーワードマッチング`MATCH_ANY`を含む
...
**2. 通常の等価・範囲クエリを高速化し、BITMAPインデックスの機能をカバー・置換**

- 文字列、数値、datetime型での =, !=, >, >=, <, <= の高速フィルタリングをサポート

- 文字列、数値、datetimeの配列型での`array_contains`の高速フィルタリングをサポート
...
1. 精度の問題がある浮動小数点型FLOATおよびDOUBLEは、精度が不正確なためinverted indexをサポートしていません。解決策は精度が正確なDECIMAL型を使用することで、これはinverted indexをサポートしています。
...
<details>
  ...
  <p>- `english`: 英語のトークン化、英語テキストを含むカラムに適しており、スペースと句読点でトークン化し、高いパフォーマンスを持ちます</p>
  <p>- `chinese`: 中国語のトークン化、主に中国語テキストを含むカラムに適しており、英語のトークン化よりもパフォーマンスが低いです</p>
  <p>- `unicode`: Unicodeトークン化、中国語と英語の混在、および多言語混在テキストに適しています。メールのプレフィックスとサフィックス、IPアドレス、文字と数字の混在文字列をトークン化でき、中国語を文字単位でトークン化できます。</p>

  トークン化の結果は `TOKENIZE` SQL関数を使用して検証できます。詳細は以下のセクションを参照してください。
</details>
...
<details>
  ...
  **インデックスが MATCH_PHRASE フレーズクエリの高速化をサポートするかどうかを指定します**
  <p>- true: サポートされますが、インデックスにはより多くのストレージ領域が必要です</p>
  <p>- false: サポートされません。よりストレージ効率が良く、複数のキーワードをクエリするために MATCH_ALL を使用できます</p>
  <p>- バージョン 2.0.14、2.1.5、3.0.1 以降では、parser が設定されている場合のデフォルトは true です。それ以外の場合は false がデフォルトです。</p>
...
<details>
  ...
  **トークン化前にテキストの前処理を指定します。通常、トークン化の動作に影響を与えるために使用されます**

  <p>char_filter_type: 異なる機能のchar_filterを指定します（現在はchar_replaceのみサポート）</p>

  <p>char_replaceはpattern内の各文字をreplacement内の文字で置換します</p>
  <p>- char_filter_pattern: 置換される文字</p>
  <p>- char_filter_replacement: 置換文字配列、オプション、デフォルトはスペース文字</p>

  例えば、以下の例では、ドットとアンダースコアをスペースに置換することで、それらを単語区切り文字として扱い、トークン化の動作に影響を与えます。
...
<details>
  ...
  **非トークン化文字列インデックスの長さ制限を指定（パーサーが指定されていない場合）**
...
`CREATE / ADD INDEX`操作はインデックス定義の追加のみを行います。この操作の後に書き込まれる新しいデータは転置インデックスを生成しますが、既存のデータについては`BUILD INDEX`を使用してインデックス作成をトリガーする必要があります：
...
バージョン2.0.12および2.1.4より前では、`BUILD INDEX`は成功するまで再試行を続けていました。これらのバージョンからは、失敗とタイムアウトメカニズムにより無限の再試行を防いでいます。3.0（Cloud Mode）は現時点でこのコマンドをサポートしていません。
...
`DROP INDEX`はインデックス定義を削除するため、新しいデータはインデックスに書き込まれなくなります。これにより、各BE上の複数のスレッドによって実行されるインデックス削除を実行する非同期タスクが作成されます。スレッド数はBEパラメータ`alter_index_worker_count`を使用して設定でき、デフォルト値は3です。
...
転置クエリの高速化は、セッション変数 `enable_inverted_index_query` を使用して切り替えることができ、これはデフォルトで true に設定されています。インデックスの高速化効果を確認するために、false に設定してオフにすることができます。

転置インデックスの高速化効果は、Query Profile の以下のメトリクスを使用して分析することができます：
- RowsInvertedIndexFiltered: 転置インデックスによってフィルタリングされた行数で、他の Rows 値と比較してインデックスのフィルタリング効果を分析できます。
...
トークン化の実際の効果を確認するため、または一部のテキストをトークン化するために、検証に `TOKENIZE` 関数を使用することができます。
...
-- 日時型の場合、USING INVERTEDはパーサーの指定を必要としません
  -- CREATE INDEXはインデックスを作成するための一つの構文です。別の方法は後で説明します
...
SELECT count() FROM hackernews_1m WHERE parent = 11189;
  ...
  -- 数値型の場合、USING INVERTEDはパーサーの指定を必要としません
  -- ALTER TABLE t ADD INDEXはインデックスを作成する2つ目の構文です
  ALTER TABLE hackernews_1m ADD INDEX idx_parent(parent) USING INVERTED;


  -- 既存データに対してinverted indexを作成するためにBUILD INDEXを実行します
...
SELECT count() FROM hackernews_1m WHERE author = 'faster';
  ...
  -- ここでは、`author`列をトークン化せず、単一の用語として扱い、USING INVERTEDを使用している
  ALTER TABLE hackernews_1m ADD INDEX idx_author(author) USING INVERTED;

  
  -- BUILD INDEXを実行して、既存のデータに対してinverted indexを追加する
  BUILD INDEX idx_author ON hackernews_1m;

  
100万件のauthorレコードに対するincremental indexの作成は、わずか1.5秒で完了した。
