...
{
  "title": "MAP_AGG",
  "description": "MAPAGG関数は、複数行のデータからキーと値のペアに基づいてマッピング構造を形成するために使用されます。",
  "language": "ja"
}
---
## 概要

MAP_AGG関数は、複数行のデータからキー・バリューペアに基づいてマッピング構造を形成するために使用されます。
...
Map型の値を返します。グループ内に有効なデータが存在しない場合、空のMapを返します。
...
+--------------------------------------+
```
`n_regionkey`、map_agg(`n_name`, `n_nationkey` % 5) を `nation` から `n_regionkey` でグループ化して選択する;
