...
{
  "title": "Hive Catalog",
  "description": "Hive MetastoreまたはHive Metastoreと互換性のあるメタデータサービスに接続することで、DorisはHiveデータベースとテーブル情報を自動的に取得できます...",
  "language": "ja"
}
---
# Hive Catalog

Hive MetastoreまたはHive Metastoreと互換性のあるメタデータサービスに接続することで、DorisはHiveデータベースとテーブル情報を自動的に取得し、データクエリを実行できます。

Hiveに加えて、多くの他のシステムがメタデータの保存にHive Metastoreを使用しています。そのため、Hive Catalogを通じて、Hiveテーブルだけでなく、IcebergやHudiなどのHive Metastoreをメタデータストレージに使用する他のテーブル形式にもアクセスできます。

## 適用シナリオ

| シナリオ | 説明 |
|----------|------|
| Query Acceleration | Dorisの分散コンピューティングエンジンを使用してHiveデータに直接アクセスし、クエリを高速化します。 |
| Data Integration | Hiveデータを読み取ってDoris内部テーブルに書き込むか、Dorisコンピューティングエンジンを使用してZeroETL操作を実行します。 |
| Data Write-back | Dorisがサポートする任意のソースからデータを処理し、Hiveテーブルに書き戻します。 |
...
* `<fs_defaultfs>`
  ...
  このパラメータは、DorisからこのHive Catalog内のテーブルにデータを書き込む際に必須です。例：
...
* `{CommonProperties}`
  ...
  CommonPropertiesセクションは、共通の属性を入力するためのものです。[Catalog Overview](../catalog-overview.md)の「Common Properties」セクションを参照してください。

* `{OtherProperties}`

  OtherPropertiesセクションは、Hive Catalogに関連するプロパティを入力するためのものです。

  * `get_schema_from_table`：デフォルト値はfalseです。デフォルトでは、DorisはHive Metastoreからテーブルスキーマ情報を取得します。しかし、場合によっては互換性の問題が発生することがあり、例えば`Storage schema reading not supported`のようなエラーが発生することがあります。この場合、このパラメータをtrueに設定すると、テーブルスキーマはTableオブジェクトから直接取得されます。ただし、この方法では列のデフォルト値情報が無視されることにご注意ください。このプロパティはバージョン2.1.10および3.0.6以降でサポートされています。

### サポートされるHiveバージョン

Hive 1.x、2.x、3.x、および4.xをサポートします。

Hiveトランザクショナルテーブルはバージョン3.x以降でサポートされます。詳細については「Hiveトランザクショナルテーブル」セクションを参照してください。
...
> Dorisを通じてHiveテーブルを作成し、データを書き込むには、Catalog属性に`fs.defaultFS`プロパティを明示的に追加する必要があります。Catalogがクエリのためだけに作成される場合、このパラメータは省略できます。
...
* `SHOW PARTITIONS FROM [catalog.][db.]hive_table`
  ...
  このステートメントは、指定されたHiveテーブルのすべてのパーティションとその値を一覧表示します。
...
* `table$partitions` Metadata Table の使用
  ...
  バージョン2.1.7および3.0.3以降では、`table$partitions` metadata tableを通じてHiveパーティション情報をクエリできます。このテーブルは本質的にリレーショナルであり、各パーティションカラムが1つのカラムとして表現されるため、任意のSELECT文で使用できます。
...
* 現在の制限事項
  ...
  Original Filesシナリオはサポートされていません。テーブルがTransactional tableに変換された場合、新しいデータファイルはHive Transactional tableスキーマを使用しますが、既存のデータファイルは変換されません。これらのファイルはOriginal Filesと呼ばれます。
...
INSERT文を使用してHiveテーブルにデータを書き込むことができます。これは、Dorisで作成されたHiveテーブルまたは互換性のある形式の既存のHiveテーブルでサポートされます。

パーティション化されたテーブルの場合、データは自動的に対応するパーティションに書き込まれるか、データに基づいて新しいパーティションが作成されます。現在、書き込み用のパーティションを指定することはサポートされていません。

### INSERT INTO

INSERT操作は、対象テーブルにデータを追加します。現在、書き込み用のパーティションを指定することはサポートされていません。
...
INSERT OVERWRITEは、テーブル内の既存データを新しいデータで完全に置き換えます。現在、書き込み用のパーティションの指定はサポートされていません。
...
* ターゲットテーブルがパーティション化されており、ソーステーブルが空の場合、操作は効果がありません。ターゲットテーブルは変更されません。

* ターゲットテーブルがパーティション化されておらず、ソーステーブルが空の場合、ターゲットテーブルはクリアされます。

* 書き込み用のパーティション指定はサポートされていないため、INSERT OVERWRITEはソーステーブルの値に基づいて、ターゲットテーブル内の関連するパーティションを自動的に処理します。ターゲットテーブルがパーティション化されている場合、影響を受けるパーティションのみが上書きされ、影響を受けないパーティションは変更されません。

### CTAS

`CTAS (CREATE TABLE AS SELECT)`ステートメントを使用してHiveテーブルを作成し、データを挿入できます：
...
| ----------| ---- | ---- | --- |
| `hive_parquet_use_column_names` | `true` | DorisがHiveテーブルのParquetデータ型を読み取る際、デフォルトではHiveテーブルのカラム名に基づいて、Parquetファイルから同じ名前のカラムを見つけてデータを読み取ります。この変数が`false`の場合、DorisはHiveテーブルのカラム順序に従ってParquetファイルからデータを読み取り、カラム名は無視されます。HiveにおけるHiveの`parquet.column.index.access`変数と同様です。このパラメータはトップレベルのカラム名にのみ適用され、Struct内では無効です。 | 2.1.6+, 3.0.3+ |
| `hive_orc_use_column_names` | `true` | `hive_parquet_use_column_names`と同様で、HiveテーブルのORCデータ型に対応します。Hiveにおける`orc.force.positional.evolution`変数と同様です。 | 2.1.6+, 3.0.3+ |
...
ユーザーはDorisを通じてHive Metastoreでデータベースとテーブルを作成および削除できます。Dorisはこれらの操作においてHive Metastore APIを呼び出すのみで、Hiveメタデータ自体を保存または永続化することはありません。
...
```
:::caution
Hive Databaseの場合、そのDatabase自体を削除する前に、まずそのDatabase配下のすべてのテーブルを削除する必要があります。そうしなければエラーが発生します。この操作により、Hive内の対応するDatabaseも削除されます。
:::
...
- **テーブルの作成**
  ...
  DorisはHiveでパーティション化されたテーブルとパーティション化されていないテーブルの両方の作成をサポートしています。
  ...
  ```
テーブルを作成した後、`SHOW CREATE TABLE`コマンドを使用してHiveテーブル作成文を表示できます。

Hiveのテーブル作成構文とは異なり、Dorisでパーティションテーブルを作成する際は、パーティションカラムをテーブルスキーマに含める必要があることに注意してください。また、パーティションカラムはスキーマの最後に配置し、同じ順序を維持する必要があります。

:::tip
ACIDトランザクション機能がデフォルトで有効になっているHiveクラスターでは、Dorisによって作成されたテーブルの`transactional`プロパティが`true`に設定されます。DorisはHiveトランザクションテーブルの特定の機能のみをサポートしているため、Dorisが作成したHiveテーブルを読み取れない問題が発生する可能性があります。これを回避するには、テーブルプロパティで`"transactional" = "false"`を明示的に設定して、非トランザクションHiveテーブルを作成してください：
...
- **テーブルの削除**
  ...
  `DROP TABLE`文を使用してHiveテーブルを削除できます。テーブルが削除されると、パーティションデータを含むすべてのデータも削除されます。

- **カラム型のマッピング**

  詳細については[Column Type Mapping]セクションを参照してください。以下の制限事項にご注意ください：
...
- **パーティショニング**
  ...
  Hiveでは、パーティション型はDorisのListパーティションに対応します。そのため、DorisでHiveパーティションテーブルを作成する際は、Listパーティション構文を使用しますが、各パーティションを明示的に列挙する必要はありません。Dorisは、データ挿入時にデータ値に基づいて対応するHiveパーティションを自動的に作成します。単一カラムまたは複数カラムのパーティションテーブルがサポートされています。
...
- **ファイル形式**
  ...
  - Text（バージョン2.1.7および3.0.3以降でサポート）
      ...
      Text形式では以下のテーブルプロパティがサポートされています：
...
FEノードがHMSからNotification Eventsを定期的に読み取ることにより、DorisはHiveテーブルメタデータのリアルタイムな変更を検出し、メタデータの適時性を向上させることができます。現在、以下のイベントがサポートされています：
...
| CREATE TABLE     | 対応するデータベースにテーブルを作成します。                            |
| DROP TABLE       | 対応するデータベースのテーブルを削除し、テーブルキャッシュを無効化します。 |
| ALTER TABLE      | 名前が変更された場合、古いテーブルを削除し、新しい名前で新しいテーブルを作成します。そうでなければ、テーブルキャッシュを無効化します。 |
| ADD PARTITION    | 対応するテーブルのキャッシュされたパーティションリストにパーティションを追加します。 |
...
:::tip
1. データインポートによってファイルが変更される場合、パーティションテーブルは`ALTER PARTITION`イベントをトリガーし、非パーティションテーブルは`ALTER TABLE`イベントをトリガーします。
...
Hiveへの書き込み操作は独立したトランザクション内で実行されます。トランザクションがコミットされる前は、データは外部から見えません。トランザクションがコミットされて初めて、関連するテーブル操作が他から見えるようになります。
...
例えば、Hiveテーブルの複数のパーティションを変更する必要があるトランザクションにおいて、タスクが2つのバッチに分割される場合、最初のバッチは2番目のバッチが完了する前に外部から見えるようになる可能性があります。これは、最初のバッチのパーティションは読み取れるが、2番目のバッチは読み取れないことを意味します。
...
通常の非トランザクショナルHiveテーブルには完全なトランザクション機能がないため、Apache Dorisのトランザクション機能は不整合ウィンドウを最小化することを目的としていますが、真のACID特性を保証することはできません。したがって、Apache DorisでのHiveテーブルへの並行書き込みは、データ一貫性の問題を引き起こす可能性があります。
...
4. 同一のテーブルまたはパーティションに対する並行`INSERT OVERWRITE`操作は、データの損失や破損を引き起こし、予測不可能な結果をもたらす可能性があります。

5. 一般的な解決策は以下の通りです：

   * パーティションテーブルの場合、異なるパーティションにデータを書き込む。異なるパーティションでの並行操作は競合しません。

   * 非パーティションテーブルの場合、競合を避けるために`INSERT OVERWRITE`の代わりに`INSERT`を使用する。

   * 競合する可能性がある操作については、ビジネス側で一度に一つの書き込み操作のみが実行されることを保証する。

### HDFS File Operations

HDFS上のHiveテーブルデータについては、データは通常最初に一時ディレクトリに書き込まれ、その後`rename`などのファイルシステム操作を使用して確定されます。以下は、異なるデータ操作に対するHDFS上での具体的なファイル操作の詳細な説明です。
...
1. **非パーティションテーブル**
   ...
   * **Append（データ追加）**
     ...
     * ターゲットテーブルディレクトリ：`hdfs://ns/usr/hive/warehouse/example.db/table1`
     ...
     * コミット段階では、すべての一時ファイルがターゲットテーブルディレクトリに移動されます。
   ...
   * **Overwrite（データ置換）**
     ...
     * コミット段階のステップ：
       ...
       1. ターゲットテーブルディレクトリを一時ディレクトリにリネーム：`hdfs://ns/usr/hive/warehouse/example.db/_temp_b35fdbcea3a4e39-86d1f36987ef1492_table1`

       2. 一時ディレクトリをターゲットテーブルディレクトリにリネーム。

       3. 一時ターゲットテーブルディレクトリを削除。

2. **パーティションテーブル**

   * **Add（新しいパーティションへの追加）**

     * ターゲットテーブルディレクトリ：`hdfs://ns/usr/hive/warehouse/example.db/table2/part_col=2024-01-01`
     ...
     * コミット段階では、一時ディレクトリがターゲットテーブルディレクトリにリネームされます。
   ...
   * **Append（既存パーティションへのデータ追加）**
     ...
     * コミット段階では、一時ディレクトリからのファイルがターゲットテーブルディレクトリに移動されます。
...
| Dorisバージョン | 機能サポート                              |
| ------------- | --------------------------------------------- |
| 2.1.6         | Hiveテーブルへの書き戻しサポート       |
| 3.0.4         | JsonSerDe形式のHiveテーブルサポート。Hive4でのトランザクショナルテーブルサポート。 |
