...
事前にコンパイルされた [trino-bigquery-435-20240724.tar.gz](https://github.com/apache/doris-thirdparty/releases/download/trino-435-20240724/trino-bigquery-435-20240724.tar.gz) を直接ダウンロードして展開することもできます。

### BigQuery Connectorのデプロイ

すべてのFEおよびBEノードのデプロイパスの `connectors/` ディレクトリに `trino-bigquery-435/` ディレクトリを配置してください。（ディレクトリが存在しない場合は、手動で作成できます。）
