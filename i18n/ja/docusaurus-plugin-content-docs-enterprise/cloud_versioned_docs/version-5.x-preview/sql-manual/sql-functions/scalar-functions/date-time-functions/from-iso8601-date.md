---
{
  ...
  "description": "ISO8601形式の日付式をDATE型の日付式に変換します。ISO 8601標準に準拠した日付文字列、",
  "language": "ja"
}
---
# FROM_ISO8601_DATE
...
- YYYY: 年のみ（その年の1月1日を返します）
- YYYY-MM: 年と月（その月の1日を返します）
- YYYY-DDD: 年 + 年間通算日（DDDの範囲は1-366、例：0000-059は0000年の59日目を表します）
- YYYY-WWW: 年 + 週番号（WWWの範囲は1-53、その週の月曜日を返します）
- YYYY-WWW-D: 年 + 週番号 + 曜日（Dの範囲は1-7、1は月曜日、7は日曜日を表します）
- この形式では、年の第1週はその週の木曜日を含む必要があり、そうでなければ前年の週として数えられます
...
| -- | -- |
| `<date>` | ISO8601形式の日付、string型 |

## 戻り値

解析された特定の日付を表すYYYY-MM-DD形式のDATE型を返します。
