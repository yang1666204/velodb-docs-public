---
{
  ...
  "description": "Apache Doris 2.1のPaimon Catalogは、複数のPaimonメタデータサービスに接続し、HDFSとクラウドオブジェクトストレージ全体でPaimonテーブルをクエリします。詳細な設定、プロパティ、クエリ操作を提供し、書き込みサポートが計画されています。",
  "language": "ja"
}
---
...
DorisはPaimonテーブルのメタデータに様々なメタデータサービス経由でアクセスし、Paimonデータをクエリすることを現在サポートしています。

現時点では、Paimonテーブルへの読み取り操作のみがサポートされています。Paimonテーブルへの書き込み操作は将来的にサポートされる予定です。
...
| シナリオ | 説明 |
| ------------ | ------------------------------------------------------------ |
| クエリ高速化 | Dorisの分散計算エンジンを使用してPaimonデータに直接アクセスし、クエリを高速化します。 |
| データ統合 | Paimonデータを読み取りDoris内部テーブルに書き込む、またはDoris計算エンジンを使用してZeroETL操作を実行します。 |
...
* `<paimon_catalog_type>`
  ...
  Paimon Catalogのタイプです。以下をサポートしています：
...
* `<paimon_warehouse>`
  ...
  `warehouse`パスは`Database`パスの1階層上を指定する必要があります。例えば、テーブルパスが`s3://bucket/path/to/db1/table1`の場合、`warehouse`は`s3://bucket/path/to/`にする必要があります。

* `{MetaStoreProperties}`

  MetaStorePropertiesセクションは、Metastoreメタデータサービスの接続および認証情報を入力するために使用されます。詳細は[Supported Metadata Services]のセクションを参照してください。

* `{StorageProperties}`

  StoragePropertiesセクションは、ストレージシステムに関連する接続および認証情報を入力するために使用されます。詳細は[Supported Storage Systems]のセクションを参照してください。

* `{PaimonProperties}`

  PaimonPropertiesセクションは、Paimon関連のプロパティを入力するために使用されます。

  DorisがPaimonメタデータにアクセスする際、部分的にPaimon Java SDKを使用するため、いくつかのカスタムパラメータをPaimon Java SDKに渡す必要がある場合があります。以下のプレフィックスを使用できます：
...
* `{CommonProperties}`
  ...
  CommonPropertiesセクションは共通プロパティを入力するために使用されます。[Common Properties]の詳細については、[Catalog Overview](../catalog-overview.md)セクションを参照してください。
...
> 注意: Dorisバージョンによってサポートされるサービスタイプとパラメータは若干異なります。[Examples]セクションを参照してください。
...
| varchar                            | string        |                                                                         |
| char                               | string        |                                                                         |
| binary, varbinary                             | string/varbinary | Catalogの`enable.mapping.varbinary`プロパティによって制御されます（4.0.2以降でサポート）。デフォルトは`false`で`string`にマップされます。`true`の場合は`varbinary`タイプにマップされます。|
| date                               | date          |                                                                         |
| timestamp\_without\_time\_zone     | datetime(N)   | 精度に応じてマップされます。精度が6より大きい場合は最大6にマップされます（精度の損失が発生する可能性があります）。 |
| timestamp\_with\_local\_time\_zone | datetime(N)   | 精度に応じてマップされます。精度が6より大きい場合は最大6にマップされます（精度の損失が発生する可能性があります）。 |
| array                              | array         |                                                                         |
| map                                | map           |                                                                         |
| row                                | struct        |                                                                         |
| other                              | UNSUPPORTED   |                                                                         |
...
> 注意:
>
> Dorisは現在、タイムゾーン付きの`Timestamp`タイプをサポートしていません。すべての`timestamp_without_time_zone`と`timestamp_with_local_time_zone`は一律に`datetime(N)`タイプにマップされます。ただし、読み取り時にDorisは実際のソースタイプに基づいてタイムゾーンを正しく処理します。例えば、`SET time_zone=<tz>`でタイムゾーンを指定した後は、`timestamp_with_local_time_zone`カラムの戻り値に影響します。
>
> `DESCRIBE table_name`文のExtraカラムでソースタイプにタイムゾーン情報が含まれているかどうかを確認できます。`WITH_TIMEZONE`と表示される場合、ソースタイプがタイムゾーン対応タイプであることを示します。（この機能は3.0.8以降でサポートされています）
...
指定されたスナップショットまたはタイムスタンプ区間内の増分データのクエリをサポートしています。この区間は左閉右開です。
...
| `endTimestamp` | 終了スナップショットタイムスタンプ。`startTimestamp`より大きい値である必要があります。オプションで、指定されない場合は`startTimestamp`から最新のスナップショットまで読み取ります。単位はミリ秒です。 | `'endTimestamp'='1750944949000'` |

> 注意:
>
> `startSnapshotId`と`endSnapshotId`はPaimonパラメータ`'incremental-between'='3,10'`を構成します
>
> `startTimestamp`と`endTimestamp`はPaimonパラメータ`'incremental-between-timestamp'='1750844949000,1750944949000'`を構成します
>
...
指定されたPaimonテーブルのスナップショットは`$snapshots`テーブル関数を通じてクエリできます:
...
+-------------+-------------------------+
```
`FOR TIME AS OF`と`FOR VERSION AS OF`文を使用して、スナップショットIDまたはスナップショットが作成された時刻に基づいて履歴バージョンデータを読み取ることができます。例：
...
指定されたPaimonテーブルのブランチとタグの読み取りをサポートします。

`$branches`と`$tags`システムテーブルを使用して、Paimonテーブルのブランチとタグを表示できます：
...
+----------+-------------+-----------+-------------------------+--------------+-------------+---------------+
```
Spark/Trinoなどのシステムとの互換性を保つために、様々な構文形式をサポートします：
...
```
`FOR VERSION AS OF`構文では、Dorisはパラメータがタイムスタンプかどうかを自動的に判定します

## System Tables

> バージョン3.1.0以降

DorisはPaimonシステムテーブルのクエリをサポートし、テーブル関連のメタデータを取得できます。システムテーブルを使用して、スナップショット履歴、マニフェストファイル、データファイル、パーティション、その他の情報を表示できます。
...
```
> Note: Doris は Paimon global system tables の読み込みをサポートしていません。これらは Flink でのみサポートされています。


### schemas

テーブルの現在および履歴のスキーマ情報を表示します。`ALTER TABLE`、`CREATE TABLE AS`、または `CREATE DATABASE AS` ステートメントを使用してテーブルスキーマを変更する際、各変更は schemas テーブル内にレコードを生成します：
...
テーブルのすべてのタグ情報を表示します。タグ名と関連するスナップショットが含まれます：
...
#### テーブルの最新のスナップショット情報を表示して現在の状態を把握する
...
1. `Could not find a file io implementation for scheme 's3a' in the classpath`
    ...
    ```
**バージョン3.1より前**
バージョン3.1より前では`s3a`プロトコルをサポートしていません。どうしても使用する必要がある場合は、回避策として`s3a`関連のパラメータを一時的に追加することができます。

**バージョン3.1以降**
バージョン3.1以降では、`s3a`パラメータを明示的に追加する必要はなくなりました。Paimonストレージパラメータをオーバーライドする必要がある場合は、`paimon.fs.`プレフィックス付きのパラメータを追加できます。Paimonは内部的にHDFS APIを使用するため、すべての`fs.s3a.*`パラメータがサポートされています。
...
例：
    ...
    ``` 
`s3`プロトコルを使用する場合、Paimon S3関連パラメータを明示的に追加する必要があります。つまり、`paimon.`プレフィックスを追加します。例：
