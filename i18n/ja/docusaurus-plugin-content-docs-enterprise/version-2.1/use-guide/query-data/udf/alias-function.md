...
{
  "title": "エイリアス関数",
  "description": "エイリアス関数とは、関数にエイリアス名を割り当てることを指します。システム内の関数や式の断片に新しいシグネチャを登録することで、",
  "language": "ja"
}
---
## はじめに

エイリアス関数とは、関数にエイリアス名を割り当てることを指します。システム内で関数または式フラグメントの新しいシグネチャを登録することで、互換性の向上や利便性の増加を目的としています。

エイリアス関数は、他のカスタム関数と同様に、`LOCAL`と`GLOBAL`の2つのスコープをサポートしています。

- `LOCAL`: エイリアス関数は現在のデータベーススコープ下で登録されます。他のデータベース下でエイリアス関数を使用する必要がある場合は、完全修飾名を使用する必要があります。つまり、`<Database Name>.<Function Name>`です。

- `GLOBAL`: エイリアス関数はグローバルスコープ下で登録されます。任意のデータベース下で関数名により直接アクセスできます。
...
このシナリオは複雑な分析でよく見られます。複雑なクエリステートメントを記述する際、ステートメント内または異なるステートメント間で大量の繰り返し式フラグメントが存在する場合があります。この複雑な式フラグメントにエイリアス関数を作成することで、クエリステートメントを簡略化し、記述の利便性と保守性を向上させることができます。
