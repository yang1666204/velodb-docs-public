...
{
  "title": "BOOL_AND",
  "description": "式の全てのnon-NULL値に対して論理AND集約を実行します。",
  "language": "ja"
}
---
## 説明

式のすべての非NULLの値に対して論理AND集計を実行します。
...
| -- | -- |
| `<expr>` | 論理ANDで集約される式。BOOLEAN型と、0/非0ルール（0はFALSE、非0はTRUE）によってブール値に変換可能な数値型をサポートします。 |

## Return Value

戻り値はBOOLEANです。すべての非NULL値がTRUEの場合のみTRUEを返し、それ以外の場合はFALSEを返します。

式のすべての値がNULLまたは式が空の場合、この関数はNULLを返します。
...
+-----------------+-----------------+-----------------+-----------------+
```
bool_andは数値型のパラメータも受け取ることができます。数値が0でない場合、`TRUE`に変換されます。
...
```
### Window function:
以下の例では、条件 (id > 2) に基づいて行をパーティション分割し、2つのグループに分けて、ウィンドウ集約の結果を表示します：
