...
戻り値の意味:
- 対応するIPv4アドレスのネットワークバイトオーダー（ビッグエンディアン）整数表現でのアドレスの数値を返します
- 無効なIPv4文字列または`NULL`入力に対して例外をスローします

### 使用上の注意
- 標準的なIPv4テキストのみをサポートし、CIDR（`/24`など）、ポート（`:80`など）、その他の拡張フォーマットはサポートしません
- 暗黙的なトリミングや型変換は実行せず、先頭/末尾に空白文字がある文字列は無効とみなされます
- 相互変換のために`inet_ntoa`、`to_ipv4`と組み合わせて使用されることが多いです
...
+--------+-----------+
```
無効な入力は例外を発生させます（セグメント値が範囲外/空白文字を含む/NULL）。
