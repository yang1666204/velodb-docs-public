...
> - **`<columns_mapping>`**（オプション）: ソーステーブルとターゲットテーブルの列間のマッピングを定義します。省略した場合、列は順序通りに一対一でマッピングされます。なお、`col_name = expr` の形式はサポートされていません。
...
| 権限 | オブジェクト | 備考                |
| :---------------- | :------------- | :---------------------------- |
| LOAD_PRIV        | テーブル   | この操作は、インポートテーブルに対してLOAD_PRIV権限を持つユーザーまたはロールのみが実行できます。 |

## 例

1. **シンプルな同期ジョブの作成**

   `test_db` データベース内に `job1` という名前の同期ジョブを作成し、MySQLソーステーブル `mysql_db1.tbl1` をDorisターゲットテーブル `test_tbl` にマッピングして、ローカルのCanalサーバーに接続します。
...
2. **複数チャネルと明示的なカラムマッピングを使用した同期ジョブの作成**
   ...
   一対一マッピングと明示的に指定されたカラム順序を持つ複数のMySQLソーステーブルに対して、`test_db`データベース内に`job1`という名前の同期ジョブを作成します。
