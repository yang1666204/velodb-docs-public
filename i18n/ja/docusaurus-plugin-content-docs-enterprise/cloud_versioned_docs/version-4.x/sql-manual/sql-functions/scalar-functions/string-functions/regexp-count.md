...
{
  "title": "REGEXP_COUNT",
  "description": "この関数は、文字列内で指定された正規表現パターンに一致する文字数をカウントするものです。",
  "language": "ja"
}
---
## 説明
これは、文字列内で指定された正規表現パターンにマッチする文字数をカウントする関数です。入力は、ユーザーが提供する文字列と正規表現パターンで構成されます。戻り値はマッチした文字の総数です。マッチするものが見つからない場合は0を返します。

'str'パラメータは'string'型で、ユーザーが正規表現でマッチさせたい文字列です。

'pattern'パラメータは'string'型で、文字列のマッチに使用される正規表現の文字列です。

文字セットマッチングを処理する際は、Utf-8標準文字クラスを使用する必要があることに注意してください。これにより、関数が異なる言語の様々な文字を正しく識別し、処理できるようになります。
...
- 'str'内で正規表現'pattern'にマッチした数を返します。これは'int'型で、マッチする文字がない場合は0を返します。
patternがNULLまたはstrがNULL、あるいは両方がNULLの場合、NULLを返します。
patternが許可されない正規表現の場合、エラーを発生させます。これは不正な操作です。
...
+------------------------+
```
特定の変数値を挿入してから、格納された行から変数を取得してマッチングを行った場合の戻り結果。
...
+----------------------------------------------------+
```
'pattern'が正規表現として許可されていない場合、エラーを投げる
