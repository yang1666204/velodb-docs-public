...
3つの点を入力します。最初の直線はpoint1（最初の端点）とpoint2（2番目の端点）を結ぶ直線です。2番目の直線はpoint2（最初の端点）とpoint3（2番目の端点）を結ぶ直線です。これら2本の直線はpoint2で交差します。この関数は、最初の直線から2番目の直線への時計回りの角度を返します。各端点の座標について、x値（経度）は[-180, 180]の範囲内である必要があり、y値（緯度）は[-90, 90]の範囲内である必要があります。
...
2つの線の間の角度をラジアンで返します。倍精度浮動小数点数として、[0, 2π)の範囲で返されます。角度は最初の線から2番目の線まで時計回りに測定されます。
...
- いずれかのポイントの座標がx（経度）またはy（緯度）の指定された範囲を超える場合、NULLを返します。
- 入力ポイントのいずれかがNULLの場合、NULLを返します。
