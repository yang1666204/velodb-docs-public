...
```
コンパイル後、`trino/plugin/trino-tpcds/target/`配下に`trino-tpcds-435/`ディレクトリが作成されます。

また、事前にコンパイルされた[trino-tpcds-435.tar.gz](https://github.com/morningman/trino-connectors/releases/download/trino-connectors/trino-tpcds-435.tar.gz)を直接ダウンロードして展開することも可能です。

## TPCDSコネクターのデプロイ

すべてのFEおよびBEデプロイメントパスの`connectors/`ディレクトリ配下に`trino-tpcds-435/`ディレクトリを配置してください。（存在しない場合は手動で作成できます）。
...
...
```
デプロイ後、Connectorが正しく読み込まれることを保証するために、FEとBEノードを再起動することを推奨します。
