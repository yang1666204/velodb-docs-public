...
---
Kudu CatalogはKudu Connectorを通じてKuduテーブルにアクセスするために[Trino Connector](https://doris.apache.org/zh-CN/community/how-to-contribute/trino-connector-developer-guide/)互換フレームワークを使用します。
...
| シナリオ       | 説明                          |
| -------------- | ------------------------------------ |
| データ統合 | Kuduデータを読み取り、Doris内部テーブルに書き込みます。 |
...
* `{TrinoProperties}`
  ...
  `TrinoProperties` セクションは、Trino Connectorに渡されるプロパティを指定するために使用されます。これらのプロパティには `trino.` のプレフィックスが付けられます。理論的には、Trinoでサポートされているすべてのプロパティがここでもサポートされます。Kuduプロパティの詳細については、[Trino documentation](https://trino.io/docs/current/connector/kudu.html)を参照してください。
...
Kuduの詳細については、[Trino documentation](https://trino.io/docs/current/connector/kudu.html)を参照してください。
