...
Table作成時に、`UNIQUE KEY`キーワードを使用してUnique KeyTableを指定できます。merge-on-readモードは、`enable_unique_key_merge_on_write`属性を明示的に無効にすることで有効にできます。Dorisバージョン2.1以前では、merge-on-readモードがデフォルトで有効でした：
