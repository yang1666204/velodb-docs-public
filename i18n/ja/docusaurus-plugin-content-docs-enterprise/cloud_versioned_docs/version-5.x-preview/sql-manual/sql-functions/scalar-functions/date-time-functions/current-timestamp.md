...
---
## 説明
この関数は現在のシステム時刻を取得するために使用され、datetime型（`DATETIME`）を返します。オプションで精度を指定して、戻り値の秒の小数部分の桁数を調整することができます。

この関数は、MySQLの[current_timestamp function](https://dev.mysql.com/doc/refman/8.4/en/date-and-time-functions.html#function_current-timestamp)と一致しています。
...
- 指定された`<precision>`が範囲外の場合（負の値や6より大きい値など）、関数はエラーを返します。
