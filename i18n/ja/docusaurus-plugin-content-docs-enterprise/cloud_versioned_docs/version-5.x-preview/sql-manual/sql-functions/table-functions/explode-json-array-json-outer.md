...
---
## 説明
`explode_json_array_json_outer`テーブル関数はJSON配列を受け取ります。その実装ロジックは、JSON配列をarray型に変換してから、処理のために`explode_outer`関数を呼び出すことです。この動作は`explode_outer(cast(<json_array> as Array<JSON>))`と同等です。
