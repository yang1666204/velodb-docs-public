...
auto partition機能は主に、ユーザーが特定の列に基づいてTableをパーティション化することを期待しているものの、その列のデータ分布が分散していたり予測不可能であったりするため、Tableの構造を構築または調整する際に必要なパーティションを正確に作成することが困難であったり、パーティション数が非常に多くて手動で作成するには煩雑すぎるという問題を解決します。
...
```
Tableには大量のビジネス履歴データが格納され、トランザクションが発生した日付に基づいてパーティション分割されています。Tableを構築する際に見ることができるように、事前に手動でパーティションを作成する必要があります。パーティション分割されたカラムのデータ範囲が変更された場合、例えば上記のTableに2022年が追加された場合、[ALTER-TABLE-PARTITION](../../sql-manual/sql-statements/table-and-view/table/ALTER-TABLE-PARTITION)によってパーティションを作成し、Tableパーティションを変更する必要があります。そのようなパーティションを変更したり、より細かい粒度レベルで細分化したりする必要がある場合、それらを修正するのは非常に煩雑です。この時点でAuto Partitionを使用してTableDDLを書き直すことができます。

## 構文

Tableを作成する際は、[CREATE-TABLE](../../sql-manual/sql-statements/table-and-view/table/CREATE-TABLE)文の`partitions_definition`セクションに以下の構文を使用してください。
...
```
2つの列のみを持つTableの例を取ってください。この時点で、新しいTableにはデフォルトのパーティションがありません：

```sql
show partitions from `DAILY_TRADE_VALUE`;
Empty set (0.12 sec)
```
データを挿入して再度確認すると、Tableが対応するパーティションを作成していることがわかります：
...
AUTO RANGE PARTITIONTableでは、`partition.retention_count`プロパティがサポートされており、これは正の整数値をパラメータとして受け取り（`N`と表記）、**すべての履歴パーティションの中で、パーティション値が最大の上位`N`個の履歴パーティションのみ**が保持されることを示します。現在および将来のパーティションはすべて保持されます。具体的には：
...
AUTO RANGE PARTITIONのみが[Auto バケット](./data-bucketing.md#auto-setting-bucket-number)機能と組み合わせて使用できます。この機能を使用する場合、Dorisはデータインポートが時間順に増分的であり、各インポートは1つのパーティションのみに関わると想定します。つまり、この使用方法はバッチごとに増分的にインポートされるTableに対してのみ推奨されます。
...
Auto Partitionが有効な場合、パーティション名は`auto_partition_name`関数を使用してパーティションにマッピングできます。`partitions`Table関数は、パーティション名から詳細なパーティション情報を生成します。`DAILY_TRADE_VALUE`Tableを例に、データを挿入した後の現在のパーティションを見てみましょう：
...
- 通常のパーティション化Tableと同様に、auto List Partitionは構文に違いがなく、マルチカラムパーティショニングをサポートします。
- データの挿入やインポートプロセス中にパーティションが作成され、インポートプロセス全体が完了しない場合（失敗またはキャンセル）、作成されたパーティションは自動的に削除されません。
- Auto Partitionを使用するTableは、パーティション作成方法のみが手動から自動に変わり、Tableと作成されたパーティションの本来の使用方法は、Auto PartitionではないTableやパーティションと同じです。
...
- Auto Partitionが有効なTableにデータをインポートする際、コーディネーターは通常のTableとは異なるポーリング間隔でデータを送信します。詳細はBE ConfigurationのOLAP_table_sink_send_interval_auto_partition_factor`を参照してください。この設定は`enable_memtable_on_sink_node`が有効になった後は影響しません。
- [insert-overwrite](../../sql-manual/sql-statements/data-modification/DML/INSERT-OVERWRITE)を使用してAuto PartitionTableにデータをロードするとINSERT OVERWRITEドキュメントに詳細な動作が記載されています。
