...
> - **`<columns_mapping>`**（オプション）: ソーステーブルとターゲットテーブルの列間のマッピングを定義します。省略された場合、列は順序に従って一対一でマッピングされます。なお、`col_name = expr`の形式はサポートされていません。
...
- `<channel_desc>`で指定されたターゲットテーブルは、バッチ削除機能が有効化されている必要があります。
...
| 権限 | オブジェクト | 注記                |
| :---------------- | :------------- | :---------------------------- |
| LOAD_PRIV        | Table   | この操作は、インポートテーブルに対するLOAD_PRIV権限を持つユーザーまたはロールのみが実行できます。 |

## 例

1. **シンプルな同期ジョブを作成する**

   `test_db`データベース内で、MySQLソーステーブル`mysql_db1.tbl1`をDorisターゲットテーブル`test_tbl`にマッピングし、ローカルのCanalサーバーに接続する`job1`という名前の同期ジョブを作成します。
...
2. **複数のチャンネルと明示的なカラムマッピングを使用した同期ジョブの作成**
   ...
   一対一マッピングを持つ複数のMySQLソーステーブルに対して、明示的に指定されたカラム順序で`test_db`データベース内に`job1`という名前の同期ジョブを作成します。
