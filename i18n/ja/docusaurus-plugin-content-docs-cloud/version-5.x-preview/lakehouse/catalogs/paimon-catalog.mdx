...
{
  "title": "Paimon カタログ",
  "description": "Apache Doris 4.xのPaimon カタログは、複数のPaimonメタデータサービスに接続し、HDFSやクラウドオブジェクトストレージ上のPaimonTableをクエリできます。詳細な設定、プロパティ、クエリ操作が提供され、書き込みサポートが計画されています。",
  "language": "ja"
}
---
...
* `<paimon_warehouse>`
  ...
  `warehouse`パスは`Database`パスの1階層上を指す必要があります。例えば、Tableパスが`s3://bucket/path/to/db1/table1`の場合、`warehouse`は`s3://bucket/path/to/`とする必要があります。
...
* `{CommonProperties}`
  ...
  CommonPropertiesセクションは共通プロパティを設定するために使用されます。[Common Properties]については[カタログ 概要](../catalog-overview.md)セクションを参照してください。
...
| varchar                            | string        |                                                                         |
| char                               | string        |                                                                         |
| binary, varbinary                             | string/varbinary | カタログの`enable.mapping.varbinary`プロパティによって制御されます（4.0.2以降でサポート）。デフォルトは`false`で`string`にマッピングされます。`true`の場合は`varbinary`タイプにマッピングされます。|
| date                               | date          |                                                                         |
