...
### 説明
    ...
    enable_decimal256のデフォルト値はfalseです。これをtrueに設定するとより正確な結果を得ることができますが、パフォーマンスの低下を招きます。

    decimal型を出力する際、末尾の桁が0であっても、小数点の後には常にS桁が表示されます。例えば、decimal(18, 6)型の数値123.456は123.456000として出力されます。

### 精度推論

DECIMALには非常に複雑な型推論規則があります。異なる式に対して、精度推論のために異なる規則が適用されます。
...
|e1 / e2|p1 + s2 + `div_precision_increment`|s1 + `div_precision_increment`|MAX_P|<ol><li>precision - s1がmax_precision - `decimal_overflow_scale`未満の場合: (max_precision - (precision - s1)) + `div_precision_increment`</li><li>precision - s1がmax_precision - `decimal_overflow_scale`より大きく、かつs1が`decimal_overflow_scale`未満の場合: s1 + `div_precision_increment`</li><li>precision - s1がmax_precision - `decimal_overflow_scale`より大きく、かつs1が`decimal_overflow_scale`以上の場合: `decimal_overflow_scale` + `div_precision_increment`</li></ol>|pは結果に応じてキャスト、sは結果+e2.scaleに応じてキャスト||
|e1 % e2|max(p1 - s1,p2 - s2) + max(s1, s2)|max(s1, s2)|MAX_P|min(MAX_P, p) - max(p1 - s1,p2 - s2)|結果に応じてキャスト|結果に応じてキャスト|

表の`Result scale if overflow`を計算する規則において、`precision`は`Result precision`列の`precision`を指し、`scale`は`Result scale`列の`scale`を指します。

`div_precision_increment`はFEの設定パラメータです。div_precision_incrementを参照してください。

`decimal_overflow_scale`はFEのセッション変数で、decimal値の計算結果の精度がオーバーフローした場合に計算結果で保持できる小数点以下の最大桁数を示します。デフォルト値は6です。

注目すべき点として、除算計算のプロセスは以下の通りです：
DECIMAL(p1, s1) / DECIMAL(p2, s2)は最初にDECIMAL(p1 + s2 + div_precision_increment, s1 + s2) / DECIMAL(p2, s2)に変換されてから計算が実行されます。そのため、DECIMAL(p1 + s2 + div_precision_increment, s1 + div_precision_increment)がDECIMALの範囲を満たしていても、
...
```
乗算結果の精度の計算ルールに従って、結果の型は decimal(38, 18) となり、オーバーフローは発生しません：
...
```
乗算結果の精度の計算ルールに従って、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）では、通常の計算結果型は`decimal(41, 11)`です。精度がオーバーフローするため、オーバーフロールールに従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - `scale` = 41 - 11 = 30 < 32、ルール1が適用され、最終結果のスケール = min(11, 38 - 30) = 8、最終結果型はdecimal(38, 8)です：
...
+----------------------+-----------------------+--------------------------------------+
```
`decimal_overflow_scale`の値が増加した場合、例えば`set decimal_overflow_scale=9;`の場合、オーバーフロールールに従って計算が実行されます：`MAX_P` - `decimal_overflow_scale` = 38 - 9 = 29、`precision` - `scale` = 41 - 11 = 30 > 29、かつscale > `decimal_overflow_scale`であるため、オーバーフロールール3が適用され、最終的な計算結果の型は`decimal(38,9)`となります：
...
+----------------------+-----------------------+---------------------------------------+
```
`decimal_overflow_scale`の値を引き続き増加させる場合、例えば`set decimal_overflow_scale=12;`とすると、オーバーフロールールに従って計算されます：`MAX_P` - `decimal_overflow_scale` = 38 - 12 = 26、`precision` - `scale` = 41 - 11 = 30 > 26、そしてscale < `decimal_overflow_scale`となります。この場合、オーバーフロールール2が適用され、最終的に計算された結果の型は`decimal(38,11)`となります：
...
```
除算結果精度の計算ルールに従い、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）の下で、通常の計算結果タイプは`decimal(19 + 9 + 4, 9 + 4)`、つまり`decimal(32, 13)`であり、精度はオーバーフローしません。結果の最終タイプは`decimal(32, 13)`です：
...
+----------------------+---------------------+-----------------+
```
結果により多くの小数点以下の桁数を保持したい場合は、`div_precision_increment`を増加させることができます。例えば、`admin set frontend config('div_precision_increment'='8');`のようにします。その後、上記の計算ルールに従って、計算結果の型は`decimal(36, 17)`になります：
...
```
除算結果精度の計算ルールに従って、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）の下では、通常の計算結果タイプは`decimal(27 + 8 + 4, 8 + 4)`、すなわち`decimal(39, 12)`です。精度がオーバーフローしたため、オーバーフロールールに従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - s1 = 39 - 8 = 31 < 32であるため、`Result scale if overflow`ルール1が適用され、結果スケールは（`MAX_P` - （`precision` - `s1`））+ `div_precision_increment` = (38 - (39 - 8)) + 4 = 11となり、結果タイプは`decimal(38, 11)`です：
...
+-----------------------------+----------------------------+---------------+
```
`decimal_overflow_scale`の値が増加した場合、例えば`set decimal_overflow_scale=8;`とすると、オーバーフロールールに従って計算が実行されます：`MAX_P` - `decimal_overflow_scale` = 38 - 8 = 30、`precision` - s1 = 39 - 8 = 31 > 30、かつs1 == `decimal_overflow_scale`であるため、オーバーフロールール3が適用され、最終的な計算結果の型は`decimalv3(38,12)`となります：
...
```
除算結果の精度の計算規則に従い、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）では、通常の計算結果の型は`decimal(38 + 4 + 4, 4 + 4)`、つまり`decimal(46, 8)`となります。精度がオーバーフローしたため、オーバーフロー規則に従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - s1 = 46 - 4 = 42 > 32、s1 = 4 < `decimal_overflow_scale`のため、`Result scale if overflow`規則2が適用され、結果のスケールは`s1` + `div_precision_increment` = 4 + 4 = 8となり、結果の型は`decimal(38, 8)`となります：
...
+-------------------------+------------------------+-------------+
```
結果により多くの小数点以下の桁数を保持したい場合は、`div_precision_increment`を増やすことができます。例えば、`admin set frontend config('div_precision_increment'='8');`のようにします。その後、上記の計算ルールに従って、計算結果の型は`decimal(38, 12)`になります：
...
```
除算結果の精度の計算規則に従って、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）では、通常の計算結果の型は`decimal(38 + 7 + 4, 7 + 4)`、つまり`decimal(49, 11)`となります。精度がオーバーフローするため、オーバーフロー規則に従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - s1 = 49 - 7 = 42 > 32、s1 = 7 > `decimal_overflow_scale`であるため、`Result scale if overflow`規則3が適用され、結果のスケールは`decimal_overflow_scale` + `div_precision_increment` = 6 + 4 = 10となり、結果の型は`decimal(38, 10)`となります：
...
上記で言及された式を除いて、他の式は精度推定にデフォルトルールを使用します。つまり、式`expr(DECIMAL(a, b))`の結果型もDECIMAL(a, b)になります。

#### 結果精度の調整

DECIMALに対する精度要件はユーザーによって異なります。上記のルールはDorisのデフォルト動作です。ユーザーが**異なる精度要件を持つ場合、以下の方法で精度を調整できます**：

* 期待される結果精度がデフォルト精度より大きい場合、パラメータの精度を調整することで結果精度を調整できます。例えば、ユーザーが`AVG(col)`を計算してDECIMAL(x, y)を結果として取得することを期待し、`col`の型がDECIMAL (a, b)の場合、式を`AVG(CAST(col as DECIMAL (x, y))`に書き換えることができます。
* 期待される結果精度がデフォルト精度より小さい場合、出力結果を近似することで望ましい精度を取得できます。例えば、ユーザーが`AVG(col)`を計算してDECIMAL(x, y)を結果として取得することを期待し、`col`の型がDECIMAL(a, b)の場合、式を`ROUND(AVG(col), y)`として書き換えることができます。

### DECIMALが必要な理由

DorisのDECIMALは真の高精度固定小数点数です。DECIMALには以下の中核的な利点があります：
1. より広い範囲を表現できます。DECIMALにおける精度とスケールの両方の値範囲が大幅に拡張されています。
2. より高いパフォーマンス。DECIMALの旧バージョンはメモリに16バイト、ストレージに12バイトを必要としましたが、DECIMALは以下に示すような適応的調整を行っています。

|     precision        | 占有スペース（メモリ/ディスク） |
|----------------------|-------------------|
...
3. より完全な精度推定。異なる式に対して、異なる精度推論ルールが適用され、結果の精度を推定します。
