...
{
  "title": "ARRAY_AGG",
  "description": "列内の値（null値を含む）を配列に連結します。これは行を列にピボットする際に使用できます。",
  "language": "ja"
}
---
## 概要

列内の値（null値を含む）を配列に連結します。これは行を列に変換するピボット処理に使用できます。
...
| パラメータ | 説明 |
| -- | -- |
| `<col>` | 配列に配置される値を決定する式。サポートされている型：Bool、TinyInt、SmallInt、Integer、BigInt、LargeInt、Float、Double、Decimal、Date、Datetime、IPV4、IPV6、String、Array、Map、Struct。 |
