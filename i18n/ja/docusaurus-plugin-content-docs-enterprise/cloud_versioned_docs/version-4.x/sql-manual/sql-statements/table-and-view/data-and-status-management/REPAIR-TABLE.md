...
`REPAIR TABLE`ステートメントは、指定されたテーブルまたはパーティションのレプリカの修復を優先して実行するために使用されます。このステートメントには以下の機能があります：
...
>
> このパラメータが指定されない場合、テーブル全体のすべてのパーティションを修復します。
...
- このコマンドはシステムの通常のレプリカ修復メカニズムには影響しません。指定されたテーブルまたはパーティションの修復の優先度を上げるだけです。
