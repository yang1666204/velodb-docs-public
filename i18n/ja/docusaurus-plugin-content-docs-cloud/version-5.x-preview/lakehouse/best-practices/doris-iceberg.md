...
- バージョン0.15以降、Apache DorisはHiveとIceberg外部テーブルを導入し、データレイクにおけるApache Icebergとの組み合わせ機能を探求してきました。
...
Apache Icebergは、大規模データの分析と管理を可能にするオープンソースの高性能で高信頼性なデータレイクテーブル形式です。Apache Dorisを含む様々な主流クエリエンジンをサポートし、HDFSと様々なオブジェクトクラウドストレージに対応し、ACID準拠、スキーマ進化、高度なフィルタリング、隠れパーティショニング、パーティションレイアウト進化などの機能を備え、高性能クエリ、データの信頼性、整合性、柔軟性を確保し、タイムトラベルやバージョンロールバックなどの機能を提供します。
...
- Time Travel機能をサポート。
- Icebergテーブルエンジンをネイティブサポート。Apache DorisがIcebergテーブルを直接作成、管理、データ書き込みを行うことを可能にします。包括的なパーティションTransform機能をサポートし、隠れパーティショニングやパーティションレイアウト進化などの機能を提供します。

ユーザーはApache Doris + Apache Icebergに基づく効率的なData Lakehouseソリューションを迅速に構築し、様々なリアルタイムデータ分析と処理のニーズに柔軟に対応できます。

- Dorisの高性能クエリエンジンを使用してIcebergテーブルデータと他のデータソースを関連付けてデータ分析を実行し、**統合されたフェデレーテッドデータ分析プラットフォーム**を構築。
- Dorisを通じてIcebergテーブルを直接管理・構築し、Doris内でデータクリーニング、処理、Icebergテーブルへの書き込みを完了し、**データレイク用統合データ処理プラットフォーム**を構築。
- Icebergテーブルエンジンを通じてDorisデータを他の上流・下流システムと共有してさらなる処理を行い、**統合されたオープンデータストレージプラットフォーム**を構築。

将来、Apache IcebergはApache Dorisのネイティブテーブルエンジンの一つとして機能し、レイク形式データに対してより包括的な分析と管理機能を提供する予定です。Apache Dorisは、Update/Delete/Merge、書き戻し時のソート、増分データ読み取り、メタデータ管理などのApache Icebergのより高度な機能を段階的にサポートし、統合された高性能なリアルタイムデータレイクプラットフォームを共同で構築していきます。
...
Doris コマンドラインにログインした後、Doris クラスター内には Iceberg という名前の Iceberg Catalog が既に作成されています（`SHOW CATALOGS`/`SHOW CREATE CATALOG iceberg` で確認可能）。以下がこの Catalog の作成文です：
...
```
Iceberg Catalog でデータベースと Iceberg テーブルを作成します：
...
```
`iceberg_meta`テーブル関数を使用して、テーブルのスナップショット情報をクエリします：
...
```
テーブルを`Polars DataFrame`として読み込み：
...
└───────────┴─────────┴───────────────┴─────────────┴────────────────────┴─────────────────────┘
```
> PyIcebergでicebergテーブルを書き込むには、[step](#write-iceberg-table-by-pyiceberg)を参照してください
