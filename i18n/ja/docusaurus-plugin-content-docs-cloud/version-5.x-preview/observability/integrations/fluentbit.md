...
{
  "title": "FluentBit",
  "description": "Fluent Bitは高速なログプロセッサおよびフォワーダーで、ストレージシステムにデータを書き込むためのカスタム出力プラグインをサポートしており、Fluent Bit Doris出力プラグインはDorisに出力するためのプラグインです",
  "language": "ja"
}
---
[Fluent Bit](https://fluentbit.io/)は、カスタム出力プラグインをサポートしてストレージシステムにデータを書き込む高速ログプロセッサーおよびフォワーダーです。Fluent Bit Doris出力プラグインは、Dorisに出力するためのプラグインです。

Fluent Bit Doris出力プラグインは、[Doris Stream Load](../../../user-guide/data-operate/import/import-way/stream-load-manual) HTTPインターフェースを呼び出すことで、リアルタイムでDorisにデータを書き込みます。マルチスレッド並行処理、障害時の再試行、カスタムStream Load形式とパラメータ、および出力書き込み速度などの機能を提供します。

Fluent Bit Doris出力プラグインを使用するには、主に3つのステップがあります：
1. Doris出力プラグインが含まれるFluent Bitバイナリプログラムをダウンロードまたはコンパイルする。
...
Fluent Bit Doris 出力プラグインの設定は以下の通りです：

構成 | デスクリプション
--- | ---
`host` | Stream Load HTTP ホスト
`port` | Stream Load HTTP ポート
...
`label_prefix` | Doris Stream Load Label プレフィックス、最終的に生成される Label は *{label_prefix}\_{timestamp}\_{uuid}* 、デフォルト値は fluentbit。false に設定すると、Label は追加されません
 `time_key` | データに追加するタイムスタンプカラムの名前。デフォルト値は date。false に設定すると、このカラムは追加されません
`header` |  Doris Stream Load headers パラメータ、複数設定可能
`log_request` | トラブルシューティングのために Doris Stream Load のリクエストとレスポンスのメタデータをログに出力するかどうか、デフォルトは true
`log_progress_interval` | ログに速度を出力する時間間隔、単位は秒、デフォルトは 10、0 に設定するとこのタイプのログを無効にできます
`retry_limit` | Doris Stream Load リクエスト失敗時の再試行回数、デフォルト値は 1、false に設定すると再試行回数を制限しません
`workers` | Doris Stream Load を実行するワーカー数、デフォルト値は 2
...
FE ログファイルは通常、Doris インストールディレクトリ下の fe/log/fe.log ファイルに配置されています。これらは典型的な Java プログラムログで、タイムスタンプ、ログレベル、スレッド名、コード位置、ログ内容などのフィールドを含んでいます。通常のログだけでなく、スタックトレースを含む例外ログも含まれており、これらは複数行にわたります。ログ収集と保存では、メインログとスタックトレースを単一のログエントリに結合する必要があります。
...
GitHub events archiveには、GitHubユーザーアクションのアーカイブデータがJSON形式で含まれています。[ここ](https://data.gharchive.org/)からダウンロードできます。例えば、2024年1月1日午後3時のデータなどです。
