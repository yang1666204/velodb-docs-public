...
---
## 説明
この関数はJSON値から引用符を削除し、結果を文字列として返します。パラメータがNULLの場合、NULLを返します。
...
* 入力パラメータがダブルクォートで囲まれた値でない場合、値そのものを返します。
* 入力パラメータが文字列でない場合、自動的に文字列に変換されてから値そのものを返します。
