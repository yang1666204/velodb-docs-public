---
{
  ...
  "description": "地球表面の閉じた領域の面積を平方キロメートル単位で計算します。入力パラメータは幾何オブジェクト（Geopolygonなど",
  "language": "ja"
}
---
## Description
...
| -- |--------|
| `<geo>` | 地球表面上の幾何オブジェクトで、GeoPolygon、GeoCircle、GeoMultiCircle型である必要があり、閉じた領域を形成する必要があります。|

## 戻り値

平方キロメートル単位での領域の面積をDOUBLE型で返します。

ST_AREA_SQUARE_KMのエッジケース：

- 入力パラメータがNULLの場合、NULLを返します。
- 入力ジオメトリが無効な場合（例：自己交差、面積ゼロ）、NULLを返します。
- 入力が非閉鎖の幾何オブジェクト（GeoPoint、線分GeoLineStringなど）の場合、0を返します。
- 座標が有効な経度/緯度の範囲（経度 [-180, 180]、緯度 [-90, 90]）を超える場合、NULLを返します。
