...
MICROSECOND_TIMESTAMP関数は、入力された日時値を1970-01-01 00:00:00から計算されたUnixタイムスタンプ（ローカルタイムゾーンオフセットによって調整）に変換するために使用され、単位はマイクロ秒です（1秒 = 1,000,000マイクロ秒）。この関数はマイクロ秒精度のDATETIME型の処理をサポートし、変換時にタイムゾーンの違いを自動的に無視します（デフォルトの基準としてUTC時間を使用）。
...
|------------------|-----------------------------------------------------------------------------------------------|
| `<datetime>`     | Unix タイムスタンプに変換される日時を表します。`DATETIME` 型をサポートします。特定の日時フォーマットについては、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion) を参照してください |

## Return Value

入力された日時に対応するマイクロ秒単位の Unix タイムスタンプを表す `BIGINT` 整数を返します（現在のタイムゾーンに変換された総マイクロ秒）。
