...
{
  "title": "LPAD",
  "description": "LPAD関数（Left Padding）は、指定された長さに達するまで、文字列の左側を指定された文字でパディングします。",
  "language": "ja"
}
---
## 説明

LPAD関数（Left Padding）は、文字列の左側を指定された文字で埋めて、指定された長さに達するまでパディングします。対象の長さが元の文字列の長さより短い場合、文字列は切り詰められます。
...
パディングルール：
- len > 元の文字列長の場合：合計長がlenに達するまで、pad文字列で左側を繰り返しパディング
