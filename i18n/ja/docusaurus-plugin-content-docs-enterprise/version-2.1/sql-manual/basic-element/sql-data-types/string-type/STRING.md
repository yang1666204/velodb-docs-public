...
### Description
STRING (M)
可変長文字列です。デフォルトサポートは1048576バイト（1M）で、最大2147483643バイト（2G）まで調整可能です。Stringタイプの長さはbeのstring_type_length_soft_limit_bytes（文字列タイプ長のソフトリミット）設定によっても制限されます。Stringタイプは値列でのみ使用でき、キー列、パーティション列、バケット列では使用できません。
