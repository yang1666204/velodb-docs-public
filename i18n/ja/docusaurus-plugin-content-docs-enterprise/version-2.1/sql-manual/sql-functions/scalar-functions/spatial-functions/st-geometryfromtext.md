...
- `POLYGON` - 1つ以上のリングで定義される閉領域で、最低3つの異なる点と閉じた端点が必要
- `MULTIPOLYGON` - ポリゴンのコレクションで、multipolygon内のポリゴンは離散的な点のみを共有可能

:::info Note
Apache Doris 2.1.10以降、MULTIPOLYGON形式の解析がサポートされています
:::

## 戻り値

WKTに対応するジオメトリックストレージ形式のWKB

入力されたWKT形式が仕様に適合しない場合、または入力がNULLの場合はNULLを返します。
