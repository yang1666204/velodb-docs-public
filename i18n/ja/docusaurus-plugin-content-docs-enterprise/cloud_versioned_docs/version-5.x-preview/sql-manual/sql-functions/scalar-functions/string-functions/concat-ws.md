...
{
  "title": "CONCAT_WS",
  "description": "CONCATWS関数（Concatenate With Separator）は、指定されたセパレータを使用して複数の文字列または配列を連結します。CONCAT関数とは異なり、",
  "language": "ja"
}
---
## 説明

CONCAT_WS関数（Concatenate With Separator）は、指定された区切り文字を使用して複数の文字列または配列を連結します。CONCAT関数とは異なり、CONCAT_WSはNULL値を自動的にスキップし（ただし空文字列はスキップしません）、NULL以外の値の間に区切り文字を挿入します。この関数は文字列引数と配列引数の両方のモードをサポートしており、CSV形式の生成、パスの結合、タグリストなどのシナリオで非常に有用です。
