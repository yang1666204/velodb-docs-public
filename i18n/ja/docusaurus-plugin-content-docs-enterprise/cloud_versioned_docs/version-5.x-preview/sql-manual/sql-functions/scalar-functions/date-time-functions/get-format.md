...
この関数はMySQLの[GET_FORMAT function](https://dev.mysql.com/doc/refman/8.4/en/date-and-time-functions.html#function_get-format)と同じ動作をします。
...
フォーマット文字列を返します。異なる引数に対する結果を以下の表に示します：
...
第二引数が`'USA', 'JIS', 'ISO', 'EUR', 'INTERNAL'`のいずれでもない場合、またはNULLの場合、関数はNULLを返します。
