---
{
  "title": "DIGITAL_MASKING",
  "description": "DIGITALMASKING関数は、中間部分を固定フォーマットで置き換えることにより、数値文字列に対してマスキングを実行します。",
  "language": "ja"
}
---
## 説明

DIGITAL_MASKING関数は、数値文字列に対してマスキングを実行し、固定フォーマットで中間部分を`****`に置き換えます。最初の3桁と最後の4桁を保持します。`CONCAT(LEFT(id, 3), '****', RIGHT(id, 4))`と同等です。

## 構文

```sql
DIGITAL_MASKING(<digital_number>)
```
## パラメータ

| パラメータ | 説明 |
| ------------------ | ----------------------------------------- |
| `<digital_number>` | マスクする数値文字列。型: VARCHAR |

## 戻り値

VARCHAR型を返します。マスクされた数値文字列を表します。

特殊なケース:
- パラメータがNULLの場合、NULLを返します
- マスク形式: 最初の3桁 + `****` + 最後の4桁
- 文字列長が7桁未満の場合、結果が重複する可能性があります

## 例

1. 基本的な使用方法: 11桁の電話番号をマスクする

```sql
SELECT digital_masking('13812345678');
```
```text
+--------------------------------+
| digital_masking('13812345678') |
+--------------------------------+
| 138****5678                    |
+--------------------------------+
```
2. 異なる長さの数値

```sql
SELECT digital_masking('1234567890');
```
```text
+-------------------------------+
| digital_masking('1234567890') |
+-------------------------------+
| 123****7890                   |
+-------------------------------+
```
3. 短い数値（7桁未満）

```sql
SELECT digital_masking('123');
```
```text
+------------------------+
| digital_masking('123') |
+------------------------+
| 123****123             |
+------------------------+
```
4. NULL値の処理

```sql
SELECT digital_masking(NULL);
```
```text
+-----------------------+
| digital_masking(NULL) |
+-----------------------+
| NULL                  |
+-----------------------+
```
5. UTF-8文字を含む文字列

```sql
SELECT digital_masking('13812ṭṛ34678');
```
```text
+-------------------------------------+
| digital_masking('13812ṭṛ34678')     |
+-------------------------------------+
| 138****4678                         |
+-------------------------------------+
```
