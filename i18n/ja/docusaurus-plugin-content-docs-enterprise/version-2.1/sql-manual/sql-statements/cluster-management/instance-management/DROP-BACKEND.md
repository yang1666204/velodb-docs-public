...
1. このコマンドを使用してBEノードをオフラインにすることは推奨されません。このコマンドはBEノードをクラスタから直接削除します。現在のノード上のデータは他のBEノードに負荷分散されません。クラスタに単一レプリカテーブルがある場合、データ損失が発生する可能性があります。より良いアプローチは、[DECOMMISSION BACKEND](./DECOMMISSION-BACKEND.md)コマンドを使用してBEノードを適切にオフラインにすることです。
