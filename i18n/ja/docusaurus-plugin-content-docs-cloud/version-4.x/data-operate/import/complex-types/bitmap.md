...
{
  "title": "ビットマップ",
  "description": "BITMAP型はDuplicateTable、UniqueTable、およびAggregateTableで使用でき、Key型としてのみ使用可能で、Value列としては使用できません。",
  "language": "ja"
}
---
BITMAP型は、DuplicateTable、UniqueTable、AggregateTableで使用でき、Key型としてのみ使用可能で、Valueカラムとしては使用できません。AggregateTableでBITMAP型を使用する場合、Tableは集約型BITMAP_UNIONで作成する必要があります。ユーザーは長さとデフォルト値を指定する必要はありません。長さは、データ集約の度合いに基づいてシステムによって制御されます。詳細なドキュメントについては、Bitmapを参照してください。

## 使用例

### ステップ1: データの準備

以下のCSVファイルを作成します: test_bitmap.csv
...
以下では、stream loadを使用して複数の要素を含むbitmapカラムをインポートする2つの方法を説明します。ユーザーはソースファイルの形式に基づいて適切な方法を選択できます。

### bitmap_from_string

`bitmap_from_string`を使用する場合、ソースファイルのarrカラムでは角括弧は使用できません。使用した場合、データ品質エラーとして扱われます。
...
`bitmap_from_array`を使用する場合、ソースファイルのarr列に角括弧を含めることができます。ただし、stream loadでは、使用前に文字列型を配列型にキャストする必要があります。
