...
{
  "title": "ICEBERG_META",
  "description": "icebergmeta table-valued-function(tvf)は、icebergメタデータの読み取り、操作履歴、テーブルのスナップショット、ファイルメタデータなどに使用されます。",
  "language": "ja"
}
---
## Description
...
| フィールド | 説明 |
|------------|------|
| `<table>` | 表示したいIcebergテーブルの完全なテーブル名。`database_name.table_name`の形式で指定する必要があります。 |
...
- スナップショット用のicebergテーブルメタデータを読み取ってアクセスします。
...
- iceberg テーブルの履歴を表示する
    ...
    ```
- icebergテーブルのmetadata_log_entriesを表示する
...
Icebergシステムテーブルに関するより詳細な情報については、Iceberg Catalog System Tablesを参照してください。
