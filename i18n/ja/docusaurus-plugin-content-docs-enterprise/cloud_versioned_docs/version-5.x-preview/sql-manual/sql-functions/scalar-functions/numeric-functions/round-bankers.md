---
{
  "title": "ROUND_BANKERS",
  "description": "banker's rounding方式を使用してxを丸め、小数点以下d桁を保持します。dのデフォルト値は0です。",
  "language": "ja"
}
---
## 説明

banker's rounding法を使用してxを丸め、小数点以下d桁を保持します。dのデフォルト値は0です。

dが負の場合、小数点の左側の|d|桁が0に設定されます。

xまたはdがnullの場合、nullを返します。

dが列を表し、最初のパラメータがDecimal型の場合、結果のDecimalは入力のDecimalと同じ小数点以下の桁数を持ちます。

banker's roundingアルゴリズムの規則に従って、指定された小数点以下の桁に丸める場合：

- 丸める対象の数字が5で、その後に他の非ゼロの数字がない場合、その直前の数字が確認されます：
   - 直前の数字が偶数の場合、5は単純に削除されます。
   - 直前の数字が奇数の場合、数値は切り上げられます。

- 丸める対象の数字が5より大きいか、その後に非ゼロの数字がある場合、従来の丸め規則が適用されます：数字が5以上の場合は切り上げ、そうでなければ切り下げ。

例：

- 値2.5の場合、5の前の数字2は偶数なので、結果は2に丸められます。

- 値3.5の場合、5の前の数字3は奇数なので、結果は4に丸められます。

- 値2.51の場合、5の後の数字が0ではないため、直接切り上げられ、結果は3になります。

## 構文

```sql
ROUND_BANKERS(<x> [ , <d>])
```
## パラメータ

| Parameter | Description |
|-----------|------------|
| `<x>` | 丸める対象の数値 |
| `<d>` | オプション、丸める小数点以下の桁数。デフォルト値は0。 |

## 戻り値

整数または浮動小数点数を返します：

- デフォルトでは、パラメータ d = 0 の場合、banker's rounding アルゴリズムを使用して x を計算した整数を返します。

- d が負の値の場合、小数点の左側の最初の桁が0である整数を返します。

- x と d の両方が NULL の場合、NULL を返します。

- d が列を表し、x が Decimal 型の場合、同じ精度の浮動小数点数を返します。

## 例

```sql
select round_bankers(0.4);
```
```text
+--------------------+
| round_bankers(0.4) |
+--------------------+
|                  0 |
+--------------------+
```
```sql
select round_bankers(-3.5);
```
```text
+---------------------+
| round_bankers(-3.5) |
+---------------------+
|                  -4 |
+---------------------+
```
```sql
select round_bankers(-3.4);
```
```text
+---------------------+
| round_bankers(-3.4) |
+---------------------+
|                  -3 |
+---------------------+
```
```sql
select round_bankers(10.755, 2);
```
```text
+--------------------------+
| round_bankers(10.755, 2) |
+--------------------------+
|                    10.76 |
+--------------------------+
```
```sql
select round_bankers(10.745, 2);
```
```text
+--------------------------+
| round_bankers(10.745, 2) |
+--------------------------+
|                    10.74 |
+--------------------------+
```
```sql
select round_bankers(1667.2725, -2);
```
```text
+------------------------------+
| round_bankers(1667.2725, -2) |
+------------------------------+
|                         1700 |
+------------------------------+
```
```sql
SELECT number
, round_bankers(number * 2.5, number - 1) AS rb_decimal_column
, round_bankers(number * 2.5, 0) AS rb_decimal_literal
, round_bankers(cast(number * 2.5 AS DOUBLE), number - 1) AS rb_double_column
, round_bankers(cast(number * 2.5 AS DOUBLE), 0) AS rb_double_literal
FROM test_enhanced_round
WHERE rid = 1;
```
```text
+--------+-------------------+--------------------+------------------+-------------------+
| number | rb_decimal_column | rb_decimal_literal | rb_double_column | rb_double_literal |
+--------+-------------------+--------------------+------------------+-------------------+
|      1 |               2.0 |                  2 |                2 |                 2 |
+--------+-------------------+--------------------+------------------+-------------------+
```
