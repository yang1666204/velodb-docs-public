...
{
  "title": "Doris カタログ",
  "description": "Doris カタログは、Arrow Flightまたはバーチャルクラスターモードを介して複数のDorisクラスター間でのクロスクラスター連携分析を可能にし、設定、型マッピング、およびクエリ最適化ガイドを提供します。",
  "language": "ja"
}
---
...
* `use_arrow_flight`
  ...
  Arrow Flightを使用してリモートDorisクラスタにアクセスするか、リモートTableを内部Tableとして扱い実行プランをリモートDorisクラスタに送信して実行するかを指定します。
...
* `{CommonProperties}`
  ...
  CommonPropertiesセクションは、共通プロパティの入力に使用されます。データカタログ Overviewの[Common Properties]セクションを参照してください。
