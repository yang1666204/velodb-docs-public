---
{
  ...
  "description": "この文は、Routine Load ジョブの実行ステータスを表示するために使用されます。特定のジョブまたはすべてのジョブのステータス情報を表示できます。",
  "language": "ja"
}
---
## 説明

この文は、Routine Loadジョブの実行ステータスを表示するために使用されます。特定のジョブまたはすべてのジョブのステータス情報を表示できます。
...
| Progress            | ジョブ実行進捗                                         |
| Lag                 | ジョブ遅延ステータス                                            |
| ReasonOfStateChanged | ジョブステート変更理由                                 |
...
| 権限    | オブジェクト | 備考                                            |
| :----------- | :----- | :----------------------------------------------- |
| LOAD_PRIV    | テーブル  | SHOW ROUTINE LOADはテーブルに対するLOAD権限が必要 |
...
- test1という名前のすべてのroutine loadジョブ（停止またはキャンセルされたものを含む）を表示
    ...
    ```
- test1という名前で現在実行中のroutine load jobsを表示する
...
- example_db内のすべてのルーチンロードジョブ（停止またはキャンセルされたものを含む）を表示します。結果は1行または複数行になる場合があります。
    ...
    ```
- example_db で現在実行中のすべてのroutine loadジョブを表示する
...
- example_db内でtest1という名前の現在実行中のroutine load jobを表示する
    ...
    ```
- example_db内でtest1という名前のすべてのroutine loadジョブ（停止またはキャンセルされたものを含む）を表示します。結果は1行または複数行になる場合があります。
