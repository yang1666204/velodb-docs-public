---
{
  "title": "Doris と LakeSoul の使用",
  "description": "新しいオープンデータ管理アーキテクチャとして、Data Lakehouseはデータウェアハウスの高性能かつリアルタイム機能と",
  "language": "ja"
}
---
新しいオープンデータ管理アーキテクチャとして、Data Lakehouseはデータウェアハウスの高性能とリアルタイム機能を、データレイクの低コストと柔軟性と統合し、ユーザーがさまざまなデータ処理と分析のニーズをより便利に満たすことを支援します。企業のビッグデータシステムにおいて、ますます応用されています。

最近のバージョンにおいて、Apache Dorisはデータレイクとの統合を深化させ、成熟したData Lakehouseソリューションを発展させました。

- バージョン0.15以降、Apache DorisはHiveとIceberg外部Tableを導入し、データレイク用のApache Icebergとの組み合わせ機能を探求しました。
- バージョン1.2から、Apache Dorisは正式にMulti-Catalog機能を導入し、さまざまなデータソースに対する自動メタデータマッピングとデータアクセスを可能にし、外部データ読み取りとクエリ実行に対する数多くのパフォーマンス最適化を行いました。現在では、高速でユーザーフレンドリーなLakehouseアーキテクチャを構築する能力を完全に備えています。
- バージョン2.1において、Apache DorisのData Lakehouseアーキテクチャは大幅に強化され、主流のデータレイク形式（Hudi、Iceberg、Paimonなど）の読み書き機能を改善し、複数のSQL方言との互換性を導入し、既存システムからApache Dorisへのシームレスな移行を実現しました。データサイエンスと大規模データ読み取りシナリオにおいて、DorisはArrow Flight高速読み取りインターフェースを統合し、データ転送効率の100倍向上を実現しました。
...
- レイクへのリアルタイムデータ取り込み：LakeSoulのアーキテクチャを活用することで、データを高効率かつ低遅延でレイクに取り込むことができ、集約、重複排除、部分列更新を含むさまざまなデータ更新機能をサポートします。
- 高性能データ処理と分析：LakeSoulのバッチストリーム統合処理やスキーマ進化などの機能は、Dorisの強力なクエリエンジンとシームレスに統合でき、レイクデータに対する高速なクエリと分析応答を可能にします。

将来、Apache DorisはCDCストリーム同期や自動スキーマ進化など、LakeSoulのより高度な機能を段階的にサポートし、統一された高性能リアルタイムlakehouseプラットフォームを共同で構築します。

本記事では、Apache Doris + LakeSoulテスト・デモンストレーション環境を迅速にセットアップする方法を説明し、さまざまな機能の使用法を実演して、lakehouseアーキテクチャにおける両システムの統合と利点を紹介します。

詳細については、[LakeSoul カタログ](../catalogs/lakesoul-catalog.md)を参照してください。


## ユーザーガイド

本記事で言及されるすべてのスクリプトとコードは、次のアドレスから取得できます：[https://github.com/apache/doris/tree/master/samples/datalake/lakesoul](https://github.com/apache/doris/tree/master/samples/datalake/lakesoul)

### 01 環境準備

本記事では、Docker Composeを使用してデプロイメントを行い、以下のコンポーネントとバージョンを使用します：
...
以下に示すように、Dorisクラスタに`lakesoul`という名前のCatalogが作成されています（`SHOW CATALOGS`を使用して確認できます）。以下がこのCatalogの作成文です：
...
- シンプルクエリ
  ...
  ```
- パーティション pruning
DorisはLakeSoulでpartition pruningを実行でき、ネイティブ読み取りによってクエリプロセスを高速化できます。これは`explain verbose`で確認できます。
...
Flink CDC JobをmysqlTableをsyncするためにLaunchします。mysqlTableはdocker composeをlaunchする際にロードされます。
...
```
Flink CDC Jobが起動されます。`doris client`でlakesoulカタログを再作成することで、起動プロセスを確認できます。Flink CDC Jobが起動された後、`doris client`で同期中のLakeSoul CDCTableを確認できます。
