...
{
  "title": "DATE_ADD",
  "description": "DATEADD関数は、指定された日付または時刻の値に指定された時間間隔を加算し、計算結果を返すために使用されます。",
  "language": "ja"
}
---
## 説明

DATE_ADD関数は、指定された日付または時刻値に指定された時間間隔を加算し、計算結果を返すために使用されます。

- サポートされる入力日付型には、DATE、DATETIME（'2023-12-31'、'2023-12-31 23:59:59'など）が含まれます。
...
| -- | -- |
| `<date_or_time_expr>` | 処理される日付/時刻の値。サポートされる型: datetime または date 型、秒の最大精度は小数点以下6桁まで（例：2022-12-28 23:59:59.999999）。具体的な datetime と date のフォーマットについては、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion) と [date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion) を参照してください |
| `<expr>` | 加算される時間間隔、`INT` 型 |
...
- DATE 型が入力された場合、DATE を返します（日付部分のみ）；
- DATETIME 型が入力された場合、DATETIME を返します（日付と時刻を含む）；
- スケール付きの入力（'2024-01-01 12:00:00.123' など）はスケールを保持し、最大で小数点以下6桁まで。

特別なケース：
- いずれかのパラメータが NULL の場合、NULL を返します；
- 不正な単位や数値以外の expr の場合、エラーを返します；
- 計算結果が日付型の範囲を超える場合（'0000-00-00 23:59:59' より前または '9999-12-31 23:59:59' より後など）、エラーを返します。
- 入力された日付に対して翌月に十分な日数がない場合、自動的に翌月の最終日に設定されます。
