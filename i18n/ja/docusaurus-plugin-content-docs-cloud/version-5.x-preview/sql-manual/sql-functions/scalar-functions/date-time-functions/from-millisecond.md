...
FROM_MILLISECOND関数は、Unixタイムスタンプ（ミリ秒）をDATETIME型の日時値に変換するために使用されます。Unixタイムスタンプの基準時刻は1970-01-01 00:00:00 UTCであり、この関数は入力されたミリ秒をその基準時刻以降の対応する具体的な日付と時刻に変換します（ミリ秒まで正確）。
...
- millisecondが有効範囲を超える場合（結果のdatetimeが9999-12-31 23:59:59を超える場合）、関数はエラーを返す。
- 入力されたmillisecondが整数の秒に変換できる場合、結果はスケールなしのdatetimeを返す；変換できない場合、結果はスケールありのdatetimeを返す
