...
- ソースデータの列とターゲットテーブルの列の順序が一致しない場合
- ソースデータの列とターゲットテーブルの列の数が一致しない場合
...
- **ステップ2：Column MappingとAssignment** - 列名により中間変数をターゲットテーブルのフィールドにマッピングする
...
4,\N,chongqing,1.4
```
ターゲットテーブルには4つのカラムがあります：k1、k2、k3、k4。以下のようにカラムをマッピングしたいと思います：
...
4,\N,chongqing,1.4
```
ターゲットテーブルには3つの列があります：k1、k2、k3。ソースファイルから必要なのは1番目、2番目、4番目の列のみで、以下のマッピング関係があります：
...
column4 -> k3
```
ソースファイル内の特定の列をスキップするには、列マッピング時にターゲットテーブルに存在しない任意の列名を使用できます。これらの列名はカスタマイズ可能で、制限はありません。これらの列のデータは、ロード中に自動的に無視されます。
...
FROM KAFKA (...);
```
> 注意: 例における `tmp_skip` は、対象テーブルの列定義に含まれていない限り、任意の名前に置き換えることができます。
...
4,\N,chongqing,1.4
```
ターゲットテーブルには5つのカラムがあります：k1、k2、k3、k4、k5。ソースファイルからは1番目、2番目、3番目、4番目のカラムのみが必要で、以下のマッピング関係があります：
...
4,\N,chongqing,1.4
```
ターゲットテーブルは4つのカラムを持ちます：k1、k2、k3、k4。以下のようにカラムの値を変換したいと考えています：
...
4,\N,chongqing,1.4
```
対象テーブルには4つの列があります：k1、k2、k3、k4。列の値を次のように変換したいと思います：
...
4,\N,chongqing,1.4
```
ターゲットテーブルには4つの列があります：k1、k2、k3、k4。列の値を以下のように変換したいと思います：
...
例えば、ソースデータに複数のテーブルのデータが含まれている場合（または複数のテーブルのデータが同じKafkaメッセージキューに書き込まれている場合）。データの各行には、そのデータがどのテーブルに属するかを示すカラムがあります。ユーザーはpre-filtering条件を使用して、ロード用の対応するテーブルデータを除外できます。
...
4,\N,chongqing,1.4
```
対象テーブルには4つの列があります：k1、k2、k3、k4。これらの列の値を以下のように変換したいと思います：
...
4,\N,chongqing,1.4
```
対象テーブルには4つの列があります：k1、k2、k3、k4。k1がNULLで、k4が1.2未満のデータを除外したいと思います。
