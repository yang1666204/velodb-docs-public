...
{
  "title": "DROP BACKEND",
  "description": "この文は、DorisクラスターからBEノードを削除するために使用されます。",
  "language": "ja"
}
---
## 説明
...
1. BEノードをオフラインにするために、このコマンドを使用することは推奨されません。このコマンドは、BEノードを直接クラスターから削除します。現在のノード上のデータは、他のBEノードに負荷分散されません。クラスター内に単一レプリカのテーブルがある場合、データ損失が発生する可能性があります。より良いアプローチは、[DECOMMISSION BACKEND](./DECOMMISSION-BACKEND.md)コマンドを使用して、BEノードを適切にオフラインにすることです。
2. この操作は高リスクな操作であるため、このコマンドを直接実行する場合：
