...
DATE_FORMAT関数は、指定されたフォーマット文字列（`format`）に従って、日付または時刻の値を文字列に変換するために使用されます。この関数はDATE（日付のみ）およびDATETIME（日付と時刻）タイプのフォーマットをサポートし、出力結果は指定されたフォーマットに準拠した文字列となります。

この関数は、MySQLの[date_format function](https://dev.mysql.com/doc/refman/8.4/en/date-and-time-functions.html#function_date-format)と互換性があります。
...
| パラメータ | 説明 |
| -- | -- |
| `<date_or_time_expr>` | 有効な日付値。datetime型またはdate型をサポート。具体的なdatetimeとdateの形式については、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)および[date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion)を参照してください |
| `<format>` | 日付/時刻の出力形式を指定。`varchar`型 |
...
| %b     | 短縮された月名（3文字）                           |
| %c     | 月の数値（0-12）                           |
| %D     | 英語の接尾辞付きの月の日（0th, 1st, 2nd, 3rd, …） |
| %d     | 月の日の数値（00-31）                |
| %e     | 月の日の数値（0-31）                 |
...
| %U     | 週（00-53）、日曜日が週の最初の日、[week](./week)モード0   |
| %u     | 週（00-53）、月曜日が週の最初の日、[week](./week)モード1   |
| %V     | 週（01-53）、日曜日が週の最初の日、[week](./week)モード2、%Xと併用 |
| %v     | 週（01-53）、月曜日が週の最初の日、[week](./week)モード3、%xと併用 |
| %W     | 完全な曜日名（Sunday-Saturday）    |
| %w     | 曜日（0 = Sunday, 6 = Saturday）        |
| %X     | 年、日曜日が週の最初の日（4桁）、%Vと併用 |
| %x     | 年、月曜日が週の最初の日（4桁）、%vと併用 |
| %Y     | 年（4桁）                            |
| %y     | 年（2桁）                            |
| %%     | %文字を表す                         |
| %**x** | 上記に記載されていない任意の**x**について、**x**自体を表す |
