---
{
  ...
  "description": "Dictionary は、JOIN操作を高速化するためにDorisが提供する特別なデータ構造です。これは通常のテーブルをベースに構築されます。",
  "language": "ja"
}
---
## 概要

Dictionaryは、JOIN操作を高速化するためにDorisが提供する特別なデータ構造です。通常のテーブルを基盤として構築され、元のテーブルの対応する列をキー・バリューの関係として扱い、これらの列のすべてのデータを事前にメモリにロードして高速な検索操作を実現し、クエリパフォーマンスを向上させます。頻繁なキー・バリュー検索が必要なシナリオに特に適しています。

当然ながら、キー・バリュー検索ソリューションとして、dictionaryテーブルは重複キーを許可しません。
...
元々LEFT OUTER JOINを使用して実装する必要があったキー・バリュー検索は、dictionaryテーブルの助けを借りて完全に排除でき、通常の関数呼び出しに変換できます。以下は完全なシナリオの例です：

### シナリオの例

eコマースシステムでは、注文テーブル（`orders`、ファクトテーブル）が大量の取引データを記録し、詳細な商品情報を取得するために商品テーブル（`products`、ディメンションテーブル）と頻繁に関連付けを行う必要があります。
...
```
以下は典型的なクエリのセットです。各カテゴリの注文数量と売上を集計するために、過去には商品テーブルから商品情報を取得する機能を実現するためにLEFT OUTER JOINを使用する必要がありました。
...
キーバリューペアの関係を設定し、対応する辞書テーブルを事前に構築することで、以前のJOIN操作を完全により軽量なキーバリュールックアップに変換でき、それによりSQLの実行効率を向上させることができます：
...
- `<key_column>`: キーとして機能するソーステーブル内のカラム名
- `<value_column>`: 値として機能するソーステーブル内のカラム名
...
- `HASH_MAP`: ハッシュテーブルベースの実装で、一般的なキー-値検索シナリオに適しています。
...
|-|-|-|-|
|`date_lifetime`|整数、単位は秒|データの有効期間。この辞書の最終更新からの経過時間がこの値を超え、ソーステーブルにデータ変更がある場合、自動的にインポートを開始します。インポートロジックの詳細は[Automatic Import](#automatic-import)を参照|はい|
...
```
テーブルに基づいて、`source_table`内の`city`値に基づいて対応する`id`をクエリするために、`dict_get`関数を通じて辞書`city_dict`を使用できます。
...
Dorisは`dictionary_auto_refresh_interval_seconds`秒ごとにすべての辞書データの有効期限を確認します。辞書が`data_lifetime`秒以上更新されておらず、前回のインポートと比較してソーステーブルデータが変更されている場合、Dorisはその辞書のインポートを自動的に送信します。

一部のBEでデータが不足しており、前回のインポートと比較してソーステーブルデータが変更されていない場合、Dorisは対応するBEの現在のバージョンのデータのみを補完し、すべてのBEに対してリフレッシュタスクを送信せず、辞書のバージョンは変更されません。
...
単一のKey、Valueリストおよび複数のKey、Valueリストの辞書テーブルクエリには、それぞれ`dict_get`および`dict_get_many`関数を使用できます。
...
3. 辞書テーブルの削除
    ...
    ```
辞書テーブルを削除した後、削除された辞書がBEからすぐに削除されない場合があります。
...
`SHOW DICTIONARIES`ステートメントを使用することで、辞書に対応するベーステーブル、現在のデータバージョン番号、およびFEとBEでの対応するステータスを確認できます：
...
辞書テーブルの列定義を確認するには、`DESC DICTIONARY` を使用できます。例:
...
1. データ整合性
   ...
   - Dorisは辞書テーブルとベーステーブル間で強いデータ整合性を維持しません。ユーザーは辞書の`data_lifetime`を適切に設定して自動更新を実現し、ビジネスロジックに基づいて必要に応じて手動更新する必要があります。
   - ソーステーブルが何らかの方法で削除された場合、対応する辞書テーブルも自動的に削除されます。

2. パフォーマンスに関する考慮事項

   - 辞書テーブルは、ディメンションテーブルデータなどの比較的静的なデータに適しています。

   - 辞書テーブルは純粋なインメモリテーブルで、すべてのBEのメモリに完全なデータが格納され、大量のメモリを占有する可能性があるため、メモリ使用量とクエリパフォーマンスを考慮して辞書を派生させる適切なテーブルを選択する必要があります。
...
3. ベストプラクティス
   ...
   3. 状態管理：
      ...
      - 辞書テーブルを使用する際は、BEメモリ監視に注意して、辞書テーブルが多すぎたり大きすぎたりして過度にメモリを占有し、BE状態の異常を引き起こすことを防ぐ
