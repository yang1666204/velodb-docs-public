...
3. 同期マテリアライズドビューを持つTableでは部分列更新はサポートされていません。
4. スキーマ変更を実行中のTableでは部分列更新は許可されていません。
:::
...
Dorisに注文Table`order_tbl`があると仮定します。ここで、注文idがKey列、注文ステータスと注文金額がValue列です。データの状態は以下のとおりです：
