...
| FRAGMENT  | 実行フラグメント。Dorisは分散実行エンジンであるため、完全な実行プランは複数の単一ノード実行フラグメントに分割されます。FRAGMENTテーブルは完全な単一ノード実行フラグメントを表します。複数のFRAGMENTが組み合わされて完全なPLANを形成します。 |
...
| tbl  | tupleの対応するテーブル、または適用されない場合は`null`  |
