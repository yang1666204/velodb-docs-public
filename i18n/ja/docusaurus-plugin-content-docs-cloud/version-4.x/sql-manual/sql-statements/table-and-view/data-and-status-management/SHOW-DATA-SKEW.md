...
- パーティション化されたテーブルと非パーティション化されたテーブルの両方をサポートします。
...
> 表示対象のテーブル名を指定します。データベース名を含めることができます。
>
...
>
> 非パーティション化テーブルの場合、パーティション名はテーブル名と同じになります。
...
| BucketIdx        | Bucketインデックス番号               |
| AvgRowCount      | 平均行数                             |
| AvgDataSize      | 平均データサイズ（バイト単位）       |
| Graph            | データ分散の可視化チャート           |
| Percent          | 総データ量に対するこのbucketのデータ量の割合 |
...
| 権限            | オブジェクト | 備考                                         |
| :-------------- | :---------- | :-------------------------------------------- |
| SELECT          | Table       | テーブルを表示するためにSELECT権限が必要です。 |
...
- 非パーティション化テーブルの場合、クエリ結果のパーティション名はテーブル名と同じになります。

## 例

- パーティション化テーブルを作成してそのデータ分散を表示する：
