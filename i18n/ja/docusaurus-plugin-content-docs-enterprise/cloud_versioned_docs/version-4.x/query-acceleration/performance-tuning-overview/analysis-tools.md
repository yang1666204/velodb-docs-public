...
実行プランは、SQLクエリの特定の実行方法とプロセスを記述します。たとえば、2つのテーブルを結合するSQLクエリの場合、実行プランは、テーブルへのアクセス方法、結合方法、結合順序などの情報を表示します。
