---
{
  "title": "processlist",
  "description": "現在のすべての接続を表示",
  "language": "ja"
}
---
# processlist

## 概要

すべての現在の接続を表示する

## データベース

`information_schema`

## テーブル情報

| Column Name       | Type           | Description                          |
| ----------------- | -------------- | ------------------------------------ |
| CURRENT_CONNECTED | varchar(16)    | 非推奨、常にNoとなる                 |
| ID                | largeint       | 接続ID                               |
| USER              | varchar(32)    | 接続ユーザー                         |
| HOST              | varchar(261)   | 接続アドレス                         |
| LOGIN_TIME        | datetime       | ログイン時刻                         |
| CATALOG           | varchar(64)    | 現在のCatalog                        |
| DB                | varchar(64)    | 現在のDatabase                       |
| COMMAND           | varchar(16)    | 現在送信されているMySQLコマンドの種類 |
| TIME              | int            | 最後のクエリの実行時間               |
| STATE             | varchar(64)    | 最後のクエリのステータス             |
| QUERY_ID          | varchar(256)   | 最後のクエリのID                     |
| INFO              | varchar(65533) | 最後のクエリのクエリ文               |
| FE                | varchar(64)    | 接続されているFront-End（FE）        |
| CLOUD_CLUSTER     | varchar(64)    | 使用されているCloud Clusterの名前    |
