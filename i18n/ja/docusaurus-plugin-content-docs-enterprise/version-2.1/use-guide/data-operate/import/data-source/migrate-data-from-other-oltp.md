...
---
MySQL/SqlServer/Oracle などの他の TP システムから Doris にデータを移行するには、さまざまな方法があります。

## Multi-Catalog

Catalog を使用して外部テーブルとしてマッピングし、INSERT INTO または CREATE-TABLE-AS-SELECT ステートメントを使用してデータロードを完了します。
...
Flinkを活用してTPシステムのオフライン同期およびリアルタイム同期を実現できます。
...
- FlinkCDCを使用してフルデータと増分データの両方を読み取ることで、リアルタイム同期を実現できます。例えば、FlinkSQLを使用する場合：
  ...
  ```
TP データベースでデータベース全体または複数のテーブルを同期する場合、Flink Doris Connector が提供するフルデータベース同期機能を使用してワンクリックで TP データベースへの書き込みを完了できます。以下に示します：
...
さらに、データ同期にはサードパーティの同期ツールも使用できます。詳細については、以下を参照してください：
- DataX
- Seatunnel
- CloudCanal
