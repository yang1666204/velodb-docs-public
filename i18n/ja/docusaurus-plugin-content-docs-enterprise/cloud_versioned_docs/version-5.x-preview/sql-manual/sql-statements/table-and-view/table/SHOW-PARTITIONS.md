...
フィルタ条件。PartitionId、PartitionName、State、Buckets、ReplicationNum、LastConsistencyCheckTime等のカラムでのフィルタリングをサポートします。

注意点：
1. 現在、where句は文字列型のPartitionName、Stateに対してのみ`=`、`!=`、`like`演算子をサポートします。その他のカラムについては`=`、`!=`、`>`、`<`、`>=`、`<=`等の演算子のみをサポートします。
2. where句で上記の演算子を使用する際、カラム名は左側に配置する必要があります。
3. Where句は`AND`を含むことができます。

**4. `<order_by_key>`**

ソート条件。PartitionId、PartitionName、State、Buckets、ReplicationNum、LastConsistencyCheckTime等のカラムでのソートをサポートします。
...
## アクセス制御要件
表示するテーブルのSHOW権限が必要です。



## 例

1. 指定されたデータベース内の指定されたテーブルの全ての非一時パーティション情報を表示します。
