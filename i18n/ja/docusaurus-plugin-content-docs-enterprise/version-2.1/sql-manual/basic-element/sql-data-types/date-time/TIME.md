...
TIME型
    時間型です。クエリ結果として表示されることがありますが、現時点ではTableストレージはサポートされていません。ストレージ範囲は`[-838:59:59, 838:59:59]`です。
    現在Dorisでは、計算結果としてのTIMEの正確性は保証されています（例：`timediff`などの関数）が、**手動でのCAST によるTIME型の生成は推奨されません**。
