...
TIME_FORMAT関数は、指定されたフォーマット文字列に従って時刻値を文字列に変換するために使用されます。TIMEおよびDATETIME型のフォーマットをサポートし、出力はフォーマット要件に準拠した文字列になります。
...
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<time_or_datetime_expr>` | 有効な時間値で、TIMEまたはDATETIME型をサポートします。                                                                                                     |
| `<format>`                | 時間の出力フォーマットを`varchar`型として指定します。フォーマット文字列に日付または非準拠のフォーマット指定子が含まれている場合、NULLを返します。 |
...
| %l        | 時 (1-12)                                   |
| %p        | AMまたはPM                                      |
| %r        | 時間、12時間制 (hh:mm:ss followed by AM or PM) |
| %S        | 秒 (00-59)                               |
| %s        | 秒 (00-59)                               |
| %T        | 時間、24時間制 (hh:mm:ss)                      |
| %%        | リテラルの%文字を表します              |
...
- 時間値が23より大きい時間部分を含む場合、`%H`と`%k`フォーマット指定子は23より大きい値を生成します。その他の時間フォーマット指定子は、時間値を12で割った余りを生成します。
...
+------+-------------------+------+------+------+------+------+------+------+------+---------+------+--------------+-----------+------------------+-----------------+-----------------------------+--------------------+------------------------------------+
```
> 注意: パラメータ`838:59:59.999999`と`-838:59:59.999999`は、TIME範囲[-838:59:59, 838:59:59]を超えているため、TIME型に変換できず、返される結果はNULLになります。

```

```sql
-- %Y、%y、%m、%d、%c、%eのプレースホルダーは対応する桁数0を返します
