---
{
  "title": "RANDOM_BYTES",
  "description": "RANDOMBYTES関数は、指定された長さのランダムなバイトシーケンスを生成します。返されるバイトシーケンスは16進文字列として表現されます。",
  "language": "ja"
}
---
## 説明

RANDOM_BYTES関数は、指定された長さのランダムなバイトシーケンスを生成します。返されるバイトシーケンスは16進文字列として表現されます。

## 構文

```sql
RANDOM_BYTES(<len>)
```
## パラメータ

| パラメータ | 説明 |
| -------- | ----------------------------------------- |
| `<len>` | 生成するランダムバイトの数。型: INT |

## 戻り値

VARCHAR型を返します。16進数エンコードされたランダムバイトシーケンス（`0x`プレフィックス付き）。

特別なケース:
- `<len>`は0より大きい値である必要があります。そうでない場合はエラーを返します
- パラメータがNULLの場合、NULLを返します
- 各呼び出しでランダムな結果を生成します

## 例

1. 基本的な使用法: 8バイトのランダムシーケンスを生成

```sql
SELECT random_bytes(8);
```
```text
+--------------------+
| random_bytes(8)    |
+--------------------+
| 0x1a2b3c4d5e6f7089 |
+--------------------+
```
2. 短いシーケンスを生成する

```sql
SELECT random_bytes(4);
```
```text
+----------------+
| random_bytes(4) |
+----------------+
| 0xab12cd34     |
+----------------+
```
3. 無効なパラメータ：負の数

```sql
SELECT random_bytes(-1);
```
```text
ERROR 1105 (HY000): errCode = 2, detailMessage = (10.16.10.3)[INVALID_ARGUMENT]argument -1 of function random_bytes at row 0 was invalid.
```
4. NULL値の処理

```sql
SELECT random_bytes(NULL);
```
```text
+--------------------+
| random_bytes(NULL) |
+--------------------+
| NULL               |
+--------------------+
```
5. より長いシーケンスを生成する（16バイト）

```sql
SELECT random_bytes(16);
```
```text
+------------------------------------+
| random_bytes(16)                   |
+------------------------------------+
| 0x1a2b3c4d5e6f708192a3b4c5d6e7f809 |
+------------------------------------+
```
### キーワード

    RANDOM_BYTES
