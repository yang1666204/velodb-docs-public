...
1. **構文説明:**
   ...
   2. select_expr, ... 取得対象テーブル（サブクエリによって生成された一時テーブルを含む1つ以上のテーブル）
   ...
   10. `Having`句はテーブル内の行データをフィルタリングするのではなく、集約関数によって生成された結果をフィルタリングします。

       通常`having`は集約関数（例：`COUNT(), SUM(), AVG(), MIN(), MAX()`）と`group by`句と一緒に使用されます。

   11. SELECTは`table_reference`内のテーブル名に続くパーティションまたはサブパーティション（またはその両方）のリストを含むPARTITIONを使用した明示的なパーティション選択をサポートします

   12. `[TABLET tids] TABLESAMPLE n [ROWS | PERCENT] [REPEATABLE seek]`: FROM句のテーブルから読み取る行数を制限し、指定された行数または割合に従ってテーブルから疑似ランダムに複数のTabletを選択し、REPEATABLEでシード数を指定して選択されたサンプルを再度返します。さらに、TableIDを手動で指定することもできます。これはOLAPテーブルでのみ使用できることに注意してください。
...
1. SELECTは任意のテーブルを参照せずに計算行を取得するためにも使用できます。
...
共通テーブル式を指定するには、1つ以上のカンマ区切りの句を持つ`WITH`句を使用します。各副句は、結果セットを生成するサブクエリを提供し、名前をサブクエリに関連付けます。次の例では、`cte1`と`cte2`という名前のCTEで`WITH`句を定義し、それらのトップレベル`SELECT`の下で`WITH`句を参照しています：
...
1. SELECT句に関する追加知識
   ...
   - FROMの後のtable_referencesは、クエリに参加する1つ以上のテーブルを示します。複数のテーブルがリストされている場合、JOIN操作が実行されます。そして、指定された各テーブルについて、エイリアスを定義できます。
