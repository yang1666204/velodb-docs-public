---
{
  ...
  "description": "この文は、指定されたdbのプロパティを設定し、db名を変更し、dbに対して様々なクォータを設定するために使用されます。",
  "language": "ja"
}
---
## 説明

このステートメントは、指定されたdbのプロパティを設定し、db名を変更し、dbに対して様々なクォータを設定するために使用されます。
...
データベース名を変更した後、必要に応じてREVOKEコマンドとGRANTコマンドを使用して、対応するユーザー権限を変更してください。データベースのデフォルトデータ容量クォータは1024 GBで、デフォルトレプリカ数クォータは1073741824です。
...
- 指定されたデータベースのコピー数にクォータを設定する
  ...
  ```
- db配下のテーブルのデフォルトレプリカ配布戦略を変更する（この操作は新しく作成されるテーブルに対してのみ有効であり、db配下の既存のテーブルは変更されません）
