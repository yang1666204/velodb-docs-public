...
ラムダ式を満たす配列内の最後の要素の位置インデックス（1から開始）を検索します。条件を満たす最後の要素を検索し、その位置インデックスを返します。
...
- `lambda`：検索条件を定義するために使用するlambda式
...
- 空の配列は0を返します。入力パラメータがNULL配列でlambda式がない場合、0を返します。入力パラメータがNULL配列でlambda式がある場合、エラーになります
- 複数の配列パラメータがある場合、すべての配列は同じ長さである必要があります
- Lambdaは任意のスカラー式を使用できますが、集計関数は使用できません
...
+----------------------------------------+
```
NULLの配列とラムダ式の組み合わせ。NULLを含むラムダ式がある場合、エラーになります。ラムダ式がない場合は、0を返します：
...
```
null値を含む配列で、lambdaはnullをチェックできます：
...
+-------------------------------------------------------------------+
```
マルチ配列検索、最初の配列が2番目の配列より大きい最後の要素の位置インデックスを見つける：
...
+-------------------------------------------------------------------------------+
```
ネストされた配列の検索で、各サブ配列の長さが2より大きい最後の要素の位置インデックスを見つける：
...
+--------------------------------------------------------------------+
```
Map型検索で、キー'a'の値が10より大きい最後の要素の位置インデックスを見つける：
