...
{
  "title": "SECONDS_ADD",
  "description": "この関数は、指定されたdatetime値に対して指定された秒数を加算または減算し、結果のdatetimeを返します。",
  "language": "ja"
}
---
## 説明
...
|--------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|
| `<datetime>` | 必須。入力するdatetime値。DATETIMEおよびDATE型をサポートします。                                                                                     |
| `<seconds>`  | 必須。追加または減算する秒数。整数（INT）をサポートします。正の数は秒を追加し、負の数は秒を減算します。                                           |

## Return Value
- 入力した`<datetime>`と同じ型のdatetime値を返します。
