---
{
  "title": "IPV6_NUM_TO_STRING | Ip Functions",
  "description": "ipv6numtostringの別名。",
  "language": "ja"
}
---
# IPV6_NUM_TO_STRING

## inet6_ntoa

`ipv6_num_to_string`のエイリアス。

## 説明
IPv6アドレスを標準のテキスト表現に変換し、このアドレスをテキスト形式で表した文字列を返します。

## 構文

```sql
IPV6_NUM_TO_STRING(<ipv6_num>)
```
### パラメータ
- `<ipv6_num>`: IPv6型カラムの値、または長さ16のバイナリ文字列

### 戻り値
戻り値の型: VARCHAR

戻り値の意味:
- IPv6のテキスト表現を返す
- 入力パラメータがNULLの場合、NULLを返す
- 無効な入力（空文字列、長さが16でないバイナリ文字列など）の場合、`NULL`を返す

### 使用上の注意
- `ipv6_num_to_string`と一貫して動作する: 長さが16でないバイナリ入力はNULLを返す
- MySQLの`INET6_NTOA`構文との互換性のために一般的に使用される

## 例

16バイトのバイナリ（`unhex`で構築）をIPv6テキストに変換する。

```sql
select ipv6_num_to_string(unhex('2A0206B8000000000000000000000011')) as addr;
+--------------+
| addr         |
+--------------+
| 2a02:6b8::11 |
+--------------+
```
パラメータがNULLの場合、NULLを返します

```sql
select ipv6_num_to_string(NULL) as null_result;
+-------------+
| null_result |
+-------------+
| NULL        |
+-------------+
```
無効なバイナリ入力（16バイト以外）はNULLを返します。

```sql
select ipv6_num_to_string('-23vno12i34nlfwlsj') as invalid;
+----------+
| invalid  |
+----------+
| NULL     |
+----------+
```
### Keywords

INET6_NTOA, IPV6_NUM_TO_STRING
