...
`QUANTILE_PERCENT`関数は、指定されたパーセンテージの分位数値を計算するために使用されます。この関数は2つのパラメータを受け取ります：quantile_state列とパーセンテージを表す定数の浮動小数点数です。この関数は、指定されたパーセンテージ位置での分位数値を表す浮動小数点数を返します。
