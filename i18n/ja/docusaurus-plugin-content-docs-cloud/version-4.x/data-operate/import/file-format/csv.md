...
{
  "title": "CSV",
  "description": "この文書では、CSV形式のデータファイルをDorisに読み込む方法について説明します。Dorisは、カスタム区切り文字を含む柔軟なCSV形式の設定をサポートしており、",
  "language": "ja"
}
---
...
|-----------|--------------|-------------|--------------|--------------|------------|-----|
| Line Delimiter | `\n` | line_delimiter | LINES TERMINATED BY | サポートしていません | LINES TERMINATED BY | line_delimiter |
...
| Skip Lines | 0 | skip_lines | PROPERTIES.skip_lines | サポートしていません | IGNORE LINES | skip_lines |
| Trim Double Quotes | false | trim_double_quotes | サポートしていません | PROPERTIES.trim_double_quotes | サポートしていません | trim_double_quotes |
| Compression Format | plain | compress_type | PROPERTIES.compress_type | サポートしていません | サポートしていません | compress_type |
