...
{
  "title": "ARRAY_SORT",
  "description": "lambda関数が指定されていない場合、配列要素は昇順にソートされます。それ以外の場合は、",
  "language": "ja"
}
---
## Function

lambda関数が指定されていない場合、配列要素は昇順でソートされます。指定されている場合は、lambda関数に従ってソートが実行されます。
...
- `lambda`: ソートルールを定義するために使用される`lambda`式で、戻り値は-1、0、または1である必要があります（それぞれ未満、以下または等しい、より大きいを表します）。
...
- `lambda`式を含む場合
  - `lambda`式の値は-1、0、または1である必要があります。`NULL`値が存在する場合は、`NULL`値を配列の前に配置するか後に配置するかを明示的に指定する必要があります。

## Examples

1. 基本: `NULL`要素は返される配列の先頭に配置されます
