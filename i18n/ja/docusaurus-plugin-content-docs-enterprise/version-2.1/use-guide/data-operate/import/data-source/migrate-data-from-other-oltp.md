...
カタログ を使用して外部Tableとしてマッピングし、INSERT INTO または CREATE-TABLE-AS-SELECT ステートメントを使用してデータロードを完了します。
...
- FlinkCDCを使用してフルデータと増分データの両方を読み取ることで、リアルタイム同期を実現できます。例えば、FlinkSQLを使用する場合：
  ...
  ```
TP データベースでデータベース全体または複数のTableを同期する場合、Flink Doris Connector が提供するフルデータベース同期機能を使用してワンクリックで TP データベースへの書き込みを完了できます。以下に示します：
