---
{
  ...
  "description": "posexplodeouter表関数は<array>列を複数行に展開し、位置を示す列を追加して、STRUCT型を返します。",
  "language": "ja"
}
---
## 説明
`posexplode_outer`テーブル関数は、`<array>`列を複数の行に展開し、位置を示す列を追加して、[`STRUCT`](../../basic-element/sql-data-types/semi-structured/STRUCT.md)型を返します。
Lateral Viewと組み合わせて使用し、複数のLateral Viewをサポートします。
...
- 単一列、複数行のSTRUCTデータを返します。STRUCTは2つの列で構成されます：
    1. 0から開始して1ずつ増加し、n – 1まで続く整数の列。ここでnは結果行数を表します。
    2. `<array>`のすべての要素を含む列。
- `<array>`がNULLまたは空の配列（要素数が0）の場合、NULLを含む1行が返されます。

## 使用上の注意
1. `<array>`はNULLや他の型にすることはできません。そうでなければエラーが報告されます。
