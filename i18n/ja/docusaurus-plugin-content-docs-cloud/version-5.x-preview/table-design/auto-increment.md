---
{
  ...
  "description": "Dorisでは、auto increment列は一意の数値を自動的に生成する機能です。",
  "language": "ja"
}
---
Dorisでは、auto increment列は一意の数値を自動生成する機能で、主キーなど各データ行の一意識別子を作成するために一般的に使用されます。新しいレコードが挿入されるたびに、auto increment列は自動的にインクリメント値を割り当て、手動で数値を指定する必要がなくなります。DorisのAUTO_INCREMENT列を使用することで、データの一意性と整合性が保証され、データ挿入プロセスが簡素化され、人的エラーが削減され、データ管理効率が向上します。これにより、auto increment列は、ユーザーIDなどの一意識別子が必要なシナリオに最適な選択肢となります。

## 機能

auto-increment列を持つTableに対して、Dorisは以下のようにデータ書き込みを処理します：

- **自動入力（列が除外されている場合）**：
  書き込まれるデータにauto-increment列が含まれていない場合、Dorisはこの列に一意の値を生成して入力します。

- **部分指定（列が含まれている場合）**：

  - **Null値**：Dorisは書き込まれるデータ内のnull値をシステム生成の一意の値で置き換えます。

  - **非Null値**：ユーザー提供の値はそのまま維持されます。
  
  :::caution Attention
  ユーザー提供の非null値は、auto-increment列の一意性を損なう可能性があります。
  :::
  
### 一意性

Dorisは、auto-increment列でDorisが生成する値について**Table全体の一意性**を保証します。ただし：

- **保証される一意性**：これはシステム生成値のみに適用されます。
- **ユーザー提供値**：Dorisは、ユーザーがauto-increment列で指定した値の一意性を検証または強制しません。これにより重複エントリが発生する可能性があります。

### 密度

Dorisによって生成されるauto-increment値は一般的に**密**ですが、いくつかの考慮事項があります：

- **潜在的なギャップ**：パフォーマンス最適化により、ギャップが現れる可能性があります。各バックエンドノード（BE）は効率性のため一意値のブロックを事前割り当てし、これらのブロックはノード間で重複しません。
- **非時系列値**：Dorisは、後の書き込みで生成される値が以前の書き込みからの値よりも大きいことを保証しません。

  :::info Note
  AUTO_INCREMENT値は書き込みの時系列順序を推測するために使用することはできません。
  :::
  
## 構文

auto-increment列を使用するには、Table作成時（[CREATE-TABLE](../sql-manual/sql-statements/table-and-view/table/CREATE-TABLE)）に対応する列に`AUTO_INCREMENT`属性を追加する必要があります。auto-increment列の開始値を手動で指定するには、Table作成時に`AUTO_INCREMENT(start_value)`文を使用して行うことができます。指定されていない場合、デフォルトの開始値は1です。

### 例

1. auto-increment列をキー列として持つ重複Tableの作成。
...
```sql
dws_agg_tblに挿入
dwd_dup_tbl.dim1、dwd_dup_tbl.dim3、dwd_dup_tbl.dim5、BITMAP_UNION(TO_BITMAP(dictionary_tbl.aid))、COUNT(1)を選択
dwd_dup_tblからdictionary_tblとdwd_dup_tbl.user_id = dictionary_tbl.user_idでINNER JOIN
