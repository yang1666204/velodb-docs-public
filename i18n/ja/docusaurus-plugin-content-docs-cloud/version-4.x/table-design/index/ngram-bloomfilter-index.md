...
1. **`idx_column_name(column_name)`** は必須です。`column_name` はインデックスを作成する対象の列で、上記の列定義に含まれている必要があります。`idx_column_name` はインデックス名で、テーブルレベルで一意である必要があります。列名の前に `idx_` プレフィックスを付けた名前にすることを推奨します。
