...
{
  "title": "並行性制御とキューイング",
  "description": "並行性制御とキューイングはリソース管理メカニズムです。",
  "language": "ja"
}
---
# Concurrency Control and Queuing

Concurrency ControlとQueuing は、リソース管理メカニズムです。複数のクエリが同時にリソースを要求し、システムの並行性制限に達した場合、Dorisは事前定義された戦略と制限に基づいてクエリを管理し、高負荷状態でもシステムが円滑に動作し、OOM (Out of Memory) やシステムフリーズなどの問題を回避することを保証します。

Dorisの Concurrency Control とQueuing メカニズムは、主にworkload groupsを通じて実装されています。workload groupは、最大並行数、キュー長、タイムアウトパラメータなど、クエリのリソース使用制限を定義します。これらのパラメータを適切に設定することで、リソース管理の目標を達成できます。
...
|-----------------|-----------|---------------|-----------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| max_concurrency | Integer   | 2147483647    | [0, 2147483647] | オプション、同時クエリの最大数。デフォルト値は整数の最大値で、同時実行に制限がないことを意味します。実行中のクエリ数が最大同時実行数に達すると、新しいクエリはキューイング処理に入ります。             |
| max_queue_size  | Integer   | 0             | [0, 2147483647] | オプション、クエリキューの長さ。キューが満杯の場合、新しいクエリは拒否されます。デフォルト値は0で、キューイングなしを意味します。                                                                                                                                |
| queue_timeout   | Integer   | 0             | [0, 2147483647] | オプション、キュー内でのクエリの最大待機時間（ミリ秒単位）。クエリがキュー内でこの時間を超えた場合、クライアントに例外がスローされます。デフォルト値は0で、キューイングなしを意味し、クエリはキューに入った時点で即座に失敗します。 |


クラスタに現在1つのFEがある場合、この設定の意味は次のとおりです：クラスタ内の同時クエリの最大数は10に制限されます。最大同時実行数に達すると、新しいクエリはキューに入り、キュー長は20に制限されます。キュー内でのクエリの最大待機時間は3秒で、キュー内で3秒を超えるクエリは直接クライアントに失敗を返します。
