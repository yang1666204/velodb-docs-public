...
Direct reads from HDFS or S3 can also be imported through HDFS TVF or S3 TVF in the Lakehouse/TVF. The current "Insert Into" based on TVF is a synchronous import, while Broker Load is an asynchronous import method.
...
For the specific syntax for usage, please refer to BROKER LOAD in the SQL manual.

### Prerequisite check

1. Grant privileges on the table

Broker Load requires `INSERT` privileges on the target table. If there are no `INSERT` privileges, it can be granted to the user through the GRANT command.
...
Broker Load is an asynchronous import method, and the specific import results can be viewed through the SHOW LOAD command.
...
When the status of a Broker Load job is not CANCELLED or FINISHED, it can be manually cancelled by the user. To cancel, the user needs to specify the label of the import task to be cancelled. The syntax for the cancel import command can be viewed by executing CANCEL LOAD.
...
For more detailed syntax and best practices for using  Broker Load , please refer to the Broker Load command manual. You can also enter HELP BROKER LOAD in the MySQL client command line to obtain more help information.
