---
{
  "title": "Apache Gravitinoとの統合",
  "description": "データレイク技術の急速な発展により、統一された安全な",
  "language": "ja"
}
---
データレイク技術の急速な発展に伴い、統一された安全で効率的なlakehouseアーキテクチャの構築は、企業のデジタル変革における中核的な課題となっています。Apache Gravitinoは、次世代の統一メタデータ管理プラットフォームとして、マルチクラウド・マルチエンジン環境におけるデータガバナンスの完全なソリューションを提供します。様々なデータソースとコンピュートエンジンの統一管理をサポートするだけでなく、credential vendingメカニズムを通じてデータアクセスのセキュリティと制御性を確保します。

本記事では、Apache DorisとApache Gravitinoを深く統合し、Iceberg REST Catalogベースの現代的なlakehouseアーキテクチャを構築する方法について詳しく紹介します。Gravitinoの統一メタデータ管理と動的credential配布機能により、S3上のIcebergデータへの効率的かつ安全なアクセスを実現できます。

**本文書により、以下を迅速に理解できます：**

* **AWS環境の準備**: AWSでのS3バケットとIAMロールの作成方法、Gravitino用のセキュアなcredential管理システムの設定、一時的なcredentialの動的配布の実装。

* **Gravitinoのデプロイと設定**: Gravitinoサービスの迅速なデプロイ方法、Iceberg REST Catalogの設定、vended-credentials機能の有効化。

* **DorisのGravitino接続**: DorisがGravitino REST APIを通じてIcebergデータにアクセスする方法の詳細説明。

## 実践ガイド

### 1. AWS環境の準備

開始前に、S3バケットと慎重に設計されたIAMロールシステムを含む完全なインフラストラクチャをAWS上で準備する必要があります。これは安全で信頼性の高いlakehouseアーキテクチャを構築するための基盤となります。

### 1.1 S3バケットの作成

まず、Icebergデータを保存するための専用S3バケットを作成します：
...
```
### 1.2 IAM ロール アーキテクチャの設計

安全な認証情報管理を実装するため、STS AssumeRole メカニズムを通じて Gravitino が使用する IAM ロールを作成する必要があります。この設計は、最小権限の原則と職務分離のセキュリティベストプラクティスに従っています。
...
2. IAMロールの作成
   ...
   デモンストレーションを簡単にするため、AWSマネージドポリシーを直接使用します。本番環境では、より細かい権限制御を使用する必要があります。
...
成功レスポンス例：
    ...
    ```
## 2. Gravitino のデプロイメントと設定

### 2.1 Gravitino のダウンロードとインストール

環境を素早くセットアップするために、Gravitino のプリコンパイル版を使用します：
