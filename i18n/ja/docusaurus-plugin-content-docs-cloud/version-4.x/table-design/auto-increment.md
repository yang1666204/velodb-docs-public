...
auto-incrementカラムを使用するには、テーブル作成時（CREATE-TABLE）に対応するカラムに`AUTO_INCREMENT`属性を追加する必要があります。auto-incrementカラムの開始値を手動で指定するには、テーブル作成時に`AUTO_INCREMENT(start_value)`ステートメントを使用して行うことができます。指定されていない場合、デフォルトの開始値は1です。

### 例

1. auto-incrementカラムをキーカラムとして持つduplicateテーブルの作成。
