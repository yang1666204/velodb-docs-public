...
{
  "title": "ST_DISJOINT",
  "description": "2つの幾何学的図形が完全に分離している（つまり、共通の点を持たない）かどうかを判定します。",
  "language": "ja"
}
---
## Description

2つの幾何学的図形が完全に分離されているかどうか（つまり、共通点を持たないかどうか）を判定します。2つの図形の境界または内部に交差がない場合（共有点がない場合）は1を返し、少なくとも1つの共通点がある場合（境界接触や内部重複を含む）は0を返します。
...
|----------|------------------------|
| `<shape1>` | もう一方の図形と重ならないかどうかを判定するために使用される最初の幾何図形。Point、Line、Polygon、Circleなどの型をサポートします。|
| `<shape2>` | 	もう一方の図形と重ならないかどうかを判定するために使用される2番目の幾何図形。Point、Line、Polygon、Circleなどの型をサポートします。 |

## 戻り値

1: shape1がshape2と完全に重ならない（共通する点がない）。
0: shape1がshape2と交差する（少なくとも1つの共通する点が存在する）。

ST_DISJOINTには以下のエッジケースがあります：

- いずれかの入力パラメータがNULLの場合、NULLを返します。
- 入力された幾何図形が無効な場合、NULLを返します。
- 空の幾何図形に対して1を返します（空の図形は点を持たず、任意の図形と重なりません）。
