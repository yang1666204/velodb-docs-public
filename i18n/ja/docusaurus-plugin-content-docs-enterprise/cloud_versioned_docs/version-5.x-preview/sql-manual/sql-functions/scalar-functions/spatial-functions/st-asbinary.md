...
{
  "title": "ST_ASBINARY",
  "description": "幾何オブジェクトを標準的なWKB（Well-Known Binary）バイナリ表現に変換します。WKBは地理空間データを表現するためのバイナリフォーマットです。",
  "language": "ja"
}
---
## 説明

幾何オブジェクトを標準的なWKB（Well-Known Binary）バイナリ表現に変換します。WKBは地理空間データを表現するためのバイナリ形式で、地理情報システム（GIS）で広く使用されています。
