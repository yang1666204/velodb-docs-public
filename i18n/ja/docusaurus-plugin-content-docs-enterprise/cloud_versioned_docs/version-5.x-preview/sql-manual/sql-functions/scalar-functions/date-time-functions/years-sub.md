...
YEARS_SUB関数は、指定された日付または時刻値から指定された年数を減算（または加算）するために使用され、調整された日付または時刻を返します（本質的にyears_value × 1年を減算します）。DATEおよびDATETIME型の処理をサポートしており、年数は正の値（減算）または負の値（加算）を指定できます。
...
|-----------|-------------|
| `<date_or_time_expr>` | 入力datetime値、date/datetimeタイプをサポートします。datetimeとdateフォーマットについては、[datetime conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)と[date conversion](../../../../sql-manual/basic-element/sql-data-types/conversion/date-conversion)を参照してください |
| `<years>` | 減算する年数、タイプINT、正の数は減算を示し、負の数は加算を示します |
...
- 入力がDATETIMEタイプの場合、戻り値はDATETIMEタイプのまま（年、月、日が調整され、時、分、秒は変更されません）。
...
- 調整された月の日数が不足する場合（例：うるう年の2月29日から1年減算して非うるう年の2月28日になる場合）、その月の実際の日数に自動調整されます。
