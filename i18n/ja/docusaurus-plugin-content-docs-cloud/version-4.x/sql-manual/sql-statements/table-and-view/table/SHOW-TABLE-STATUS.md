...
> LIKE句はTable名に基づいてファジー検索を実行できます。
...
| Row_format          | String   | 行フォーマット。MyISAMエンジンの場合、Dynamic、Fixed、またはCompressedになる場合があります。Dynamic行はVarcharやBlob型フィールドのような可変長です。Fixed行はCharやInteger型フィールドのような固定長です。 |
| Rows                | String   | Tableの行数。非トランザクショナルTableの場合、この値は正確で、トランザクショナルエンジンの場合、この値は通常推定値です。                                                                               |
| Avg_row_length      | Integer  | 1行あたりの平均バイト数                                                                                                                                                                           |
| Data_length         | Integer  | Table全体のデータ量（バイト単位）                                                                                                                                                                         |
| Max_data_length     | Integer  | Tableが保持できる最大データ量                                                                                                                                                                          |
...
| Checksum            | String   | 有効な場合、Table全体の内容に対して計算されたチェックサム                                                                                                          |
| Create_options      | String   | Table作成時のその他すべてのオプションを指します                                                                                     |
...
- 現在のデータベース内のすべてのTableに関する情報を表示
    ...
    +------------+--------+---------+------------+------+----------------+-------------+-----------------+--------------+-----------+----------------+---------------------+---------------------+------------+-----------+----------+----------------+---------+
    ```
- 指定されたデータベース配下で、名前にexampleを含むTableに関する情報を表示する
